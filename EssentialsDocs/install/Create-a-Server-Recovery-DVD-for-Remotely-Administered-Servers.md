...
1.  На диске должна находиться папка en-us. Если приложение восстановления сервера не обнаружит файлы языка, соответствующие используемому компьютеру, оно переключится на язык en-us.  
  
2.  В создаваемые папки для каждого языка добавьте три файла настроек (PNG, CHM и RTF).  
  
3.  Скопируйте обе папки языка и региональных параметров из языковых пакетов\\< CultureName\>\Server Recovery в корень DVD-диска восстановления сервера. Пример: Папки ES и ES-ES будут скопированы в корневой каталог DVD для поддержки испанского языка.  
