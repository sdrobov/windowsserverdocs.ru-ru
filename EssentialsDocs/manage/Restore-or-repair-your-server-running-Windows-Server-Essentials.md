...
-   **Повторная настройка сервера.** Используйте мастер настройки сервера, чтобы повторно установить параметры. Чтобы настроить удаленно управляемый сервер Windows Server Essentials с клиентского компьютера, откройте веб-браузер и введите в адресной строке **http://** _< йоурсервернаме\>_ .  
