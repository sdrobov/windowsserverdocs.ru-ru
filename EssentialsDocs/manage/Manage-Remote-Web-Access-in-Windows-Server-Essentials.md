...
>Область применения. Windows Server 2016 Essentials, Windows Server 2012 R2 Essentials, Windows Server 2012 Essentials
 ...
 Удаленного веб-доступа в Windows Server Essentials или в Windows Server 2012 R2 с установленной ролью Windows Server Essentials Experience обеспечивает взаимодействие с браузером оптимальное и поддержкой сенсорного ввода для доступа к приложениям и данным практически из любого места наличие подключения к Интернету и с помощью практически с любого устройства. Для использования функции удаленного веб-доступа нужно сначала включить ее при помощи мастера настройки повсеместного доступа, а затем настроить маршрутизатор и имя домена.  
...
-   [Защита удаленного веб-доступа](Manage-Remote-Web-Access-in-Windows-Server-Essentials.md#BKMK_SecureRWA)  
  
-   [Управление пользователями удаленного веб-доступа и VPN](Manage-Remote-Web-Access-in-Windows-Server-Essentials.md#BKMK_ManageRWAVPN)  
  
###  <a name="BKMK_Overview"></a> Обзор удаленного веб-доступа  
 Находясь за пределами офиса, можно открыть веб-браузер и получить доступ к удаленного веб-доступа из любого места с доступом к Интернету. В удаленного веб-доступа вы можете:  
...
-   Доступ к серверу и компьютерам в сети. Это означает, что у вас есть доступ к рабочему столу сетевого компьютера, как если бы вы сидели за компьютером в своем офисе.  
  ...
  Удаленный веб-доступ не включен по умолчанию. При запуске мастера настройки повсеместного доступа мастер пытается настроить маршрутизатор и подключение к Интернету. После включения удаленного веб-доступа можно настроить доменное имя сервера и настройка удаленного веб-доступа. Кроме того, можно повторно настроить маршрутизатор при смене последнего.  
  
 Разрешение на доступ к Remote Web Access не предоставляется автоматически при добавлении учетной записи пользователя. При добавлении учетной записи пользователя можно предоставить ей доступ к общим папкам, библиотеке мультимедиа, компьютерам, ссылкам на главную страницу и серверной панели мониторинга. Можно также указать, что пользователю не разрешается использовать удаленный веб-доступ.  
  
 Параметр Remote Web Access отображается для каждой учетной записи пользователя на **пользователей** вкладку панели мониторинга Windows Server Essentials. Чтобы изменить настройку удаленного веб-доступа, щелкните правой кнопкой мыши учетную запись пользователя и нажмите кнопку **Просмотр настроек учетной записи**.  
...
###  <a name="BKMK_SecureRWA"></a> Защита удаленного веб-доступа  
...
###  <a name="BKMK_ManageRWAVPN"></a> Управление пользователями удаленного веб-доступа и VPN  
...
###  <a name="BKMK_ReplaceRouter"></a> Замените маршрутизатор  
 Замените маршрутизатор в соответствии с инструкциями производителя s и запустите набор мастер настройки маршрутизатора для настройки нового устройства.  
...
###  <a name="BKMK_ActiveX"></a> Включение элементов управления ActiveX служб удаленных рабочих столов  
 Элементы управления ActiveX служб удаленных рабочих столов позволяет доступ к домашнему или рабочему компьютеру через Интернет, с другого компьютера с помощью удаленного веб-доступа.  
...
-   [Обзор службы доменных имен](Manage-Remote-Web-Access-in-Windows-Server-Essentials.md#BKMK_DNOverview)  
  
-   [Сведения об именах Microsoft персонализированного домена](Manage-Remote-Web-Access-in-Windows-Server-Essentials.md#BKMK_PersonalizedNames)  
  
-   [Использование нового или существующего доменного имени](Manage-Remote-Web-Access-in-Windows-Server-Essentials.md#BKMK_UseNewName)  
...
-   [Найти поставщика службы доменных имен](Manage-Remote-Web-Access-in-Windows-Server-Essentials.md#BKMK_Find)  
  
###  <a name="BKMK_DNOverview"></a> Обзор службы доменных имен  
 Доменное имя уникально идентифицирует сервер в Интернете. Доменные имена состоят, по меньшей мере, из двух частей: доменного имени верхнего уровня и доменного имени второго уровня. Например в домене contoso.com, com является Доменным и contoso является доменным именем второго уровня.  
...
###  <a name="BKMK_PersonalizedNames"></a> Сведения об именах Microsoft персонализированного домена  
 Персонализированное имя домена Microsoft включает следующие компоненты.  
  
-   Имя личного домена для удаленного веб-доступа (например, *имя_хоста*. remotewebaccess.com). Доменное имя связано с общедоступным IP-адресом.  
  
-   Динамический DNS обновить службу протокола удаленного веб-доступа, использующего ваше доменное имя не будет прерван при изменении общедоступного IP-адрес. Как правило поставщики услуг Интернета (ISP) для вашей организации, s широкополосным соединениям предоставляют динамические общедоступные IP-адреса, которые могут изменяться.  
...
###  <a name="BKMK_UseNewName"></a> Использование нового или существующего доменного имени  
...
-   При наличии существующего доменного имени, приобретенного у поставщика служб доменных имен, который не поддерживается Windows Server Essentials и желании использовать мастер настройки доменных имен для настройки доменного имени сервера можно перенести доменное имя одному из поставщиков служб доменных имен, указанных в мастере. Нажмите кнопку **я хочу использовать уже имеющееся имя домена**, введите имя домена в **доменное имя** текстовое поле, а затем следуйте инструкциям на сайте поставщика s имя службы домена для переноса доменного имени .  
...
###  <a name="BKMK_ChooseProvider"></a> Выберите поставщика службы доменных имен  
 Следует выбирать поставщика служб доменных имен, поддерживающего расширение, которое требуется использовать для доменного имени. Домен имени мастер настройки содержит список удовлетворяющих требованиям поставщиков, которые можно использовать со ссылкой для каждого веб-сайта поставщика s. Нажмите кнопку **детали** ссылку рядом с именем каждого поставщика s, чтобы получить сведения об услугах и ценах, предлагаемых поставщиком.  
 ...
 **Доменное имя второго уровня** — это метка рядом с расширением доменного имени верхнего уровня. Доменное имя второго уровня часто создается на основе названия компании или производимых ею товаров и услуг. Например в www.contoso.com, contoso — это имя домена второго уровня и выбранное по названию компании Contoso Pharmaceuticals. Домен второго уровня часто называют именем хоста, с которым связан определенный IP-адрес.  
...
###  <a name="BKMK_Extension"></a> Выбор расширения доменного имени  
 При выборе имени домена для веб-сайта в Интернете необходимо также указать желаемое расширение имени домена. Расширение обозначается буквами, следующими за последней точкой любого доменного имени. (Формальное название расширения — верхнего уровня домена или домена верхнего УРОВНЯ).  
 ...
 **Примеры доменов верхнего уровня код страны**  
...
-   В мастере не указано нужное расширение доменного имени, но оно доступно у поставщика доменных имен, который не является партнером в данное время.  
 ...
 Если вы решили настроить имя домена вручную, работаете с вашего поставщика службы доменных имен для создания записи A для вашего домена.  
  
##### <a name="to-create-an-a-record"></a>Чтобы создать запись a.  
  
1.  Выберите имя узла, такими как удаленное. Это будет префикс имени домена. Префикс имени домена имя домена определяют URL-адрес для открытия страницы входа в систему удаленного веб-доступа; например **http://remote.contoso.com**.  
  
2.  В домен имя поставщики конфигурации мониторинга службы (обычно на своих веб-странице) создайте запись A для имени узла, как указано в шаге 1. Убедитесь, что IP-адрес, указанный в записи A IP-адрес на стороне WAN вашего маршрутизатора (на стороне сети Интернет). Обратитесь к документации маршрутизатора, чтобы узнать свой IP-адрес WAN.  
...
###  <a name="BKMK_Find"></a> Найти поставщика службы доменных имен  
...
-   [Настройка образов для фона и эмблем](Manage-Remote-Web-Access-in-Windows-Server-Essentials.md#BKMK_CustomizeImages)  
...
###  <a name="BKMK_CustomizeImages"></a> Настройка образов для фона и эмблем  
...
-   Точечный рисунок (*.bmp, \*.dib, \*загружаемого)  
...
> [!NOTE]
>  Мастер восстановления не доступен до тех пор, пока не будет включен удаленный веб-доступ. Сведения о включении удаленного веб-доступа см. в разделе [Turn on Remote Web Access](Manage-Remote-Web-Access-in-Windows-Server-Essentials.md#BKMK_TurnOnRWA).  
