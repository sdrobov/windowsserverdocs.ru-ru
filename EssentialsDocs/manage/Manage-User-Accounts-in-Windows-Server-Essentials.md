...
-   Назначьте учетную запись Microsoft Online Services (называется учетной записью Office 365 в Windows Server Essentials), если это применимо.  
  
-   Назначение групп пользователей (только для Windows Server Essentials).  
  
> [!NOTE]
>  -   Символы, отличные от ASCII, не поддерживаются в Microsoft Azure Active Directory (Azure AD). Не используйте символы не из набора ASCII в свой пароль, если ваш сервер интегрирован с Azure AD.  
...
> [!IMPORTANT]
>  Если пользователь учетной записи Microsoft online учетной записи назначены, то при удалении учетной записи пользователя, то его учетная запись также удаляется из службы Microsoft Online Services и s данные пользователя, включая сообщения электронной почты, подлежит данных политики хранения в Microsoft Online Services. Если вы хотите сохранить данные пользователя для учетной записи интернет-служб, то не удаляйте эту учетную запись, а отключите ее. Дополнительные сведения см. в разделе [управление Online учетных записей пользователей](Manage-Online-Accounts-for-Users.md).  
...
4.  В **< учетная запись пользователя\> задачи** панели щелкните **удалить учетную запись**. Появляется мастер удаления учетных записей пользователей.  
  
5.  На **вы хотите сохранить файлы?** страницы мастера, вы можете удалить файлы пользователя s, включая резервные копии истории файлов и перенаправленную папку для учетной записи пользователя. Чтобы пользователь оставался файлы s, оставьте флажок пустым. После выбора нажмите кнопку **Далее**.  
  
6.  Щелкните **Удалить учетную запись**.  
  
> [!NOTE]
>  После удаления эта учетная запись пользователя в списке учетных записей пользователей больше не отображается. Если вы решили удалить файлы, сервер окончательно удалит папку пользователя s из **пользователей** папки сервера и от **резервные копии истории файлов** папки сервера.  
>   
...
2.  В **< учетная запись пользователя\> задачи** панели щелкните **Просмотр настроек учетной записи**. Отобразится страница **Свойства** учетной записи пользователя.  
...
4.  В **< учетная запись пользователя\> задачи** панели щелкните **активация учетной записи пользователя**.  
...
4.  В **< учетная запись пользователя\> задачи** панели щелкните **отключение учетной записи пользователя**.  
...
###  <a name="BKMK_Manage8"></a> Управление учетными записями пользователей с помощью панели мониторинга  
 Windows Server Essentials позволяет выполнять стандартные административные задачи, используя панель мониторинга Windows Server Essentials. По умолчанию **пользователей** страница панели мониторинга содержит две вкладки **пользователей** и **пользователи группы**.  
  
> [!NOTE]
>  -   При интеграции сервера, на котором выполняется Windows Server Essentials с Office 365, вызывается новая вкладка **группы распространения** добавляется **пользователей** страница панели мониторинга.  
> -   В Windows Server Essentials **пользователей** страница панели мониторинга имеется только одна вкладка — **пользователей**.  
...
-   Список учетных записей пользователей, в котором отображаются:  
    ...
    -   Если сервер интегрирован с Microsoft Office 365 в Windows Server Essentials отображается состояние учетной записи Office 365 (известный в Windows Server Essentials в качестве учетной записи Майкрософт) для учетной записи пользователя.  
...
-   Область задач, которая включает в себя:  
 ...
 В следующей таблице описаны различные задачи учетной записи пользователя, доступные на вкладке **Пользователи**. Некоторые задачи зависят от конкретной учетной записи пользователя и отображаются только при выборе учетной записи пользователя в списке.  
  
> [!NOTE]
>  При интеграции Office 365 с Windows Server Essentials, будут доступны дополнительные задачи. Дополнительные сведения см. в разделе [управление Online учетных записей пользователей](Manage-Online-Accounts-for-Users.md).  
...
|Обновить|Обновляет вкладку "Пользователи".<br /><br /> Эта задача предназначена для Windows Server Essentials.|  
...
-   [Изменить или сбросить пароль для учетной записи пользователя](Manage-User-Accounts-in-Windows-Server-Essentials.md#BKMK_Access1)  
  
-   [Что нужно знать о политиках паролей](Manage-User-Accounts-in-Windows-Server-Essentials.md#BKMK_Access3)  
...
-   [Разрешить учетные записи пользователей удаленного рабочего стола](Manage-User-Accounts-in-Windows-Server-Essentials.md#BKMK_Access8)  
  
-   [Предоставить пользователям доступ к ресурсам на сервере](Manage-User-Accounts-in-Windows-Server-Essentials.md#BKMK_Access9)  
...
-   [Изменение разрешения виртуальной частной сети для учетной записи пользователя](Manage-User-Accounts-in-Windows-Server-Essentials.md#BKMK_Access11)  
...
-   [Разрешить учетные записи пользователей установить сеанс удаленного рабочего стола на свой компьютер](Manage-User-Accounts-in-Windows-Server-Essentials.md#BKMK_Access13)  
  
###  <a name="BKMK_Access1"></a> Изменить или сбросить пароль для учетной записи пользователя  
...
4.  В **< учетная запись пользователя\> задачи** панели щелкните **изменение пароля учетной записи пользователя**. Появляется мастер изменения пароля учетной записи пользователя.  
...
7.  Предоставьте новый пароль пользователю.  
    ...
    > -   Символы, отличные от ASCII, не поддерживаются в Azure AD. Таким образом Если ваш сервер интегрирован с Azure AD, не используйте отличные от ASCII символы в паролях.  
    > -   Если пользователю назначена online учетной записи Майкрософт (известный в Windows Server Essentials в качестве учетной записи Office 365), то пароль синхронизируется с паролем учетной записи Интернет. Пользователь будет использовать новый пароль для входа в систему на сервере или входа в Office 365. Дополнительные сведения см. в разделе [управление Online учетных записей пользователей](Manage-Online-Accounts-for-Users.md).  
  
###  <a name="BKMK_Access3"></a> Что нужно знать о политиках паролей  
...
###  <a name="BKMK_Access6"></a> Сохранение и управление доступом к файлам для удаленных учетных записей  
 Администратор сети можно удалить учетную запись пользователя и выбрать Сохранить пользователя s файлы для использования в будущем. В этом сценарии удаленная учетная запись пользователя больше не может использоваться для входа в сеть. Тем не менее, файлы для этого пользователя будут сохранены в общей папке, которая может использоваться совместно с другим пользователем.  
  
> [!IMPORTANT]
>  Следует отметить, что в случае удаления учетной записи пользователя, которой назначена учетная запись интернет-служб Майкрософт, последняя также будет удалена, а данные пользователя, включая сообщения электронной почты, будут подчиняться политикам хранения данных в Microsoft Online Services. Для сохранения данных пользователя для учетной записи интернет-служб не удаляйте эту учетную запись, а отключите ее. Дополнительные сведения см. в разделе [управление Online учетных записей пользователей](Manage-Online-Accounts-for-Users.md).  
  
##### <a name="to-remove-a-user-account-but-retain-access-to-the-user-s-files"></a>Чтобы удалить учетную запись пользователя, но сохранить доступ к файлам пользователя s  
...
6.  В **< учетная запись пользователя\> свойства**, нажмите кнопку **доступ** , а затем щелкните **общего ресурса**.  
...
3.  Чтобы синхронизировать Пароль DSRM на контроллере домена с текущей сети s учетной записи администратора, введите:  
...
5.  Определите действия задач:  
    ...
    3.  В **добавить аргументы**текста (необязательно) введите следующее (необходимо использовать кавычки): **задать синхронизации пароля dsrm из учетной записи домена SBS_network_administrator_account q q** где  *SBS_network_administrator_account* текущее имя учетной записи администратора s сети.  
...
###  <a name="BKMK_Access8"></a> Разрешить учетные записи пользователей удаленного рабочего стола  
...
-   Удаленный веб-доступ  
 ...
 Дополнительные сведения о повсеместном доступе см. в разделе [Manage Anywhere Access](Manage-Anywhere-Access-in-Windows-Server-Essentials.md).  
  
###  <a name="BKMK_Access9"></a> Предоставить пользователям доступ к ресурсам на сервере  
  Этот раздел относится к серверу под управлением Windows Server Essentials или Windows Server Essentials, либо на сервер под управлением Windows Server 2012 R2 Standard или Windows Server 2012 R2 Datacenter с установленной ролью Windows Server Essentials Experience.  
  
 Если требуется, чтобы пользователи использовали удаленный доступ и (или) имели отдельные учетные записи после завершения подключения компьютера к серверу, с помощью панели мониторинга на сервере можно создать новые учетные записи для пользователей компьютера, подключенного к сети. Дополнительные сведения о создании учетной записи пользователя см. в разделе [Add a user account](Manage-User-Accounts-in-Windows-Server-Essentials.md#BKMK_Manage1). После создания учетных записей пользователей сведения о имени и пароле пользователя сети необходимо предоставить пользователям клиентских компьютеров, чтобы они могли обращаться к ресурсам на сервере с помощью панели запуска.  
...
4.  В **< учетная запись пользователя\> задачи** панели щелкните **Просмотр настроек учетной записи**.  
  
5.  В **< учетная запись пользователя\> свойства**, выполните следующие действия:  
...
###  <a name="BKMK_Access11"></a> Изменение разрешения виртуальной частной сети для учетной записи пользователя  
...
###  <a name="BKMK_Access13"></a> Разрешить учетные записи пользователей установить сеанс удаленного рабочего стола на свой компьютер  
  Этот раздел относится к серверу под управлением Windows Server Essentials или Windows Server Essentials, либо на сервер под управлением Windows Server 2012 R2 Standard или Windows Server 2012 R2 Datacenter с установленной ролью Windows Server Essentials Experience.  
...
5.  В **< учетная запись пользователя\> свойства**, нажмите кнопку **доступ к компьютеру** вкладки.  
...
-   [Управление учетными записями Интернет для пользователей](Manage-Online-Accounts-for-Users.md)  
