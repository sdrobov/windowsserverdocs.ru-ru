...
ms.locfileid: "66812665"
---
# <a name="deploy-work-folders-with-ad-fs-and-web-application-proxy-step-3-set-up-work-folders"></a>Развертывание рабочих папок с помощью AD FS и прокси веб-приложения: Шаг 3, настройки рабочих папок
...
Измените IP-адрес сервера на статический IP-адрес. Пример тестирования, используйте IP-адрес класса A, который является 192.168.0.170 / маска подсети: 255.255.0.0 / шлюз по умолчанию: 192.168.0.1 / предпочитаемый DNS: 192.168.0.150 (IP-адрес контроллера домена). 
...
Существует два метода, которые можно использовать для привязки сертификата к порту с помощью Windows PowerShell: Командлеты служб IIS и netsh.  
...
4.  В окне **Параметры рабочих папок** выберите **Службы федерации Active Directory (AD FS)** и введите URL-адрес службы федерации. Щелкните **Применить**.  
    ...
    В примере теста является URL-адрес **https://blueadfs.contoso.com** .  
...
Чтобы экспортировать сертификат, выполните те же действия, которые вы использовали ранее, экспортировать сертификат AD FS, как описано в разделе [развертывание рабочих папок с AD FS и прокси веб-приложения: Шаг 2, AD FS после конфигурации рабочих](deploy-work-folders-adfs-step2.md), экспортировать сертификат AD FS.  
  
Далее: [Развертывание рабочих папок с помощью AD FS и прокси веб-приложения: Шаг 4 — Настройка прокси веб-приложения](deploy-work-folders-adfs-step4.md)  
