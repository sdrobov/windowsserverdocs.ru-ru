...
ms.locfileid: "70867025"
---
# <a name="deploying-work-folders"></a>Развертывание рабочих папок

>Относится к: Windows Server (половина ежегодного канала), Windows Server 2016, Windows Server 2012 R2, Windows 10, Windows 8.1, Windows 7
...
6.  Введите имена пользователей или группы, к которым вы предоставить доступ к конкретному общему ресурсу синхронизации (Если вы создаете группу для управления доступом к такому ресурсу), или введите имена администраторов рабочих папок (Если вы собираетесь настроить учетные записи пользователей, чтобы автоматически обнаружить соответствующий сервер синхронизации), нажмите кнопку **ОК**, а затем нажмите кнопку **ОК** еще раз.
...
## <a name="step-8-optionally-specify-a-tech-support-email-address"></a>Шаг 8. Укажите адрес электронной почты службы технической поддержки (необязательно)   
...
>[!NOTE]
>Свойство msDS-SyncServerURL в Active Directory не следует определять для удаленных пользователей, которые обращаются к рабочим папкам через обратный прокси-сервер, например прокси-сервер веб-приложения или прокси-сервер приложения Azure AD. Если определено свойство msDS-Синксерверурл, клиент рабочих папок попытается получить доступ к внутреннему URL-адресу, который недоступен через решение для обратных прокси-серверов. При использовании прокси-сервера веб-приложения или прокси-сервера приложения Azure AD необходимо создать уникальные приложения прокси-сервера для каждого сервера рабочих папок. Дополнительные сведения см. в [разделе Развертывание рабочих папок с помощью AD FS и прокси веб-приложения. Обзор](deploy-work-folders-adfs-overview.md) или [Развертывание рабочих папок с помощью Azure AD application proxy](https://blogs.technet.microsoft.com/filecab/2017/05/31/enable-remote-access-to-work-folders-using-azure-active-directory-application-proxy/).
...
## <a name="step-10-optionally-configure-web-application-proxy-azure-ad-application-proxy-or-another-reverse-proxy"></a>Шаг 10. При необходимости настройте прокси веб-приложения, AD Application Proxy Azure или другой обратный прокси-сервер.  
...
|------------------|----------------|  
|**Поможет**|-   [Рабочие папки](work-folders-overview.md)|  
|**Планирование**|-   [Разработка реализации рабочих папок](plan-work-folders.md)|
...
|**Техническая справочная информация**|-   [Интерактивный вход в систему: Порог блокировки учетной записи компьютера](https://technet.microsoft.com/library/jj966264(v=ws.11).aspx)<br />-   [Командлеты общего доступа синхронизации](https://docs.microsoft.com/powershell/module/syncshare/?view=win10-ps)|
