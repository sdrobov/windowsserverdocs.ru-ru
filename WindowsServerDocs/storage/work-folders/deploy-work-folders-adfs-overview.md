...
-   Контроллер домена: Сервер с включенной ролью AD DS и настроен с доменом (например теста, contoso.com).  
...
![Схема с изображением трех сегментов сети: Интернета, промежуточной подсети и Contoso. В сегменте Интернета: CLIENT2; в сети Периметра: WAP-сервера; в сегменте Contoso: Рабочие папки сервера, контроллер домена, сервер AD FS и Client1](media/deploy-work-folders-adfs/WF_ADFS_WAP_Diagram.png)
