...
В этой статье описано планирование реализации рабочих папок; предполагается наличие следующих знаний:  
...
> [!NOTE]
>  Установите накопительный пакет обновления общей доступности для Windows 8.1 и Windows Server 2012 R2 на все серверы рабочих папок и все клиентские компьютеры с Windows 8.1 или Windows Server 2012 R2. Дополнительную информацию см. в статье [2883200](http://support.microsoft.com/kb/2883200) в библиотеке базы знаний Майкрософт.  
...
### <a name="active-directory-domain-services"></a>Доменные службы Active Directory  
 ...
 Во-вторых, службы ADDS (с обновлением схемы Windows Server 2012 R2) поддерживают атрибут msDS-SyncServerURL для каждого пользователя. При помощи этого атрибута пользователи автоматически перенаправляются на соответствующий сервер синхронизации.  
...
###  <a name="GroupPolicy"></a> Групповая политика  
 При наличии компьютеров с Windows 7 в вашей среде рекомендуется следующее.  
...
### <a name="windows-intune"></a>Windows Intune  
 Windows Intune также обеспечивает определенный уровень безопасности и управляемости для устройств, не присоединенных к домену, которые иначе не были бы представлены. При помощи Windows Intune можно настраивать личные устройства пользователей — например, планшеты, подключающиеся к рабочим папкам через Интернет, — и управлять такими устройствами. Windows Intune может предоставить устройствам URL-адрес сервера синхронизации. В противном случае пользователи должны ввести свой рабочий адрес электронной почты для просмотра параметров (если вы публикуете общедоступный URL-адрес рабочих папок в виде https://workfolders.*contoso.com*) или ввести URL-адрес сервера синхронизации напрямую.  
...
-  Прокси-сервер приложения Azure AD— это обратный прокси-сервер для облачной среды. Дополнительные сведения см. в разделе [Предоставление безопасного удаленного доступа для локальных приложений](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-application-proxy-get-started)
...
-   Безопасность данных  
    ...
    -   Требуется ли многофакторная идентификация (MFA)?  
...
|**Развертывание**|-   [Проектирование реализации рабочих папок](plan-work-folders.md)<br />-   [Развертывание рабочих папок](deploy-work-folders.md)<br />-   [Развертывание рабочих папок с помощью AD FS и прокси-сервера веб-приложения (WAP)](deploy-work-folders-adfs-overview.md)<br />- [Развертывание рабочих папок с помощью прокси-сервера приложения Azure AD](https://blogs.technet.microsoft.com/filecab/2017/05/31/enable-remote-access-to-work-folders-using-azure-active-directory-application-proxy/)<br />-   [Факторы производительности при развертывании рабочих папок](http://blogs.technet.com/b/filecab/archive/2013/11/01/performance-considerations-for-large-scale-work-folders-deployments.aspx)<br />-   [Рабочие папки для Windows 7 (скачать 64-разрядную версию)](http://www.microsoft.com/download/details.aspx?id=42558)<br />-   [Рабочие папки для Windows 7 (скачать 32-разрядную версию)](http://www.microsoft.com/download/details.aspx?id=42559)<br />-   [Развертывание лаборатории тестирования рабочих папок](http://blogs.technet.com/b/filecab/archive/2013/07/10/work-folders-test-lab-deployment.aspx) (запись блога)|
