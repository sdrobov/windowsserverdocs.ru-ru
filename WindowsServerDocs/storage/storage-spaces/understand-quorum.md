...
- **Кворум кластера**: Это работает на уровне кластера (т. е. можно потерять узлов и кластера, в курсе)
- **Пул кворума**: Это происходит на уровне пула при включении дисковых пространств (т. е. можно потерять узлов и дисков, устраняя пула). Пулы носителей были разработаны для использования в сценариях кластеризованный и некластеризованный, поэтому они имеют разные кворума механизм.
...
- Если у вас есть два узла, следящий сервер, то **требуется**.
- При наличии трех или четырех узлов, следящий сервер находится **настоятельно рекомендуется**.
- Если у вас есть доступ к Интернету, используйте  **[облако-свидетель](../../failover-clustering/deploy-cloud-witness.md)**
...
- Если у вас есть **даже** количество узлов и без свидетеля *один узел получает свой голос за zeroed*. К примеру, только три из четырех узлов получения голосов, поэтому *общее число голосов* равно 3, а два оставшихся объектов с голосами считаются большинства.
- Если у вас есть **нечетное** количество узлов и без свидетеля *всех них голосов*.
- Если у вас есть **даже** число узлов, а также для следящего сервера, *голосов следящий сервер*, поэтому общее число нечетное.
- Если у вас есть **нечетное** число узлов, а также для следящего сервера, *следящий сервер не проголосовать*.
...
Описанный выше сценарий применяется к общие кластера, у которых дисковых пространств включено. Тем не менее при включении Storage Spaces Direct, кластер может поддерживать только двух сбоев узлов. Это объясняется более [пула разделе кворума](#pool-quorum-overview).

### <a name="examples"></a>Примеры

#### <a name="two-nodes-without-a-witness"></a>Два узла без следящего сервера. 
Один узел голосов обнуляется, поэтому *большинство* голоса определяется из общего числа **1 голос**. Если без права голоса узел выйдет из строя неожиданно, «выжившую» имеет 1/1, и выдерживает его кластера. Если голосования узел выйдет из строя неожиданно, «выжившую» имеет 0/1 и кластера, выходит из строя. Если узел голосования корректно выключено, голосов переносится на другой узел, и выдерживает его кластера. ***Именно поэтому крайне важно, чтобы настроить следящий сервер.***

![Как описано в случае с двумя узлами без следящего сервера кворума](media/understand-quorum/2-node-no-witness.png)

- Сохраняется в случае сбоя одного сервера: **Вероятность 50%** .
- Сохраняется в случае сбоя одного сервера, а затем создается другой: **Нет**.
- За один раз может выдержать два сбоя сервера: **Нет**. 

#### <a name="two-nodes-with-a-witness"></a>Два узла со следящим сервером. 
Проголосовать оба узла, а также голосов следящий сервер, поэтому *большинство* определяется из общего числа **3 голосов**. Если либо узел выйдет из строя, «выжившую» имеет 2/3, и выдерживает его кластера.

![Кворум, описано в случае с двумя узлами со следящим сервером](media/understand-quorum/2-node-witness.png)

- Сохраняется в случае сбоя одного сервера: **Да**.
- Сохраняется в случае сбоя одного сервера, а затем создается другой: **Нет**.
- За один раз может выдержать два сбоя сервера: **Нет**. 

#### <a name="three-nodes-without-a-witness"></a>Три узла без следящего сервера.
Проголосовать все узлы, поэтому *большинство* определяется из общего числа **3 голосов**. Если любой узел выйдет из строя, оставшихся объектов, 2 и 3, и выдерживает его кластера. Кластер становится двух узлов без следящего сервера — на этом этапе вы находитесь в сценарии 1.

![Как описано в случае с тремя узлами без следящего сервера кворума](media/understand-quorum/3-node-no-witness.png)

- Сохраняется в случае сбоя одного сервера: **Да**.
- Сохраняется в случае сбоя одного сервера, а затем создается другой: **Вероятность 50%** .
- За один раз может выдержать два сбоя сервера: **Нет**. 

#### <a name="three-nodes-with-a-witness"></a>Три узла со следящим сервером.
Голосовать все узлы, поэтому изначально не проголосовать следящий сервер. *Большинство* определяется из общего числа **3 голосов**. После обнаружения первой ошибки в кластере есть два узла со следящим сервером — то есть сценарий 2. Итак теперь двумя узлами и следящий сервер проголосовать.

![Кворум, описано в случае с тремя узлами со следящим сервером](media/understand-quorum/3-node-witness.png)

- Сохраняется в случае сбоя одного сервера: **Да**.
- Сохраняется в случае сбоя одного сервера, а затем создается другой: **Да**.
- За один раз может выдержать два сбоя сервера: **Нет**. 

#### <a name="four-nodes-without-a-witness"></a>Четыре узла без следящего сервера
Один узел голосов обнуляется, поэтому *большинство* определяется из общего числа **3 голосов**. После обнаружения первой ошибки кластер становится три узла, и вы находитесь в сценарий 3.

![Как описано в случае с четырьмя узлами без следящего сервера кворума](media/understand-quorum/4-node-no-witness.png)

- Сохраняется в случае сбоя одного сервера: **Да**.
- Сохраняется в случае сбоя одного сервера, а затем создается другой: **Да**.
- За один раз может выдержать два сбоя сервера: **Вероятность 50%** . 

#### <a name="four-nodes-with-a-witness"></a>Четыре узла со следящим сервером.
Все голоса узлов и голоса свидетеля, поэтому *большинство* определяется из общего числа **5 голосов**. После обнаружения первой ошибки вы перейдете на сценарий 4. После двух одновременных отказов Перейдите вниз, чтобы сценарий 2.

![Кворум, описано в случае с четырьмя узлами со следящим сервером](media/understand-quorum/4-node-witness.png)

- Сохраняется в случае сбоя одного сервера: **Да**.
- Сохраняется в случае сбоя одного сервера, а затем создается другой: **Да**.
- За один раз может выдержать два сбоя сервера: **Да**. 
...
- **[Облако-свидетель](../../failover-clustering/deploy-cloud-witness.md)**  -BLOB-объектов хранилища в Azure доступны всем узлам кластера. Он поддерживает кластеризации информацию в файле witness.log, но не хранит копию базы данных кластера.
- **Файл ресурса-свидетеля** — файловые ресурсы SMB, настроенный на файловом сервере под управлением Windows Server. Он поддерживает кластеризации информацию в файле witness.log, но не хранит копию базы данных кластера.
- **Диск-свидетель** -small кластеризованный диск, который входит в группу доступное хранилище кластера. Этот диск высокой доступности и отработки отказа между узлами. Он содержит копию базы данных кластера.  ***Диск-свидетель не поддерживается с дисковыми пространствами***.

## <a name="pool-quorum-overview"></a>Общие сведения о пуле кворума
...
Каждый из 16 дисков имеет один голос, два также имеет один голос (так как он является владельцем ресурса пула). *Большинство* определяется из общего числа **16 голосов**. Если узлы трех и четырех вышли из строя, уцелевшие подмножество состоит из 8 дисков и владелец ресурса пула, являющийся голосов 9/16. Таким образом пул сохраняется.
...
Каждый из 24 дисков имеет один голос, два также имеет один голос (так как он является владельцем ресурса пула). *Большинство* определяется из общего числа **24 голосов**. Если узлы трех и четырех вышли из строя, уцелевшие подмножество состоит из 8 дисков и владелец ресурса пула, который является голосов 9/24. Таким образом пул не имеет большую и выходит из строя.

![Кворума пула 3](media/understand-quorum/pool-3.png)

- Сохраняется в случае сбоя одного сервера: **Да**.
- Можно избежать простоев в случае сбоя одного сервера, а затем создается другой: ** определяется ** (не удается избежать простоев в случае если оба узла, трех и четырех вышли из строя, но может выдержать всех других сценариев.
- Может выдержать два сбоя сервера за один раз: ** определяется ** (не удается избежать простоев в случае если оба узла, трех и четырех вышли из строя, но может выдержать всех других сценариев.
