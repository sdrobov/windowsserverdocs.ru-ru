...
author: cosmosdarwin
ms.date: 11/06/2017
description: Добавление серверов или дисков в кластере локальных дисковых пространств
...
ms.locfileid: "4678603"
---
# Добавление серверов или дисков в локальные дисковые пространства
...
1. Запустите [мастер проверки кластера](https://technet.microsoft.com/library/cc732035(v=ws.10).aspx) с помощью оснастки отказоустойчивого кластера или командлета **Test-Cluster** в PowerShell (запустите его от имени администратора). Добавьте новый сервер *\<NewNode>*, который вы хотите добавить.
   ...
   Это позволит убедиться в том, что новый сервер работает под управлением Windows Server2016 Datacenter Edition, присоединен к тому же домену доменных служб Active Directory, что и существующие серверы, имеет все необходимые роли и компоненты и для него правильно настроено сетевое взаимодействие.
...
![Добавление третий сервер в кластер с двумя узлами](media/add-nodes/Scaling-2-to-3.png)

При наличии двух серверов можно создать только тома с двухсторонним зеркалированием (сопоставимо с распределенными массивами RAID-1). При наличии трех серверов вы можете создать трехсторонние зеркальные тома для улучшенной отказоустойчивости. Рекомендуем по возможности использовать трехстороннее зеркалирование.
...
![Добавление четвертый сервер в кластер из трех узлов](media/add-nodes/Scaling-3-to-4.png)
...
![Анимация, демонстрирующая добавить диски в систему](media/add-nodes/Scale-Up.gif)
...
## Оптимизировать использование дисков после добавления дисков или серверов

Со временем диски при добавлении или удалении распределения данных между диски в пул может стать разного размера. В некоторых случаях это может привести некоторые диски, заполненными, тогда как другие диски в пуле будут использоваться гораздо меньше.

Для обеспечения распределения диска даже по всему пулу локальных дисковых пространств автоматически оптимизирует использование диска после добавления дисков или серверов в пул (это ручной процесс для дисковых пространств системах, которые используют полками SAS Общие). Оптимизация начинается 15 минут после добавления новых дисков в пул. Оптимизация пула работает как операцию с низким приоритетом фона, поэтому может занять несколько часов или дней для завершения, особенно в том случае, если вы используете больших жестких дисков.

Оптимизация использует два задания — один под названием *оптимизировать* и один под названием *заново сбалансировать* — и вы можете отслеживать их выполнение с помощью следующей команды:

```powershell
Get-StorageJob
```

Вы можете вручную оптимизировать пул носителей с помощью командлета [Оптимизация StoragePool](https://docs.microsoft.com/powershell/module/storage/optimize-storagepool?view=win10-ps) . Вот пример.
