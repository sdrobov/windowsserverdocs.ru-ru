...
Сначала создайте новые шаблоны уровня хранилища с помощью командлета `New-StorageTier`. Это необходимо сделать только один раз, а затем каждый созданный том может ссылаться на этот шаблон. Укажите `-MediaType` дисков емкости и, при необходимости, `-FriendlyName` по своему усмотрению. Не изменяйте другие параметры.
...
Если диски емкости являются твердотельными накопителями (SSD), установите вместо этого `-MediaType` значение `SSD`. Не изменяйте другие параметры.
...
Чтобы использовать вложенное двустороннее зеркало, сослаться на шаблон уровня `NestedMirror` и укажите размер. Например:
...
Чтобы использовать вложенную контрольную четность с зеркальным отображением, сослаться на шаблоны уровней `NestedMirror` и `NestedParity` и указать два размера: по одному для каждой части тома (сначала зеркальный, а второй, четность). Например, чтобы создать том 1 500 ГБ с 20% вложенной двусторонней зеркальной копии и 80% вложенной четности, выполните:
