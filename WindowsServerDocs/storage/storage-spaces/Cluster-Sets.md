...
1. Настройте клиент управления под управлением Windows Server 2019.
...
С наборами кластера эти шаги не обязательны, и только одна команда требуется.  Во-первых следует установить все сети, должны быть доступны для миграции с помощью команды:

    Set-VMHost -UseAnyNetworkMigration $true

Например я хочу переместить виртуальной машины значение кластера CLUSTER1 NODE2 CL3 на CLUSTER3.  Одной команды будет следующим:
...
**Ответ:** Наборы кластера — это новая технология, представленные в Windows Server 2019 года, поэтому таким образом, не существует в предыдущих выпусках. Кластеров под управлением ОС нижнего уровня не могут присоединяться к набора кластера. Тем не менее последовательного обновления технологии кластера операционной системы должен предоставить функции миграции, которые вы ищете путем обновления до Windows Server 2019 эти кластеры.
