...
|Suboptimal (Неоптимальное)    |Данные не записываются на диски равномерно. <br><br>**Действие**: Оптимизируйте использование дисков в пуле носителей, запустив командлет [optimize-StoragePool](https://technet.microsoft.com/itpro/powershell/windows/storage/optimize-storagepool) .|
...
|---------            |---------          |
|«ОК»,|Диск исправен.|
|Эксплуатация|Диск выполняет некоторые внутренние действия по обслуживанию. После завершения действия диск должен вернуться в состояние работоспособности *ОК* .|
...
|Starting|Диск подготавливается к работе. Это должно быть временное состояние. После завершения подготовки диск должен перейти в другое рабочее состояние.|
