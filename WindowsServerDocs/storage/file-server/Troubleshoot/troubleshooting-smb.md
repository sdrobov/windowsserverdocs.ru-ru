...
> [!NOTE]  
> Команда netsh trace создает ETL-файл. ETL-файлы можно открывать только в анализаторе сообщений (MA) и сетевой монитор 3,4 (Настройка средства синтаксического анализа для сетевой монитор \> окон анализаторов).
...
1. Всегда проверяйте ошибки SMB по спецификации протокола MS-SMB2. Многие ошибки SMB являются небезопасными (неопасными). Ознакомьтесь со следующими сведениями, чтобы определить причину, по которой SMB возвращала ошибку, прежде чем вы завершите, что эта ошибка связана с одной из следующих проблем.
   ...
   - В разделе [синтаксис сообщений MS-SMB2](/openspecs/windows_protocols/ms-smb2/6eaf6e75-9c23-4eda-be99-c9223c60b181) описывается каждая команда SMB и ее параметры.
    
   - В разделе об [обработке клиента MS-SMB2](/openspecs/windows_protocols/ms-smb2/df0625a5-6516-4fbe-bf97-01bef451cab2) описывается, как клиент SMB создает запросы и реагирует на сообщения сервера.

   - В разделе [обработка сервера MS-SMB2](/openspecs/windows_protocols/ms-smb2/e1d08834-42e0-41ca-a833-fc26f5132a6f) описывается, как SMB-сервер создает запросы и отвечает на запросы клиентов.

2. Проверьте, отправляет ли команда TCP Reset сразу после \_ команды фсктл Validate \_ \_ info (проверка согласованности). Если это так, см. следующие сведения:
...
Двоичные файлы клиента SMB, перечисленные в разделе **% WINDIR% \\ \\ драйверы system32**:
...
Двоичные файлы SMB Server, указанные в разделе **% WINDIR% \\ \\ драйверы system32**:
...
[Сценарий обмена пакетами протокола SMB (Майкрософт)](/windows/win32/fileio/microsoft-smb-protocol-packet-exchange-scenario)
