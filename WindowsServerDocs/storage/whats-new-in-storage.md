...
Дополнительные сведения о миграции службы хранилища, см. в разделе [Общие сведения о службе миграции хранилища](storage-migration-service/overview.md).
...
- **Сжатие томов ReFS и дедупликации**
    ...
    Store в 10 раз больше данных на одном томе с сжатие файловой системы ReFS и дедупликации. (Он имеет [одним щелчком мыши](https://www.youtube.com/watch?v=PRibTacyKko&feature=youtu.be) Включение с помощью Windows Admin Center.) Хранилище блоков переменного размера с необязательным сжатие увеличивает показатели сохранения, хотя многопоточных постобработки архитектуры сохраняет минимальное влияние на производительность. Поддерживает тома размером до 64 ТБ и будут дедуплицировать первые 4 ТБ каждого файла.
