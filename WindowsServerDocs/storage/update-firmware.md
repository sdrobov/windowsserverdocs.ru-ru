...
title: "Обновление встроенного ПО дисков в Windows Server2016"
...
---
# <a name="updating-drive-firmware-in-windows-server-2016"></a>Обновление встроенного ПО дисков в Windows Server2016
>Область применения: Windows 10, Windows Server (Semi-Annual Channel), Windows Server 2016
...
Диски обычно не выполняют запросы ввода-вывода во время активации нового образа встроенного ПО. Время, требуемое для активации диска, зависит от его конструкции и типа обновляемого встроенного ПО. По нашим наблюдениям, этот период составляет от менее чем 5секунд до более чем 30секунд.
...
Чтобы обеспечить стабильное развертывание и достаточное время для проверки нового образа встроенного ПО, между обновлениями нескольких серверов присутствует значительная задержка. По умолчанию служба работоспособности ожидает 7дней перед обновлением второго<sup></sup> сервера. Все последующие обновления серверов (третьего<sup></sup>, четвертого<sup></sup> и т.д.) выполняются с задержкой в 1день. Если администратор находит встроенное ПО нестабильным или нежелательным по иным причинам, он может в любое время остановить дальнейшее развертывание службой работоспособности. Если встроенное ПО прошло предварительную проверку и требуется ускорить развертывание, можно уменьшить эти значения по умолчанию с дней до часов или минут.
...
Если вы хотите увидеть работу службы работоспособности и узнать дополнительные сведения о ее механизме развертывания, просмотрите этот видеоролик: https://channel9.msdn.com/Blogs/windowsserver/Update-Drive-Firmware-Without-Downtime-in-Storage-Spaces-Direct
...
В Windows Server2016 со службой работоспособности, развернутой в локальных дисковых пространствах, эту операцию можно выполнять без перевода рабочих нагрузок в автономный режим, при условии, что диски поддерживают обновление встроенного ПО в Windows Server.

### <a name="what-happens-if-the-update-fails"></a>Что произойдет в случае сбоя обновления?

Обновление может завершиться ошибкой по различным причинам, некоторые из которых указаны далее. 1) Диск не поддерживает нужные команды для обновления его встроенного ПО системой Windows. В этом случае новый образ встроенного ПО никогда не активируется, и диск продолжает работать со старым образом. 2) Не удается скачать образ или применить его к диску (несоответствие версий, повреждение образа и т.д). В этом случае диск выполняет команду активации с ошибкой. При этом старый образ по-прежнему продолжит работать.
