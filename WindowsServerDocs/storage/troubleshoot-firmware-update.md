...
- [Обновление встроенного по дисков без простоев в Storage Spaces Direct](https://channel9.msdn.com/Blogs/windowsserver/Update-Drive-Firmware-Without-Downtime-in-Storage-Spaces-Direct)
...
2.  См. в каталог Windows Server Catalog для идентификации устройства SAS, которые успешно получили (AQ обновления встроенного по https://www.windowsservercatalog.com/)
...
Событие 507 ETW из канала показывает, что запрос SCSI SRB завершился сбоем, и предоставляет дополнительные сведения о том, что SenseKey был равен "5" (неправомерный запрос) и что сведения AdditionalSense были равны "36" (неправомерное поле в CDB).
...
Помимо рабочего канала ClassPnP StorAHCI и StorNVMe будут записывать в журнал коды возврата для протокола драйвера в следующем канале ETW:
...
Ниже приведен пример обновления встроенного по на неудачное устройства SATA, за недопустимого скачивание образа (идентификатор события: 258):
...
- Код возврата: 0 (0000 0000) (н/д - смысла, так как полезные данные не было передано)
- Особенности 15 (0000 1111) (бит 1 имеет значение "1" и указывает «прервать»)
- SectorCount: 0 (0000 0000) (НЕДОСТУПЕН)
- DriveHead: 160 (1010 0000) (н/д: только устаревшие разряды)
