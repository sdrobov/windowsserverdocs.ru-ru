...
| System                        | Операционная система                                            | Требуется для     |
|-------------------------------|-------------------------------------------------------------|------------------|
...
**Рис. 1. репликация между серверами**  
...
7.  Настройте роли следующим образом.  
    ...
    -   **Метод центра администрирования Windows**
        ...
        3. Выберите **компоненты** > **Реплика хранилища**, а затем нажмите кнопку **установить**.
...
3. Предоставьте общий доступ к данным на исходном сервере и сделайте его доступным через пространство имен DFS. Это важно для того, чтобы пользователи сохранили доступ к данным в случае аварийного изменения имени сервера на имя резервного расположения.  
   а)  На сервере назначения можно создать аналогичные общие файловые ресурсы. Во время обычной работы они будут недоступны по сети.   
   б.  Не добавляйте целевой сервер в пространство имен пространства имен DFS или, если это необходимо, убедитесь, что все целевые объекты папки отключены.  
...
![портал Azure Показать группу ресурсов, добавленную с помощью ExpressRoute](media/Server-to-Server-Storage-Replication/express-route-resource-group.png)
...
1. [Создайте виртуальную машину Azure](https://docs.microsoft.com/azure/virtual-machines/windows/quick-create-portal) со следующими параметрами (показаны на рис. 5):
    ...
    ![создать виртуальную машину, в которой отображаются параметры сети ExpressRoute](media/Server-to-Server-Storage-Replication/azure-vm-express-route.png)
