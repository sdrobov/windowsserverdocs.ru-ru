---
title: Репликация сервера на сервер хранилища
description: Как настроить и использовать реплику хранилища для репликации между серверами в Windows Server, включая Windows Admin Center и PowerShell.
...
ms.locfileid: "66447621"
---
# <a name="server-to-server-storage-replication-with-storage-replica"></a>Репликация хранилища сервера на сервер с репликой хранилища
...
Управление репликой хранилища, можно использовать [Windows Admin Center](../../manage/windows-admin-center/overview.md) или PowerShell.

Вот обзорный Видеоролик с помощью реплики хранилища в Windows Admin Center.
> [!video https://www.microsoft.com/videoplayer/embed/3aa09fd4-867b-45e9-953e-064008468c4b?autoplay=false]


## <a name="prerequisites"></a>предварительные требования  

* Лес Active Directory доменные службы (не обязательно для запуска Windows Server 2016).  
* Два сервера под управлением Windows Server 2019 или Windows Server 2016 Datacenter Edition. Если вы используете Windows Server 2019, воспользуйтесь Standard Edition при ОК репликации только одного тома размером до 2 ТБ.  
...
* Сеть между серверами должна иметь достаточную пропускную способность для ваших рабочих нагрузок ввода-вывода, а средняя задержка приема-передачи должна составлять 5 мс для синхронной репликации. Асинхронная репликация не рекомендацию задержки.<br>
Если вы выполняете репликацию между локальными серверами и виртуальными машинами Azure, необходимо создать сетевого соединения между локальными серверами и виртуальные машины Azure. Чтобы сделать это, используйте [Express Route](#add-azure-vm-expressroute), [сеть-сеть VPN-шлюза](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-portal), и устанавливать программное обеспечение VPN на виртуальных машинах Azure, чтобы подключить их с локальной сетью.
...
## <a name="windows-admin-center-requirements"></a>Требования к Windows Admin Center

Чтобы использовать реплику хранилища и Windows Admin Center вместе, вам потребуется следующее:

| Система                        | Операционная система                                            | Требуется для     |
|-------------------------------|-------------------------------------------------------------|------------------|
| Два сервера <br>(любое сочетание локального оборудования, виртуальных машин и облачных виртуальных машинах, включая виртуальные машины Azure)| Windows Server 2019 г., Windows Server 2016 или Windows Server (Semi-Annual Channel) | Реплика хранилища  |
| Один компьютер                     | Windows 10                                                  | Windows Admin Center |

> [!NOTE]
> Сейчас нельзя использовать на сервере Windows Admin Center для управления репликой хранилища.
...
## <a name="step-1-install-and-configure-windows-admin-center-on-your-pc"></a>Шаг 1. Установка и настройка Windows Admin Center на Компьютере

Если вы используете Windows Admin Center для управления репликой хранилища, следуйте инструкциям ниже, чтобы подготовить компьютер для управления репликой хранилища.
1. Скачайте и установите [Windows Admin Center](../../manage/windows-admin-center/overview.md).
2. Скачайте и установите [средства удаленного администрирования сервера](https://www.microsoft.com/download/details.aspx?id=45520).
    - Если вы используете Windows 10, версия 1809 или более поздней версии, установите «средства удаленного администрирования сервера: Модуль службы хранилища реплики для Windows PowerShell» из компонентов по требованию.
3. Откройте сеанс PowerShell от имени администратора, выбрав **запустить** кнопку, введя **PowerShell**, щелкните правой кнопкой мыши **Windows PowerShell,** и выбрав  **Запуск от имени администратора**.
4. Введите следующую команду, чтобы включить протокол WS-Management на локальном компьютере и установить конфигурацию по умолчанию для удаленного управления на стороне клиента.

    ```PowerShell
    winrm quickconfig
    ```

5. Тип **Y** включить службы удаленного управления Windows и включить исключение брандмауэра для WinRM.

## <a name="provision-os"></a>Шаг 2. Подготовка операционной системы, функций, ролей, хранилища и сети

1.  Установите Windows Server на обоих узлах серверов, указав тип установки Windows Server **(возможности рабочего стола)** . 
 
    Для использования виртуальной Машины Azure, подключенная к сети через подключение ExpressRoute, см. в разделе [Добавление виртуальной Машины Azure, подключенная к сети через ExpressRoute](#add-azure-vm-expressroute).

3.  Добавьте сведения о сети, присоединение серверов к домену, в котором управление Windows 10 ПК (Если вы используете одну) и затем перезапустите серверы.  
...
3.  Подключите первый набор корпуса хранилища JBOD, целевому объекту iSCSI, сеть SAN FC или хранилище на локальном жестком диске (DAS) на сервер, на сайте **Redmond**.  

4.  Второй набор хранилищ соединиться с сервером узла **Bellevue**.  
...
6.  Убедитесь, что для параметров BIOS или UEFI настроена высокая производительность, например отключено C-состояние, установлена скорость QPI, включена архитектура NUMA и установлена максимально возможная частота памяти. Убедитесь, что управление питанием в Windows Server имеет значение для высокой производительности. При необходимости перезагрузите компьютер.  

7.  Настройте роли следующим образом.  

    -   **Метод Windows Admin Center**
        1. В Windows Admin Center перейдите в диспетчер серверов и затем выберите один из серверов.
        2. Перейдите к **роли и компоненты**.
        3. Выберите **функции** > **реплики хранилища**, а затем нажмите кнопку **установить**.
...
8.  Настройте хранилище следующим образом:  
    ...
    - **Для хранилища на локальном жестком диске:**  

        -   Убедитесь, хранилища не содержит системные тома, файл подкачки или файлы дампа.  
...
10. Изучите **TestSrTopologyReport.html** отчета показано на рисунке 2, чтобы убедиться, что требования для хранения реплики.  
...
## <a name="step-3-set-up-server-to-server-replication"></a>Шаг 3. Настройка репликации между серверами
### <a name="using-windows-admin-center"></a>С помощью Windows Admin Center

1. Добавьте исходный сервер.
    1. Выберите **добавить** кнопки.
    2. Выберите **добавить подключение к серверу**.
    3. Введите имя сервера, а затем выберите **отправить**.
2. На **все подключения** выберите исходный сервер.
3. Выберите **реплики хранилища** на панели инструментов.
4. Выберите **New** для создания новой связи.
5. Укажите сведения о партнерстве, а затем выберите **создать**. <br>
   ![Экран новой связи, которой отображаются сведения о связи, таких как размер журнала 8 ГБ.](media/Storage-Replica-UI/Honolulu_SR_Create_Partnership.png)

    **Рис. 3. Создание новой связи**

> [!NOTE]
> Удаление партнерства с репликой хранилища в Windows Admin Center не удаляет имя группы репликации.

### <a name="using-windows-powershell"></a>Использование Windows PowerShell

Теперь мы перейдем к настройке межсерверной репликации с помощью Windows PowerShell. Все описанные ниже шаги следует выполнять на узлах непосредственно или с помощью компьютера удаленного управления, содержащий средства удаленного администрирования сервера Windows Server.  
...
Теперь можно приступить к управлению инфраструктурой межсерверной репликации и ее использованию. Все описанные ниже шаги можно выполнять на узлах непосредственно или с помощью компьютера удаленного управления, содержащий средства удаленного администрирования сервера Windows Server.  
...
* При этом не реплицируются в используемые и открытые файлы.  
* При этом не реплицируются синхронно.  
...
* Как правило, он настраивается несколько главных серверов, которое позволяет вносить изменения в обоих направлениях, возможной перезаписью более новые данные.  
...
* Оно поддерживает только репликацию "один к одному" между томами. Существует возможность реплицировать разные тома между несколькими серверами.  
* Она поддерживает асинхронную репликацию, она не предназначена для низкой пропускной способностью и сетей с высокой задержкой.  
* Запрещает пользователю доступ к защищенным данным на сервере назначения во время репликации  
...
3. Совместное использование данных на исходном сервере и сделайте его доступным через пространство имен DFS. Это важно для того, чтобы пользователи сохранили доступ к данным в случае аварийного изменения имени сервера на имя резервного расположения.  
   ...
   2.  Не добавляйте конечного сервера к пространству имен пространств имен DFS, и в противном случае убедитесь, что всем целевым папкам на нем отключено.  
...
8. Добавьте сервер и его общие папки в качестве конечного объекта папки пространства имен DFS.   
...
## <a name="add-azure-vm-expressroute"></a>Добавление виртуальной Машины Azure, подключенная к сети через ExpressRoute

1. [Создание канала ExpressRoute на портале Azure](https://docs.microsoft.com/azure/expressroute/expressroute-howto-circuit-portal-resource-manager).<br>После утверждения ExpressRoute добавляется подписка — группу ресурсов, перейдите к **групп ресурсов** на просмотр этой новой группы. Запишите имя виртуальной сети.
![Портала Azure: группа ресурсов, добавленные с помощью ExpressRoute](media/Server-to-Server-Storage-Replication/express-route-resource-group.png)
    
    **Рис. 4. Ресурсы, связанные с ExpressRoute - запишите имя виртуальной сети**
1. [Создать новую группу ресурсов](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-portal).
1. [Добавьте группу безопасности сети](https://docs.microsoft.com/azure/virtual-network/virtual-networks-create-nsg-arm-pportal). При его создании, выберите идентификатор подписки, связанный с ExpressRoute, вы создали и выберите группу ресурсов, который вы только что создали также.
<br><br>Все правила безопасности относительно входящего и исходящего трафика, которые необходимо добавьте в группу безопасности сети. Например может потребоваться разрешить доступ с удаленного рабочего стола к виртуальной Машине.
1. [Создайте виртуальную Машину Azure](https://docs.microsoft.com/azure/virtual-machines/windows/quick-create-portal) со следующими параметрами (показано на рис. 5):
    - **Общедоступный IP-адрес**: Нет
    - **Виртуальная сеть**: Выберите виртуальную сеть, которому был на из группы ресурсов, добавленные с помощью ExpressRoute.
    - **Группа безопасности сети (брандмауэр)** : Выберите группу безопасности сети, который вы создали ранее.
    ![Создание виртуальной машины, показывающий параметры сети для ExpressRoute](media/Server-to-Server-Storage-Replication/azure-vm-express-route.png)
    **рис. 5: Создание виртуальной Машины при выделении параметры сети для ExpressRoute**
1. После создания виртуальной Машины, см. в разделе [шаг 2: Подготовка операционной системы, функций, ролей, хранилища и сети](#provision-os).
