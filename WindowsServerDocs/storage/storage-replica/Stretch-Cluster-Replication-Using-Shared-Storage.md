...
14. **(Необязательно.)** Настройте сеть кластера и Active Directory для ускоренной отработки отказа сайта DNS. Для этого можно использовать программно-конфигурируемую сеть Hyper-V, распределенные виртуальные локальные сети, устройства для абстрагирования сети, сокращение срока жизни DNS и другие распространенные методы.
    ...
    Дополнительные сведения см. в сеансе Microsoft Ignite. [Растягивание отказоустойчивых кластеров и использование реплики хранилища в Windows Server vNext](http://channel9.msdn.com/Events/Ignite/2015/BRK3487) и [Включение уведомлений об изменениях между сайтами — как и зачем?](http://blogs.technet.com/b/qzaidi/archive/2010/09/23/enable-change-notifications-between-sites-how-and-why.aspx) запись блога.  
...
4.  Чтобы изменить размер журнала с 8 ГБ по умолчанию, используйте **Set-срграуп** как в исходной, так и в целевой группах реплики хранилища.   Например, чтобы задать для всех журналов размер в 2ГБ, выполните следующее команды:  
