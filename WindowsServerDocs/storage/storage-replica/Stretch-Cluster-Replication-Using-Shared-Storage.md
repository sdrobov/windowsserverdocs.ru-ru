...
-   Минимум два сервера с установленным выпуском Windows Server 2016 Datacenter. Поддержка кластера, содержащего до 64 узлов.  
...
1.  Установите Windows Server 2016 (выпуск Datacenter) на всех узлах сервера. Если доступен выпуск Standard, не выбирайте его, так как он не содержит реплику хранилища. Поддерживается использование режимов установки Windows Server Desktop Experience, Core и Nano.  
    > [!IMPORTANT]
...
4.  Настройте файловый ресурс-свидетель или облако-свидетель для предоставления кворума в случае потери сайта.  
    ...
    > [!WARNING]  
    > Дополнительные сведения о конфигурации кворума см. в разделе [Настройка свидетеля](https://technet.microsoft.com/library/jj612870.aspx) руководства о настройке кворума и управлении им на отказоустойчивом кластере Windows Server 2012. Дополнительные сведения о командлете `Set-ClusterQuorum` см. в статье [Set-ClusterQuorum](https://technet.microsoft.com/library/hh847275.aspx).  
...
Теперь вы приступите к управлению растянутым кластером и к его эксплуатации. Все описанные ниже шаги можно выполнять на узлах кластера непосредственно или с помощью компьютера удаленного управления, на котором установлены средства удаленного администрирования сервера Windows Server 2016.  
...
4.  Чтобы изменить размер журнала (8ГБ по умолчанию) в Windows Server 2016, щелкните исходный и конечный диски журналов правой кнопкой мыши, выберите вкладку **Журнал репликации**, а затем установите одинаковый размер для дисков.  
...
4.  Чтобы изменить размер журнала (8 ГБ по умолчанию) в Windows Server 2016, используйте командлет **Set-SRGroup** в группах реплик хранилищ источника и назначения.   Например, чтобы задать для всех журналов размер в 2ГБ, выполните следующее команды:  
