...
-   Минимум два сервера с установленным выпуском Windows Server2016 Datacenter. Поддержка кластера, содержащего до 64узлов.  
...
1.  Установите Windows Server2016 (выпуск Datacenter) на всех узлах сервера. Если доступен выпуск Standard, не выбирайте его, так как он не содержит реплику хранилища. Поддерживается использование режимов установки Windows Server Desktop Experience, Core и Nano.  
    > [!IMPORTANT]
...
3.  Создайте вычислительный кластер Hyper-V. Убедитесь, что имя кластера содержит не более 15 символов. В примере ниже использован кластер **SR-SRVCLUS**.  
...
4.  Настройте файловый ресурс-свидетель или облако-свидетель для предоставления кворума в случае потери сайта.  
    ...
    > [!WARNING]  
    > Дополнительные сведения о конфигурации кворума см. в разделе [Настройка свидетеля](http://technet.microsoft.com/library/jj612870.aspx) руководства о настройке кворума и управлении им на отказоустойчивом кластере Windows Server2012. Дополнительные сведения о командлете `Set-ClusterQuorum` см. в статье [Set-ClusterQuorum](http://technet.microsoft.com/library/hh847275.aspx).  

5.  Просмотрите статью [Network Recommendations for a Hyper-V Cluster in Windows Server 2012](http://technet.microsoft.com/library/dn550728.aspx) (Рекомендации в отношении сети для кластера Hyper-V в Windows Server2012) и убедитесь, что сеть кластера настроена оптимально.  
...
7.  Выполните шаги 7–10 в руководстве [Deploy a Hyper-V Cluster](http://technet.microsoft.com/library/jj863389.aspx) (Развертывание кластера Hyper-V) на сайте **Redmond**, чтобы создать тестовую виртуальную машину. Это нужно только для того, чтобы обеспечить нормальную работу кластера с двумя узлами, которые совместно используют хранилище на первом тестовом сайте.  

9.  Если вы создаете растянутый кластер с двумя узлами, перед продолжением необходимо добавить все хранилища. Для этого откройте сеанс PowerShell с разрешениями администратора на узлах кластера и выполните следующую команду: `Get-ClusterAvailableDisk -All | Add-ClusterDisk`.

    Это ожидаемое поведение вWindows Server 2016.

10. Запустите Windows PowerShell и используйте командлет `Test-SRTopology`, чтобы определить, все ли требования для реплики хранилища выполнены.  
...
11. Изучите отчет **TestSrTopologyReport-<дата>.html**, чтобы убедиться в выполнении всех требований к реплике хранилища, и запишите прогноз относительно времени начальной синхронизации и рекомендации для журнала.  
...
8.  После этого удалите тестовую виртуальную машину. Добавьте настоящие тестовые виртуальные машины, необходимые для дальнейшей оценки, в предложенный исходный узел.  

9. Настройте доступность сведений о сайтах растянутого кластера таким образом, чтобы сервер **SR-SRV01** и **SR-SRV02** находились на сайте **Redmond**, **SR-SRV03** и **SR SRV04**— на сайте **Bellevue**, а сайт **Redmond** был предпочтительным для хранения узлов исходного хранилища и виртуальных машин.  
   ...
   > [!NOTE]
   > Невозможно настроить доступность сведений о сайтах с помощью диспетчера отказоустойчивости кластеров в Windows Server2016.  

10. **(Необязательно.)** Настройте сеть кластера и Active Directory для ускоренной отработки отказа сайта DNS. Для этого можно использовать программно-конфигурируемую сеть Hyper-V, распределенные виртуальные локальные сети, устройства для абстрагирования сети, сокращение срока жизни DNS и другие распространенные методы.
...
11. **(Необязательно.)** Настройте устойчивость виртуальной машины, чтобы работа гостей не прерывалась надолго во время сбоев узлов. Вместо этого будет выполняться отработка отказа с переключением на новое хранилище источника репликации за 10секунд.  
    ...
    > [!NOTE]
    >  Невозможно настроить устойчивость виртуальной машины с помощью диспетчера отказоустойчивости кластеров в Windows Server2016.  
...
2.  Создайте вычислительный кластер Hyper-V (необходимо указать собственный статический IP-адрес для кластера). Убедитесь, что имя кластера содержит не более 15 символов.  
...
4.  Просмотрите статью [Network Recommendations for a Hyper-V Cluster in Windows Server 2012](http://technet.microsoft.com/library/dn550728.aspx) (Рекомендации в отношении сети для кластера Hyper-V в Windows Server 2012) и убедитесь, что сеть кластера настроена оптимально.  

9.  Если вы создаете растянутый кластер с двумя узлами, перед продолжением необходимо добавить все хранилища. Для этого откройте сеанс PowerShell с разрешениями администратора на узлах кластера и выполните следующую команду: `Get-ClusterAvailableDisk -All | Add-ClusterDisk`.

    Это ожидаемое поведение в Windows Server 2016.

5.  Выполните шаги 7–10 в руководстве [Deploy a Hyper-V Cluster](http://technet.microsoft.com/library/jj863389.aspx) (Развертывание кластера Hyper-V) на сайте **Redmond**, чтобы создать тестовую виртуальную машину. Это нужно только для того, чтобы обеспечить нормальную работу кластера с двумя узлами, которые совместно используют хранилище на первом тестовом сайте.  

6.  После этого удалите тестовую виртуальную машину. Добавьте настоящие тестовые виртуальные машины, необходимые для дальнейшей оценки, в предложенный исходный узел.  
...
8.  **(Необязательно.)** Настройте сеть кластера и Active Directory для ускоренной отработки отказа сайта DNS. Для этого можно использовать программно-конфигурируемую сеть Hyper-V, распределенные виртуальные локальные сети, устройства для абстрагирования сети, сокращение срока жизни DNS и другие распространенные методы.  

    Чтобы получить дополнительные сведения, ознакомьтесь с материалами семинара конференции Microsoft Ignite [Stretching Failover Clusters and Using Storage Replica in Windows Server vNext](http://channel9.msdn.com/Events/Ignite/2015/BRK3487) (Растягивание отказоустойчивых кластеров и использование реплики хранилища в Windows Server vNext) и записью блога [Enable Change Notifications between Sites— How and Why](http://blogs.technet.com/b/qzaidi/archive/2010/09/23/enable-change-notifications-between-sites-how-and-why.aspx) (Включение уведомлений об изменениях между сайтами: выполнение процесса и преимущества).  

9. **(Необязательно.)** Настройте устойчивость виртуальной машины, чтобы работа гостей не прерывалась надолго во время сбоев узлов. Вместо этого будет выполняться отработка отказа с переключением на новое хранилище источника репликации за 10секунд.  
...
### <a name="BKMK_FileServer"></a> Настройка кластерного файлового сервера для общего использования  
...
2.  Проверьте предложенный кластер и проанализируйте результаты, чтобы убедиться, что можно продолжить.  
    >[!NOTE]
    >Из-за использования асимметричного хранилища при проверке кластера могут возникнуть ошибки хранилища.   
3. Создайте кластер хранилища файлового сервера для общего использования. Убедитесь, что имя кластера содержит не более 15 символов. В примере ниже использован кластер SR-SRVCLUS.  

2.  Настройте файловый ресурс-свидетель или облако-свидетель для предоставления кворума в случае потери сайта.  
    >[!NOTE]
    > Windows Server2016 теперь предоставляет возможность использования облака-свидетеля Azure. Можно выбрать этот вариант кворума вместо файлового ресурса-свидетеля.                                                                                                                                                                             
    >[!NOTE]
    >  Дополнительные сведения о конфигурации кворума см. в разделе [Настройка свидетеля](https://technet.microsoft.com/library/jj612870.aspx) руководства о настройке кворума и управлении им на отказоустойчивом кластере Windows Server2012. Дополнительные сведения о командлете Set-ClusterQuorum см. в статье [Set-ClusterQuorum](https://technet.microsoft.com/library/hh847275.aspx). 

9.  Если вы создаете растянутый кластер с двумя узлами, перед продолжением необходимо добавить все хранилища. Для этого откройте сеанс PowerShell с разрешениями администратора на узлах кластера и выполните следующую команду: `Get-ClusterAvailableDisk -All | Add-ClusterDisk`.

    Это ожидаемое поведение в Windows Server 2016.

1.  Убедитесь, что сеть кластера настроена оптимально.  
    >[!NOTE]
    > Прежде чем перейти к следующему шагу, установите роль файлового сервера.   |  

1.  В разделе **Роли** щелкните **Настройка роли**. Просмотрите экран **Перед началом работы** и нажмите кнопку **Далее**.  

2.  Выберите **Файловый сервер** и нажмите кнопку **Далее**.  

3.  Не снимайте флажок **Файловый сервер для общего использования** и нажмите кнопку **Далее**.  

4.  Укажите имя **точки доступа клиента** (не более 15 символов) и нажмите кнопку **Далее**.  

5.  Выберите диск, который будет использоваться в качестве тома данных, и нажмите кнопку **Далее**.  

6.  Проверьте параметры и нажмите кнопку **Далее**. Нажмите кнопку **Готово**.  

7.  Щелкните новую роль файлового сервера правой кнопкой мыши и выберите пункт **Добавить общий файловый ресурс**. Следуйте указаниям мастера для настройки общих ресурсов.  

8.  (Необязательно.) Добавьте другую роль файлового сервера, использующую другое хранилище на этом сайте.  
...
1.  (Необязательно.) Настройте сеть кластера и Active Directory для ускоренной отработки отказа сайта DNS. Для этого можно использовать распределенные виртуальные локальные сети, устройства для абстрагирования сети, сокращение срока жизни DNS и другие распространенные методы.  
...
4. Убедитесь, что сеть кластера настроена оптимально.  

5.  Настройте роль файлового сервера. Например:   
 ...
 6. Настройте доступность сведений о сайтах растянутого кластера таким образом, чтобы сервер SR-SRV01 и SR-SRV02 находились на сайте Redmond, SR-SRV03 и SR SRV04— на сайте Bellevue, а сайт Redmond был предпочтительным для хранения узлов исходного хранилища и виртуальных машин.  
...
7.  (Необязательно.) Настройте сеть кластера и Active Directory для ускоренной отработки отказа сайта DNS. Для этого можно использовать распределенные виртуальные локальные сети, устройства для абстрагирования сети, сокращение срока жизни DNS и другие распространенные методы.  
...
9. На экране "Сводка" обратите внимание на результаты в диалоговом окне завершения. Отчет можно просмотреть в браузере.  

10. На этом этапе настроено партнерство реплики хранилища между двумя частями кластера. Репликация все еще выполняется. Просмотреть состояние репликации с помощью графического средства можно несколькими способами.  
...
1.  Добавьте исходное хранилище данных в качестве CSV-файла только в кластер. Чтобы получить сведения о размере, разделах и структуре томов доступных дисков, используйте следующие команды:  
...
2.  Задайте правильный диск для CSV-файла с помощью следующих команд:  
...
3.  Настройте растянутый кластер, указав следующее:  
...
5.  Для получения состояния источника и назначения репликации в растянутом кластере используйте `Get-SRGroup` и `Get-SRPartnership`, чтобы просмотреть настроенное состояние репликации в растянутом кластере.  
...
Теперь вы приступите к управлению растянутым кластером и к его эксплуатации. Все описанные ниже шаги можно выполнять на узлах кластера непосредственно или с помощью компьютера удаленного управления, на котором установлены средства удаленного администрирования сервера Windows Server2016.  
...
3.  Чтобы изменить источник и назначение репликации в растянутом кластере, используйте следующие методы:  
    ...
    4.  Чтобы выполнить незапланированную отработку отказа для изменения направления репликации с одного сайта на другой, отключите оба узла на одном сайте.  
        ...
        > [!NOTE]
        > В Windows Server2016 может потребоваться использовать диспетчер отказоустойчивости кластеров или Move-ClusterGroup для перемещения конечных дисков на другой сайт вручную после перехода узлов в оперативный режим.  

        > [!NOTE]
        > Реплика хранилища отключает конечные тома. Это сделано намеренно.  

4.  Чтобы изменить размер журнала (8ГБ по умолчанию) в Windows Server2016, щелкните исходный и конечный диски журналов правой кнопкой мыши, выберите вкладку **Журнал репликации**, а затем установите одинаковый размер для дисков.  
...
3.  Чтобы изменить источник и назначение репликации в растянутом кластере, используйте следующие методы:  
    ...
    3.  Чтобы выполнить незапланированную отработку отказа для изменения направления репликации с одного сайта на другой, отключите питание обоих узлов на одном сайте.  

        > [!NOTE]  
        > Реплика хранилища отключает конечные тома. Это сделано намеренно.  

4.  Чтобы изменить размер журнала (8ГБ по умолчанию) в Windows Server2016, используйте командлет **Set-SRGroup** в группах реплик хранилищ источника и назначения.   Например, чтобы задать для всех журналов размер в 2ГБ, выполните следующее команды:  
