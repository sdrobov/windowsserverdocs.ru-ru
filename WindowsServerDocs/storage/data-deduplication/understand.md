...
1. **Оптимизация не должна мешать операции записи на диск**  
...
| <a id="job-info-unoptimization"></a>Отмены оптимизации | Задание **отмены оптимизации**, особое задание, которое может выполняться только вручную, отменяет всю оптимизацию, выполненную службой дедупликации, и отключает дедупликацию данных для тома. | [Только по запросу](run.md#disabling-dedup) |
...
| <a id="dedup-term-dedup"></a>дедупликации | Сокращенная форма англоязычного названия дедупликации данных, которая часто используется в PowerShell, интерфейсах API и компонентах Windows Server, а также в сообществе Windows Server. |
...
* *Чем отличается Дедупликация данных из одного экземпляра Store?*  
...
**Можно отслеживать историю запусков заданий дедупликации данных?**  
Да, [все задания дедупликации данных создают записи в журнале событий Windows](run.md#monitoring-dedup).

**Можно изменить расписание по умолчанию для заданий дедупликации данных в моей системе?**  
