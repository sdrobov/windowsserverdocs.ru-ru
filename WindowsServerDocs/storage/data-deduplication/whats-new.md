...
[Дедупликация данных](overview.md) в Windows Server была оптимизирована в контексте производительности, гибкость и управляемость в масштабе частного облака. Дополнительные сведения о программно определяемого хранилища в Windows Server см. в разделе [новые возможности хранилища в Windows Server](../whats-new-in-storage.md).

Дедупликация данных имеет следующие улучшения в Windows Server 2019:

| Функция | Новинка или обновление | Описание |
|---------------|----------------|-------------|
| Поддержка reFS  | Оператор new            | Store до 10 раз больше данных на одном томе с сжатие файловой системы ReFS и дедупликации. (Он имеет [одним щелчком мыши](https://www.youtube.com/watch?v=PRibTacyKko&feature=youtu.be) Включение с помощью Windows Admin Center.) Хранилище блоков переменного размера с необязательным сжатие увеличивает показатели сохранения, хотя многопоточных постобработки архитектуры сохраняет минимальное влияние на производительность. Поддерживает тома размером до 64 ТБ и будут дедуплицировать первые 4 ТБ каждого файла.|

Дедупликация данных имеет следующие дополнительные возможности, начиная с Windows Server 2016:
