...
Если вы не установили прокси-службу службы миграции хранилища на конечный компьютер с Windows Server 2019 или на дестинатон компьютере установлена ОС Windows Server 2016 или Windows Server 2012 R2, это поведение предназначено для разработки. Мы рекомендуем перейти на компьютер под Windows Server 2019 с установленным прокси-сервером, чтобы значительно повысить производительность переноса.  
...
При использовании службы миграции хранилища для переноса файлов в новое место назначения, а также настройки репликация DFS (DFSR) для репликации этих данных с существующего сервера DFSR с помощью нажатой репликации или клонирования базы данных DFSR все файлы експериемце хэш несоответствие и повторно реплицируются. Все потоки данных, потоки безопасности, размеры и атрибуты полностью совпадают после использования SMS для их перемещения. Анализ файлов с помощью ICACLS или журнала отладки клонирования базы данных DFSR открывает:
...
## <a name="error-couldnt-transfer-storage-on-any-of-the-endpoints-when-transfering-from-windows-server-2008-r2"></a>Ошибка "не удалось переместить хранилище на любую из конечных точек" при переносе из Windows Server 2008 R2
