...
При попытке передачи данных с исходного компьютера Windows Server 2008 R2, без передачи данных и возникает ошибка:  
...
## <a name="error-couldnt-transfer-storage-on-any-of-the-endpoints-and-check-if-the-source-device-is-online---we-couldnt-access-it"></a>Ошибка «не удалось перенести хранилище на любой конечной точке» и «Установите флажок, если исходное устройство подключено к сети — мы не удалось получить доступ к его».

При попытке передачи данных с исходного компьютера, некоторые или все общие папки не передаются, сводки ошибкой:

   Не удалось перенести хранилище на любой из конечных точек.
0x9044

Изучив особенности SMB передачи показана ошибка.

   Проверьте, если исходное устройство подключено к сети — мы не удалось получить доступ к его.

Показано, изучив журнал событий StorageMigrationService/Admin:

   Не удалось перенести хранилище.

   Задание: Идентификатор Job1:  
   Область: Завершилось с ошибкой: 36931 появляется сообщение об ошибке 

   Руководство. Проверьте подробности ошибки и убедитесь, что в соответствии с требованиями передачи. Задание передачи не удалось передать любой исходный и конечный компьютеры. Это может быть вызвано компьютере orchestrator не удалось установить подключение источника или назначения компьютеров, из-за правила брандмауэра, или отсутствуют разрешения.

Изучение журнала показано в StorageMigrationService-прокси-сервера/Debug:

   Не удалось проверить передачи 07/02/2019-13:35:57.231 [ошибка]. Код ошибки: 40961, конечной точки источника недоступен или не существует, или учетные данные источника являются недопустимыми или прошедший проверку пользователь не имеет достаточных прав на доступ к нему.
в Microsoft.StorageMigration.Proxy.Service.Transfer.TransferOperation.Validate() в Microsoft.StorageMigration.Proxy.Service.Transfer.TransferRequestHandler.ProcessRequest (FileTransferRequest fileTransferRequest, Guid operationId)    [d:\os\src\base\dms\proxy\transfer\transferproxy\TransferRequestHandler.cs::

Эта ошибка возникает в том случае, если учетной записи миграции не имеет по крайней мере разрешения на чтение к общим папкам SMB. Чтобы обойти эту ошибку, добавьте группу безопасности, содержащую исходной миграции учетной записи к общим ресурсам SMB на исходном компьютере и обеспечьте ей, чтение, изменение или полный доступ. После завершения миграции, можно удалить эту группу. Будущих версиях Windows Server может изменить это поведение, чтобы больше не требуются явные разрешения исходными общими ресурсами.
