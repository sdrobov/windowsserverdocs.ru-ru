...
При использовании службы миграции хранилища для переноса файлов в новое место назначения и последующей настройки репликация DFS (DFSR) для репликации этих данных с существующего сервера DFSR через предварительно заполненную репликацию или клонирование базы данных DFSR все файлы используют хэш. несоответствие и повторно реплицируются. Все потоки данных, потоки безопасности, размеры и атрибуты полностью совпадают после использования SMS для их перемещения. При проверке файлов с помощью ICACLS или журнала отладки клонирования базы данных DFSR выясняется следующее:
...
## <a name="uninstalling-a-cumulutative-update-prevents-storage-migration-service-from-starting"></a>Удаление обновления кумулутативе предотвращает запуск службы миграции хранилища
...
Если вы уже выполнили перемещение один или несколько раз:
 ...
 1. Используйте следующую команду AD PowerShell для контроллера домена, чтобы найти всех измененных пользователей или группы (при изменении Сеарчбасе в соответствии с именем динстрингуишед домена): 
...
При попытке выполнить вырезание с исходного компьютера повышена блокировка на этапе "38% сопоставления сетевых интерфейсов в источнике комнпутер..." в журнале событий SMS появится следующая ошибка:
