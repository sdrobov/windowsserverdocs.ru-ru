...
-   **Проблем "шумный сосед".** По умолчанию служба качества обслуживания хранилища гарантирует, что одна виртуальная машина не может использовать все ресурсы хранилища и влиять на пропускную способность для других виртуальных машин.  

-   **Мониторинг производительности сквозного хранилища.** Отслеживание производительности виртуальных машин, хранящихся на масштабируемом файловом сервере, начинается сразу после их запуска. Сведения о производительности всех работающих виртуальных машин и конфигурации кластера масштабируемых файловых серверов можно просмотреть из одного расположения.  
...
## <a name="BKMK_Overview"></a>Общие сведения о  
...
### <a name="BKMK_Requirements"></a>Требования к качеству обслуживания хранилища  
...
-   **Hyper-V с использованием масштабируемого файлового сервера.** Этот сценарий предусматривает наличие следующих компонентов:  
    ...
    Если у вас нет кластера масштабируемого файлового сервера, развернутого для оценки, пошаговые инструкции по его созданию с помощью существующих серверов или виртуальных машин, см. в разделе [хранилище Windows Server 2012 R2: Пошаговое руководство с дисковыми пространствами, SMB масштабирования и общего VHDX (физического)](http://blogs.technet.com/b/josebda/archive/2013/07/31/windows-server-2012-r2-storage-step-by-step-with-storage-spaces-smb-scale-out-and-shared-vhdx-physical.aspx).  

-   **Hyper-V с помощью общих томов кластера.** Этот сценарий предусматривает наличие следующих компонентов:  
...
### <a name="BKMK_SolutionOverview"></a>Использование службы качества обслуживания хранилища в решении для программно определяемого хранилища  
...
**Рис. 1. Использование службы качества обслуживания хранилища в программно определяемого хранилища решении горизонтально масштабируемого файлового сервера**  
...
### <a name="BKMK_Glossary"></a>Глоссарий  

|Термин|Описание|  
|--------|---------------|  
|Нормализованные операции ввода-вывода в секунду|В этих единицах измеряется использование хранилища.  Это число операций ввода-вывода хранилища в секунду.  Операция ввода-вывода размером 8 КБ или меньше считается одной нормализованной операцией ввода-вывода.  Если размер такой операции превышает 8 КБ, она рассматривается как несколько нормализованных операций ввода-вывода. Например, запрос размером 256 КБ рассматривается как 32 нормализованные операции ввода-вывода в секунду.<br /><br />В Windows Server 2016 можно указать размер для нормализации операций ввода-вывода.  В кластере хранилища можно указать нормализованный размер, который будет применяться при вычислении нормализации на уровне всего кластера.  Значение по умолчанию — 8 КБ.|  
...
|Политика|Политики качества обслуживания хранилища хранятся в базе данных кластера и имеют следующие свойства: PolicyId, минимальное, MaximumIOPS, ParentPolicy и PolicyType.|  
...
## <a name="BKMK_SetUpQoS"></a>Настройке качества обслуживания хранилища и монитор базовых показателей производительности  
...
**Рис. 2. Ресурс качества обслуживания хранилища отображается как основной ресурс кластера в диспетчере отказоустойчивости кластеров**  
...
На масштабируемом файловом сервере потребуется развернуть несколько виртуальных машин с соответствующими рабочими нагрузками.  Советы по моделированию нагрузки и нагрузочному тестированию см. в разделе на следующей странице рекомендуемое средство (DiskSpd) и приведены некоторые примеры использования: [DiskSpd, PowerShell и производительность хранилища: измерение операций ввода-вывода, пропускной способности и задержки для локальных дисков и SMB файловые ресурсы.](http://blogs.technet.com/b/josebda/archive/2014/10/13/diskspd-powershell-and-storage-performance-measuring-iops-throughput-and-latency-for-both-local-disks-and-smb-file-shares.aspx)  
...
## <a name="BKMK_CreateQoSPolicies"></a>Как создавать и отслеживать политики качества обслуживания хранилища  
...
Существует два типа политик: Статистическая обработка проводится (ранее называвшаяся SingleInstance) и выделенной (ранее известной как на нескольких экземплярах). Объединенные политики применяют максимальные и минимальные значения для объединенного набора VHD- или VHDX-файлов и виртуальных машин, которым они назначены. Таким образом, у них общий набор операций ввода-вывода в секунду и пропускная способность. Выделенные политики применяют максимальные и минимальные значения для каждого VHD- или VHDX-файла отдельно. Поэтому можно создать одну политику, которая применяет одинаковые ограничения к нескольким VHD- или VHDX-файлам.  
...
Хранить количество VHD или vhdx-файлы, назначенные одну объединенную политику, 20 или меньше.  Этот тип политики предназначены для выполнения статистической обработки с нескольких виртуальных машин в кластере.
...
`Get-StorageQosPolicy` список всех настроенных политик и их состояние на файловом сервере горизонтального масштабирования.  
...
#### <a name="BKMK_RemovePolicyFromVM"></a>Удаление политики качества обслуживания хранилища  
...
## <a name="BKMK_Health"></a>Монитор работоспособности, с помощью службы качества обслуживания хранилища  
...
### <a name="BKMK_Updates"></a>Какие новые функциональные возможности была добавлена в Windows Server 2016
...
-   **Настраивается нормализации операций ввода-ВЫВОДА**  
...
#### <a name="example-2-get-iops-normalization-settings-and-specify--a-new-value"></a>Пример 2: Получение параметров нормализации операций ввода-ВЫВОДА и укажите новое значение  
