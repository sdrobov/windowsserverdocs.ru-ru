...
-   **Hyper-V с использованием масштабируемого файлового сервера.** Этот сценарий предусматривает наличие следующих компонентов:  
    ...
    Если у вас нет кластера масштабируемый файловый сервер, развернутого в целях оценки, пошаговые инструкции по его созданию с помощью существующих серверов или виртуальных машин см. в разделе хранилище [Windows Server 2012 R2: Пошаговая операция с дисковыми пространствами, масштабируемым и общим VHDX-форматом SMB (физический) ](http://blogs.technet.com/b/josebda/archive/2013/07/31/windows-server-2012-r2-storage-step-by-step-with-storage-spaces-smb-scale-out-and-shared-vhdx-physical.aspx).  
...
### <a name="BKMK_SolutionOverview"></a>Использование Storage QoS в программно-определяемом решении для хранения данных  
...
**Рис. 1. Использование Storage QoS в программно-определяемом решении для хранения данных в масштабируемый файловый сервер @ no__t-0  
...
|Политика|Политики качества обслуживания хранилища хранятся в базе данных кластера и имеют следующие свойства. PolicyId, Минимумиопс, MaximumIOPS, Парентполици и PolicyType.|  
...
## <a name="BKMK_SetUpQoS"></a>Настройка качества обслуживания хранилища и мониторинг базовой производительности  
...
### <a name="BKMK_SetupStorageQoSonStorageCluster"></a>Настройка качества обслуживания хранилища в кластере хранения  
...
**Рис. 2. Ресурс качества обслуживания хранилища отображается как ресурс ядра кластера в диспетчер отказоустойчивости кластеров @ no__t-0  
...
### <a name="BKMK_SetupStorageQoSonComputeCluster"></a>Настройка качества обслуживания хранилища в кластере вычислений  
...
-   Windows PowerShell: Добавление RSAT-WindowsFeature. кластеризация  

Дополнительный компонент **RSAT-Hyper-V-Tools** включает в себя модуль Windows PowerShell для удаленного управления Hyper-V.  

-   Windows PowerShell: Добавление-WindowsFeature RSAT-Hyper-V-Tools  
...
## <a name="BKMK_CreateQoSPolicies"></a>Создание и мониторинг политик качества обслуживания в хранилище  
...
Существует два типа политик: Агрегированный (ранее известный как Синглеинстанце) и выделенный (ранее известный как многоэкземплярный). Объединенные политики применяют максимальные и минимальные значения для объединенного набора VHD- или VHDX-файлов и виртуальных машин, которым они назначены. Таким образом, у них общий набор операций ввода-вывода в секунду и пропускная способность. Выделенные политики применяют максимальные и минимальные значения для каждого VHD- или VHDX-файла отдельно. Поэтому можно создать одну политику, которая применяет одинаковые ограничения к нескольким VHD- или VHDX-файлам.  
...
`Get-StorageQosPolicy` перечисляет все настроенные политики и их состояние на масштабируемый файловый сервер.  
...
## <a name="BKMK_KnownIssues"></a>Определение и устранение распространенных проблем  
...
### <a name="BKMK_FindingVMsWithInvalidPolicies"></a>Выявление виртуальных машин с недопустимыми политиками  
...
### <a name="BKMK_VMsThatDoNotMeetStorageQoSPoilicies"></a>Поиск виртуальных машин, не отвечающих за соблюдение политик качества обслуживания хранилища  
...
## <a name="BKMK_Health"></a>Мониторинг работоспособности с помощью качества обслуживания хранилища  
...
### <a name="BKMK_Updates"></a>Новые функции, добавленные в Windows Server 2016
...
#### <a name="example-1-creating-a-new-policy-and-viewing-the-maximum-bandwidth-on-the-storage-cluster"></a>Пример 1: Создание новой политики и просмотр максимальной пропускной способности кластера хранилища  
...
#### <a name="example-2-get-iops-normalization-settings-and-specify--a-new-value"></a>Пример 2: Получение параметров нормализации операций ввода-вывода и указание нового значения  
