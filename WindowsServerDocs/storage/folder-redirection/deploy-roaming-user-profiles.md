...
ms.locfileid: "59857145"
---
# <a name="deploying-roaming-user-profiles"></a>Развертывание перемещаемых профилей пользователей

>Относится к: Windows 10, Windows 8.1, Windows 8, Windows 7, Windows Server 2016, Windows Server 2012 R2, Windows Server 2012, Windows Server 2008 R2
...
>
> Таким образом меню "Пуск" сбрасывается до новой версии операционной системы по умолчанию после ОС обновление на месте. Обойти это ограничение, см. в разделе [приложение в: Рабочий около Сброс макетов меню Пуск после обновления](#appendix-c-workaround).
...
<tr class="odd">
<td>Группа безопасности пользователей, которым требуется обеспечить общий доступ к данным (пользователи перемещаемых профилей и компьютеры)</td>
<td>Содержимое папки/чтение данных<sup>1</sup><br />
<br />
Создание папок/добавление данных<sup>1</sup></td>
<td>Только эта папка</td>
</tr>
<tr class="even">
...
Можно использовать групповую политику, чтобы применить перемещаемые профили пользователей на компьютерах под управлением Windows 8.1, Windows 8, Windows 7, Windows Vista, Windows Server 2016, Windows Server 2012 R2, Windows Server 2012, Windows Server 2008 R2 или Windows Server 2008.
...
4. Используйте групповую политику, чтобы задать следующий параметр реестра на компьютерах Windows 10. Чтобы сделать это, см. в разделе [Настройка элемента реестра](<https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc753092(v=ws.11)>).
    ...
    <tr class="odd">
    <td>«Значение»</td>
    <td><strong>1</strong> (или <strong>0</strong> отключение)</td>
    </tr>
    <tr class="even">
    <td>Base</td>
...
6. (Необязательно) Дополнительно сокращают время входа в систему, удалив лишние приложения из базовый образ Windows 10, который используется для развертывания на клиентских компьютерах. Windows Server 2016 нет любого предварительно подготовленные приложения, поэтому вы можете пропустить этот шаг в образах сервера.
...
## <a id="appendix-c-workaround"></a>Приложение c. Рабочий около Сброс макетов меню Пуск после обновления
...
|14 марта 2017 г.|Добавлен необязательный шаг для указания обязательных макет начального для компьютеров с Windows 10 в [приложении a. Контрольный список для развертывания перемещаемых профилей пользователей](#appendix-a:-checklist-for-displaying-roaming-user-profiles).|Функция изменения в последнем обновлении Windows.|
...
|29 декабря 2016 г.|Добавлена ссылка в [шаг 7: Включение GPO перемещаемых профилей пользователя](#step-7:-enable-the-roaming-user-profiles-gpo) для упрощения получения сведений о настройке групповой политики для основных компьютеров. Также исправлено несколько ссылок на шаги 5 и 6, для номера Неизвестная ошибка.|Отзывы пользователей.|
