...
&#171;  [**Предыдущих:** Шаг 5. Настроить DNS и параметров брандмауэра](vpn-deploy-dns-firewall.md)<br>
&#187; [ **Next:** Шаг 7. (Необязательно) Условный доступ для VPN-подключения с помощью Azure AD](../../ad-ca-vpn-connectivity-windows10.md)

На этом шаге вы Узнайте о параметрах ProfileXML и схемы и настроить клиентские компьютеры Windows 10 для взаимодействия с этой инфраструктурой с помощью VPN-подключения. 
...
## <a name="bkmk_profile"></a>Создать профиль подключения шаблона вручную

На этом шаге вы использовать защищенный наращиваемый протокол аутентификации \(PEAP\) для безопасного обмена данными между клиентом и сервером. В отличие от простого имени пользователя и пароля это подключение требует уникального раздела EAPConfiguration профиля VPN для работы. 
...
3.  Щелкните правой кнопкой мыши **виртуальной частной сети \(VPN\) подключений**и нажмите кнопку **свойства**.
...
6.  Запишите значения для **сертификат, выданный** и **издателя**.<p>Эти значения используются в предстоящих шаблоне конфигурации VPN. Например если имя узла — NPS01 nps01.corp.contoso.com указано полное доменное имя сервера, имя сертификата основан на полное доменное имя или DNS-имя сервера — например, nps01.corp.contoso.com.
...
### <a name="configure-the-template-vpn-profile-on-a-domain-joined-client-computer"></a>Настройка шаблона профиля VPN в домене\-присоединившихся клиентских компьютеров

Теперь, когда у вас есть данные, необходимые настройки шаблона профиля VPN на компьютере клиента на присоединенных к домену. Тип учетной записи пользователя, используемой \(то есть обычный пользователь или администратор\) для этой части процесса не имеет значения. 
...
6.  В имя или адрес сервера, введите **внешних** полное ДОМЕННОЕ имя VPN-серверу \(к примеру, **vpn.contoso.com**\).
...
11. Закрытие окна сетевых подключений.

12. В параметрах, проверьте VPN-Подключение, щелкнув **шаблона**и щелкнув **Connect**.
...
## <a name="bkmk_ProfileXML"></a>Создать файлы конфигурации ProfileXML

Прежде чем выполнять в этом разделе, убедитесь, что вы создал и протестировал шаблон VPN-подключение, разделе [вручную создать профиль подключения шаблона](#bkmk_profile) описывает. Проверка VPN-подключение необходимо, чтобы гарантировать, что профиль содержит все сведения, необходимые для подключения к виртуальной частной сети.
...
1. Войдите в компьютер клиента на присоединенных к домену, содержащий шаблон профиля VPN с тем же пользователем учетной записи, разделе «Вручную создать профиль подключения шаблона» описано.

2. Вставьте интегрированной среды сценариев Windows PowerShell в листинге 1 \(интегрированной среды Сценариев\)и настроить параметры, описанные в комментариях. Это $Template, $ProfileName, $Servers, $DnsSuffix, $DomainName, $TrustedNetwork и $DNSServers. Полное описание каждого параметра находится в комментарии.
...
>[!NOTE]
>Чтобы просмотреть полный пример сценария, см. в разделе [MakeProfile.ps1 полный сценарий](#bkmk_fullscript).
...
- Суффикс - суффикса домена, которое будет добавлено к запросу shortname для разрешения DNS. Чтобы указать суффикс, добавить в начало периода \(.) для DNS-суффикс.
...
## <a name="bkmk_fullscript"></a>Полный сценарий MakeProfile.ps1
...
## <a name="vpn-deploy-client-sccm"></a>Настройка VPN-клиента с помощью System Center Configuration Manager

Профили VPN в System Center Configuration Manager, можно развернуть с помощью ProfileXML CSP узла, так же, как в Windows PowerShell. Здесь используется сценарий VPN_Profile.ps1 Windows PowerShell, который вы создали в разделе [создания файлов конфигурации ProfileXML](#bkmk_ProfileXML).
...
После создания группы пользователей для получения профиля VPN, можно создать пакет и программу для развертывания Windows PowerShell скрипт конфигурации, созданный в разделе [создания файлов конфигурации ProfileXML](#bkmk_ProfileXML).
...
4.  На странице "пакет" выполните следующие действия:
    ...
    В. В диалоговом окне задать исходную папку, нажмите кнопку **Обзор**выберите общей папкой, содержащей VPN_Profile.ps1 и нажмите кнопку **ОК**.<p>Убедитесь, что выбран сетевой путь, а не локальный путь. Другими словами, путь должен быть примерно  *\\fileserver\\vpnscript*, а не *c:\\vpnscript*.
...
Чтобы использовать Intune для развертывания удаленного доступа всегда на профили Windows 10 VPN, можно настроить узел ProfileXML CSP с помощью профиль VPN, созданный в разделе [создания файлов конфигурации ProfileXML](#bkmk_ProfileXML), или можно использовать базовый EAP Приведенный ниже образец XML.
...
5.   В разделе **платформы**выберите **Windows 10 или более поздней версии**и выберите **VPN** из раскрывающегося списка тип профиля.
...
6. В разделе **базовая VPN** Подтвердите или задайте следующие параметры:
...
7.  Скопируйте следующую XML-строку в текстовый редактор:<p>
...
8.  Замените  **\<TrustedRootCA > 5а 89 fe cb 5b 49 a7 0b 1a 52 63 b7 35 ee d7 1c c2 68 быть 4b <\/TrustedRootCA >** в примере на отпечаток сертификата корневого центра сертификации на предприятии в обоих местах.
...
10. Замените  **\<ServerNames > NPS.contoso.com\</ServerNames >** в образце XML полным доменным именем NPS присоединенных к домену, где будет выполнена проверка подлинности. 

11. Скопируйте измененный XML-строку и вставьте в **XML-файл EAP** на вкладке базовая VPN и нажмите кнопку **ОК**.<p>В Intune создается политика всегда в конфигурации VPN-устройства, с помощью EAP.
