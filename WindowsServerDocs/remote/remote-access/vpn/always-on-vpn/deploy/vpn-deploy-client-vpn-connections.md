...
- [**Прошлом** Шаг 5. Настройка параметров DNS и брандмауэра](vpn-deploy-dns-firewall.md)<br>
- [**Очеред** Шаг 7. Используемых Условный доступ для VPN-подключений с помощью Azure AD](../../ad-ca-vpn-connectivity-windows10.md)
...
VPN-клиент Always On можно настроить с помощью PowerShell, SCCM или Intune. Для всех трех требуется профиль VPN в формате XML, чтобы настроить соответствующие параметры VPN. Автоматизация регистрации PowerShell для организаций без SCCM или Intune невозможна.
...
Профилексмл используется во всех методах доставки, описанных в этом развертывании, включая Windows PowerShell, System Center Configuration Manager и Intune. Существует два способа настройки узла Профилексмл поддержка vpnv2 CSP в этом развертывании:
...
- **Инструментарий управления Windows (WMI) (WMI) для моста CSP**. Второй способ настройки узла CSP Профилексмл — использовать мост WMI в CSP — класс WMI с именем **MDM_VPNv2_01**—, который может получить доступ к поддержка vpnv2 CSP и узлу профилексмл. При создании нового экземпляра класса WMI Инструментарий WMI использует CSP для создания профиля VPN при использовании Windows PowerShell и System Center Configuration Manager.
...
**Маршруты** Раздельное туннелирование
...
**Идентификаци** PEAP-TLS с сертификатами пользователей, защищенными доверенным платформенным модулем
...
12. Щелкните **использовать протокол расширенной проверки подлинности (EAP)** . затем в окне **использование протокола EAP**выберите **Майкрософт: Защищенный EAP (PEAP) (шифрование включено**).

13. Щелкните **Свойства** , чтобы открыть диалоговое окно Свойства защищенного EAP, и выполните следующие действия.

    1\. В поле **Подключение к этим серверам** введите имя сервера политики сети, полученное из параметров проверки подлинности сервера NPS, ранее в этом разделе (например, NPS01).
    ...
    2\.  В разделе Доверенные корневые центры сертификации выберите корневой ЦС, выдавший сертификат сервера политики сети (например, Contoso-CA).
    ...
    з.  Установите флажок **не запрашивать пользователя авторизовать новые серверы или доверенные центры сертификации** .
...
3. Запустите скрипт, чтобы создать **VPN_Profile. XML** и **VPN_Profile. ps1** на рабочем столе.
...
Используйте сценарий **VPN_Profile. ps1** в Windows PowerShell или System Center Configuration Manager, чтобы настроить профилексмл на рабочем столе Windows 10.
...
Этот сценарий можно использовать на рабочем столе Windows 10 или в System Center Configuration Manager.
...
В большинстве примеров для вставки Профилексмл в новый экземпляр класса WMI MDM_VPNv2_01 используется командлет Windows PowerShell Set-WmiInstance. 

Однако это не работает в System Center Configuration Manager, так как пакет нельзя запустить в контексте конечных пользователей. Таким образом, этот скрипт использует модель CIM для создания сеанса WMI в контексте пользователя, а затем создает новый экземпляр класса WMI MDM_VPNv2_01 в этом сеансе. Этот класс WMI использует мост WMI-CSP для настройки CSP поддержка vpnv2. Поэтому, добавив экземпляр класса, вы настроите CSP. 

>[!IMPORTANT]
>Для моста WMI в CSP требуются права локального администратора. Для развертывания профилей VPN каждого пользователя следует использовать SCCM или MDM.

>[!NOTE]
>В скрипте VPN_Profile. ps1 используется идентификатор безопасности текущего пользователя для обнаружения контекста пользователя. Так как в удаленный рабочий стол сеансе нет доступных идентификаторов безопасности, сценарий не работает в сеансе удаленный рабочий стол. Аналогично, он не работает в расширенном сеансе Hyper-V. Если вы тестируете удаленный доступ Always On VPN в виртуальных машинах, отключите расширенный сеанс на клиентских виртуальных машинах перед выполнением этого скрипта.
...
В System Center Configuration Manager можно развернуть профили VPN с помощью узла CSP Профилексмл, как и в Windows PowerShell. Здесь вы используете сценарий Windows PowerShell VPN_Profile. ps1, созданный в разделе [Создание файлов конфигурации профилексмл](#create-the-profilexml-configuration-files).

Чтобы использовать System Center Configuration Manager для развертывания профиля VPN удаленного доступа Always On на клиентских компьютерах Windows 10, необходимо сначала создать группу компьютеров или пользователей, которым вы развертываете профиль. В этом сценарии создайте группу пользователей для развертывания скрипта конфигурации.
...
4.  На странице правила членства выполните следующие действия.
    ...
    f.  На странице Завершение нажмите кнопку **Закрыть**.
...
8.  На странице Завершение нажмите кнопку **Закрыть**.
...
1.  Разместите сценарий VPN_Profile. ps1 в общей сетевой папке, к которой имеет доступ учетная запись компьютера сервера сайта.
...
4.  На странице пакет выполните следующие действия.
    ...
    1\. В списке **имя**введите **Windows 10 Always on профиль VPN**.

    2\. Установите флажок **Этот пакет содержит исходные файлы** и нажмите кнопку **Обзор**.
...
3.  На странице Стандартная программа выполните следующие действия.
    ...
    1\.  В списке **имя**введите **сценарий профиля VPN**.

    2\.  В **командной строке**введите **PowerShell. exe-ExecutionPolicy обход файла "VPN_Profile. ps1"** .
...
4.  На странице требования выполните следующие действия.
    ...
    2\.  Установите флажки **все Windows 10 (32-bit)** и **все windows 10 (64-bit)** .
...
3.  На вкладке **программы** в нижней части области сведений щелкните правой кнопкой мыши элемент **Скрипт профиля VPN**, выберите пункт **Свойства**и выполните следующие действия.
    ...
    1\.  На вкладке **Дополнительно** в поле **когда эта программа назначена компьютеру**выберите **один раз для каждого пользователя, который входит**в систему.
...
5.  На странице Общие выполните следующие действия.
    ...
    1\.  Рядом с элементом **коллекция**нажмите кнопку **Обзор**.

    2\.  В списке **типы коллекций** (вверху слева) щелкните **коллекции пользователей**.
...
6.  На странице содержимое выполните следующие действия.
    ...
    1\.  Нажмите кнопку **Добавить**и выберите **пункт точка распространения**.

    2\.  В списке **Доступные точки распространения**выберите точки распространения, для которых требуется распространить скрипт конфигурации профилексмл, и нажмите кнопку **ОК**.
...
8.  На странице Расписание выполните следующие действия.
    ...
    1\.  Нажмите кнопку **создать** , чтобы открыть диалоговое окно Расписание назначений.

    2\.  Щелкните **назначить немедленно после этого события**и нажмите кнопку **ОК**.
...
11. На странице Завершение нажмите кнопку **Закрыть**.
...
2.  В диалоговом окне Свойства Configuration Manager на вкладке **действия** выполните следующие действия.
    ...
    1\.  Щелкните **цикл оценки & получения политики компьютера**, щелкните **Запустить сейчас**и нажмите кнопку **ОК**.

    2\.  Щелкните **цикл оценки & получения политики пользователя**, щелкните **Запустить сейчас**и нажмите кнопку **ОК**.
...
Чтобы использовать Intune для развертывания Always On профилей VPN удаленного доступа Windows 10, можно настроить узел CSP Профилексмл с помощью профиля VPN, созданного в разделе [Создание файлов конфигурации профилексмл](#create-the-profilexml-configuration-files), или можно использовать базовый образец EAP XML. см.
...
2. На вкладке **Базовая VPN** проверьте или задайте следующие параметры.
    ...
    - **Серверах** Добавьте один или несколько VPN-серверов, нажав кнопку **Добавить.**
    ...
    - **Запоминать учетные данные при каждом входе**:  Логическое значение (true или false) для кэширования учетных данных. Если задано значение true, учетные данные кэшируются везде, где это возможно.
