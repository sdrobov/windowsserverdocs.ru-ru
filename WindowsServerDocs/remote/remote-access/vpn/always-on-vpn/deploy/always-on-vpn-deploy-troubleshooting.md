...
NPS создает и сохраняет журналы учета NPS. По умолчанию они хранятся в папке% SYSTEMROOT%\\system32\\файлы журнала\\ в файле с именем*XXXX*. txt, где *XXXX* — это Дата создания файла.
...
## <a name="vpn_profileps1-script-issues"></a>Проблемы с сценариями VPN_Profile PS1

Наиболее распространенные проблемы, возникающие при ручном запуске скрипта VPN_ Profile. ps1:
...
2. Можно ли разрешить имя сервера удаленного доступа или VPN-адреса в адрес? На **панели управления** > **сеть** и **Интернет** > **Сетевые подключения**, откройте свойства профиля VPN. Значение на вкладке **Общие** должно быть открыто разрешимым с помощью DNS.
...
- **Возможное решение.** Для экранирования этого цикла выполните следующие действия.
  ...
  2. Убедитесь в том, что разделы **\<тлсекстенсионс >** , **\<екунаме >** и **\<екуоид >** существуют и показывают правильное имя и идентификатор объекта.
  ...
  4. Если в личном хранилище пользователя существует допустимый сертификат проверки подлинности клиента, соединение не будет выполнено (как оно должно) после того, как пользователь выберет **X** , а **\<тлсекстенсионс >** , **\<екунаме >** и **\<екуоид >** уже существуют и содержат правильные сведения.
