...
> Относится к: Windows Server 2019, Windows Server (полугодовой канал), Windows Server 2016, Windows Server 2012 R2, Windows 10
...
На этом шаге вы установите сервера политики сети (NPS) для обработки запросов на подключение, отправленных с VPN-сервера:
...
Для выполнения этой процедуры с помощью Windows PowerShell, запустите Windows PowerShell от имени администратора, введите следующий командлет:
...
1.  В диспетчере серверов выберите **управление**, а затем выберите **Добавить роли и компоненты**.
        Откроется мастер добавления ролей и компонентов.

2.  В перед началом работы, выберите **Далее**.
...
3.  Выбор типа установки, убедитесь, что **Установка ролей или компонентов** , а кнопку **Далее**.

4.  Выбор целевого сервера убедитесь, что **выберите сервер из пула серверов** выбран.

5.  В пул серверов, убедитесь, что локальный компьютер и нажмите кнопку **Далее**.

6.  В выбранных ролей сервера в **ролей**выберите **службы политики сети и доступа**. Откроется диалоговое окно с вопросом, следует добавить компоненты, необходимые для служб политики сети и доступа.

7.  Выберите **добавить компоненты**, а затем выберите **Далее**

8.  В разделе выбора возможностей выберите **Далее**и в политике сети и доступа к службам, ознакомиться со сведениями, а затем выберите **Далее**.

9.  В службы выберите роль, выберите **сервера политики сети**.

10. Компоненты, необходимые для сервера политики сети, выберите **добавить компоненты**, а затем выберите **Далее**.

11. В подтверждение установки компонентов выберите **автоматический перезапуск конечного сервера, если требуется**.

12. Выберите **Да** для подтверждения выбранного, а затем выберите **установить**.
    
    На странице хода установки отображается состояние во время установки. После завершения процесса, сообщение «Установка выполнена на *ComputerName*"отображается, где *ComputerName* — это имя компьютера, на котором установлен сервер политики сети.
...
1.  В диспетчере серверов выберите **средства**, а затем выберите **сервера политики сети**. Откроется консоль NPS.

2.  В консоли сервера политики сети, щелкните правой кнопкой мыши **сервер сетевых Политик (локальный)** , а затем выберите **зарегистрировать сервер в Active Directory**.
   
     Будет открыто диалоговое окно сервера политики сети.

3.  В диалоговом окне сервера политики сети выберите **ОК** дважды.
...
2. Щелкните правой кнопкой мыши **RADIUS-клиенты** и выберите **New**. Откроется диалоговое окно новый RADIUS-клиент.

3. Убедитесь, что **включить RADIUS-клиент** установлен флажок.

4. В **понятное имя**, введите отображаемое имя VPN-сервера.

5. В **адрес (IP-адрес или DNS)** , введите NAS IP-адрес или полное доменное имя.
     
     Если ввести полное доменное имя, выберите **Verify** Если вы хотите убедиться, что имя указано правильно и сопоставляется допустимый IP-адрес.
...
2. Выберите **Настройка VPN или удаленного доступа**.
        
    Откроется мастер настройки VPN или удаленного доступа.

3. Выберите **подключений виртуальной частной сети (VPN)** и выберите **Далее**.

4. Укажите удаленного доступа или VPN-сервер, в клиентов RADIUS выберите имя VPN-сервера, добавленного на предыдущем шаге. Например, если имя NetBIOS сервера VPN RAS1, выберите **RAS1**.
...
6. В Настройка методов проверки подлинности выполните следующие действия:
    ...
    2. Выберите **расширяемого протокола проверки подлинности** флажок, чтобы выбрать его.

    3. В типе (на основе метода доступа и сетевой конфигурации), выберите **Microsoft: Защищенный EAP (PEAP)** , а затем выберите **Настройка**.
      
        Будет открыто диалоговое окно редактирования свойства защищенного EAP.

    4. Выберите **удалить** удалить тип EAP защищенный пароль (EAP-MSCHAP v2).

    5. Нажмите кнопку **Добавить**. Будет открыто диалоговое окно добавления EAP.

    6. Выберите **смарт-карта или иной сертификат**, а затем выберите **ОК**.

    7. Выберите **ОК** закрыть изменение свойства защищенного EAP.
...
8. В Настройка групп пользователей выполните следующие действия:
    ...
    2. Введите **пользователей VPN**, а затем выберите **ОК**.

    3. Выберите **Далее**.

9. В задайте IP-фильтры, выберите **Далее**.

10. В задайте параметры шифрования выберите **Далее**. Не вносить изменения.

    Эти параметры применяются только для подключения шифрования Point-to-Point Microsoft (MPPE), которые не поддерживает этот сценарий.

11. В списке укажите имя сферы выберите **Далее**.

12. Выберите **Готово** чтобы закрыть мастер.
