...
**Получено сообщение об ошибке** (журнал событий клиента). Не удалось выполнить регистрацию сертификата OTP для <username> пользователя на сервере ЦС < CA_name >, сбой запроса, возможные причины сбоя: не удается разрешить имя сервера ЦС, не удается получить доступ к серверу ЦС через первый туннель DirectAccess или не удается установить подключение к серверу ЦС.  
...
Если действительный сертификат не найден, Удалите недопустимый сертификат (если он существует) и повторно зарегистрируйте сертификат компьютера, либо запустив `gpupdate /Force` из командной строки с повышенными привилегиями, либо перезапустив клиентский компьютер.  
...
2.  Если ЦС не настроен:  
    ...
    1.  Используйте командную `Set-DAOtpAuthentication` или консоль управления удаленным доступом для настройки центров сертификации, которые выдают сертификат входа DirectAccess.  
...
Убедитесь, что на клиентском компьютере развернуты последние параметры, выполнив `gpupdate /force` из командной строки с повышенными привилегиями или перезапустите клиентский компьютер.  
...
**Получено сообщение об ошибке** (журнал событий клиента). Для проверки подлинности OTP с помощью сервера удаленного доступа (< DirectAccess_server_name >) пользователю (<username>) требуется запрос от пользователя.  
...
1.  Проверьте настроенное имя шаблона сертификата для подписи OTP, выполнив командлет PowerShell `Get-DAOtpAuthentication` и изучите значение `SigningCertificateTemplateName`.  
...
-   Не удается проверить подлинность пользователя <username> с OTP. Убедитесь, что имя участника-пользователя определено в Active Directory. Код ошибки: < error_code >.  
  
-   Не удается проверить подлинность пользователя <username> с OTP. Убедитесь, что для имени пользователя в Active Directory определено РАЗЛИЧАЮЩЕеся имя. Код ошибки: < error_code >.  
