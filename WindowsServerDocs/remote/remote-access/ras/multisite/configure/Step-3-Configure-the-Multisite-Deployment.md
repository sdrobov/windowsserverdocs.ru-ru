...
11.  В диалоге **Ход выполнения установки**, проверьте, что установка прошла успешно, затем щелкните **Закрыть**.  
...
На каждом сервере удаленного доступа, который будет добавлен к многосайтовому развертыванию, для проверки HTTPS-подключения к веб-серверу с поддержкой протокола IP HTTPS требуется сертификат SSL. Для выполнения данной процедуры требуется членство в локальной группе **Администраторы** или в эквивалентной группе.  
  
#### <a name="to-obtain-an-ip-https-certificate"></a>Получение сертификата IP-HTTPS  
  
1.  На каждом сервере удаленного доступа: на **начальном** экране введите **MMC**и нажмите клавишу ВВОД. Если появится диалог **Контроль учетных записей**, подтвердите, что действие, которое оно отображает то, что нужно, затем щелкните **Да**.  
...
1.  На имеющемся сервере удаленного доступа: на **начальном** экране введите **рамгмтуи. exe**и нажмите клавишу ВВОД. Если появится диалог **Контроль учетных записей**, подтвердите, что действие, которое оно отображает то, что нужно, затем щелкните **Да**.  
