...
0Windows команды PowerShell для](../../../../media/Step-3-Configure-the-Multisite-Deployment/PowerShellLogoSmall.gif)***<em>Windows PowerShell</em> @no__t***  
...
1.  На DNS-сервере выполните следующие действия. На **начальном** экране введите **днсмгмт. msc**и нажмите клавишу ВВОД.  
...
2.  Чтобы удалить компьютеры из группы безопасности, дважды щелкните группу безопасности, а затем в диалоговом окне **< Group_Name > свойства** откройте вкладку **члены** .  
...
Чтобы разрешить клиентским компьютерам Windows 7 доступ через первую точку входа через группу безопасности DA_Clients_US и использовать DA_W7_Clients_GPO_US объекта групповой политики.  
...
15. В диалоговом окне **Добавление точки входа** нажмите кнопку **Закрыть** , а затем в мастере добавления точки входа нажмите кнопку **Закрыть**.  
    ...
    > [!NOTE]  
    > Если добавленная точка входа находится в другом лесу, чем существующие точки входа или клиентские компьютеры, то для обнаружения контроллеров домена и System Center необходимо нажать кнопку **обновить серверы управления** в области **задачи** . Configuration Manager в новом лесу.  
...
Чтобы разрешить клиентским компьютерам Windows 7 доступ через вторую точку входа через группу безопасности DA_Clients_Europe и использовать DA_W7_Clients_GPO_Europe объекта групповой политики.  
