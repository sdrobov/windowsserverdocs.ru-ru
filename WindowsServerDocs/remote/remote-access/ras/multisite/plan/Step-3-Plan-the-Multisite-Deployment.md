...
## <a name="bkmk_3_1_IPHTTPS"></a>3,1. Планирование сертификатов IP-HTTPS  
...
Если сервер сетевого расположения размещен на сервере удаленного доступа, необходимо добавить запись DNS для веб-сайта сервера сетевого расположения для каждой точки входа в развертывании. Обратите внимание на следующее условия:  
...
## <a name="bkmk_3_3_IPsec"></a>3,3. Спланируйте корневой сертификат IPsec для всех серверов удаленного доступа  
...
## <a name="bkmk_3_5_EP_Selection"></a>3,5. Планирование выбора точки входа клиента DirectAccess  
...
## <a name="bkmk_3_6_IPv6"></a>3,6. префиксы и маршрутизация плана  
...
3. Для каждой точки входа в внутренней сети должен быть один или три маршрута:  
   ...
   3. Префикс Teredo (необязательно). Этот префикс важен только в том случае, если сервер удаленного доступа настроен с двумя последовательными общедоступными IPv4-адресами на внешнем адаптере. Префикс основан на первом общедоступном IPv4-адресе пары адресов. Например, если внешние адреса:  
      ...
      Затем префикс Teredo для настройки — 2001:0: ВВКСКС: ИИЗЗ::/64, где ВВКСКС: ИИЗЗ — шестнадцатеричное представление IPv4-адреса www\.xxx.yyy.zzz.  
...
### <a name="TransitionIPv4toMixed"></a>Переход с IPv4-объекта на корпоративную сеть IPv6 + IPv4  
...
### <a name="TransitionMixedtoIPv6"></a>Переход с IPv6 + IPv4 на корпоративную сеть только с IPv6  
...
### <a name="DualDeployment"></a>Переход с IPv4-компонента на развертывание только по протоколу IPv6 с использованием двойных развертываний DirectAccess  
...
1.  Установите второе развертывание DirectAccess. Можно установить DirectAccess на новых серверах или удалить серверы из первого развертывания и использовать их для второго развертывания.  
    ...
    >   
    > Если вы устанавливаете DirectAccess с помощью мастера начало работы или командлета `Install-RemoteAccess`, удаленный доступ автоматически устанавливает префикс клиента первой точки входа в развертывании на значение по умолчанию < подсети IPv6 @ no__t-1prefix >: 1000::/64. При необходимости необходимо изменить префикс.  
...
4.  Дополнительно Удалите точки входа DirectAccess из первого развертывания и добавьте эти серверы в качестве новых точек входа во второй.  
