...
> [!NOTE]  
> -   Для каждого леса DirectAccess требуется по крайней мере одна группа безопасности клиента Windows 10&reg; или Windows&reg; 8. Однако рекомендуется иметь одну группу безопасности клиента Windows 10 или Windows 8 для каждого домена, содержащего клиентов Windows 10 или Windows 8.  
> -   Если включена поддержка нескольких сайтов, для каждой точки входа DirectAccess, в которой поддерживаются клиентские компьютеры Windows 7, требуется по крайней мере одна группа безопасности клиента Windows 7&reg; каждого леса. Однако рекомендуется иметь отдельную группу безопасности клиента Windows 7 для каждой точки входа для каждого домена, содержащего клиентов Windows 7.  
>   
