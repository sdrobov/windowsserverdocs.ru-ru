...
## <a name="AccessForest2"></a>Доступ к ресурсам из леса Forest2  
...
## <a name="EnableForest2DA"></a>Обеспечение клиентов из леса Forest2 возможностью подключения через DirectAccess  
...
2.  Если DNS-суффикс леса forest2 не является частью DNS-суффикс леса forest1, добавьте правила NRPT с суффиксами домена клиентов в лесу Forest2, чтобы включить доступ к контроллерам домена для проверки подлинности и при желании добавьте суффиксы доменов в лесу Forest2 DNS suf Исправьте список поиска. 
...
## <a name="AddEPForest2"></a>Добавление точек входа из леса Forest2  
...
-   Корневой ЦС леса главный ЦС дерева PKI.  
  
-   ЦС предприятия — все остальные ЦС.  
  
-   Для леса ресурсов лес, содержащий корневой ЦС и считается «лес\домен».  
  
-   Учетной записи леса-All других лесов в топологии.  
  
Для этой процедуры требуется сценарий PowerShell, PKISync.ps1. См. в разделе [AD CS: Сценарий PKISync.ps1 для регистрации сертификатов между лесами](https://technet.microsoft.com/library/ff961506.aspx).  
...
4.  Извлеките сертификат корневого ЦС, выполнив следующую команду из командной строки с повышенными привилегиями:  
    ...
    (Если вы выполняете команду для корневого ЦС можно опустить сведения о соединении, - config < имя_компьютера >\\< имя корневого ЦС >)  
    ...
    2.  Предоставление ресурсов леса чтения и записи разрешения на шаблонов сертификатов \<леса учетных записей\>\\< учетная запись администратора\>.  
...
### <a name="BKMK_DelImp"></a>Удаление и импорт шаблонов сертификатов для OTP  
...
### <a name="BKMK_Publish"></a>Публикация шаблонов сертификатов для OTP  
...
### <a name="NRPT_DNSSearchSuffix"></a>Добавление правил NRPT и DNS-суффиксы  
...
3.  На странице **DNS** в таблице введите любые дополнительные суффиксы имен, которые являются частью корпоративной сети в лесу Forest2. В поле **Адрес DNS-сервера** введите адрес DNS-сервера вручную, либо щелкнув **Обнаружить**. Если не указывать этот адрес, новые записи применяются как исключения NRPT. Затем нажмите кнопку **Далее**.  
  
4.  Дополнительно Добавьте любые суффиксы DNS на странице **Порядок просмотра DNS-суффиксов**. Для этого введите суффикс в поле **Новый суффикс** и нажмите кнопку **Добавить**. Затем нажмите кнопку **Далее**.  
...
### <a name="IPv6Prefix"></a>Добавление внутреннего префикса IPv6  
  
> [!NOTE]  
> Добавление внутреннего префикса IPv6 имеет смысл только при развертывании IPv6 во внутренней сети.  
  
Сервер удаленного доступа ведет список префиксов IPv6 для корпоративных ресурсов. Клиентам, подключенным через DirectAccess, доступны только ресурсы с этими префиксами IPv6. Так как консоль управления удаленным доступом и команды Windows PowerShell автоматически добавляют префиксы IPv6 леса forest1 и могут не добавлять префиксы других лесов, необходимо добавить все отсутствующие префиксы леса forest2 вручную.  
...
### <a name="SGs"></a>Добавление клиентских групп безопасности  
Чтобы клиентские компьютеры Windows 8 из леса Forest2 доступ к ресурсам через DirectAccess, необходимо добавить в группу безопасности из леса Forest2 в развертывание удаленного доступа.  
...
Чтобы включить Windows 7, клиентским компьютерам из леса Forest2 доступ к ресурсам через DirectAccess при многосайтовом включен, необходимо добавить группу безопасности из леса Forest2 в развертывание удаленного доступа для каждой точки входа. Сведения о добавлении групп безопасности Windows 7 см. описание **поддержка клиента** страницы в 3.6. Включение многосайтового развертывания.  
...
Процесс автоматического обнаружения серверов инфраструктуры обеспечивает доступ через DirectAccess к контроллерам домена и к System Center Configuration Manager. Вы должны запустить процесс обнаружения вручную.  
