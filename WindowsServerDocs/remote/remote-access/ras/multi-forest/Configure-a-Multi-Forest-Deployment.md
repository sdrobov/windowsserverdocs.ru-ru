...
Для этой процедуры требуется сценарий PowerShell, PKISync.ps1. См. раздел [AD CS: Скрипт Пкисинк. ps1 для регистрации сертификатов между лесами @ no__t-0.  
...
4.  Извлеките сертификат корневого ЦС, выполнив следующую команду из командной строки с повышенными привилегиями:  
    ...
    (Если выполнить команду в корневом ЦС, можно опустить сведения о подключении,-config < Computer-name > \\ < корневой ЦС-name >)  
    ...
    2.  Предоставьте шаблонам сертификатов леса ресурсов разрешения на чтение и запись для \<Account леса @ no__t-1 @ no__t-2 < учетной записи администратора @ no__t-3.  
