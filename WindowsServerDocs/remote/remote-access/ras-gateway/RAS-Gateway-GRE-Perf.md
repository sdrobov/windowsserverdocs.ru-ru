...
>Относится к: Windows Server \(Semi-годовой канал @ no__t-1

С помощью этого раздела вы узнаете о сервере удаленного доступа \(RAS @ no__t-1 "Инкапсуляция шлюза с общей маршрутизацией \(GRE @ no__t-3" производительность туннеля в Windows Server версии 1709 в непрограммно-определяемой сети \(SDN @ no__t-5. PXE.
...
Одноклиентский режим позволяет организациям любого размера развертывать Шлюз в внешней сети или в Интернете @ no__t-0facing Virtual частная сеть \(VPN @ no__t-2 Server. В режиме одного клиента можно развернуть шлюз RAS на физическом сервере или виртуальной машине \(VM @ no__t-1. В этом разделе описывается развертывание шлюза RAS на двух виртуальных машинах \(VMs @ no__t-1, настроенных в отказоустойчивом кластере.
...
GRE — это упрощенный протокол туннелирования, который может инкапсулировать разнообразные протоколы сетевого уровня в виртуальные точки @ no__t-0to @ no__t-1Point Links по протоколу IP. Реализация Microsoft GRE инкапсулирует как IPv4, так и IPv6.
...
В тестовой среде виртуальные машины шлюза RAS развертываются на узлах Hyper @ no__t-0V в отказоустойчивом кластере для обеспечения высокой доступности.

### <a name="hyper-v-host-configuration"></a>Конфигурация узла Hyper @ no__t-0V

Два узла Hyper @ no__t-0V настроены для поддержки сценария тестирования следующим образом. 

- Два двойных @ no__t-0homed физических компьютера настроены с помощью Windows Server, версия 1709
...
- Роль сервера Hyper @ no__t-0V устанавливается на обоих серверах и настраивается с помощью двух внешних коммутаторов Hyper @ no__t-1V, по одному для каждого физического сетевого адаптера.
- Так как оба сервера подключены к одной интрасети, серверы могут взаимодействовать друг с другом.
- Узлы Hyper @ no__t-0V настраиваются в отказоустойчивом кластере по сети интрасети. 
...
- На каждой виртуальной машине установлен шлюз RAS, который настраивается как VPN-сервер GRE @ no__t-0based.
...
### <a name="workload-hyper-v-hosts-and-vms"></a>Рабочая нагрузка Hyper @ no__t-0V Hosts and VMS

Для этого теста в интрасети устанавливаются два узла рабочей нагрузки Hyper @ no__t-0V, на каждом из которых установлена одна виртуальная машина. Если вы дублируете этот тест в собственной тестовой среде, можно установить столько серверов рабочей нагрузки и виртуальных машин, сколько подходит для ваших целей.

- На узлах рабочей нагрузки Hyper @ no__t-0V установлен один физический сетевой адаптер, подключенный к интрасети организации.
- В виртуальном коммутаторе Hyper @ no__t-0V на каждом узле создается один виртуальный коммутатор. Параметр является внешним и привязан к одному сетевому адаптеру, подключенному к интрасети.
...
Вы можете добиться пропускной способности до 2,0 Гбит/с на нескольких виртуальных машинах @ no__t-0core, настроенных как шлюзы RAS GRE.
