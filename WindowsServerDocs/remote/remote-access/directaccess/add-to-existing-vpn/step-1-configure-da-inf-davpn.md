...
ms.locfileid: "66446984"
---
# <a name="step-1-configure-the-directaccess-infrastructure"></a>Шаг 1 Настройка инфраструктуры DirectAccess

>Область применения. Windows Server (полугодовой канал), Windows Server 2016

В этом разделе описывается настройка необходимой инфраструктуры для создания DirectAccess в уже развернутой сети VPN. Перед планированием этапов развертывания убедитесь, что выполнены шаги планирования в [шаг 1: Планирование инфраструктуры DirectAccess](Step-1-Plan-DirectAccess-Infrastructure.md).  
...
-   Протокол управления IP-HTTPS-передачей (TCP) порт назначения 443 и TCP-порт источника 443 для исходящих подключений. Когда сервер удаленного доступа имеет один сетевой адаптер, а сервер сетевых расположений находится на сервере удаленного доступа, тогда также требуется TCP-порт 62000.  
...
-   **Открытый**— предоставленный сторонним поставщиком.  
...
-   **Закрытый**-требуются следующие сертификаты, если они еще не существует:  
...
-   **Самозаверяющий**-требуются следующие сертификаты, если они еще не существует:  
...
7.  На **запрос сертификатов** странице, установите флажок для шаблона сертификата и при необходимости щелкните **требуется больше данных для подачи заявки на этот сертификат**.  
...
### <a name="NLS_DNS"></a>Создание записей DNS для сетевого расположения сервера и веб-пробы  
  
1.  На DNS-сервере внутренней сети: На **запустить** экрана, введите ** dnsmgmt.msc**, и нажмите клавишу ВВОД.  
...
![Windows PowerShell](../../../media/Step-1-Configure-the-DirectAccess-Infrastructure_3/PowerShellLogoSmall.gif)***<em>эквивалентные команды Windows PowerShell</em>***  
...
Для развертывания службы удаленного доступа требуется как минимум два объекта групповой политики: один объект групповой политики содержит параметры для сервера удаленного доступа, один содержит параметры для клиентских компьютеров DirectAccess. При настройке удаленного доступа мастер автоматически создает необходимые объекты групповой политики. Тем не менее если организация принудительно применяет соглашение об именовании, или у вас нет необходимых разрешений для создания или изменения объектов групповой политики, они должны быть созданы до настройки удаленного доступа.  
...
> [!IMPORTANT]  
> Администратор может вручную связать объекты групповой политики DirectAccess с подразделением, выполнив следующие действия:  
>   
...
> 4.  Если связывание с подразделением не было сделано до запуска мастера DirectAccess, после завершения настройки администратор домена может свяжите объекты групповой политики DirectAccess необходимыми подразделениями. Связь с доменом можно удалить. Шаги для связывания объекта групповой политики с подразделением можно найти [здесь](https://technet.microsoft.com/library/cc732979.aspx).  
...
Настройки DirectAccess на клиентском компьютере объект групповой политики применяются только к компьютерам, которые являются членами группы безопасности, которые вы указываете при настройке удаленного доступа. Кроме того, если вы используете группы безопасности для управления серверами приложений, необходимо создать группу безопасности для этих серверов.  
...
![Windows PowerShell](../../../media/Step-1-Configure-the-DirectAccess-Infrastructure_3/PowerShellLogoSmall.gif)**эквивалентные команды Windows PowerShell**  
