...
1. Определите, какую топологию сетевых адаптеров хотите использовать. Для установки удаленного доступа могут использоваться следующие варианты:  
    ...
    - С двумя сетевыми адаптерами: Либо на границе с одним сетевым адаптером, подключенных к Интернету, а другой к внутренней сети или за NAT брандмауэр или маршрутизатор, один сетевой адаптер подключен к сети периметра, а другой для внутренней сети.  
...
|------------|----------|--------------|  
|Необходим внутренний центр сертификации для выдачи сертификатов сервера удаленного доступа и клиентам для проверки подлинности IPsec, если не используется прокси-сервер Kerberos для проверки подлинности|Общедоступный ЦС: Мы рекомендуем использовать общедоступный ЦС для выдачи сертификата IP-HTTPS, это гарантирует, что точка распространения CRL имеется внешний доступ.|Внутренний ЦС: Вы можете использовать внутренний ЦС для выдачи сертификата веб-сайта сервера сетевых расположений. Убедитесь в абсолютной доступности точки распределения списка отзыва сертификатов из внутренней сети.|  
||Внутренний ЦС: Вы можете использовать внутренний ЦС для выдачи сертификата IP-HTTPS. Однако следует убедиться, что точка распространения CRL доступна из внешней сети.|Самозаверяющий сертификат: Можно использовать самозаверяющий сертификат сетевого расположения сервера веб-сайта; Тем не менее нельзя использовать самозаверяющий сертификат в развертываниях на нескольких сайтах.|  
||Самозаверяющий сертификат: Вы можете использовать самозаверяющий сертификат для сервера IP-HTTPS. Однако следует убедиться, что точка распространения CRL доступна из внешней сети. Самозаверяющие сертификаты неприменимы к многосайтовым развертываниям.||  
...
-  **Серверы инфраструктуры**  
    ...
    1. **Сервер сетевых расположений**: Клиенты DirectAccess пытаются подключиться к серверу сетевых расположений, чтобы определить, находятся ли они во внутренней сети. Клиенты во внутренней сети должны иметь возможность разрешения имени сервера сетевых расположений. Если клиент находится во внешней сети, эта возможность должна исключаться. Для этого по умолчанию происходит добавление полного доменного имени сервера сетевых расположений в качестве правила исключения в NRPT. Кроме того, при настройке удаленного доступа автоматически создаются следующие правила:  
        ...
        **Средства проверки подключения**: Удаленного доступа создает веб-пробы по умолчанию, который используется, используемого клиентскими компьютерами DirectAccess, чтобы проверить подключение к внутренней сети. Для надлежащей работы пробы необходимо вручную зарегистрировать следующие доменные имена:  
        ...
        1.  DirectAccess-webprobehost должно разрешаться на внутренний адрес IPv4 сервера удаленного доступа или IPv6-адресом в среде IPv6.  
  
        2.  DirectAccess-corpconnectivityhost следует разрешить в адрес localhost (замыкание на себя). Следует создать записи узла A (со значением 127.0.0.1) и AAAA (со значением из префикса NAT64, последние 32 бита которого заменяются значением 127.0.0.1). Префикс NAT64 можно узнать при помощи командлета get-netnattransitionconfiguration.  
...
- **Объект групповой Политики сервера DirectAccess**: Этот GPO содержит параметры конфигурации, которые применяются к любому серверу, настроенный как сервер удаленного доступа в развертывании. Кроме того, в них записываются правила брандмауэра Windows в режиме повышенной безопасности.  
  
- **Объект групповой Политики серверов приложений**: Этот GPO содержит параметры выбранных серверов приложений, для которых расширяется проверка подлинности и шифрование с клиентов DirectAccess. Если проверка подлинности и шифрование не расширяются, данные объекты групповой политики не используются.  
...
1. **Автоматически**: Вы можете выбрать автоматическое создание. Каждому объекту присваивается имя по умолчанию.  
...
Консоль **Управление удаленным доступом** отобразит следующее сообщение об ошибке: **Не удается найти объект групповой Политики (имя объекта групповой Политики)** . Чтобы удалить параметры конфигурации, выполните следующие действия:  
  
1. Запустите командлет PowerShell **Uninstall-remoteaccess**.  
  
2. Снова откройте **управление удаленным доступом**.  
