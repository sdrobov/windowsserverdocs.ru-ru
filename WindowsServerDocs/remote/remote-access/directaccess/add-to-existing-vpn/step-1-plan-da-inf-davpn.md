...
1. Определите, какую топологию сетевых адаптеров хотите использовать. Для установки удаленного доступа могут использоваться следующие варианты:  
    ...
    - С двумя сетевыми адаптерами: На границе с одним сетевым адаптером, подключенным к Интернету, а другой — к внутренней сети или за устройством NAT, брандмауэром или маршрутизатором, с одним сетевым адаптером, подключенным к сети периметра, а другим — к внутренней сети.  
  
    - За устройством NAT с одним сетевым адаптером: Сервер удаленного доступа устанавливается за пределами устройства NAT, а один сетевой адаптер подключен к внутренней сети.  
...
3. Выполните необходимые настройки адаптеров и адресации в соответствии со следующей таблицей. Для развертываний за устройством NAT с помощью одного сетевого адаптера настройте IP-адреса, используя только столбец "внутренний сетевой адаптер".  
    ...
    |-|--------------|--------------------|------------|  
    |IPv4-интрасеть и IPv4-интернет|Необходимо установить:<br/><br/>— Один статический общедоступный IPv4-адрес с подходящими масками подсети.<br/>— IPv4-адрес шлюза по умолчанию брандмауэра Интернета или локального маршрутизатора поставщика услуг Интернета (ISP).|Необходимо установить:<br/><br/>— Адрес интрасети IPv4 с соответствующей маской подсети.<br/>— Зависящий от подключения DNS-суффикс пространства имен интрасети. Необходимо также настроить DNS-сервер во внутреннем интерфейсе.<br/>Не настраивайте шлюз по умолчанию для всех интерфейсов интрасети.|Чтобы настроить север удаленного доступа для работы со всеми подсетями внутренней IPv4-сети:<br/><br/>1.  Сформируйте список адресных пространств IPv4 для всех расположений интрасети.<br/>2.  Чтобы добавить адресные пространства IPv4 в таблицу маршрутизации IPv4 сервера удаленного доступа в качестве статических маршрутов, используйте команду **route add -p** или **netsh interface ipv4 add route**.|  
...
-  **Серверы инфраструктуры**  
    ...
    1. **Сервер сетевого расположения**: Клиенты DirectAccess пытаются подключиться к серверу сетевых расположений, чтобы определить, находятся ли они во внутренней сети. Клиенты во внутренней сети должны иметь возможность разрешения имени сервера сетевых расположений. Если клиент находится во внешней сети, эта возможность должна исключаться. Для этого по умолчанию происходит добавление полного доменного имени сервера сетевых расположений в качестве правила исключения в NRPT. Кроме того, при настройке удаленного доступа автоматически создаются следующие правила:  
        ...
        **Сервер IP-HTTPS**: Сервер удаленного доступа выступает в качестве прослушивателя IP-HTTPS и использует его сертификат сервера для проверки подлинности клиентов IP-HTTPS. Имя IP-HTTPS должно разрешаться для клиентов DirectAccess с помощью публичных DNS-серверов.  
        ...
        Средства, **проверяющие подключение**: Удаленный доступ создает веб-зонд по умолчанию, используемый клиентскими компьютерами DirectAccess для проверки подключения к внутренней сети. Для надлежащей работы пробы необходимо вручную зарегистрировать следующие доменные имена:  
...
Консоль **Управление удаленным доступом** отобразит следующее сообщение об ошибке: **Не удается найти объект GPO (имя объекта групповой политики)** . Чтобы удалить параметры конфигурации, выполните следующие действия:  
