...
![Installation успешное выполнение](../../../media/Step-2-Configuring-DirectAccess-Servers/PowerShellLogoSmall.gif)***<em>эквивалентных команд Windows PowerShell</em>***  
...
1.  На сервере удаленного доступа откройте консоль управления удаленным доступом: На **начальном** экране введите**рамгмтуи. exe**и нажмите клавишу ВВОД. Если появится диалоговое окно **контроля учетных записей**, подтвердите, что отображаемое в нем действие именно то, которое требуется, и нажмите кнопку **Да**.  
...
5.  На странице **Управление** добавьте все серверы управления, которые не были определены автоматически, и нажмите кнопку **Далее**. Служба удаленного доступа автоматически добавляет контроллеры домена и серверы System Center Configuration Manager.  
    ...
    > [!NOTE]  
    > Хотя серверы добавляются автоматически, они не отображаются в списке. Серверы System Center Configuration Manager будут показаны в списке после первого применения конфигурации.  
...
4.  Чтобы использовать проверку подлинности без шифрования, выберите параметр **Do не шифровать трафик. Флажок использовать только проверку подлинности @ no__t-0.  
