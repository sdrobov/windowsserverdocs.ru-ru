---
title: Шаг 1 настраивать сложную инфраструктуру DirectAccess
...
В этом разделе описывается, как настроить инфраструктуру, необходимую для расширенного развертывания службы удаленного доступа, в котором используется один сервер DirectAccess в смешанной среде с поддержкой IPv4 и IPv6. Перед началом развертывания убедитесь, что выполнены действия по планированию, описанные в [Планирование расширенного развертывания DirectAccess](../../../remote-access/directaccess/single-server-advanced/Plan-an-Advanced-DirectAccess-Deployment.md).  
...
## <a name="ConfigNetworkSettings"></a>1.1. Настройка сетевых параметров сервера  
...
> -   Если невозможно настроить второй сетевой адаптер для профиля домена, политик DirectAccess IPsec должны быть вручную установить область действия для всех профилей с помощью следующей команды Windows PowerShell, после настройки DirectAccess:  
>   
>     ```  
...
-   Вручную настройте организации «s маршрутах IPv4 и IPv6 на серверах DirectAccess. Добавьте опубликованный маршрут, чтобы весь трафик с префиксом IPv6 организации (/48) пересылался во внутреннюю сеть Для IPv4-трафика добавьте явные маршруты, чтобы пересылать его во внутреннюю сеть.  
...
-   Трафик Teredo "протокола UDP (User Datagram) порт назначения 3544 входящих подключений и UDP-порт источника 3544 для исходящих подключений.  
  
-   трафик 6to4 «протокол 41 для входящих и исходящих.  
  
-   IP-HTTPS «протокола управления передачей (TCP) порт назначения 443 и TCP-порт источника 443 для исходящих подключений. Если на сервере DirectAccess один сетевой адаптер, а сервер сетевых расположений находится на сервере DirectAccess, также требуется TCP-порт 62000.  
...
Удаленный доступ в Windows Server 2012 позволяет выбрать использование сертификатов для проверки подлинности компьютеров или встроенный прокси-сервера Kerberos, который проверяет подлинность с помощью имен пользователей и паролей. Также необходимо настроить сертификат IP-HTTPS на сервере DirectAccess.  
  
Дополнительные сведения см. в разделе [служб сертификатов Active Directory](https://technet.microsoft.com/library/cc770357.aspx).  
...
1.  Во внутреннем ЦС, решите, если будет использоваться **компьютера** шаблон сертификата, или вы создадите новый шаблон сертификата, как описано в статье [Создание шаблонов сертификатов](https://technet.microsoft.com/library/cc731705.aspx).  
...
3.  Если необходимо, настройте для шаблона сертификата автоматическую регистрацию. Дополнительные сведения см. в разделе [Настройка автоматической регистрации сертификатов](https://technet.microsoft.com/library/cc731522.aspx).  
...
1.  Во внутреннем ЦС, создайте шаблон сертификата, как описано в [Создание шаблонов сертификатов](https://technet.microsoft.com/library/cc731705.aspx).  
...
-   Сертификат веб-сайта, используемый для проверки подлинности IP-HTTPS. Субъектом сертификата должно быть полное доменное имя с возможностью внешнего разрешения, доступное из Интернета. Сертификат основан на шаблоне сертификата, созданный, следуя инструкциям в 1.5.2 Настройка шаблонов сертификатов.  
...
-   В **субъекта** укажите полное ДОМЕННОЕ имя URL-адрес IP-HTTPS.  
...
1.  На сервере DirectAccess: На **запустить** введите**mmc.exe**, и нажмите клавишу ВВОД.  
...
7.  На **запрос сертификатов** странице, установите флажок для шаблона сертификата, созданную ранее (Дополнительные сведения см. в разделе 1.5.2 Настройка сертификата шаблонов). При необходимости щелкните **Требуется больше данных для регистрации этого сертификата**.  
...
### <a name="NLS_DNS"></a>Создание сервера сетевых расположений  
...
Для развертывания удаленного доступа необходимы как минимум два объекта групповой политики:  
  
-   один содержит параметры для сервера DirectAccess;  
  
-   другой содержит параметры для клиентских компьютеров DirectAccess.  
  
При настройке удаленного доступа мастер автоматически создает необходимые объекты групповой политики. Но если организация принудительно применяет соглашение об именовании, вы можете ввести имя в диалоговом окне "Объект групповой политики" в консоли управления удаленным доступом. Дополнительные сведения см. в разделе 2.7. Конфигурации сводки и альтернативные объекты групповой политики. Если у вас есть разрешения на создание, будет создан GPO. Если у вас нет требуемых разрешений для создания GPO, их необходимо настроить до настройки службы удаленного доступа.  
...
> [!IMPORTANT]  
> Администраторы могут вручную связать объекты групповой политики DirectAccess с подразделением (OU), сделав следующее:  
>   
...
> 3.  Администратор удаленного доступа может иметь или не иметь разрешения связывать объекты групповой политики к домену. В любом из этих случаев объекты групповой политики будут настроены автоматически. Если объекты групповой политики уже привязаны к подразделению, связи не будут удалены и эти GPO не будут привязаны к домену. Для объекта групповой политики сервера подразделение должно включать объект-компьютер сервера. В противном случае объект групповой политики будет связан с корневым каналом домена.  
> 4.  Если вы не создали связь с подразделением перед запуском мастера DirectAccess, после завершения настройки, администратор домена может связать объекты групповой политики DirectAccess к требуемым подразделениям. Связь с доменом можно удалить. Дополнительные сведения см. в разделе [связать объект групповой политики](https://technet.microsoft.com/library/cc732979.aspx).  
...
3.  Производственных GPO см. в статье [связать объект групповой политики](https://technet.microsoft.com/library/cc732979).  
  
4.  Предоставьте администратору удаленного доступа разрешение **Изменение параметров, удаление и изменение разрешений безопасности** для всех промежуточных GPO. Дополнительные сведения см. в разделе [Делегирование разрешений для группы или пользователя на объект групповой политики](https://technet.microsoft.com/library/cc754542).  
  
5.  DENY, запрет разрешений администратора удаленного доступа, чтобы связать объекты групповой политики во всех доменах (или убедитесь, что администратор удаленного доступа «t разрешение таких). Дополнительные сведения см. в разделе [Делегирование разрешений на связи объектов групповой политики](https://technet.microsoft.com/library/cc755086).  
...
2.  Экспортируйте параметры, создав резервную копию всех промежуточных GPO в развертывании удаленного доступа. Дополнительные сведения см. в разделе обратно вверх объекта групповой политики.  
  
3.  Для каждого производственного GPO измените фильтры безопасности в соответствии с фильтрами соответствующего промежуточного GPO. Дополнительные сведения см. в разделе фильтра с использованием групп безопасности.  
...
4.  Для каждого производственного GPO импортируйте параметры из резервной копии соответствующего промежуточного GPO следующим образом:  
    ...
    1.  В политике управления консоли Групповыми политиками, разверните узел объекты групповой политики в лесу и домене, содержащем производственный объект групповой политики, в которую будут импортированы параметры.  
...
-   Для резервного копирования промежуточный GPO клиента «Параметры клиента DirectAccess — промежуточный» в домене «corp.contoso.com» в папке резервного копирования «C:\Backups\":  
  
    ```  
    $backup = Backup-GPO "Name 'DirectAccess Client Settings - Staging' "Domain 'corp.contoso.com' "Path 'C:\Backups\'  
    ```  
  
-   Чтобы просмотреть фильтры безопасности промежуточного GPO «Клиента параметры DirectAccess — промежуточный» в домене «corp.contoso.com» клиента:  
  
    ```  
    Get-GPPermission "Name 'DirectAccess Client Settings - Staging' "Domain 'corp.contoso.com' "All | ?{ $_.Permission "eq 'GpoApply'}  
    ```  
  
-   Чтобы добавить группу безопасности «corp.contoso.com\DirectAccess clients» в фильтр безопасности производственного GPO клиента «параметры клиента DirectAccess «Production» в домене «corp.contoso.com»:  
...
-   Чтобы импортировать параметры из резервной копии в производственный GPO клиента «параметры клиента DirectAccess «Production» в домене «corp.contoso.com»:  
...
Параметры DirectAccess, которые содержатся в клиентский компьютер, объект групповой политики применяются только к компьютерам, которые являются членами группы безопасности, указанные вами при настройке удаленного доступа. Кроме того, если вы используете группы безопасности для управления серверами приложений, необходимо создать группу безопасности для этих серверов.  
  
### <a name="Sec_Group"></a>Чтобы создать группу безопасности для клиентов DirectAccess  
  
1.  На **запустить** введите**dsa.msc**, и нажмите клавишу ВВОД. В консоли **Active Directory — пользователи и компьютеры** разверните в левой области домен, к которому будет принадлежать группа безопасности, щелкните правой кнопкой мыши **Пользователи**, выберите **Новые**, после чего щелкните **Группа**.  
...
![Windows PowerShell](../../../media/Step-1-Configuring-DirectAccess-Infrastructure/PowerShellLogoSmall.gif)**эквивалентные команды Windows PowerShell**  
...
-   **Закрытый сертификат**  
    ...
    Этот сертификат основан на шаблоне сертификата, который вы создали, следуя инструкциям в [1.5.2 Настройка шаблонов сертификатов](#ConfigCertTemp).  
...
7.  На **запрос сертификатов** страницы, установите флажок для шаблона сертификата, созданного по инструкциям в 1.5.2 Настройка шаблонов сертификатов. При необходимости щелкните **Требуется больше данных для регистрации этого сертификата**.  
...
3.  Настройте сайт CRL с высоким уровнем доступности из внутренней сети.  
    ...
    -   Веб-серверов с помощью URL-адрес на основе HTTP, таких как: https://crl.corp.contoso.com/crld/corp-APP1-CA.crl  
