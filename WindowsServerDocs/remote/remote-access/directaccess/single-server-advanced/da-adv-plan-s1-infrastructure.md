...
|Задача|Описание|
|----|--------|  
|[1,1. Планирование топологии сети и параметров](#11-plan-network-topology-and-settings)|Выберите место размещения сервера DirectAccess (на границе либо за устройством преобразования сетевых адресов (NAT) или брандмауэром) и запланируйте IP-адресацию, маршрутизацию и принудительное туннелирование.|  
|[1,2. Планирование требований к брандмауэру](#12-plan-firewall-requirements)|Определите, какой трафик DirectAccess будет пропускаться через пограничные брандмауэры.|  
|[1,3. Планирование требований к сертификатам](#13-plan-certificate-requirements)|Решите, будет ли использоваться для клиентской проверки подлинности протокол Kerberos или сертификаты и запланируйте сертификаты веб-сайта. IP-HTTPS — это протокол перехода, используемый клиентами DirectAccess для туннелирования трафика IPv6 через сети IPv4. Определите, будет ли проверка подлинности на сервере IP-HTTPS выполняться с помощью сертификата, выданного центром сертификации (ЦС), или с помощью самозаверяющего сертификата, автоматически выдаваемого сервером DirectAccess.|  
|[1,4. Планирование требований к DNS](#14-plan-dns-requirements)|Запланируйте параметры DNS для сервера DirectAccess, серверы инфраструктуры, параметры локального разрешения имен и подключение клиентов.|  
...
|[1,6. серверы управления планированием](#16-plan-management-servers)|Вы можете удаленно управлять клиентскими компьютерами DirectAccess, размещенными за пределами корпоративной сети в Интернете. Планируйте для серверов управления (например, серверов обновления), которые используются во время управления удаленными клиентами.|  
|[1,7. Планирование домен Active Directory служб](#17-plan-active-directory-domain-services)|Запланируйте контроллеры домена, требования Active Directory, клиентскую проверку подлинности и несколько доменов.|  
|[1,8. планирование объектов групповая политика](#18-plan-group-policy-objects)|Определите, какие объекты групповой политики требуются вашей организации и как производить их создание и редактирование.|  
...
- [Планирование сетевых адаптеров и IP-адресов для 1.1.1](#111-plan-network-adapters-and-ip-addressing)  
  
- [подключение к интрасети IPv6 плана 1.1.2](#112-plan-ipv6-intranet-connectivity)  
  
- [план 1.1.3 для принудительного туннелирования](#113-plan-for-force-tunneling)  
...
3. Настройте требуемые адаптеры и адреса, опираясь на следующую таблицу. Для развертываний, использующих один сетевой адаптер, которые настроены за устройством NAT, настройте IP-адреса, используя только столбец **Адаптер внутренней сети**.  
    ...
    > [!NOTE]  
    > - Если клиенту DirectAccess был присвоен публичный IPv4-адрес, для подключения к интрасети он будет использовать технологию туннелирования 6to4. Если ему назначен частный IPv4-адрес, будет использоваться Teredo. Если клиенту DirectAccess не удается подключиться к серверу DirectAccess с помощью 6to4 или Teredo, будет использоваться IP-HTTPS.  
...
- [1.3.1. Планирование сертификатов компьютеров для проверки подлинности IPsec](#131-plan-computer-certificates-for-ipsec-authentication)  
...
-   [сертификаты плана 1.3.2 для IP-HTTPS](#132-plan-certificates-for-ip-https)  
...
-   [1.3.3 план сертификатов веб-сайта для сервера сетевых расположений](#133-plan-website-certificates-for-the-network-location-server)  
...
||**Такую**<br/><br/>Общедоступный ЦС:<br/><br/>Рекомендуется использовать общедоступный ЦС для выдачи сертификата  IP-HTTPS. Это обеспечивает внешний доступ к точке распространения CRL.|  
...
-   [план 1.4.1 для требований DNS-сервера](#141-plan-for-dns-server-requirements)  
  
-   [план 1.4.2 для локального разрешения имен](#142-plan-for-local-name-resolution)  
...
**Имена с одной меткой**  
  
Имена одиночных меток, например <https://paycheck>, иногда используются для серверов интрасети. Если запрошено однокомпонентное имя и настроен список поиска суффикса DNS, к однокомпонентному имени будут добавлены суффиксы из списка. Например, если пользователь компьютера, который является членом домена corp.contoso.com, <https://paycheck> в веб-браузере, то полное доменное имя, созданное в качестве имени, — paycheck.corp.contoso.com. По умолчанию добавляемый суффикс основан на основном суффиксе DNS клиентского компьютера.  
...
-   System Center Configuration Manager серверы. Автоматическое обнаружение серверов System Center Configuration Manager выполняется для всех доменов в том же лесу, что и сервер DirectAccess и клиентские компьютеры.  
  
Контроллеры домена и серверы System Center Configuration Manager автоматически обнаруживаются при начальной настройке DirectAccess. Обнаруженные контроллеры домена не отображаются в консоли, но параметры можно получить с помощью командлета Windows PowerShell **Get-дамгмтсервер-Type All**. Если контроллер домена или серверы System Center Configuration Manager изменяются, щелкните **Обновить серверы управления** в консоли управления удаленным доступом, чтобы обновить список серверов управления.  
...
-   [1.7.1 план проверки подлинности клиента](#171-plan-client-authentication)  
...
-   [1.8.1 настроить автоматически созданные объекты групповой политики](#181-configure-automatically-created-gpos)  
  
-   [1.8.2 Настройка объектов групповой политики, созданных вручную](#182-configure-manually-created-gpos)  
  
-   [1.8.3 Управление объектами групповой политики в среде с несколькими доменами](#183-manage-gpos-in-a-multi-domain-controller-environment)  
...
-   [1.8.5 восстановление из удаленного объекта групповой политики](#185-recover-from-a-deleted-gpo)  
...
Независимо от того, используете ли вы автоматически или вручную настраиваемые GPO, вам следует добавить политику обнаружения медленного канала, если клиенты будут применять 3G-сети. Путь для @no__t 0Policy: Настройте групповая политика обнаружения медленного канала @ no__t-0: **Computer configuration/Polices/Administrative Templates/System/Group Policy**.  
...
В консоли управления удаленным доступом может появиться следующее сообщение об ошибке: **Не удается найти объект GPO (имя объекта групповой политики)** . Чтобы удалить параметры конфигурации, выполните следующие действия:  
...
-   [Шаг 2. Планирование развертываний DirectAccess @ no__t-0  
