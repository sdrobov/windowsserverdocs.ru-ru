---
title: Шаг 1 плана расширенной инфраструктуры DirectAccess
...
ms.locfileid: "59880965"
---
# <a name="step-1-plan-the-advanced-directaccess-infrastructure"></a>Шаг 1 плана расширенной инфраструктуры DirectAccess
...
|Задача|Описание|  
|----|--------|  
|[1.1. Планирование топологии и параметров сети](#bkmk_11Networksvrtopsettings)|Выберите место размещения сервера DirectAccess (на границе либо за устройством преобразования сетевых адресов (NAT) или брандмауэром) и запланируйте IP-адресацию, маршрутизацию и принудительное туннелирование.|  
|[1.2. Планирование требований брандмауэра](#bkmk_ConfigFirewalls)|Определите, какой трафик DirectAccess будет пропускаться через пограничные брандмауэры.|  
|[1.3. Планирование требований сертификатов](#bkmk_12CAsandCerts)|Решите, будет ли использоваться для клиентской проверки подлинности протокол Kerberos или сертификаты и запланируйте сертификаты веб-сайта. IP-HTTPS — это протокол перехода, используемый клиентами DirectAccess для туннелирования трафика IPv6 через сети IPv4. Определите, будет ли проверка подлинности на сервере IP-HTTPS выполняться с помощью сертификата, выданного центром сертификации (ЦС), или с помощью самозаверяющего сертификата, автоматически выдаваемого сервером DirectAccess.|  
|[1.4. Планирование требований DNS](#bkmk_14Dns)|Запланируйте параметры DNS для сервера DirectAccess, серверы инфраструктуры, параметры локального разрешения имен и подключение клиентов.|  
|[1.5. Планирование сервера сетевых расположений](#bkmk_14NLS)|Сервер сетевых расположений используется клиентами DirectAccess, чтобы определить, находятся ли они во внутренней сети. Выберите, где будет размещен веб-сайта сервера сетевых расположений в вашей организации (на сервере DirectAccess или другом сервере) и определите требования для сертификатов, если сервер сетевых расположений расположен на сервере DirectAccess.|  
|[1.6. Планирование серверов управления](#bkmk_15mgmtservers)|Вы можете удаленно управлять клиентскими компьютерами DirectAccess, размещенными за пределами корпоративной сети в Интернете. Планируйте для серверов управления (например, серверов обновления), которые используются во время управления удаленными клиентами.|  
|[1.7. Планирование доменных служб Active Directory](#bkmk_16AD)|Запланируйте контроллеры домена, требования Active Directory, клиентскую проверку подлинности и несколько доменов.|  
|[1.8. Планирование объектов групповой политики](#bkmk_17GPOs)|Определите, какие объекты групповой политики требуются вашей организации и как производить их создание и редактирование.|  
  
## <a name="bkmk_11Networksvrtopsettings"></a>1.1. Планирование топологии и параметров сети  
В этом разделе описывается планирование сети, в том числе:  
  
-   [1.1.1. Планирование сетевых адаптеров и IP-адреса](#BKMK_NA)  
  
-   [1.1.2. Планирование возможностей подключения к интрасети IPv6](#bkmk_intranet)  
  
-   [1.1.3. Планирование принудительного туннелирования](#bkmk_force)  
  
### <a name="BKMK_NA"></a>1.1.1. Планирование сетевых адаптеров и IP-адреса  
...
3.  Настройте требуемые адаптеры и адреса, опираясь на следующую таблицу. Для развертываний, использующих один сетевой адаптер, которые настроены за устройством NAT, настройте IP-адреса, используя только столбец **Адаптер внутренней сети**.  
    ...
    |-|--------------|--------------|------------|  
    |Интернет с IPv4-адресацией и интрасеть с IPv4-адресацией|Настройте два статичных последовательных IPv4-адреса с соответствующими масками подсети (требуется только для Teredo).<br /><br />Кроме того, настройте IPv4-адрес шлюза по умолчанию интернет-брандмауэра или локального маршрутизатора поставщика услуг Интернета (ISP). **Примечание.** Серверу DirectAccess необходимы два последовательных IPv4-адреса, чтобы действовать как сервер Teredo и чтобы клиенты под управлением Windows могли использовать сервер DirectAccess для обнаружения типа устройства NAT.|Необходимо установить:<br /><br />Интрасети IPv4-адрес с подходящей маской подсети.<br />-Зависящий от подключения DNS-суффикс пространства имен интрасети. DNS-сервер во внутреннем интерфейсе. **Внимание!** Не настраивайте шлюз по умолчанию для интерфейсов интрасети.|Чтобы настроить сервер DirectAccess для связи со всеми подсетями во внутренней IPv4-сети, выполните следующие действия.<br /><br />-Список адресных пространств IPv4 для всех расположений интрасети.<br />— Используйте **маршрута добавьте -p** или**netsh interface ipv4 добавления маршрута** команду, чтобы добавить адресные пространства IPv4 как статические маршруты в таблицу маршрутизации IPv4 сервера DirectAccess.|  
    |IPv6-интернет и IPv6-интрасеть|Необходимо установить:<br /><br />-Используйте конфигурацию адресов, предоставленную поставщиком услуг Интернета.<br />— Используйте **Route Print** команду, чтобы убедиться, что маршрут IPv6 по умолчанию существует и указывает на маршрутизатор поставщика услуг Интернета в таблице маршрутизации IPv6.<br />— Определите, используют ли маршрутизаторы поставщика услуг Интернета и интрасети, параметры маршрутизатора по умолчанию описано в документе RFC 4191, а также использование большего приоритета по умолчанию, чем у локальных маршрутизаторов интрасети.<br />    Если эти условия выполняются, дальнейшие настройки маршрута по умолчанию не требуются. Использование большего приоритета для маршрутизатора провайдера подразумевает, что активный маршрут IPv6 по умолчанию сервера DirectAccess направлен в IPv6-интернет.<br /><br />Поскольку сервером DirectAccess служит IPv6-маршрутизатор, при наличии собственной инфраструктуры IPv6 веб-интерфейс может также получить доступ к контроллерам домена в интрасети. В этом случае на контроллере домена в сети периметра следует использовать фильтрацию пакетов, чтобы предотвратить подключение к IPv6-адресу имеющего доступ в Интернет интерфейса сервера DirectAccess.|Необходимо установить:<br /><br />— Если не используются Уровни предпочтений по умолчанию, можно настроить интерфейсы интрасети с помощью следующей команды**ipv6 интерфейса netsh set InterfaceIndex ignoredefaultroutes = включен**.<br />    После выполнения этой команды дополнительные маршруты по умолчанию, указывающие на маршрутизаторы интрасети, не будут добавлены в таблицу маршрутизации IPv6. Индекс интерфейсов интрасети можно получить, используя следующую команду: **netsh interface ipv6 show interface**.|Если ваша интрасеть использует протокол IPv6, для настройки сервера DirectAccess и осуществления доступа ко всем расположениям IPv6 нужно выполнить следующие действия:<br /><br />-Список адресных пространств IPv6 для всех расположений интрасети.<br />— Используйте **netsh interface ipv6 добавления маршрута** команду, чтобы добавить адресные пространства IPv6 в качестве статических маршрутов в таблице маршрутизации IPv6 сервера DirectAccess.|  
    |IPv4-Интернет и IPv6-интрасеть|Сервер DirectAccess перенаправляет IPv6-трафик по умолчанию через адаптер Microsoft 6to4 в ретранслятор 6to4 в IPv4-интернете. Можно настроить сервер DirectAccess для адреса IPv4 адаптера Microsoft 6to4 с помощью следующей команды: **ретрансляции имя набора netsh interface ipv6 6to4 =<ipaddress> состояние = включен**.|||  
...
### <a name="bkmk_intranet"></a>1.1.2. Планирование возможностей подключения к интрасети IPv6  
...
### <a name="bkmk_force"></a>1.1.3. Планирование принудительного туннелирования  
...
## <a name="bkmk_ConfigFirewalls"></a>1.2. Планирование требований брандмауэра  
...
## <a name="bkmk_12CAsandCerts"></a>1.3. Планирование требований сертификатов  
Существуют определенные сценарии, в которых при развертывании одного сервера DirectAccess требуются сертификаты:  
  
-   [1.3.1. Планирование сертификатов компьютера для проверки подлинности IPsec](#BKMK_compcert)  
...
-   [1.3.3. Планирование сертификатов веб-сайт для сервера сетевых расположений](#bkmk_webnlc)  
...
|------------|----------|--------------|  
|Необходим внутренний центр сертификации для выдачи сертификатов серверу DirectAccess и клиентам для проверки подлинности IPsec, если не используется прокси-сервер Kerberos для проверки подлинности|Внутренний ЦС:<br /><br />Вы можете использовать внутренний ЦС для выдачи сертификата IP-HTTPS. Однако следует убедиться, что точка распространения CRL доступна из внешней сети.|Внутренний ЦС:<br /><br />Вы можете использовать внутренний ЦС для выдачи сертификата веб-сайта сервера сетевых расположений. Убедитесь, что точка распространения CRL обладает высоким уровнем доступности из внутренней сети.|  
...
||**Рекомендуется использовать**<br /><br />Общедоступный ЦС:<br /><br />Рекомендуется использовать общедоступный ЦС для выдачи сертификата  IP-HTTPS. Это обеспечивает внешний доступ к точке распространения CRL.|  
  
### <a name="BKMK_compcert"></a>1.3.1. Планирование сертификатов компьютера для проверки подлинности IPsec  
...
### <a name="bkmk_iph"></a>1.3.2. Планирование сертификатов для IP-HTTPS  
...
### <a name="bkmk_webnlc"></a>1.3.3. Планирование сертификатов веб-сайт для сервера сетевых расположений  
...
## <a name="bkmk_14Dns"></a>1.4. Планирование требований DNS  
...
-   [1.4.1. Планирование требований DNS-сервера](#bkmk_dnsserverrequirements)  
  
-   [1.4.2. Планирование локального разрешения имен](#bkmk_dnslocalname)  
...
### <a name="bkmk_dnsserverrequirements"></a>1.4.1. Планирование требований DNS-сервера  
...
### <a name="bkmk_dnslocalname"></a>1.4.2. Планирование локального разрешения имен  
...
## <a name="bkmk_14NLS"></a>1.5. Планирование сервера сетевых расположений  
...
## <a name="bkmk_15mgmtservers"></a>1.6. Планирование серверов управления  
...
## <a name="bkmk_16AD"></a>1.7. Планирование доменных служб Active Directory  
...
-   [1.7.1. планирование проверки подлинности клиента](#bkmk_clientauth)  
  
-   [1.7.2. Планирование нескольких доменов](#bkmk_multiple)  
...
### <a name="bkmk_clientauth"></a>1.7.1. планирование проверки подлинности клиента  
...
### <a name="bkmk_multiple"></a>1.7.2. Планирование нескольких доменов  
...
Если служба WINS развернута в среде с несколькими доменами, следует развернуть зону прямого просмотра WINS в DNS. Дополнительные сведения см. в разделе **единый имена меток** в [1.4.2. Планирование локального разрешения имен](#bkmk_dnslocalname) настоящего документа.  
  
## <a name="bkmk_17GPOs"></a>1.8. Планирование объектов групповой политики  
...
-   [1.8.1. Настройка автоматически созданных объектов групповой политики](#bkmk_autoGPO)  
  
-   [1.8.2. Настройка вручную созданных объектов групповой политики](#bkmk_manualGPO)  
  
-   [1.8.3 Управление объектами групповой политики в среде с несколькими контроллерами домена](#bkmk_multiDC)  
  
-   [1.8.4. Управление GPO удаленного доступа с ограниченными разрешениями](#bkmk_manageGPO)  
  
-   [1.8.5. Восстановление удаленного объекта групповой Политики](#bkmk_delGPO)  
...
### <a name="bkmk_autoGPO"></a>1.8.1. Настройка автоматически созданных объектов групповой политики  
...
### <a name="bkmk_manualGPO"></a>1.8.2. Настройка вручную созданных объектов групповой политики  
...
### <a name="bkmk_multiDC"></a>1.8.3 Управление объектами групповой политики в среде с несколькими контроллерами домена  
...
### <a name="bkmk_manageGPO"></a>1.8.4. Управление GPO удаленного доступа с ограниченными разрешениями  
...
### <a name="bkmk_delGPO"></a>1.8.5. Восстановление удаленного объекта групповой Политики  
...
В консоли управления удаленным доступом может появиться следующее сообщение об ошибке: **Объект групповой Политики <GPO name> не удается найти**. Чтобы удалить параметры конфигурации, выполните следующие действия:  
