---
title: Начало работы с удаленными рабочими столами в iOS
...
---
# <a name="get-started-with-remote-desktop-on-ios"></a>Начало работы с удаленными рабочими столами в iOS
...
1. Загрузите клиент удаленного рабочего стола (Майкрософт) из [iTunes](https://itunes.apple.com/us/app/microsoft-remote-desktop/id714464092?mt=8).
...
### <a name="download-the-remote-desktop-ios-beta-client"></a>Загрузите бета-версию клиента удаленного рабочего стола для iOS
На устройстве iOS следуйте [этим инструкциям](https://aka.ms/rdiosbeta), чтобы загрузить бета-версию клиента удаленного рабочего стола для iOS.

### <a name="add-a-remote-desktop-connection"></a>Добавьте подключение к удаленному рабочему столу
...
- Настроенные подключения удаленного приложения RemoteApp и подключения к удаленным рабочим столам отображаются в списке.
...
1. В Центре подключений нажмите **Параметры > Шлюзы**. 
...
3. Введите следующие сведения:
   ...
   - **Имя пользователя** — имя пользователя и пароль, используемые для шлюза удаленных рабочих столов, к которому вы подключаетесь. Можно также выбрать **Использовать учетные подключения**, чтобы использовать те же имя пользователя и пароль, что и для подключения к удаленному рабочему столу.


## <a name="manage-your-user-accounts"></a>Управление учетными записями пользователей 

При подключении к рабочему столу или удаленным ресурсам можно сохранить учетные записи пользователей, чтобы выбрать их снова. Вы можете управлять своими учетными записями с помощью клиента удаленного рабочего стола.

Чтобы создать учетную запись пользователя, сделайте следующее:

1. В Центре подключений нажмите **Параметры**, а затем **Имена пользователей**.
...
1. В Центре подключений нажмите **Параметры > Имена пользователей**.
2. Прокрутите строку справа налево, чтобы выбрать пользователя.
3. Нажмите **Удалить**.
