...
С помощью клиента удаленного доступа к рабочему столу для Android можно работать с приложениями для Windows и рабочими столами Windows непосредственно с устройства Android или Chromebook, которое поддерживает Google Play Маркет.
...
## <a name="set-up-the-remote-desktop-client-for-android"></a>Настройка клиента удаленного доступа к рабочему столу для Android

### <a name="download-the-remote-desktop-client-from-the-google-play-store"></a>Скачайте клиент удаленного доступа к рабочему столу из Google Play Маркет.

Вот как можно установит клиент удаленного доступа к рабочему столу на устройстве Android.

1. Скачайте клиент удаленного доступа к рабочему столу (Майкрософт) из [Google Play](https://play.google.com/store/apps/details?id=com.microsoft.rdc.android).
2. Запустите **клиент удаленного доступа к рабочему столу** из списка приложений.
...
> [!NOTE]
∟> Если вы хотите протестировать новые возможности еще до их выпуска, мы рекомендуем скачать [бета-версию клиента удаленного доступа к рабочему столу (Майкрософт)](https://play.google.com/store/apps/details?id=com.microsoft.rdc.android.beta) из Google Play Маркет.
...
4. Вы также можете нажать **Show additional options** (Показать дополнительные параметры), чтобы задать следующие необязательные параметры.
   ...
   - Параметр **Customize display resolution** (Настроить разрешение экрана) задает разрешение экрана для удаленного сеанса. Если этот параметр отключен, то используется разрешение, указанное в глобальных параметрах.   - Параметр **Переключить кнопки мыши** переключает команды, отправляемые жестами щелчка правой и левой кнопок мыши. Идеально подходит для пользователей-левшей.
...
2. Введите значение параметра **Feed URL** (URL-адрес веб-канала). Это может быть URL-адрес или адрес электронной почты.
   - В поле **URL** (URL-адрес) указывается сервер веб-доступа к удаленным рабочим столам, предоставленный администратором. Для обращения к ресурсам из Виртуального рабочего стола Windows можно использовать `https://rdweb.wvd.microsoft.com`.
...
клиента удаленного доступа к рабочему столу поддерживают закрепление подключений на начальном экране с помощью функции мини-приложения Android. Способ добавления мини-приложения зависит от типа используемого устройства Android и его операционной системы. Вот наиболее распространенный способ добавления мини-приложения:
...
> [!NOTE]
> При переименовании подключения к рабочему столу в клиенте удаленного доступа к рабочему столу его закрепленная метка не изменяется.
...
- **Help improve Remote Desktop** (Помощь в улучшении ) отправляет анонимные данные о том, как вы используете Удаленный рабочий стол для Android в Microsoft. Мы используем эти данные, чтобы улучшить клиент. Дополнительные сведения о политике конфиденциальности и типах собранных данных см. в [Заявление о конфиденциальности корпорации Майкрософт](https://privacy.microsoft.com/privacystatement). Эта настройка включена по умолчанию.
...
Шлюз удаленных рабочих столов (шлюз RD) позволяет подключаться к удаленному компьютеру в частной сети из любой точки Интернета. Вы можете создать шлюзы и управлять ими с помощью клиента удаленного доступа к рабочему столу.
...
9   - Выберите **учетную запись пользователя**, которая будет использоваться для доступа к шлюзу удаленных рабочих столов.
...
Ниже приведены краткие сведения о том, как открыть сеанс даленного рабочего стола и перемещаться по нему.
