...
8. Дисковыми пространствами масштабируемый файловый сервер с репликой хранилища в двух развертываниях. Используйте [сценарий PowerShell](https://github.com/robotechredmond/301-s2d-sr-dr-md/tree/master/scripts) развертывание [шаблона](https://github.com/robotechredmond/301-s2d-sr-dr-md) между группами ресурсов.
   ...
   >1. Развертывание [двумя узлами хранилища пробелы прямой SOFS](rds-storage-spaces-direct-deployment.md) in RG объект для хранения на диски профилей пользователей (UPD).
   >2. Развертывание во-вторых, идентичные хранилища пробелы прямой масштабируемого файлового сервера в RG B – необходимо использовать одинаковый объем хранилища в каждом кластере.
...
1. Запустите [командлет Set-RDSessionCollectionConfiguration](https://docs.microsoft.com/powershell/module/remotedesktop/set-rdsessioncollectionconfiguration) включить диски профилей пользователей для развертывания основной (активный) — укажите путь к общей папке на исходном томе (которая была создана на шаге 7 на этапах развертывания).
...
В случае активное — пассивное развертывание отработки отказа необходимо запустить виртуальные машины дополнительного развертывания. Это можно сделать вручную или с помощью сценарий автоматизации. В случае катастрофической отработку отказа прямой SOFS пространства хранения измените направление партнерство реплики хранилища, таким образом, чтобы конечный том становится исходного тома. Пример:
