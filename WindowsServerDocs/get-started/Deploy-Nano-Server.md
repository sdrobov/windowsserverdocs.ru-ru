...
ms.locfileid: "6121503"
---
# Развертывание сервера Nano Server
...
Кроме того, можно найти и установить эти пакеты с помощью поставщика NanoServerPackage модуля PackageManagement (OneGet) PowerShell. См. раздел "Установка ролей и компонентов в сети" этой статьи.  
...
|Агент System Center Operations Manager| Устанавливается отдельно. См. в документации System Center Operations Manager Дополнительные сведения о https://technet.microsoft.com/system-center-docs/om/manage/install-agent-on-nano-server.|
...
|Агент протокола SNMP|-Package Microsoft-NanoServer-SNMP-Agent-Package.cab<br />**Примечание.** Не содержится на установочном носителе Windows Server2016. Доступно только в Интернете. См. раздел [Установка ролей и компонентов в сети](https://technet.microsoft.com/windows-server-docs/get-started/deploy-nano-server#a-namebkmkonlineainstalling-roles-and-features-online).|
|Служба IPHelper, которая обеспечивает возможность туннельного подключения с помощью технологий туннелирования для IP версии6 (6to4, ISATAP, Port Proxy и Teredo) и IP-HTTPS|-Package Microsoft-NanoServer-IPHelper-Service-Package.cab<br />**Примечание.** Не содержится на установочном носителе Windows Server2016. Доступно только в Интернете. См. раздел [Установка ролей и компонентов в сети](https://technet.microsoft.com/windows-server-docs/get-started/deploy-nano-server#a-namebkmkonlineainstalling-roles-and-features-online).|
...
### Элементы отказоустойчивого кластера, которые устанавливаются при использовании параметра -Clustering
...
### Файлы и элементы хранилища, установленные при использовании параметра -Storage
...
### Установка виртуального жесткого диска Nano Server  
...
3.  Копирование установочного носителя из \\\Path\To\Media\server_en-us into .\Base  
...
### Установка WIM Nano Server  
  
1.  Скопируйте папку *NanoServerImageGenerator* из папки \NanoServer ISO-образа Windows Server2016 в локальную папку на компьютере.  
...
### Локальное и удаленное изменение файлов на Nano Server  
...
### Установка ролей и компонентов из репозитория пакетов  
...
### Скачивание пакетов Nano Server  

`Save-NanoServerPackage` или `Save-Package` позволяют скачать и сохранить пакеты без установки. Оба командлета принимают входные данные из конвейера.
...
### Установка ролей и компонентов из локального источника  
Хотя для ролей сервера и других пакетов мы рекомендуем автономную установку, в сценариях с контейнерами может потребоваться установить их в сети (с Nano Server). Для этого выполните следующие действия:  
...
5.  Убедитесь, что пакет и связанный с ним языковой пакет установлены правильно, выполнив следующую команду:  
    ...
    **dism /online /get-packages**  
    ...
    Сообщение "Идентификатор пакета: Microsoft-NanoServer-IIS-Package~31bf3856ad364e35~amd64~en-US~10.0.10586.0" должно появиться дважды: один раз для типа выпуска— языковой пакет и один раз для типа выпуска— пакет дополнительных компонентов.  
  
## Настройка существующего виртуального жесткого диска Nano Server  
...
## Дополнительные задачи, которые можно выполнить с помощью New-NanoServerImage и Edit-NanoServerImage  
...
### Размер пользовательского образа  
...
### Внедрение пользовательских данных  
...
### Выполнение пользовательских команд после первой загрузки
...
### Выполнение пользовательских сценариев PowerShell при создании образов
...
### Пользовательский файл автоматической установки  
...
[comment]: # (Из Xumin Солнце, ошибки #68620.)  

### Установщик приложения Windows Server
...
1. Установить [пакет SDK для Windows10](https://developer.microsoft.com/windows/downloads/windows-10-sdk), который включает необходимые средства для создания пакета WSA: MakeAppx, MakeCert, Pvk2Pfx, SignTool.
2. Объявить манифест, следуя [схеме расширения манифеста WSA](https://msdn.microsoft.com/library/windows/apps/mt670653.aspx) для создания файла манифеста AppxManifest.xml.
...
1. Запустить командлет [*Import-Certificate*](https://technet.microsoft.com/library/hh848630) в PowerShell для импорта сертификата издателя, описанного выше на шаге 4, на сервер Nano Server с certStoreLocation в Cert:\LocalMachine\TrustedPeople. Например: `Import-Certificate -FilePath ".\xyz.cer" -CertStoreLocation "Cert:\LocalMachine\TrustedPeople"`
2. Установить приложение на сервере Nano Server, запустив командлет [**Add-AppxPackage**](https://technet.microsoft.com/library/mt575516(v=wps.620).aspx) PowerShell, чтобы установить пакет WSA на Nano Server. Например: `Add-AppxPackage wsaSample.appx`

#### Дополнительные ресурсы для создания приложений
...
### Установка драйверов на сервере NanoServer
...
#### Установка пакетов драйверов в автономном режиме
...
#### Установка пакетов драйверов в сети
...
### Порядок добавления сервера Nano Server к домену в сети  
...
7. Для обеспечения безопасности удалите Nano Server из списка доверенных узлов с помощью следующей команды:
...
## Работа с ролями сервера на сервере Nano Server

###Использование Hyper-V на сервере Nano Server  
...
Можно создать новый кластер с помощью `New-Cluster -Name <clustername> -Node <comma-separated cluster node list>`  
...
Удалите узел из кластера с помощью  `Remove-ClusterNode -Name <comma-separated cluster node list>  -Cluster <clustername>`  
  
Создайте масштабируемый файловый сервер с помощью `Add-ClusterScaleoutFileServerRole -name <sofsname> -cluster <clustername>`  
...
### <a name="BKMK_IIS"></a>Использование служб IIS на сервере NanoServer  
Действия для использования роли служб IIS см. в статье [IIS на сервере Nano Server](IIS-on-Nano-Server.md). 

### Использование MPIO на сервере NanoServer
...
## Приложение: пример файла Unattend.xml, присоединяющий сервер Nano Server к домену  
