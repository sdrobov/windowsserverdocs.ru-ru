...
> [!IMPORTANT]
> Пользователь, который будет выполнять `Initialize-HgsServer`, требует **полного контроля** над объектами CNO и VCO в Active Directory.
...
**Оправдан** Отказоустойчивая кластеризация использует локальную учетную запись без прав администратора с именем КЛИУСР для управления узлами кластера. Блокировка сетевого входа для этого пользователя приведет к неправильной работе кластера.
...
**Действие**: Если эта политика настроена, необходимо обновить учетную запись gMSA с помощью [Set-адсервицеаккаунт](https://docs.microsoft.com/powershell/module/addsadministration/set-adserviceaccount?view=win10-ps) , чтобы использовать в этой политике только поддерживаемые типы шифрования. Например, если политика разрешает только AES128 @ no__t-0HMAC @ no__t-1SHA1 и AES256 @ no__t-2HMAC @ no__t-3SHA1, следует запустить `Set-ADServiceAccount -Identity HGSgMSA -KerberosEncryptionType AES128,AES256`.
