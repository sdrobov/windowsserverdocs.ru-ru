...
| Изменение размера | Каждый средний узел сервера HGS (8-ядерный/4 ГБ) может обслуживать узлы Hyper-V 1 000. |
