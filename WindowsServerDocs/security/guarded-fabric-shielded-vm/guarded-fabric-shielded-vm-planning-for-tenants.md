...
Экранированные виртуальные машины помогают защититься от атак скомпрометированной структуры виртуализации, поэтому нам нужен надежный механизм для передачи конфиденциальных данных инициализации, таких как пароль администратора, учетные данные присоединение к домену или сертификаты RDP, без раскрытия сама структура виртуализации или ее Администраторы. Кроме того, данные экранирования содержат следующие сведения:
...
Хотя для каждой экранированной виртуальной машины можно использовать отдельные файлы данных экранирования, это необязательно является оптимальным выбором и должно выполняться по правильным причинам. Например, если для каждой экранированной виртуальной машины требуется другой пароль администратора, рассмотрите возможность использования службы управления паролями или средства, например, [решения Microsoft локального администратора (Lap)](https://www.microsoft.com/en-us/download/details.aspx?id=46899).
