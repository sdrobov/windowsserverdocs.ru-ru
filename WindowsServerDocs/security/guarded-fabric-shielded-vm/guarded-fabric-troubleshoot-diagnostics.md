...
1. Попросите каждого администратора узла запустить `Get-HgsTrace` указав известное `-Path` и список диагностических данных, которые будут выполняться с результирующими трассировками.  Например:
