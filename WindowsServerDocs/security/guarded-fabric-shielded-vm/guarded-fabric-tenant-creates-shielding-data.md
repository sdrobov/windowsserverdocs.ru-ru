...
> [!IMPORTANT]
> Действия, описанные в этом разделе, необходимо выполнить на компьютере клиента под Windows Server 2016. Этот компьютер не должен входить в защищенную структуру (то есть не должен быть настроен на использование кластера HGS).

Чтобы подготовиться к созданию файла данных экранирования, выполните следующие действия.

- [Получение сертификата для подключение к удаленному рабочему столу](#obtain-a-certificate-for-remote-desktop-connection)
...
## <a name="obtain-a-certificate-for-remote-desktop-connection"></a>Получение сертификата для подключение к удаленному рабочему столу
...
Если вы оцениваете экранированные виртуальные машины и еще не готовы запросить сертификат в центре сертификации, можно создать самозаверяющий сертификат на компьютере клиента, выполнив следующую команду Windows PowerShell (где *contoso.com* — это Домен клиента):
...
- Будет ли использоваться сертификат протокол удаленного рабочего стола (RDP), который будет использоваться для подтверждения принадлежности виртуальной машины к вашей организации?
- Вы хотите выполнить сценарий в конце инициализации?
- Вы используете сервер конфигурации требуемого состояния (DSC) для дальнейшей настройки?
...
-   Файл автоматической установки должен привести к отключению виртуальной машины после ее настройки. Это позволяет VMM узнавать, когда он должен сообщить клиенту о том, что виртуальная машина завершила подготовку и готова к использованию. VMM автоматически снова включит виртуальную машину после того, как она обнаружит, что она была отключена во время подготовки.

-   Настоятельно рекомендуется настроить сертификат RDP, чтобы обеспечить подключение к правильной виртуальной машине, а не другой компьютер, настроенный для атаки "злоумышленник в середине".
...
6.  На странице **значения специализации** нажмите кнопку **Обзор** , чтобы выбрать файл Unattend. XML, который будет использоваться для специализации виртуальных машин.
    ...
    Используйте кнопку **Добавить** внизу, чтобы добавить дополнительные файлы в PDK, необходимые в процессе специализации. Например, если файл автоматической установки устанавливает сертификат RDP на виртуальную машину (как описано в разделе [Создание файла ответов с помощью функции New-шиелдингдатаансверфиле](guarded-fabric-sample-unattend-xml-file.md)), следует добавить файл рдпцерт. pfx, указанный в файле Unattend. Обратите внимание, что все указанные здесь файлы будут автоматически скопированы на диск C: \\temp @ no__t-1 на созданной виртуальной машине. Файл автоматической установки должен содержать файлы в этой папке при ссылке на них по пути.
