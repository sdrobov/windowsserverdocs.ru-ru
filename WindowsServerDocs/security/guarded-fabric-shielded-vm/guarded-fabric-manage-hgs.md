...
Если вы использовали `Install-HgsServer` для настройки нового леса для HGS, эти группы будут называться "Администраторы*ServiceName*" и "*имя_службы*проверяющими" соответственно, где *ServiceName* — сетевое имя кластера HGS.
Если вы присоединились к существующему домену, вы должны обратиться к именам групп, указанным в `Initialize-HgsServer`.
...
После завершения работы с сеансом JEA введите команду `Exit-PSSession` или ее псевдоним `exit`. 
...
Чтобы записать имя службы HGS, выполните `Get-HgsServer` и обратите внимание на неструктурированное имя в URL-адресах аттестации и защиты ключей.
...
Если вы хотите импортировать политики аттестации, доверенные для администраторов, или политики аттестации доверенного платформенного модуля, это можно сделать, указав флаги `-ImportActiveDirectoryModeState` или `-ImportTpmModeState` для [Import-хгссерверстате](https://technet.microsoft.com/library/mt652168.aspx).
...
Чтобы проверить режим работы сервера HGS, выполните `Get-HgsServer`.
...
Hgs_DumpEncryption             | Требуются дампы памяти, если они включены на узле Hyper-V, которые должны быть зашифрованы с помощью ключа шифрования, доверенного для HGS. Эта политика не применяется, если на узле не включены дампы. Если эта политика и *HGS @ no__t-1NoDumps* отключены, память ЭКРАНИРОВАННОЙ виртуальной машины может быть сохранена в незашифрованном файле дампа.
Hgs_DumpEncryptionKey          | Отрицательная политика, чтобы убедиться, что узлы, настроенные на разрешение дампов памяти, используют определенный администратором ключ шифрования файла дампа, известный для HGS. Эта политика не применяется, если функция *HGS @ no__t-1DumpEncryption* отключена.
...
> Кроме того, вы можете пропустить проверку, добавив флаг `-SkipValidation` к команде.
...
Если вы захватываете базовый модуль TPM на узле Hyper-V, на котором настроено шифрование дампа, хэш включается в ткглог и может быть предоставлен командлету `Add-HgsAttestationDumpPolicy`.
...
Если полученное состояние не указывает "Ишостгуардед: True "вам потребуется устранить неполадки конфигурации.
...
Если политика больше не нужна и вы хотите удалить ее со всех узлов HGS, выполните `Remove-HgsAttestationPolicy -Name 'PolicyName'`, чтобы окончательно удалить политику.
...
**Вариант 1. Добавление сертификата, хранящегося в HSM @ no__t-0
...
**Вариант 2. Добавление неэкспортируемых сертификатов программного обеспечения @ no__t-0
...
2. Предоставьте учетной записи управляемой службы группы HGS доступ на чтение к закрытому ключу сертификата. Имя учетной записи gMSA HGS можно найти, выполнив `(Get-IISAppPool -Name KeyProtection).ProcessModel.UserName`.
...
@no__t 0Option 3: Добавление сертификатов, хранящихся в файлах PFX @ no__t-0
...
> Чтобы вернуться к последнему работоспособному предохранителю ключа, выполните `Set-VMKeyProtector -RestoreLastKnownGoodKeyProtector`.
