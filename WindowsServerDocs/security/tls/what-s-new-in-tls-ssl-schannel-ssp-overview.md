...
В этом разделе для ИТ-специалистов описываются изменения в работе поставщика услуг безопасности SChannel, в том числе протокол TLS, SSL (SSL) и Security Transport Layer (DTLS). протоколы проверки подлинности для Windows Server 2012 R2, Windows Server 2012, Windows 8.1 и Windows 8.
...
### <a name="BKMK_TrustedIssuers"></a>Управление доверенными поставщиками для проверки подлинности клиента
...
Хотя максимальный размер списка доверенных центров сертификации, поддерживаемого SChannel SSP (16 КБ), остается таким же, как в Windows Server 2008 R2, в Windows Server 2012 существует новое выделенное хранилище сертификатов для издателей проверки подлинности клиентов, чтобы несвязанные сертификаты не включаются в сообщение.
...
### <a name="BKMK_SNI"></a>Поддержка TLS для расширений с индикатором имени сервера (SNI)
...
### <a name="BKMK_DTLS"></a>Безопасность транспортного уровня датаграмм (DTLS)
...
### <a name="BKMK_Deprecated"></a>Устаревшие функции
