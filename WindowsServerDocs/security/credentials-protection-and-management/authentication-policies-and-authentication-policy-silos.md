...
Возможностях, появившихся в Windows Server 2012 R2, позволяют вам создавать приемники команд политик, в которых размещается набор пользователей с высоким уровнем привилегий для проверки подлинности. После этого вы можете назначить для каждого контейнера политики проверки подлинности, чтобы ограничить использование привилегированных учетных записей в домене. Когда учетные записи принадлежат группе безопасности "Защищенные пользователи", применяются дополнительные средства управления, такие как исключительное использование протокола Kerberos.
...
Подробную информацию о настройке приемников команд и соответствующих политик проверки подлинности см. в разделе [How to Configure Protected Accounts](how-to-configure-protected-accounts.md).
...
|Приемник команд политик проверки подлинности включен|Определяет, включен ли приемник команд политик проверки подлинности.<br /><br />Когда приемник не включен, политика по умолчанию находится в режиме аудита. Генерируются события, отражающие возможность успеха или неудачи, но защита к системе не применяется.|
...
Приемники команд политик проверки подлинности можно настроить с помощью консоли администрирования Active Directory или Windows PowerShell. Подробную информацию см. в разделе [Настройка защищенных учетных записей](how-to-configure-protected-accounts.md).
...
Тип учетной записи Active Directory определяет роли вызывающей стороны, как один из следующих:
...
-   **Службы**
    ...
    Изолированные управляемые учетные записи служб, групповые управляемые учетные записи служб или произвольные объекты учетных записей, являющиеся производными этих двух типов. Политик можно установить условия управления, которые используются для ограничения на определенные устройства с помощью удостоверения Active Directory учетные данные учетной записи управляемой службы доступа к устройству. Службы не должны входить в группу безопасности "Защищенные пользователи", поскольку в этом случае все входящие попытки пройти проверку подлинности потерпят неудачу.
...
|Политика|Политика проверки подлинности включена|Определяет, включена ли политика проверки подлинности.<br /><br />Если политика выключена, по умолчанию она находится в режиме аудита. Генерируются события, отражающие возможность успеха или неудачи, но защита к системе не применяется.|
...
|Пользовательская|Политика проверки подлинности пользователя|Определяет, какая политика проверки подлинности должна применяться к пользователям, связанным с этим объектом приемника команд.|
|Пользовательская|Обратная ссылка политики проверки подлинности пользователя|Этот атрибут представляет собой обратную ссылку для  msDS-UserAuthNPolicy.|
|Пользовательская|ms-DS-User-Allowed-To-Authenticate-To|Этот атрибут используется для определения набора субъектов, которым разрешается прохождение проверки подлинности для подключения к службе, запущенной из учетной записи пользователя.|
|Пользовательская|ms-DS-User-Allowed-To-Authenticate-From|Этот атрибут используется для определения набора устройств, на подключение к которым учетная запись пользователя располагает разрешением.|
|Пользовательская|Время действия пользовательских TGT|Определяет максимальный срок действия билетов предоставления билетов Kerberos, выдаваемых пользователю (в секундах). Полученные в итоге TGT нельзя продлить|
|Computer|Политика проверки подлинности компьютера|Определяет, какая политика проверки подлинности должна применяться к компьютерам, связанным с этим объектом приемника команд.|
|Computer|Обратная ссылка политики проверки подлинности компьютера|Этот атрибут представляет собой обратную ссылку для msDS-ComputerAuthNPolicy.|
|Computer|ms-DS-Computer-Allowed-To-Authenticate-To|Этот атрибут используется для определения набора субъектов, которым разрешается прохождение проверки подлинности для подключения к службе, запущенной из учетной записи компьютера.|
|Computer|Время жизни TGT компьютера|Определяет максимальный срок действия билетов предоставления билетов Kerberos, выдаваемых компьютеру (в секундах). Изменять этот параметр не рекомендуется.|
|Обслуживание|Политика проверки подлинности службы|Определяет, какая политика проверки подлинности должна применяться к службам, связанным с этим объектом приемника команд.|
|Обслуживание|Обратная ссылка политики проверки подлинности службы|Этот атрибут представляет собой обратную ссылку для msDS-ServiceAuthNPolicy.|
|Обслуживание|ms-DS-Service-Allowed-To-Authenticate-To|Этот атрибут используется для определения набора субъектов, которым разрешается проверка подлинности для подключения к службе, запущенной из учетной записи службы.|
|Обслуживание|ms-DS-Service-Allowed-To-Authenticate-From|Этот атрибут используется для определения набора устройств, на подключение к которым учетная запись службы располагает разрешением.|
|Обслуживание|Время жизни TGT службы|Определяет максимальный срок действия билетов предоставления билетов Kerberos, выдаваемых службе (в секундах).|

Политики проверки подлинности можно настроить для каждого приемника команд с помощью консоли администрирования Active Directory или Windows PowerShell. Подробную информацию см. в разделе [Настройка защищенных учетных записей](how-to-configure-protected-accounts.md).
...
-   [Использование протокола Kerberos с политиками и приемники команд проверки подлинности](#BKMK_HowKerbUsed)

-   [Как ограничения входа пользователя работает](#BKMK_HowRestrictingSignOn)

-   [Как работает ограничение выдачи билетов служб](#BKMK_HowRestrictingServiceTicket)

**Защищенные учетные записи**

Группы безопасности защищенных пользователей активирует ненастраиваемую защиту на устройствах и главных компьютеров под управлением Windows Server 2012 R2 и Windows 8.1, а также на контроллерах домена в доменах с основной контроллер домена под управлением Windows Server 2012 R2. В зависимости от режима работы домена учетной записи, члены группы безопасности "Защищенные пользователи" подвергаются дополнительной защите благодаря изменениям в способах проверки подлинности, поддерживаемых Windows.

-   Члены группы безопасности "Защищенные пользователи" не могут пройти проверку подлинности с помощью NTLM, дайджест-проверки или CredSSP. На устройстве под управлением Windows 8.1, используется один из этих поставщиков поддержки безопасности (SSP) пройдет проверку подлинности в домен, когда учетная запись является участником группы безопасности "защищенные пользователи".
...
-   Учетная запись пользователя не может быть делегирована с Kerberos ограниченное или неограниченное делегирование. Это означает, что если компьютер входит в группу безопасности "Защищенные пользователи", старые подключения к другим системам могут выйти из строя.
...
Дополнительные сведения об этой группе безопасности см. в разделе [How the Protected Users group works](protected-users-security-group.md#BKMK_HowItWorks).

**Политик проверки подлинности и приемники команд**
...
-   [Как работает протокол Kerberos версии 5 проверки подлинности](https://technet.microsoft.com/library/cc772815(v=ws.10).aspx)
...
Если политика проверки подлинности находится в режиме аудита, и получает запрос службы предоставления билетов контроллером домена для учетной записи домена, контроллером домена проверяется, если разрешена проверка подлинности на основании билета запроса сертификата атрибута привилегий Данные (PAC) и записывает в журнал предупреждение в случае неудачи. PAC содержит различные данные для проверки подлинности, включая информацию о группах, в которых состоит пользователь, его права, а также политики, которым он подчиняется. Эта информация используется для создания маркера доступа пользователя. Если это применяемая политика проверки подлинности позволяют проходить проверку подлинности пользователя, устройства или службы, контроллер домена проверяет, разрешена проверка подлинности основана на данных PAC для билета запроса. В случае неудачи контроллер домена возвращает сообщение об ошибке и создает соответствующую запись в журнале.
...
### <a name="BKMK_HowRestrictingSignOn"></a>Как ограничения входа пользователя работает
...
-   Поскольку требуется защита, пользователь может попытаться выполнить вход с помощью компьютера под управлением Windows 8.1 или Windows 8, для которого поддерживается защита Kerberos для повторного запуска процесса входа в систему.
...
-   Контроллер домена выполняет проверку доступа при помощи настроенных условий управления доступом и сведения об удостоверении операционной системы клиента TGT, который использовался для защиты запроса.

-   Если проверка доступа терпит неудачу, контроллер домена отклоняет запрос.

Даже если операционная система поддерживает защиту Kerberos, могут использоваться требования управления доступом, которые должны быть выполнены для установки подключения. Пользователи входят в Windows или вводят свои учетные данные домена при запросе приложением. По умолчанию Windows посылает контроллеру домена незащищенное сообщение AS-REQ. Если пользователь посылает запрос с компьютера, который поддерживает защиту, например, Windows 8.1 или Windows 8, политики проверки подлинности вычисляются следующим образом:
...
2.  Контроллер домена выполняет проверку доступа при помощи настроенных условий управления доступом и сведения об удостоверении системы TGT, используемый для защиты запроса. Проверка доступа проходит успешно.
...
### <a name="BKMK_HowRestrictingServiceTicket"></a>Как работает ограничение выдачи билетов служб
Когда не разрешается, и пользователь, имеющий TGT пытается подключиться к службе (например, открыв приложение, требующее проверки подлинности к службе, которая идентифицируется по имени участника-службы службы (SPN), происходит следующая последовательность:
...
2.  Контроллер домена в домене под управлением Windows Server 2012 R2 выполняет поиск SPN1 найти учетную запись доменных служб Active Directory для службы и определяет, что учетная запись настроена политика проверки подлинности, ограничивающая выдачу билетов службы.

3.  Контроллер домена выполняет проверку доступа при помощи настроенных условий управления доступом и информации удостоверение пользователя в TGT об Проверка доступа терпит неудачу.

4.  Контроллер домена отклоняет запрос.

Когда разрешается, так как учетная запись соответствует условиям управления доступом, которые заданы с помощью политики проверки подлинности, а пользователь, имеющий TGT пытается подключиться к службе (например, открыв приложение, которому требуется проверка подлинности к службе то есть определяется по имени участника-службы для службы), выполняются следующие действия:
...
3.  Контроллер домена выполняет проверку доступа при помощи настроенных условий управления доступом и информации удостоверение пользователя в TGT об Проверка доступа проходит успешно.
...
|----------|--------|
|101<br /><br />**AuthenticationPolicyFailures-DomainController**|Причина: ошибка подключения при помощи протокола NTLM из-за настроек политики проверки подлинности.<br /><br />Событие фиксируется на контроллере домена, чтобы указать, что проверка подлинности с помощью NTLM оказалась неудачной, поскольку требуются ограничения управления доступом, которые не могут применяться для NTLM.<br /><br />Отображаются имена учетной записи, устройства, политики и приемника команд.|
|105<br /><br />**AuthenticationPolicyFailures-DomainController**|Причина: отклонение из-за ограничений Kerberos произошло, потому что проверка подлинности не была разрешена для конкретного устройства.<br /><br />Событие фиксируется на контроллере домена, чтобы указать, что билет предоставления билетов Kerberos был отклонен, поскольку устройство не соответствует установленным ограничениям управления доступом.<br /><br />Отображаются имена учетной записи, устройства, политики и приемника команд, а также время жизни TGT.|
|305<br /><br />**AuthenticationPolicyFailures-DomainController**|Причина: отклонение из-за ограничений Kerberos могло произойти, потому что не была разрешена проверка подлинности для конкретного устройства.<br /><br />В режиме аудита информация о событии фиксируется на контроллере домена, чтобы определить, был ли отклонен билет предоставления билета Kerberos из-за несоответствия устройства ограничениям управления доступом.<br /><br />Отображаются имена учетной записи, устройства, политики и приемника команд, а также время жизни TGT.|
|106<br /><br />**AuthenticationPolicyFailures-DomainController**|Причина: отклонение из-за ограничений Kerberos могло произойти, потому что устройству пользователя не была разрешена проверка подлинности для подключения к серверу.<br /><br />Событие фиксируется на контроллере домена, чтобы указать, что билет службы Kerberos был отклонен, поскольку пользователь или устройство не соответствует установленным ограничениям управления доступом.<br /><br />Отображаются имена устройства, политики и приемника команд.|
|306<br /><br />**AuthenticationPolicyFailures-DomainController**|Причина: отклонение из-за ограничений Kerberos могло произойти, потому что устройству пользователя не была разрешена проверка подлинности для подключения к серверу.<br /><br />В режиме аудита информация о событии фиксируется на контроллере домена, чтобы указать, что билет службы Kerberos будет отклонен, поскольку пользователь или устройство не соответствует ограничениям управления доступом.<br /><br />Отображаются имена устройства, политики и приемника команд.|
