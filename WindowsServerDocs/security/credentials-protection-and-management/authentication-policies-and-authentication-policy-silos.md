...
**Схема Active Directory для приемников политик проверки подлинности**
...
|Type|Отображаемое имя|Описание|
|----|--------|--------|
...
-   [Использование протокола Kerberos с приемниками команд и политик проверки подлинности](#BKMK_HowKerbUsed)

-   [Как выполняется Запрещение входа пользователя](#BKMK_HowRestrictingSignOn)

-   [Как ограничиваются выдача билетов на обслуживание](#BKMK_HowRestrictingServiceTicket)
...
**Приемники и политики проверки подлинности**
...
-   [Как работает протокол проверки подлинности Kerberos версии 5](https://technet.microsoft.com/library/cc772815(v=ws.10).aspx)
...
### <a name="BKMK_HowKerbUsed"></a>Использование протокола Kerberos с приемниками и политиками проверки подлинности
...
Если политика проверки подлинности находится в режиме аудита и в контроллере домена для доменной учетной записи получен запрос на предоставление билета, контроллер домена проверяет, разрешена ли проверка подлинности на основе сертификата атрибута привилегий запроса билета. (PAC) и записывает в журнал предупреждающее сообщение в случае сбоя. PAC содержит различные данные для проверки подлинности, включая информацию о группах, в которых состоит пользователь, его права, а также политики, которым он подчиняется. Эти сведения используются для создания маркера доступа пользователя. Если это обязательная политика проверки подлинности, позволяющая выполнять проверку подлинности для пользователя, устройства или службы, контроллер домена проверяет, разрешена ли проверка подлинности на основе данных запроса PAC билета. В случае неудачи контроллер домена возвращает сообщение об ошибке и создает соответствующую запись в журнале.
...
### <a name="BKMK_HowRestrictingSignOn"></a>Как выполняется Запрещение входа пользователя
...
### <a name="BKMK_HowRestrictingServiceTicket"></a>Как ограничиваются выдача билетов на обслуживание
...
Если учетная запись разрешена, так как учетная запись соответствует условиям управления доступом, заданным политикой проверки подлинности, и пользователь с билетом TGT пытается подключиться к службе (например, путем открытия приложения, требующего проверки подлинности для службы, которая определяется именем участника службы (SPN), выполняется следующая последовательность:
...
## <a name="BKMK_ErrorandEvents"></a>Связанные сообщения об ошибках и информационные события
