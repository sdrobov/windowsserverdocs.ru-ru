...
Для установки сертификатов подписи и шифрования используйте следующий сценарий Windows PowerShell. Дополнительные сведения о отпечатке см. в [разделе как Получение отпечатка сертификата](https://msdn.microsoft.com/library/ms734695.aspx).
...
Перед установкой пакета сертификата можно скачать последний список доверенных корневых сертификатов доверенного платформенного модуля [https://go.microsoft.com/fwlink/?linkid=2097925](https://go.microsoft.com/fwlink/?linkid=2097925)из.

> **Важно!** Перед установкой пакета убедитесь, что он подписан цифровой подписью корпорации Майкрософт.
...
> **Примечание.** Если используется режим сертификата AIK, то для проверки выдачи сертификатов AIK корпорацией Майкрософт требуется папка Майкрософт.
...
> **Примечание.** Этот сертификат должен быть развернут на сервере, на котором выполняется служба DHA, в хранилище сертификатов **LocalMachine\My** . Когда устанавливается новый активный сертификат подписи, бывший активный сертификат подписи перемещается в список неактивных сертификатов подписи.
...
> **Примечание.** Только *один* неактивный сертификат (любого типа) может существовать в службе в любое время. Сертификаты следует удалять из списка неактивных сертификатов, когда они больше не требуются.
