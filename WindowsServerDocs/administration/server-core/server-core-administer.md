...
|                  Присоединение компьютера к домену                  |                                                                                                                                                       **netdom присоединение к% ComputerName%** **/Domain:\<домен\> /усерд:\<домен\\имя_пользователя/пассвордд:\>** \* <br> Перезагрузите компьютер.                                                                                                                                                        |
|              Подтверждение изменения домена              |                                                                                                                                                                                                                 **set**                                                                                                                                                                                                                 |
|                Удаление компьютера из домена                |                                                                                                                                                                                                   **Netdom Remove \<ComputerName\>**                                                                                                                                                                                                    |
|         Добавление пользователя в локальную группу администраторов          |                                                                                                                                                                                       **net localgroup администраторы/Add \<домен\\имя_пользователя\>**                                                                                                                                                                                       |
|       Удаление пользователя из локальной группы администраторов       |                                                                                                                                                                                     **net localgroup администраторы/DELETE \<домен\\имя_пользователя\>**                                                                                                                                                                                      |
|               Добавление пользователя на локальный компьютер                |                                                                                                                                                                                                **пользователь \<{имя_пользователя\>/Add \***                                                                                                                                                                                                 |
|               Добавление группы на локальный компьютер               |                                                                                                                                                                                                 **net localgroup \<группа имя\> /Add**                                                                                                                                                                                                  |
|          Изменение имени компьютера в домене          |                                                                                                                                                           **netdom ренамекомпутер% ComputerName%/newname.:\<новое имя\> компьютера/усерд:\<домен\\имя_пользователя\> /пассвордд:** \*                                                                                                                                                            |
...
|                 Изменение статического IP-адреса                 | **ipconfig/all** <br>Запишите соответствующие сведения или перенаправьте их в текстовый файл (**ipconfig/all > ipconfig. txt**).<br>**Интерфейс Netsh интерфейса IPv4 показывать интерфейсы**<br>Убедитесь в наличии списка интерфейсов.<br>**netsh interface IPv4 Set адрес имя \<адреса в списке\> интерфейс источник = статический адрес =\<предпочитаемый IP\> -адрес\<шлюз = адрес шлюза\>**<br>Выполните **команду ipconfig/all** , чтобы убедиться, что параметр DHCP включен в значение **нет**. |
|                   Задайте статический DNS-адрес.                   |   <strong>netsh interface IPv4 Add dnsserver Name =\<имя или идентификатор сетевой карты\> сетевого интерфейса =\<IP-адрес первичного DNS-сервера\> index = 1 <br></strong>netsh interface IPv4 Add dnsserver Name =\<имя дополнительного DNS-сервера\> address =\<IP-адрес вторичного DNS-\> сервера index = 2\*\* <br> Повторите эти действия, чтобы добавить дополнительные серверы.<br>Выполните **команду ipconfig/all** , чтобы убедиться в правильности адресов.   |
| Изменение статического IP-адреса на IP-адрес, получаемый по протоколу DHCP |                                                                                                                                      **netsh interface IPv4 Set адрес имя =\<IP-адрес локального системного\> источника = DHCP** <br>Выполните **команду ipconfig/all** , чтобы убедиться, что для параметра DCHP Enabled задано значение **Да**.                                                                                                                                      |
|                      Ввод ключа продукта                      |                                                                                                                                                                                                   **slmgr. vbs – ИПК \<ключ продукта\>**                                                                                                                                                                                                    |
|                  Локальная активация сервера                  |                                                                                                                                                                                                           **slmgr. vbs-ATO**                                                                                                                                                                                                            |
|                 Удаленная активация сервера                  |                                            **cscript slmgr. vbs — ИПК \<ключ\>\<\>сервера продукта имя\<пользователя\>пароль\<\>** <br>**cscript slmgr. vbs-ATO \<имя_сервера\> \<имя\> пользователя \<пароль\>** <br>Получите идентификатор GUID компьютера, выполнив **cscript slmgr. vbs.** <br> Запустите **cscript slmgr. vbs-GUID \<dli\>** <br>Убедитесь, что для состояния лицензии задано значение **лицензировано (активировано)** .                                             |
...
|                         Удаление обновления                          |                                 **Разверните/f:\* \<Update\>. msu c:\test** <br>Перейдите по адресу к:\тест\ \<и\>откройте файл Update. XML в текстовом редакторе.<br>Замените **Install** на **Remove** и сохраните файл.<br>**pkgmgr/n:\<Update\>. XML**                                 |
...
|                          Остановка службы                          |                                                                                                                                                                                  **SC останавливает \<службуилиимяслужбыNETостанавливают\>**  **\<\>**                                                                                                                                                                                   |
...
|Запрос событий в указанном журнале|**wevtutil QE/f: имя \<текстового журнала\>**| 
...
|Задача|Command| 
|----|-------|
|Добавление драйвера нового устройства|Скопируйте драйвер в папку в папке\\\>% хомедриве%\<Driver. Выполните команду **PnPUtil-i-a% хомедриве\\%\<драйвер\>папки\>\\\<драйвера. INF**|
