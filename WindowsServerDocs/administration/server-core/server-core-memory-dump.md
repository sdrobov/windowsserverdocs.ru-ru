...
6. В **параметре тип данных** **\<диск\>:\\\<дедикатеддумпфиле. sys\>** , а затем нажмите кнопку **ОК**.

   >[!NOTE] 
   > Замените \<Drive\> диском, на котором достаточно места на диске для файла подкачки, и замените \<Дедикатеддумпфиле. dmp\> на полный путь к выделенному файлу.
...
Задайте для параметра \<FilePath\> путь назначения. Например, следующая команда устанавливает путь назначения дампа памяти в К:\ВИНДОВС\МЕМОРИ. DMP 
