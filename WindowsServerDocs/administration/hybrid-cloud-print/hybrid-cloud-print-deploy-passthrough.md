---
title: Развертывание Windows Server гибридной облачной печати - сквозной проверки подлинности
description: Как настроить Microsoft гибридной облачной печати с помощью сквозной проверки подлинности
...
ms.locfileid: "59887115"
---
# <a name="deploy-windows-server-hybrid-cloud-print-with-passthrough-authentication"></a>Развертывание Windows Server гибридной облачной печати с помощью сквозной проверки подлинности
...
- Шаг 3. Установка прокси приложения Azure (AAP) с помощью сквозной проверки подлинности
...
### <a name="step-3---install-azure-application-proxy-aap-with-passthrough-authentication"></a>Шаг 3: Установка прокси приложения Azure (AAP) с помощью сквозной проверки подлинности
...
3. Вернитесь на портал управления клиента AAD и добавьте прокси приложений
    ...
    - Выберите **локальное приложение** и заполните поля
        ...
        - Метод предварительной проверки подлинности: Сквозной режим
...
4. Повторите 3 выше, для облачной службы печати Enterprise и укажите внутренний URL-адрес к облачной службе печати Enterprise

    >   Примечание. Https://&lt;конечной точки службы машины &gt; /mcs приведенные ниже URL-адрес должен быть внешний URL-адрес, настроить для Mopria облачной службы и/или Корпоративная облачная служба печати.
...
- Найдите группу политики корпоративной облачной печати и настроить политики следуя приведенным ниже рекомендациям:
    ...
    - MopriaDiscoveryResourceId = «URI идентификатора приложения» веб-приложения API для обнаружения конечной точки, зарегистрированные в шаге 2.8 или.  Это значение можно найти в разделе Параметры "->" Свойства приложения
    - CloudPrintResourceId = «URI идентификатора приложения» веб-приложения API для печати конечной точки, зарегистрированные в шаге 2.8 или. Это значение можно найти в разделе Параметры "->" Свойства приложения
...
- OMA-URI
    ...
    - `MopriaDiscoveryResourceId = ./Vendor/MSFT/Policy/Config/EnterpriseCloudPrint/MopriaDiscoveryResourceId`
        - Значение = «URI идентификатора приложения» веб-приложения API для обнаружения конечной точки, зарегистрированные в шаге 2.8 или.  Это значение можно найти в разделе Параметры "->" Свойства приложения
    - `CloudPrintResourceId = ./Vendor/MSFT/Policy/Config/EnterpriseCloudPrint/CloudPrintResourceId`
        - Значение = «URI идентификатора приложения» веб-приложения API для печати конечной точки, зарегистрированные в шаге 2.8 или. Это значение можно найти в разделе Параметры "->" Свойства приложения
...
5. С компьютера Windows 10 Fall Creator Update откройте командную строку с повышенными привилегиями Windows PowerShell
    1. Выполните следующие команды
        ...
        - Публикация CloudPrinter
            ...
            - DiscoveryEndpoint = https://&lt;конечной точки службы машины&gt;/mcs
            - PrintServerEndpoint = https://&lt;конечной точки службы машины&gt;/ecp
...
## <a name="verifing-the-deployment"></a>Проверяется развертывания
На устройства присоединены к Azure AD с настройки политик управления мобильными Устройствами:
- Откройте браузер и перейти к https://&lt;конечной точки службы машины &gt; /mcs и служб
