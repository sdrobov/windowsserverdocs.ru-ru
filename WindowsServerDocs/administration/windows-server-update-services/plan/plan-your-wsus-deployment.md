...
description: Раздел службы Windows Server Update Service (WSUS) — Обзор процесса публикации со ссылками на связанные разделы планирования развертывания
...
ms.locfileid: "59822125"
---
# <a name="plan-your-wsus-deployment"></a>Планирование развертывания WSUS

>Область применения. Windows Server (полугодовой канал), Windows Server 2016, Windows Server 2012 R2, Windows Server 2012

Первым шагом в процессе развертывания службы Windows Server Update Services (WSUS) является принятие таких важных решений, как выбор сценария развертывания службы WSUS, топологии сети и осмысливание требований к системе. Следующий контрольный список описаны шаги, которые участвуют в подготовку к развертыванию.

|Задача|Описание|
|----|--------|
|[1.1. Анализ существующих факторов и требований к системе](plan-your-wsus-deployment.md#BKMK_1.1)|Проанализируйте список факторов, которые необходимо учесть, а также требования к системе, чтобы убедиться, что у вас есть все необходимое оборудование и программное обеспечение для развертывания службы WSUS.|
|[1.2. Выбор сценария развертывания службы WSUS](plan-your-wsus-deployment.md#BKMK_1.2)|Выберите сценарий развертывания службы WSUS, который будет использован.|
|[1.3. Выбор стратегии хранения данных WSUS](plan-your-wsus-deployment.md#BKMK_1.3.)|Выберите, какая из стратегий хранения данных для службы WSUS наиболее подходит для данного развертывания.|
|[1.4. Выберите языки обновления WSUS](plan-your-wsus-deployment.md#BKMK_1.4.)|Выберите те языки обновления для службы WSUS, которые будут установлены.|
|[1.5. Планирование групп компьютеров WSUS](plan-your-wsus-deployment.md#BKMK_1.5)|Спланируйте, какой подход относительно групп компьютеров службы WSUS вы будете использовать в ходе развертывания.|
|[1.6. WSUS рекомендации по планированию производительности: Фоновая интеллектуальная служба передачи](plan-your-wsus-deployment.md#BKMK_1.6.)|Разработайте модель службы WSUS, которая позволяла бы добиться оптимизированной производительности.|
|[1.7. Планирование параметров автоматического обновления](plan-your-wsus-deployment.md#BKMK_1.7.)|Продумайте, как будете настраивать параметры автоматического обновления в своем сценарии.|

## <a name="BKMK_1.1"></a>1.1. Анализ существующих факторов и требований к системе

### <a name="system-requirements"></a>Требования к системе

Перед активацией роли сервера службы WSUS подтвердите, что сервер соответствует требованиям к системе, а также что у вас есть необходимые разрешения для завершения установки, руководствуясь следующими указаниями.
...
-   Требования к оборудованию сервера для включения роли WSUS привязаны к требованиям к оборудованию. Минимальные аппаратные требования к WSUS
    ...
    -   **Процессор:** 1,4 гигагерц (ГГц) x64 процессора (с частотой 2 ГГц и выше)
    ...
    -   **Доступное дисковое пространство:** 10 ГБ (40 ГБ или больше рекомендуется)

    -   **Сетевой адаптер:** 100 мегабит в секунду (Мбит/с) или более поздней версии

-   Требования к программному обеспечению:

    -   Для просмотра отчетов WSUS требуется пакет [Microsoft Report Viewer Redistributable 2008](https://www.microsoft.com/download/details.aspx?id=6576). В Windows Server 2016, службам WSUS требуется [2012 среды выполнения средства просмотра отчетов Майкрософт](https://www.microsoft.com/download/details.aspx?id=35747)
...
2.  Клиентские выпуски Windows не смогут установить .NET 3.5 по запросу из Интернета. К .NET 3.5 применяются те же рекомендации, что и к ролям сервера.
    ...
    > [!NOTE]
    > Настройка компонентов на источник установки по запросу не включает использование служб WSUS. Сведения о настройке компонентов см. в разделе [Настройка компонентов по требованию в Windows Server](https://technet.microsoft.com/library/jj127275.aspx).

3. Корпоративные устройства под управлением Windows 10 версии 1709 или версии 1803, не удается установить какие-либо функции по запросу непосредственно из WSUS. Чтобы установить компоненты по требованию, [создайте файл (store side-by-side) компонентов](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj127275%28v=ws.11%29#create-a-feature-file-or-side-by-side-store) или получить эту функцию на пакет по запросу от одного из следующих источников:
    - [Volume Licensing Service Center](https://www.microsoft.com/licensing/servicecenter) (VLSC) - необходим доступ Корпоративное Лицензирование
    - Портал OEM - необходим доступ OEM
    - Скачивание MSDN — требуется подписка MSDN

    Можно установить по отдельности получить функцию на пакеты по запросу с помощью [параметры командной строки DISM](https://docs.microsoft.com/windows-hardware/manufacture/desktop/dism-operating-system-package-servicing-command-line-options).

### <a name="BKM_1.1.1."></a>Требования к базе данных WSUS
...
> [!NOTE]
> Размер базы данных SQL Server Express 2008 R2 ограничен десятью гигабайтами. Базы данных такого размера будет достаточно для служб WSUS, хотя значительного выигрыша при использовании этой базы вместо внутренней базы данных Windows нет. Внутренняя база данных имеет более минимальное ОЗУ требования к памяти 2 ГБ за стандартные требования к системе Windows Server.
...
3.  Сервер базы данных должен находиться в домене active directory, что и сервер WSUS, либо он должен иметь отношение доверия с доменом active directory сервера WSUS.

4.  Сервер WSUS и сервера базы данных должны находиться в том же часовом поясе, либо быть синхронизированными один и тот же источник формате UTC (время по Гринвичу).

## <a name="BKMK_1.2"></a>1.2. Выбор сценария развертывания службы WSUS
...
> [!NOTE] 
> Не рекомендуется создавать иерархию серверов WSUS глубиной более трех уровней. Каждый уровень увеличивает время распространения обновлений через подключенные серверы. Несмотря на то, что нет теоретического ограничения в иерархию, корпорацией Майкрософт были протестированы только развертывания, которые имеют иерархию из пяти уровней вложенности.
>
> Также подчиненных серверов должен быть в той же версии или более ранней версии служб WSUS в качестве источника синхронизации для вышестоящего сервера.
...
Балансировка сетевой нагрузки (NLB) повышает надежность и производительность вашей сети на WSUS. Можно настроить несколько серверов WSUS, которые совместно используют один отказоустойчивый кластер под управлением QL Server, таких как SQL Server 2008 R2 SP1. При такой конфигурации необходимо использовать полную установку SQL Server , а не установку внутренней базы данных Windows, которая поставляется WSUS, при этом роль базы данных должна быть установлена на всех серверах WSUS переднего плана. Можно также сделать так, чтобы все серверы WSUS использовали распределенную файловую систему (DFS), чтобы хранить свое содержимое.

**Настройка WSUS для балансировки сетевой Нагрузки:** по сравнению с установкой WSUS 3.2 для балансировки сетевой Нагрузки, специальный вызов программы установки и параметры больше не требуются для настройки служб WSUS для балансировки сетевой Нагрузки. Требуется только настроить каждый сервер WSUS, учитывая приведенные ниже аспекты.
...
Если в сети есть мобильные пользователи, которые выполняют вход в сеть из различных мест, можно настроить службы WSUS так, чтобы позволить пользователям в роуминге осуществлять обновление на своих клиентских компьютерах с ближайшего к ним географически сервера WSUS. Например может развернуть один сервер WSUS каждого региона и используйте другую подсеть DNS для каждого региона. Все клиентские компьютеры может быть перенаправлен на тот же сервер WSUS, который подключает каждую подсеть к ближайшему физически серверу WSUS.

## <a name="BKMK_1.3."></a>1.3. Выбор стратегии хранения данных для службы WSUS
...
SQL Server, SQL Server Express и внутренняя база данных Windows обеспечивают одинаковые характеристики производительности для конфигурации, в которой используется один сервер, а база данных и служба WSUS расположены на одном компьютере. Односерверная конфигурация может поддерживать несколько тысяч клиентских компьютеров WSUS.

> [!NOTE]
> Не пытайтесь управлять службами WSUS через прямой доступ к базе данных. прямые манипуляции с базы данных может привести к повреждению базы данных. Такое повреждение может незаметно сразу, но помешает обновлению продукта до следующей версии. Управление WSUS можно осуществлять через консоль WSUS или программный интерфейс WSUS.
...
> [!NOTE]
> Внутреннюю базу данных Windows (WID) появилась в Windows Server 2012.
...
Внутренняя база данных Windows не предоставляет пользовательский интерфейс или какие-либо инструменты управления базой данных. Если эта база данных будет выбрана для WSUS, то возникнет необходимость использования внешних инструментов управления базой данных. Дополнительные сведения см. в следующих разделах:

-   [Резервное копирование и восстановление данных службы WSUS и Архивация сервера](https://technet.microsoft.com/library/dd939904(WS.10).aspx)

-   [Переиндексации базы данных WSUS](https://technet.microsoft.com/library/dd939795(WS.10).aspx)
...
Данный параметр требует, чтобы на сервере было достаточно дискового пространства для хранения всех необходимых обновлений. как минимум службам WSUS требуется 20 ГБ для хранения обновлений локально; Тем не менее рекомендуется иметь 30 ГБ, по результатам тестирования.
...
## <a name="BKMK_1.4."></a>1.4. Выбор языков обновления для службы WSUS
...
Страница **Выбор языков** мастера настройки WSUS позволяет получать обновления на всех языках или на какой-то определенной подгруппе языков. Выбор подмножества языков экономит место на диске, но это важно, чтобы выбрать все языки, которые необходимы для всех подчиненных серверов и клиентских компьютеров WSUS-сервера.

Ниже приведены некоторые важные замечания о языка обновления, вам необходимо иметь в виду, прежде чем настроить этот параметр:
...
Обновления будут отображаться как **неприменимые** на клиентских компьютерах, требующих данный язык. Чтобы избежать этого, убедитесь, что все языки операционных систем включены в параметры синхронизации сервера WSUS. Вы увидите все языки операционных систем, перейдя к **компьютеров** представление консоли администрирования WSUS и примените сортировку компьютеров по языку операционной системы. Тем не менее может потребоваться включить дополнительные языки при наличии приложений Майкрософт, в более чем один язык (например, если французской версии Microsoft Word установлен на некоторых компьютерах, использующих английскую версию Windows 8.
...
1.  Если вышестоящий сервер был настроен для загрузки файлов обновлений в некоторый набор языков: В мастере настройки служб WSUS щелкните **загружать обновления только этих языков (вышестоящий сервер поддерживаются только языки, отмеченные звездочкой)**, а затем выберите языки, для которых требуются обновления.
...
2.  Если вышестоящий сервер был настроен для загрузки файлов обновлений на всех языках: В мастере настройки служб WSUS щелкните **загрузку обновлений на всех языках, поддерживаемых вышестоящим сервером**.
...
## <a name="BKMK_1.5"></a>1.5. Планирование групп компьютеров службы WSUS
...
> [!NOTE]
> Если сервер WSUS работает в режиме реплики, группы компьютеров нельзя создать на сервере. Все группы компьютеров, которые необходимы для клиентских компьютеров сервера-реплики, должны быть созданы на корневом сервере WSUS иерархии серверов WSUS. Дополнительные сведения о режиме реплики см. в разделе Управление серверами-репликами WSUS [управление серверами-репликами WSUS](https://technet.microsoft.com/library/dd939893(WS.10).aspx) в руководстве WSUS 3.0 с пакетом обновления 2.

Компьютеры всегда входят в **все компьютеры** группы они остаются назначенными **неназначенные компьютеры** группе, пока вы не добавите их в другую группу. Компьютеры могут входить в несколько групп.
...
-   **Указание на стороне сервера**: Вы вручную назначить один или несколько клиентских компьютеров к нескольким группам одновременно.

-   **Указание на стороне клиента**: Используется групповая политика или изменяются настройки реестра на клиентских компьютерах, чтобы эти компьютеры для автоматического добавления клиентских компьютеров в ранее созданную в группы.
...
#### <a name="BKMK_Priority"></a>Приоритет
...
В следующем текстовом примере области иерархии консоли служб обновления для сервера WSUS с именем WSUS-01, группы компьютеров с именем настольных компьютеров и сервера были добавлены по умолчанию **все компьютеры** группы. Группы серверов и настольных компьютеров, на том же иерархическом уровне.
...
В этом примере группа двумя уровнями ниже ветви настольных компьютеров (настольные компьютеры-у2) имеет более высокий приоритет, чем группа на один уровень ниже ветви "сервер" (серверы-у1). Соответственно, для компьютера, являющегося членом сразу двух групп, "Настольные компьютеры-У2" и "Серверы-У1", все действия для группы "Настольные компьютеры-У2" обладают более высоким приоритетом по сравнению с действиями, определенными для группы "Серверы-У1".

#### <a name="BKMK_Install"></a>Приоритет установки и удаления
...
#### <a name="BKMK_Deadline"></a>Приоритет крайних сроков
...
## <a name="BKMK_1.6."></a>1.6. Рекомендации по планированию производительности службы WSUS
Существуют несколько моментов, которые необходимо тщательно спланировать перед тем, как разворачивать WSUS. Это поможет оптимизировать производительность. Ключевые моменты
...
### <a name="BKMK_1.6.Network"></a>Настройка сети
...
### <a name="BKMK_1.6.Deferred"></a>Отложенная загрузка
...
При развертывании иерархии подключенных серверов WSUS рекомендуется не располагать серверы глубоко. Если отложенная загрузка активирована, а подчиненный сервер запрашивает обновление, которое не утверждено на вышестоящем сервере, запрос подчиненного сервера вызывает загрузку на вышестоящем сервере. Затем подчиненный сервер загружает обновление при последующей синхронизации. На глубоких уровнях иерархии серверов WSUS при запросе обновлений, их загрузке и прохождении через иерархию могут возникать задержки. По умолчанию отложенные загрузки активированы при локальном хранении обновлений. Данный параметр можно поменять вручную.

### <a name="BKMK_1.6.Filters"></a>Фильтры
...
### <a name="BKMK_1.6.Installation"></a>Установка
...
Иногда эта функция называется «дельта-доставка», поскольку она загружает только дельту (разницу) между двумя версиями файла. Файлы экспресс-установки имеют больший размер, чем обновления, которые распространяются на клиентские компьютеры, поскольку эти файлы содержат все возможные версии каждого обновляемого файла.
...
### <a name="BKMK_1.6.LargeUpdates"></a>Развертывание крупных обновлений
...
### <a name="BKMK_1.6.BITS"></a>Фоновая интеллектуальная служба передачи
...
## <a name="BKMK_1.7."></a>1.7. Планирование параметров автоматического обновления
...
По умолчанию автоматическое обновление опрашивает сервер WSUS на наличие утвержденных обновлений каждые 22 часа за вычетом случайной задержки. Если новые обновления должны быть установлены, они загружаются. Время между каждым циклом обнаружения можно менять от 1 до 22 часов.
