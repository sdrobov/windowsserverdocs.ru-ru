...
|Задача|Описание|
|----|--------|
|[1,1. рекомендации по проверке и требования к системе](#11-review-considerations-and-system-requirements)|Проанализируйте список факторов, которые необходимо учесть, а также требования к системе, чтобы убедиться, что у вас есть все необходимое оборудование и программное обеспечение для развертывания службы WSUS.|
|[1,2. Выбор сценария развертывания WSUS](#12-choose-a-wsus-deployment-scenario)|Выберите сценарий развертывания службы WSUS, который будет использован.|
|[1,3. Выберите стратегию хранилища WSUS](#13-choose-a-wsus-storage-strategy)|Выберите, какая из стратегий хранения данных для службы WSUS наиболее подходит для данного развертывания.|
...
|[1,6. планирование вопросов производительности WSUS: фоновая интеллектуальная служба передачи](#16-plan-wsus-performance-considerations)|Разработайте модель службы WSUS, которая позволяла бы добиться оптимизированной производительности.|
|[1,7. Планирование параметров автоматическое обновление](#17-plan-automatic-updates-settings)|Продумайте, как будете настраивать параметры автоматического обновления в своем сценарии.|
...
-   Требования к оборудованию сервера для включения роли WSUS привязаны к требованиям к оборудованию. Минимальные аппаратные требования к WSUS
    ...
    -   **Память:** Для WSUS требуется еще 2 ГБ ОЗУ, чем требуется для сервера, а также для других служб и программного обеспечения.

    -   **Доступно места на диске:** 10 ГБ (рекомендуется 40 ГБ или больше)
...
1.  Если вышестоящий сервер был настроен для загрузки файлов обновлений на языках определенного набора, выполните следующие действия. В мастере настройки служб WSUS щелкните **Загружать обновления только для следующих языков (вышестоящий сервер поддерживает только языки, отмеченные звездочкой)** , а затем выберите языки, для которых требуются обновления.
...
2. Если на вышестоящем сервере настроена загрузка файлов обновления на всех языках, в мастере настройки служб WSUS щелкните **Загрузки обновления на всех языках, поддерживаемых вышестоящим сервером**.
...
Иногда эта функция называется "разностная доставка", так как она скачивает только те объекты, которые отличаются в двух версиях файлов. Файлы экспресс-установки имеют больший размер, чем обновления, которые распространяются на клиентские компьютеры, поскольку эти файлы содержат все возможные версии каждого обновляемого файла.
