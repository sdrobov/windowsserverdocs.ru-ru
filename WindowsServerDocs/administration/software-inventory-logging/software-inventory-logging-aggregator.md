...
-   Число пометок с высоким преводами и имя узла, одновременно выполняющие управляемый \(Агент System Center, присутствующее\) виртуальных машин Windows Server на каждом узле со временем.

-   Имя агентов System Center, установленных на виртуальных машинах, подсчитанных с учетом управляемой высокой\-маркировки воды

-   Количество и расположение SQL Server установок в течение времени \(только номера SKU и выпуски, для которых требуется лицензия\)
...
|`Add-SilVmHost`|Используется для добавления конкретных имен узлов или массивов имен узлов, которые должны быть опрошены в обычном интервале \(по умолчанию — интервал в один час\).|
...
-   В средстве инвентаризации программного обеспечения:
    ...
    -   Выполните `Set-SilAggregator –AddCertificateThumbprint` "отпечаток сертификата клиента".

        Это необходимо для получения данных по HTTPS с серверов Windows Server, где запущена инвентаризация программного обеспечения. Отпечаток будет добавлен в список отпечатков, по которым средство инвентаризации программного обеспечения будет получать данные. Средство инвентаризации программного обеспечения может принимать действительные корпоративные сертификаты проверки подлинности клиента. Используемый сертификат необходимо установить в хранилище **\\localmachine\MY (локальный компьютер — > личное**) на сервере, на котором пересылается данные.
...
-   `New-PSDrive -Name $firstAvailableDriveLetter -PSProvider filesystem -root` **<\\сервер\пас для предоставления общего доступа, который содержит файл сертификата pfx >** `-credential $mycreds`

-   `Copy-Item ${firstAvailableDriveLetter}:\` **< файл CertificateName. pfx в каталоге New drive > c:\<расположение по своему усмотрению >**
...
|Имя узла|Сетевое имя или полное доменное имя для физического узла, который успешно опрашивается средством инвентаризации программного обеспечения.<p>Воспользуйтесь командлетом Get-SilVMHost, чтобы найти узлы, которые были добавлены, но не опрашиваются или больше не опрашиваются. Отображается последний успешный опрос.|
...
|Дата и время опросов|Дата и время последней отметки пиковой активности для одновременно работающих виртуальных машин Windows Server на этом физическом узле.<p>См. раздел **Время и дата опросов** в этой документации.|
...
-   Если командлеты `SilLogging` или `Publish-Sildata` завершаются ошибкой, необходимо проверить следующее.
    ...
    -   Убедитесь, что все необходимые обновления для Windows Server установлены (см. предварительные требования для инвентаризации программного обеспечения).  Чтобы быстро проверить, нужно ли найти их с помощью следующего командлета: `Get-SilWindowsUpdate *3060*, *3000*`
    ...
    -   Вы можете проверить следующее расположение кэшированных файлов SIL на сервере, пытающемся выполнить пересылку или принудительную отправку, \Windows\System32.\\файлы журнала\\SIL. Если команда `SilLogging` запущена и работает более часа или команда `Publish-SilData` была недавно выполнена и в этом каталоге нет файлов, значит, отправка данных по инвентаризации прошла успешно.
...
[Агрегатор 1,0 для ведения журнала инвентаризации программного обеспечения для Windows Server](https://www.microsoft.com/download/details.aspx?id=49046)<br>
