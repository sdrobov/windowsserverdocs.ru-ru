---
title: certutil
description: Справочный раздел о команде certutil, которая является программой командной строки, которая выводит дампы и отображает сведения о конфигурации центра сертификации (ЦС), настраивает службы сертификации, компоненты ЦС для резервного копирования и восстановления, а также проверяет сертификаты, пары ключей и цепочки сертификатов.
...
- **Флаги** задает приоритет расширения. `0`При `1` задании расширения как критическое, `2` отключение расширения и и то, и `3` другое.
...
- **инфонаме** указывает отображаемое свойство CA на основе следующего синтаксиса аргумента инфонаме:
  ...
  - **выход `[index]` из** описания модуля выхода
  ...
  - **ксчгчаин `[index]` ** — цепочка сертификатов Exchange CA

  - **ксчгкрлчаин `[index]` ** — цепочка сертификатов Exchange CA с CRL
  
  - **KRA `[index]` ** -сертификат
  
  - перекрестный перекрестный перекрестный сертификат ** `[index]` **
  
  - **кросс- `[index]` ** обратный перекрестный сертификат
  
  - **CRL `[index]` ** — базовый CRL
  
  - **делтакрл `[index]` ** — разностный CRL
  
  - состояние публикации **крлстатус `[index]` ** -CRL
...
- **цертидлист** — это разделенный запятыми список маркеров сертификата или списка отзыва сертификатов. Дополнительные сведения см. в `-store certID` описании в этой статье.
...
- **KeyContainerName** — это имя контейнера ключей для проверяемого ключа. По умолчанию этот параметр имеет значение ключи компьютера. Чтобы переключиться на пользовательские ключи `-user`, используйте.
...
- **Ктлобжект** определяет список проверяемых сертификатов (CTL), включая:
  ...
  - **Аусрутву** — считывает CAB-файл аусрут и соответствующие сертификаты из кэша URL. Вместо `-f` этого используйте для скачивания из центр обновления Windows.

  - **Дисалловедву** — чтение CAB-файла запрещенных сертификатов и неразрешенный файл хранилища сертификатов из кэша URL. Вместо `-f` этого используйте для скачивания из центр обновления Windows.
...
- **цертдир** указывает папку, содержащую сертификаты, соответствующие записям CTL. По умолчанию используется та же папка или веб-сайт, что и **ктлобжект**. Для использования пути к папке HTTP в конце необходимо указать разделитель пути. Если не указать **аусрут** или не **разрешено**, будет выполнен поиск соответствующих сертификатов в нескольких расположениях, включая локальные хранилища сертификатов, ресурсы Crypt32. dll и локальный кэш URL. При `-f` необходимости используйте для скачивания из центр обновления Windows.
...
| — без уведомления | Используйте флаг `silent` , чтобы получить контекст шифрования. |
...
- [Экспорт двоичных запросов с помощью программы командной строки Certutil. exe](https://social.technet.microsoft.com/wiki/contents/articles/7573.active-directory-certificate-services-pki-key-archival-and-management.aspx)
