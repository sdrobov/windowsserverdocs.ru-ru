...
CertUtil [параметры]-сетекстенсион RequestId ExtensionName flags {Long | Дата | Строка | \@InFile}
...
CertUtil [параметры]-Скрутс Save \@OutputRootFile [Реадернаме]
...
CertUtil [параметры]-Sign Инфилелист | SerialNumber | CRL Аутфилелист [StartDate + ДД: ЧЧ] [+ Сериалнумберлист |-Сериалнумберлист |-Обжектидлист | \@ExtensionFile]
...
CertUtil [параметры]-жетрег [{CA | Restore | Policy | Exit | шаблон | регистрация | цепочка | Полицисерверс} \[ProgId @ no__t-1] [Регистривалуенаме]
...
CertUtil [параметры]-Setreg [{ЦС | восстановление | политика | выход | шаблон | регистрация | цепочка | Полицисерверс} \[ProgId @ no__t-1] Регистривалуенаме значение
...
CertUtil [параметры]-делрег [{CA | Restore | Policy | Exit | шаблон | регистрация | цепочка | Полицисерверс} \[ProgId @ no__t-1] [Регистривалуенаме]
