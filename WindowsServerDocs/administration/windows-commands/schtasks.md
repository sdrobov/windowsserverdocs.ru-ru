...
## <a name="BKMK_create"></a>команда Schtasks CREATE
...
### <a name="BKMK_syntax"></a>Объединенный синтаксис и описания параметров
...
##### <a name="sc-scheduletype"></a>/SC @no__t — 0ScheduleType >
...
##### <a name="s-computer"></a>/s @no__t 0Computer >
...
##### <a name="ru-domainuser--system"></a>/ru {[\<Domain > \] @ no__t-2 | Системой
...
|       Значение        |                                                    Описание                                                    |
|--------------------|-------------------------------------------------------------------------------------------------------------------|
| [\<Domain > \] @ no__t-2 |                                       Указывает альтернативную учетную запись пользователя.                                        |
...
##### <a name="rp-password"></a>/RP @no__t — 0Password >
...
##### <a name="mo-modifier"></a>/Mo @no__t — 0Modifier >
...
|-------------|---------------|-----------|
|ИНТЕРВАЛ|1 - 1439|Задача выполняется каждые \<N > минут.|
|КАЖДЫЙ ЧАС|1 - 23|Задача выполняется каждые \<N > часов.|
|ОБНОВЛЯЕТСЯ|1 - 365|Задача выполняется каждые \<N > дней.|
...
|ЕЖЕМЕСЯЧНО|1 - 12|Задача выполняется каждые \<N > месяцев.|
|ЕЖЕМЕСЯЧНО|ЛАСТДАЙ|Задача выполняется в последний день месяца.|
|ЕЖЕМЕСЯЧНО|ПЕРВЫЙ, ВТОРОЙ, ТРЕТИЙ, ЧЕТВЕРТЫЙ, ПОСЛЕДНИЙ|Используйте с параметром **/d**\<Day > для выполнения задачи в определенной неделе и дне. Например, в третью среду месяца.|
...
|    ЕЖЕМЕСЯЧНО    | ПЕРВЫЙ, ВТОРОЙ, ТРЕТИЙ, ЧЕТВЕРТЫЙ, ПОСЛЕДНИЙ |        ПОНЕДЕЛЬНИК — SUN         |                                                                                   Требуется для расписания конкретной недели.                                                                                    |
|    ЕЖЕМЕСЯЧНО    |          Нет или {1-12}          |          1 - 31          | Необязательный и допустимый только с параметром No модификатора ( **/mo**) (конкретное расписание даты) или если параметру **/mo** присвоено значение 1-12 ("каждое расписание \<N > месяцев"). Значение по умолчанию — день 1 (первый день месяца). |
...
##### <a name="i-idletime"></a>/i @no__t — 0IdleTime >
...
##### <a name="st-starttime"></a>/ST @no__t — 0StartTime >

Указывает время запуска задачи (при каждом запуске) в 24-часовом формате \<HH: MM >. Значение по умолчанию — текущее время на локальном компьютере. Параметр **/St** действителен с почасовой, ежечасной, ежедневной, еженедельно, ежемесячно и раз в расписаниях. Оно требуется для ОДНОКРАТНОго расписания.

##### <a name="ri-interval"></a>/RI \<Interval >

Указывает интервал повторения в минутах. Это неприменимо для типов расписания: В минуту, ЕЖЕЧАСно, в начале, в OnStart и OnStart. Допустимый диапазон — от 1 до 599940 минут (599940 минут = 9999 часов). Если указан параметр/ET или/DU, интервал повтора по умолчанию равен 10 минутам.

##### <a name="et-endtime"></a>/ET @no__t — 0EndTime >

Указывает время суток, в течение которого расписание выполнения задачи в минуту или ежечасном расписании \<HH: MM > 24-часовом формате. После указанного времени окончания программа **schtasks** не запускает задачу повторно, пока не будет повторяться время запуска. По умолчанию расписания задач не имеют времени окончания. Этот параметр является необязательным и действителен только с поминутным или ЕЖЕЧАСным расписанием.

Пример см. в следующих статьях:
-   "Чтобы запланировать задание, которое выполняется каждые 100 минут в нерабочее время", в разделе " **чтобы запланировать задачу, выполняемую каждые** \<N > **минут** ".

##### <a name="du-duration"></a>/Du @no__t — 0Duration >

Задает максимальный интервал времени в минутах или ежечасном расписании \<HHHH: MM > 24-часовом формате. По истечении указанного времени **команда Schtasks** не запускает задачу повторно, пока не будет повторяться время запуска. По умолчанию расписания задач не имеют максимальной длительности. Этот параметр является необязательным и действителен только с поминутным или ЕЖЕЧАСным расписанием.

Пример см. в следующих статьях:
-   «Чтобы запланировать задание, которое выполняется каждые 3 часа в течение 10 часов» в разделе, **чтобы запланировать задание, которое выполняется каждые** \<N > **часов** .
...
##### <a name="sd-startdate"></a>/SD @no__t — 0StartDate >
...
|       Значение       |                                        Описание                                         |
|-------------------|--------------------------------------------------------------------------------------------|
| @NO__T РАВНЫМИ 0 ММ >/<DD>/<YYYY> | Используйте для форматов с первого месяца, например для **английского (США)** и **испанского (Панама)** . |
| \<DD >/<MM> @ NO__T-2 @ NO__T-3 |       Используйте в качестве формата для первых дней, например **болгарского** и **нидерландского (Нидерланды)** .        |
| \<YYYY >/<MM> @ NO__T-2<DD> |          Используйте для форматов с первым годом, например **шведского** и **французского (Канада)** .          |
...
Задает выполнение задачи только в том случае, если пользователь с правами запуска от имени (учетная запись пользователя, под которой выполняется задача) входит в систему на компьютере. Этот параметр не влияет на задачи, выполняемые с разрешениями системы.

По умолчанию пользователь с именем "Запуск от имени" является текущим пользователем локального компьютера, когда задача запланирована, или учетной записью, указанной параметром **/u** , если она используется. Однако если команда включает параметр **/ru** , то пользователь «Запуск от имени» является учетной записью, указанной параметром **/ru** .

Примеры см. в следующих статьях:
-   "Чтобы запланировать задачу, которая выполняется каждые 70 дней, если я вошел в систему" в разделе " **чтобы запланировать задачу, которая выполняется каждые** *N* **дней** ".
-   "Выполнение задачи только при входе конкретного пользователя в систему" в разделе " **планирование задачи, выполняемой с разными разрешениями** ".
...
### <a name="BKMK_minutes"></a>Планирование задачи, выполняемой каждые N минут
...
### <a name="BKMK_hours"></a>Планирование задачи, выполняемой каждые N часов
...
### <a name="BKMK_days"></a>Планирование задачи, выполняемой каждые N дней
...
### <a name="BKMK_weeks"></a>Планирование задачи, выполняемой каждые N недель
...
Еженедельные расписания также имеют необязательный параметр **/d** , чтобы запланировать выполнение задачи в указанные дни недели или за все дни ( *). Значение по умолчанию — Пн (понедельник). Параметр ежедневно (* ) эквивалентен планированию ежедневной задачи.
...
### <a name="BKMK_months"></a>Планирование задачи, выполняемой каждые N месяцев
...
### <a name="BKMK_spec_day"></a>Планирование задачи, выполняемой в конкретный день недели
...
### <a name="BKMK_spec_week"></a>Планирование задачи, выполняемой в течение определенной недели месяца
...
### <a name="BKMK_spec_date"></a>Планирование задачи, которая выполняется с определенной датой, каждый месяц
...
### <a name="BKMK_last_day"></a>Планирование задачи, выполняемой в последний день месяца
...
### <a name="BKMK_once"></a>Планирование задачи, которая выполняется один раз
...
### <a name="BKMK_startup"></a>Планирование задачи, выполняемой при каждом запуске системы
...
### <a name="BKMK_logon"></a>Планирование задачи, выполняемой при входе пользователя в систему
...
Тип расписания "при входе в систему" планирует задачу, которая выполняется каждый раз, когда пользователь входит в систему на компьютере. В расписании "при входе в систему" требуется параметр **/sc onlogon** . Параметр **/SD** (Дата начала) является необязательным, и по умолчанию используется текущая дата.
...
### <a name="BKMK_idle"></a>Планирование задачи, выполняемой при простое системы
...
Тип расписания "в режиме простоя" планирует задачу, которая выполняется при отсутствии активности пользователя в течение времени, заданного параметром **/i** . В расписании "в режиме простоя" требуется параметр **/SC OnIdle** и параметр **/i** . Параметр **/SD** (Дата начала) является необязательным, и по умолчанию используется текущая дата.
...
### <a name="BKMK_now"></a>Планирование задачи, которая выполняется сейчас
...
Следующая команда планирует выполнение задачи с 13 ноября 2002 по 2:18 вечера. Местное время.
...
### <a name="BKMK_diff_perms"></a>Планирование задачи, выполняемой с разными разрешениями
...
```
В ответе программа **SchTasks. exe** запрашивает пароль для запуска от имени для учетной записи Admin06, а затем выводит сообщение об успешном выполнении.
```
...
### <a name="BKMK_sys_perms"></a>Планирование задачи, выполняемой с разрешениями системы

Задачи всех типов могут выполняться с разрешениями системной учетной записи как на локальном, так и на удаленном компьютере. В дополнение к параметрам, необходимым для конкретного типа расписания, требуется параметр **системы/ru** (или **/ru ""** ), а параметр **/RP** является недопустимым.
...
### <a name="BKMK_multi_progs"></a>Планирование задачи, запускающей более одной программы
...
### <a name="BKMK_remote"></a>Планирование задачи, выполняемой на удаленном компьютере
...
## <a name="BKMK_change"></a>команда Schtasks Change
...
| /ru {[\<Domain > \] @ no__t-2 |                                                                                                                                                                                                                                                                                                                                       Системой                                                                                                                                                                                                                                                                                                                                       |
|     /RP @no__t — 0Password >     |                                                                                                                                                                                                                                                 Указывает новый пароль для существующей учетной записи пользователя или учетную запись пользователя, указанную параметром **/ru** . Этот параметр не учитывается при использовании с учетной записью локальной системы.                                                                                                                                                                                                                                                  |
|     /TR \<TaskRun >      |                                                                                                                                                                                  Изменяет программу, выполняемую задачей. Введите полный путь и имя файла исполняемого файла, файла скрипта или пакетного файла. Если опустить путь, программа **schtasks** предполагает, что файл находится в каталоге \<systemroot > выполнять. Указанная программа заменяет исходную программу, выполняемую задачей.                                                                                                                                                                                  |
|    /ST @no__t — 0Starttime >     |                                                                                                                                                                                                                                                              Указывает время начала задачи с использованием 24-часового формата времени, чч: мм. Например, значение 14:30 эквивалентно 12-часовому времени 2:30 PM.                                                                                                                                                                                                                                                               |
...
|     /ET @no__t — 0EndTime >      |                                                                                                                                                                                                                                                               Задает время окончания задачи с использованием 24-часового формата времени, чч: мм. Например, значение 14:30 эквивалентно 12-часовому времени 2:30 PM.                                                                                                                                                                                                                                                                |
|     /Du @no__t — 0Duration >     |                                                                                                                                                                                                                                                                                                     Задает Закрытие задачи в \<EndTime > или <Duration>, если она указана.                                                                                                                                                                                                                                                                                                      |
|           /k            |                                                                                                                                                                   Останавливает программу, выполняемую задачей в момент, заданный параметром **/et** или **/du**. Без **/k**программа **schtasks** не запускает программу снова после того, как она достигнет времени, заданного параметром **/et** или **/du**, но не останавливает программу, если она все еще выполняется. Этот параметр является необязательным и действителен только с поминутным или ЕЖЕЧАСным расписанием.                                                                                                                                                                   |
|    /SD @no__t — 0StartDate >     |                                                                                                                                                                                                                                                                                              Указывает первую дату выполнения задачи. Формат даты — MM/ДД/ГГГГ.                                                                                                                                                                                                                                                                                               |
|     /ED @no__t — 0EndDate >      |                                                                                                                                                                                                                                                                                                 Указывает последнюю дату выполнения задачи. Формат: мм/дд/гггг.                                                                                                                                                                                                                                                                                                  |
|         РАЗРЕШЕНИЕ         |                                                                                                                                                                                                                                                                                                                       Задает включение запланированной задачи.                                                                                                                                                                                                                                                                                                                       |
|        /DISABLE         |                                                                                                                                                                                                                                                                                                                      Задает отключение запланированной задачи.                                                                                                                                                                                                                                                                                                                       |
|           разрешает           | Указывает, что выполнение запланированной задачи выполняется только при входе пользователя в систему (учетная запись пользователя, под которой выполняется задача) на компьютер.</br>Этот параметр не влияет на задачи, выполняемые с системными разрешениями или задачами, для которых уже задан интерактивный набор свойств. Нельзя использовать команду Change для удаления интерактивного свойства из задачи.</br>По умолчанию пользователь с именем "Запуск от имени" является текущим пользователем локального компьютера, когда задача запланирована, или учетной записью, указанной параметром **/u** , если она используется. Однако если команда включает параметр **/ru** , то пользователь «Запуск от имени» является учетной записью, указанной параметром **/ru** . |
...
Следующая команда изменяет пароль учетной записи пользователя для задачи Реминдме на удаленном компьютере SVR01. Команда использует параметр **/TN** для определения задачи, а параметр **/s** — для указания удаленного компьютера. В нем используется параметр **/RP** для указания нового пароля, p@ssWord3.
...
Следующая команда добавляет интерактивное свойство в MyApp, существующую задачу. Это свойство гарантирует, что задача выполняется только в том случае, если пользователь, запускающий эту задачу, выполняет вход в систему на компьютере.
...
## <a name="BKMK_run"></a>команда Schtasks Run
...
## <a name="BKMK_end"></a>команда Schtasks End
...
|    /s @no__t 0Computer >     |                                                                                                                        Указывает имя или IP-адрес удаленного компьютера. По умолчанию используется локальный компьютер.                                                                                                                        |
| /u [\<Domain > \] @ no__t-2 | Выполняет эту команду с разрешениями указанной учетной записи пользователя. По умолчанию команда выполняется с разрешениями текущего пользователя локального компьютера. Указанная учетная запись пользователя должна быть членом группы "Администраторы" на удаленном компьютере. Параметры **/u** и **/p** допустимы только при использовании параметра **/s**. |
|    /p \<пароль >     |                        Указывает пароль учетной записи пользователя, указанной в параметре **/u** . Если используется параметр **/u** , но пропущен параметр **/p** или аргумент password, **команда Schtasks** запрашивает пароль.</br>Параметры **/u** и **/p** допустимы только при использовании параметра **/s**.                         |
|          /?           |                                                                                                                                                             Отображает справку.                                                                                                                                                              |
...
## <a name="BKMK_delete"></a>команда Schtasks Delete
...
## <a name="BKMK_query"></a>запрос SchTasks
