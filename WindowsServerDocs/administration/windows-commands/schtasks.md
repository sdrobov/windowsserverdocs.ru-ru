...
|Каждый час|1 - 23|Задача выполняется каждые \<N > ч.|
...
|    ЕЖЕМЕСЯЧНО    |          Нет или {1-12}          |          1 - 31          | Необязательный параметр и допустимый только без модификатора ( **/mo**) (определенное расписание даты) или если параметру **/mo** присвоено значение 1-12 ("каждые \<N > месяцев"). Значение по умолчанию — день 1 (первый день месяца). |
...
Указывает время запуска задачи (при каждом запуске) в \<чч: мм > 24-часовом формате. Значение по умолчанию — текущее время на локальном компьютере. Параметр **/St** действителен с почасовой, ежечасной, ежедневной, еженедельно, ежемесячно и раз в расписаниях. Оно требуется для ОДНОКРАТНОго расписания.
...
-   "Чтобы запланировать задание, которое выполняется каждые 100 минут в нерабочее время", в разделе " **планирование задачи, выполняемой каждые** \<N > **минут** ".

##### <a name="du-duration"></a>Время \</du >
...
|     Время \</du >     |                                                                                                                                                                                                                                                                                                     Указывает, что задача будет закрыта на \<EndTime > или <Duration>, если она указана.                                                                                                                                                                                                                                                                                                      |
...
Следующая команда удаляет все задачи из расписания на локальном компьютере, включая задачи, запланированные другими пользователями. Он использует параметр **/tn \\** * для представления всех задач на компьютере и параметр **/f** для подавления сообщения подтверждения.
```
