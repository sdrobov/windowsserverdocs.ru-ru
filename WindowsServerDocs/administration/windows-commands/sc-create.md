...
|зависимый = \<dependencies >|Указывает имена служб или групп, которые должны быть запущены перед запуском этой службы. Имена разделяются косой чертой (/).|
...
|Password = \<Password >|Указывает пароль. Это необходимо, если используется учетная запись, отличная от LocalSystem.|
