...
|@no__t 0VolumeName >|Указывает имя тома, которое является целевым объектом точки подключения. Имя тома использует следующий синтаксис, где *GUID* является глобальным уникальным идентификатором:</br>`\\\\?\Volume\{GUID}\`</br>Квадратные скобки {} являются обязательными.|
