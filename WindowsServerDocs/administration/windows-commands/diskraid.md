---
title: diskraid
description: Справочный раздел об DiskRAID, который представляет собой средство командной строки, позволяющее настраивать избыточные массивы независимых дисков (или недорогых), а также управлять ими.
...
Чтобы показать, как связать и добавить порты в LUN, использующий поставщик VDS 1,1:
```
DISKRAID> SEL LUN 5
...
Удаляет указанные флаги. Ключевое слово **ALL** очищает все флаги автоволшебия.
...
### <a name="delete"></a><a name=BKMK_7></a>удалить
...
### <a name="detail"></a><a name=BKMK_8></a>Описание
...
Указывает политику балансировки нагрузки. Если тип не указан, необходимо указать параметр **path** . Параметр Type может иметь одно из следующих значений:
...
diskraid
```
Нажмите клавишу ВВОД. Отобразится следующее:
```
...
select subsystem 0
```
Нажмите клавишу ВВОД. Выходные данные должны выглядеть примерно так:
```
