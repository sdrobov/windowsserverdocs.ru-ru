...
| путь | Указывает, является ли путь **первичным** или имеет определенный тип `<weight>` . Все пути, которые не указаны, неявно устанавливаются в качестве резервной копии. Все указанные пути должны быть одним из путей выбранного в данный момент LUN. |
