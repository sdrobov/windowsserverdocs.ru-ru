...
### <a name="BKMK_2"></a>связана

Устанавливает указанный список портов контроллера как активный для текущего выбранного LUN (другие порты контроллера становятся неактивными) или добавляет указанные порты контроллера в список существующих активных портов контроллера для текущего выбранного LUN или связывает указанный целевой объект iSCSI для выбранного в данный момент LUN.
...
### <a name="BKMK_3"></a>Автоматическое магическое
...
|СЛУЖБА|Размер полосы|
|ВТК|Кэширование сквозной записи включено|
|ИНК|Службе|

### <a name="BKMK_4"></a>разбиени
...
Необходимо указать либо *Размер* , \<либо параметр > диска. Их также можно использовать вместе.

### <a name="BKMK_12"></a>флушкаче
...
### <a name="BKMK_16"></a>инвалидатекаче
...
**ВЕНДОРСПЕЦИФИК**: Использует политику конкретного поставщика.
...
### <a name="BKMK_20"></a>пользователей
...
**EMP**: Включить Multipath

**ЕХД**: Включить дайджест заголовка

**EDD**: Включить дайджест данных

### <a name="BKMK_21"></a>Войдите
...
Указывает > \<drive_number для заменяемого диска.

#### <a name="remarks"></a>Примечания

-   Возможно, указанный диск не является выбранным в данный момент.

### <a name="BKMK_32"></a>перезапуск
...
Указывает тип объекта для выбора. Тип > объектаможетбытьпоставщиком,подсистемой,контроллером,дискомилиLUN.\<
...
Указывает > номер объекта для выбора. \< Если указанный <object number> объект является недопустимым, все существующие элементы указанного типа будут удалены. Если значение <object number> не указано, отображается текущий объект.
...
### <a name="BKMK_shrink"></a>соответствии
...
|----------|-----------------|
|0|Ошибки не обнаружены. Весь сценарий выполнялся без ошибок.|
...
## <a name="example-interactively-view-status-of-subsystem"></a>Пример. Просмотр состояния подсистемы в интерактивном режиме
