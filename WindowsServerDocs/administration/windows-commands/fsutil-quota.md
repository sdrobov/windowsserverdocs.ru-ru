...
| @no__t 0VolumePath > |                                  Обязательный. Указывает имя диска, за которым следует двоеточие или идентификатор GUID в формате **тома {** <em>GUID</em> **}** .                                  |
| @no__t 0Threshold >  |                            Задает предельный размер (в байтах), по истечение которого выдаются предупреждения. Этот параметр является обязательным для команды **изменения квоты fsutil** .                            |
|   @no__t 0Limit >    |                                Задает максимально допустимое использование места на диске (в байтах). Этот параметр является обязательным для команды **изменения квоты fsutil** .                                |
|  @no__t 0UserName >  |                                      Указывает домен или имя пользователя. Этот параметр является обязательным для команды **изменения квоты fsutil** .                                       |
