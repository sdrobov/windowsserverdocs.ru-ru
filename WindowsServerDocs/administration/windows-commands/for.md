...
|Параметр|Описание|
|---------|-----------|
|{%%\|%}\<ая переменная >|Обязательное. Представляет заменяемый параметр. Используйте один знак процента ( **%** ), чтобы выполнить команду **for** в командной строке. Используйте двойные знаки процента ( **%%** ) для выполнения команды **for** в пакетном файле. Переменные чувствительны к регистру и должны быть представлены в алфавитном значении, например **% A**, **% B**или **% C**.|
|(набор\<>)|Обязательное. Указывает один или несколько файлов, каталогов или текстовых строк или диапазон значений, по которым должна выполняться команда. Круглые скобки являются обязательными.|
|Команда \<>|Обязательное. Указывает команду, которую нужно выполнить для каждого файла, каталога или текстовой строки, или диапазон значений, включенных в *набор*.|
...
- Использование параметров пакетной службы
  ...
  - Команда **for** <em>заменяет переменную</em> **%** или **%%** <em>переменную</em> на каждую текстовую строку в указанном наборе, пока указанная команда не обработает все файлы.
...
- Указание группы файлов
  ...
  (ar??1991.* ap??1991.*)
  ```  
  При использовании команды **for** первое *заданное* значение заменяется **%** <em>переменной</em> или **%%** <em>переменной</em>, а затем заданная команда обрабатывает это значение. Это будет продолжаться до тех пор, пока не будут обработаны все файлы (или группы файлов), соответствующие *заданному значению* .
...
- Использование дополнительных форм **для**
  ...
  - Итерация и синтаксический анализ файлов
    ...
    |     усебаккк      |                                                                                             Указывает: выполнение строки, заключенной в кавычки, в виде команды, использование строки с одинарной кавычкой в качестве литеральной строки или для длинных имен файлов, содержащих пробелы, позволяет заключать имена файлов в *\<Set\>* в кавычки в двойные кавычки.                                                                                              |
...
- Синтаксический анализ строки
  ...
  Логику синтаксического анализа **for/f** можно использовать в непосредственной строке путем заключения *\<литералстринг\>* в двойные кавычки (*без* усебаккк) или в одинарные кавычки (*с* усебаккк), например (MyString) или (' MyString '). *\<литералстринг\>* рассматривается как одна строка входных данных из файла. При синтаксическом анализе *\<литералстринг\>* в двойных кавычках символы команд (например, **\\ \& \| \> \< \^** ) обрабатываются как обычные символы.
- Синтаксический анализ выходных данных

  Можно использовать команду **for/f** для анализа выходных данных команды, поместив в скобки *\<команду* , заключенную в кавычки\>. Он рассматривается как Командная строка, которая передается в дочерний файл cmd. exe. Выходные данные фиксируются в памяти и анализируются так, как если бы это был файл.
