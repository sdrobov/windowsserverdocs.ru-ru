---
title: at
description: Справочный раздел для команды at, которая планирует выполнение команд и программ на компьютере в указанное время и дату.
...
- Эта команда не выполняет автоматическую загрузку cmd. exe перед выполнением команд. Если исполняемый файл (exe) не используется, необходимо явно загрузить cmd. exe в начале команды следующим образом:
...
Чтобы выполнить команду, которая не является исполняемым файлом (exe), перед командой с помощью **cmd/c** Загрузите cmd. exe следующим образом:
