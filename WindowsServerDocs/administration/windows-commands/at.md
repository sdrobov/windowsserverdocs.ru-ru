---
title: at
description: Раздел команд Windows для **at** — команды и программы, выполняемые на компьютере в указанное время и дату.
ms.custom: na
...
|      Параметр       |                                                                                                                                                                                                               Описание                                                                                                                                                                                                                |
|----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| \\ @ no__t-1 @ no__t-2computerName @ no__t-3 |                                                                                                                                                        Указывает удаленный компьютер. Если этот параметр не указан, **по** расписанию команды и программы на локальном компьютере.                                                                                                                                                        |
|        \<id @ no__t-1        |                                                                                                                                                                                   Указывает идентификационный номер, назначенный команде по расписанию.                                                                                                                                                                                   |
...
|     /интерактиве     |                                                                                                                                                                  Позволяет *команде* взаимодействовать с рабочим столом пользователя, выполнившего вход во время выполнения *команды* .                                                                                                                                                                  |
...
|     \<command @ no__t-1      | Указывает команду Windows, программу (т. е. exe-или COM-файл) или пакетную программу (файл. bat или. cmd), которую требуется выполнить. Если команде требуется путь в качестве аргумента, используйте абсолютный путь (то есть весь путь, начинающийся с буквы диска). Если команда находится на удаленном компьютере, укажите для имени сервера и общего ресурса нотацию UNC, а не букву удаленного диска. |
|          /?          |                                                                                                                                                                                                   Отображение справки в командной строке.                                                                                                                                                                                                   |

## <a name="remarks"></a>Примечания
- **schtasks** — это еще одно средство планирования командной строки, которое можно использовать для создания и управления запланированными задачами. Дополнительные сведения о команде **schtasks**см. в разделе связанные разделы.
- Использование **в**  
  Для использования **в**необходимо быть членом локальной группы администраторов.
- Загрузка cmd. exe  
  **в** не загружается автоматически cmd. exe, интерпретатор команд, перед выполнением команд. Если исполняемый файл (exe) не используется, необходимо явно загрузить cmd. exe в начале команды следующим образом: **cmd/c dir > к:\тест.аут**
- Просмотр запланированных команд  
  ...
  ```
- Включение идентификационного номера (*ID*)  
...
- Задачи, останавливаемые через 72 часов  
  ...
  3.  В меню Правка выберите команду Добавить значение, а затем добавьте следующее значение реестра: Имя значения: тип данных Аттаскмаксхаурс: reg_DWOrd, основание системы счисления: Данные десятичного значения: 0. Значение 0 в поле данных значения указывает на отсутствие ограничения, не останавливается. Значения от 1 до 99 указывают количество часов.
  **Внимание!**
...
- Планировщик задач и команда **at**  
