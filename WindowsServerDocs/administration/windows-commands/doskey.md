...
|[/ INSERT | / overstrike]|Указывает, следует ли вставлять или перезаписать текста при вводе. Если вы используете **или вставку**, новый текст вставляется в выполнении существующий текст. Если вы используете **/overstrike**, новый заменяет текст в существующий текст. Значение по умолчанию — **/overstrike**.|
...
-   Создание макроса
    ...
    |$B или $b|Отправляет выходные данные макрос команды. Эти специальные символы — это эквивалентно использованию канала (**|*) в командной строке.|
    ...
    |$*|Представляет все параметры командной строки, чтобы указать, при вводе имени макроса. Специальный символ **$ \*** — это подставляемый, аналогичную параметры пакета **$1** через **$9**, с одним важным различие: все, что введено в командной строке после имени макроса подставляется **$ \*** в макросе.|
