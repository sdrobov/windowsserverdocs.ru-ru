...
|   /листсизе = @no__t — 0Size >    |                                                                                                Указывает максимальное количество команд в буфере журнала.                                                                                                 |
...
|  /ексенаме = @no__t — 0ExeName >   |                                                                                        Указывает программу (исполняемую), в которой выполняется макрос **Doskey** .                                                                                         |
| /макрофиле = @no__t — 0FileName > |                                                                                              Указывает файл, содержащий макросы, которые необходимо установить.                                                                                               |
...
- Создание макроса
  ...
  |   $B или $b    |                                                                                                                                    Отправляет выходные данные макроса в команду. Эти специальные символы эквивалентны использованию канала (\* @ no__t-1                                                                                                                                     |
  ...
  |      $\*      | Представляет все сведения командной строки, которые необходимо указать при вводе имени макроса. Специальный символ **$ @ no__t-2**\* — это заменяемый параметр, аналогичный пакетным параметрам с **$1** по **$9**, с одним важным отличием: все, что вы вводите в командной строке после имени макроса, заменяется на **$ @ no__t-8**\* в макросе. |


- Запуск макроса **Doskey**

  Чтобы запустить макрос, введите имя макроса в командной строке, начиная с первой позиции. Если макрос был определен с **$ @ no__t-2*** или любыми пакетными параметрами **$1** – **$9**, используйте пробел для разделения параметров. Макросы **Doskey** нельзя запускать из пакетной программы.
...
```
В предыдущем примере макрос TX изменяет текущий каталог на temp, а затем отображает список каталогов в расширенном формате отображения. Можно использовать **$ @ no__t-2*** в конце макроса, чтобы добавить другие параметры командной строки для **dir** при запуске TX.
