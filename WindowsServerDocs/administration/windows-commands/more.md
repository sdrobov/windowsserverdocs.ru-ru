...
|           Параметр            |                               Описание                               |
|--------------------------------|-------------------------------------------------------------------------|
|           @no__t 0Command >           |      Указывает команду, для которой необходимо отобразить выходные данные.      |
...
|             /t @ no__t-0N >             |         Отображает вкладки как количество пробелов, заданное параметром *N*.         |
|             + @ NO__T-1N >              |     Отображает первый файл, начиная с строки, заданной параметром *N*.     |
| [@no__t 0Drive >:] [\<Path >] @no__t 2FileName > |          Указывает расположение и имя файла для вывода.          |
|            @no__t 0Files >            | Указывает список отображаемых файлов. Разделяйте имена файлов пробелами. |
...
-   Следующие подкоманды принимаются **в приглашении** (`-- More --`). 
    ...
    | p @no__t 0N > | Отображает следующие *N* строк. |
    | s @no__t — 0N > |Кипс следующие *N* строк. |
...
Следующие примеры можно использовать **в запросе (** `-- More --`).
