...
|Параметр|Описание|
|-------|--------|
|параметра|Предотвращает попытки **pathping** разрешить IP-адреса промежуточных маршрутизаторов к их именам. Это может ускорить отображение результатов **pathping** .|
...
|/g @no__t 0Hostlist >|Указывает, что сообщения эхо-запроса используют параметр свободного исходного маршрута в заголовке IP с набором промежуточных назначений, указанных в *hostlist*. При свободной маршрутизации последовательные промежуточные назначения могут быть разделены одним или несколькими маршрутизаторами. Максимальное число адресов или имен в списке узлов равно 9. *Hostlist* — это серия IP-адресов (в точечно-десятичной нотации), разделенных пробелами.|
|/p \<Period >|Указывает время ожидания между последовательными проверками связи (в миллисекундах). Значение по умолчанию — 250 миллисекунд (1/4 секунд).|
|/q @no__t — 0NumQueries >|Указывает число сообщений эхо-запросов, отправляемых каждому маршрутизатору в пути. Значение по умолчанию — 100 запросов.|
|/w @no__t — 0timeout >|Указывает количество миллисекунд ожидания для каждого ответа. Значение по умолчанию — 3000 миллисекунд (3 секунды).|
|/i @no__t — 0IPaddress >|Указывает исходный адрес.|
|/4 \<IPv4 >|Указывает, что pathping использует только IPv4.|
|/6 \<IPv6 >|Указывает, что pathping использует только IPv6.|
|@no__t 0TargetName >|Указывает назначение, которое определяется по IP-адресу или имени узла.|
...
В следующем примере показан вывод команды **pathping** :
