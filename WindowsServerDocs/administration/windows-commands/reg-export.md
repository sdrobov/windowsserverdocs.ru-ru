...
|Параметр|Описание|
|---------|-----------|
|@no__t 0KeyName >|Указывает полный путь к подразделу. Операция экспорта работает только с локальным компьютером. KeyName должен содержать допустимый корневой ключ. Допустимые корневые ключи: HKLM, HKCU, HKCR, HKU и ХККК.|
