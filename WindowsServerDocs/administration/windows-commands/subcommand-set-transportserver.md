...
description: Справочный раздел по подкоманде Set-Транспортсервер, который задает параметры конфигурации для транспортного сервера.
...
, которая включает команду[Start-транспортсервер](subcommand-start-transportserver.md)
