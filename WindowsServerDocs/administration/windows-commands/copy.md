---
title: копирование
description: Раздел команд Windows для копирования, при котором один или несколько файлов копируются из одного расположения в другое.
...
|> источника \<|Обязательное. Указывает расположение, из которого необходимо скопировать файл или набор файлов. *Источник* может состоять из буквы диска и двоеточия, имени каталога, имени файла или их сочетания.|
|> назначения \<|Обязательное. Указывает расположение, в которое необходимо скопировать файл или набор файлов. *Назначение* может состоять из буквы диска и двоеточия, имени каталога, имени файла или их сочетания.|
...
-   Использование **/a**
    ...
    Когда параметр **/a** предшествует или следует за списком файлов в командной строке, он применяется ко всем файлам, перечисленным до тех **/b**пор, пока не встретится **копия** . В этом случае **/b** применяется к файлу, предшествующему **/b**.
...
```
Если требуется объединить несколько двоичных файлов в один, используйте подстановочные знаки, включите **/b**. Это предотвращает расинтерпретацию CTRL + Z в качестве символа конца файла в Windows. Введите например
```
