---
title: auditpol Get
description: Справочный раздел по команде auditpol Get, который получает системные политики, политику для отдельных пользователей, параметры аудита и аудитный объект дескриптора безопасности.
...
> [!NOTE]
> Эта команда полезна в двух сценариях. 1) при мониторинге определенной учетной записи пользователя на наличие подозрительных действий можно `/get` использовать команду для получения результатов в определенные категории с помощью политики включения для включения дополнительного аудита. 2) Если в параметрах аудита учетной записи регистрируются многочисленные, но избыточные события, `/get` можно использовать команду, чтобы отфильтровать внешние события для этой учетной записи с помощью политики исключения. Для вывода списка всех категорий используйте `auditpol /list /category` команду.
