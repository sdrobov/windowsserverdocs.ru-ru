---
title: set
description: Справочный раздел по набору, который отображает, устанавливает или удаляет CMD. EXE переменные среды.
...
|\<Промптстринг>|Необязательный элемент. Указывает сообщение, предлагающее пользователю ввести данные. Этот параметр используется с параметром командной строки **/p** .|
