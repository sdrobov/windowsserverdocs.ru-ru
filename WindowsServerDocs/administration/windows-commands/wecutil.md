---
title: wecutil
description: Раздел команд Windows для wecutil, который позволяет создавать и администрировать подписки на события, пересылаемые с удаленных компьютеров.
...
|{SS \| Set-Subscription} \<subid > [/e: [\<подактивированные >]] [/ЕСА:\<адрес >] [/ЕСЕ: [\<Срценаблед >]] [/АЕС] [/RES] [/Ун:\<username >] [/Up:\<пароль >] [/d:\<DESC >] [/Ури:\<URI >] [/cm:\<Конфигмоде >] [/ЕКС:\<срок действия >] [/q:\<query >] [/Диа:\<диалект >] [/TN:\<транспорта >] [/TP:\<Транспортпорт >] [/дм:\<Деливеримоде >] [/ДМИ:\<Deliverymax >] [/dmlt:\<Deliverytime >] [/HI:\<пульс >] [/CF:\<Content >] [/l :\<locale >] [/ри: [\<Реадексист >]] [/ЛФ:\<файл_журнала >] [/Пн:\<PublisherName >] [/ессп:\<Енаблепорт >] [/ХН:\<hostname >] [/КТ:\<тип >]</br>или</br>{SS \| Set-Subscription/c:\<ConfigFile > [/кун:\<Комусернаме >/КУП:\<Компассворд >]|Изменяет конфигурацию подписки. Можно указать идентификатор подписки и соответствующие параметры для изменения параметров подписки. также можно указать XML-файл конфигурации для изменения параметров подписки.|
