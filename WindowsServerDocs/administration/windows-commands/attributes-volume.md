...
-   В основной основной загрузочной записи \(диски MBR\), параметры **Hidden**, **ReadOnly**и **нодефаултдривелеттер** применяются ко всем томам на диске.  
  
-   В основной таблице разделов GUID \(GPT\) диски, а также на динамических дисках MBR и GPT параметры **Hidden**, **ReadOnly**и **нодефаултдривелеттер** применяются только к выбранному тому.  
...
Чтобы удалить атрибуты "скрытый" и "только чтение\-" на выбранном томе, введите:  
