...
|@no__t 0filename >|Указывает полный путь к файлу, включая имя файла и расширение, например К:\документс\филенаме.ткст.|
|@no__t 0length >|Указывает допустимую длину данных файла.|
|финдбисид|Поиск файлов, принадлежащих указанному пользователю на томах NTFS с включенными квотами на диск.|
|@no__t 0username >|Указывает имя пользователя или имя для входа.|
|@no__t 0directory >|Указывает полный путь к каталогу, например папку c:\Users.|
|оптимиземетадата|Это делает немедленную операцию сжатия метаданных для заданного файла.|
|/|Анализ метаданных файла до и после оптимизации.|
...
|Length = \<length >|Задает длину диапазона (в байтах).|
|куерекстентс|Запрашивает экстенты для файла.|
|/R|Если <filename> является точкой повторного анализа, откройте ее, а не цель.|
|@no__t 0startingvcn >|Указывает первый VCN для запроса. Если не указано, начните с VCN 0.|
|@no__t 0numvcns >|Число Вкнс для запроса. Если параметр опущен или равен 0, то запрос будет выполняться до EOF.|
|куерифилеид|Запрашивает идентификатор файла на томе NTFS.<br /><br />Этот параметр применяется к:  Windows Server 2008 R2 и Windows 7.|
|@no__t 0volume >|Указывает том как имя диска, за которым следует двоеточие.|
|куерифиленамебид|Отображает произвольное имя ссылки для указанного идентификатора файла на томе NTFS. Так как файл может иметь несколько имен ссылок, указывающих на этот файл, не гарантируется, какая ссылка на файл будет предоставлена в результате запроса на имя файла.<br /><br />Этот параметр применяется к:  Windows Server 2008 R2 и Windows 7.|
|@no__t 0fileid >|Указывает идентификатор файла на томе NTFS.|
...
|@no__t 0shortname >|Указывает короткое имя файла.|
|сетвалиддата|Задает допустимую длину данных для файла на томе NTFS.|
|@no__t 0datalength >|Указывает длину файла в байтах.|
