...
|m = @no__t 0MaxSize >|Указывает максимальный размер (в байтах), выделяемый файловой системой NTFS для журнала изменений.|
...
|c = \<chunk-размер >|Указывает размер фрагмента для записи в томе.|
|s = пороговое значение \<file-size >|Указывает пороговое значение размера файла для отслеживания диапазона.|
|енумдата|Перечисляет и перечисляет записи журнала изменений между двумя указанными границами.|
|@no__t 0FileRef >|Задает порядковый номер в файлах на томе, с которого начинается перечисление.|
