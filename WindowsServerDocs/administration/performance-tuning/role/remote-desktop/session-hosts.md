...
Конфигурация памяти зависит от приложений, которые применяются пользователями; Однако требуемый объем памяти можно оценить с помощью следующей формулы: Тоталмем = Осмем + Сессионмем @no__t — 0 НС
...
Сжатие Microsoft RemoteFX можно настроить с помощью групповая политика в разделе **Конфигурация компьютера &gt; административные шаблоны &gt; компонентов Windows &gt; службы удаленных рабочих столов &gt; удаленный рабочий стол узле сеансов &gt; Remote. Среда сеанса &gt; Настройка сжатия данных RemoteFX**. Возможны три значения:
...
Также можно выбрать не использовать алгоритм сжатия RemoteFX. Выбор не использовать алгоритм сжатия RemoteFX будет использовать больше пропускной способности сети, поэтому рекомендуется только при использовании аппаратного устройства, предназначенного для оптимизации сетевого трафика. Даже если вы решили не использовать алгоритм сжатия RemoteFX, некоторые графические данные будут сжиматься.

### <a name="device-redirection"></a>Перенаправление устройств

Перенаправление устройств можно настроить с помощью групповая политика в разделе **Конфигурация компьютера &gt; административные шаблоны &gt; компонентов Windows &gt; службы удаленных рабочих столов &gt; удаленный рабочий стол узле сеансов &gt; устройства и ресурса. Перенаправление** или с помощью поля свойства **коллекции сеансов** в Диспетчер сервера.
...
-   **Анимация меню и окон** (отключить меню анимс: i: 1 и отключить параметр курсора: i: 1): При отключении этих параметров снижается пропускная способность за счет отключения анимации меню (например, плавности) и курсоров.
