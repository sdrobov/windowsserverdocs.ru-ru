...
-   Использование логического процессора-\\Hyper-V логический процессор гипервизора (\*) \\% общего времени выполнения

-   Использование виртуальных процессоров — \\Hyper-V. виртуальный процессор гипервизора (\*) \\% общего времени выполнения

-   Использование корневого виртуального процессора — \\Hyper-V корневой виртуальный процессор гипервизора (\*) \\% общего времени выполнения

Если **логический процессор гипервизора Hyper-V (\_Total) \\% счетчика общего времени выполнения** превышает 90%, узел перегружен. Необходимо добавить дополнительные вычислительные мощности или переместить некоторые виртуальные машины на другой узел.

Если **виртуальный процессор гипервизора Hyper-V (имя виртуальной машины: вице-президент x) @no__t – 1% общего счетчика времени выполнения** превышает 90% для всех виртуальных процессоров, необходимо выполнить следующие действия.
...
Если **виртуальный процессор гипервизора Hyper-V (имя виртуальной машины: вице-президент x) @no__t – 1% общего счетчика времени выполнения** превышает 90% для некоторых, но не всех виртуальных процессоров, необходимо выполнить следующие действия.
...
Если **корневой виртуальный процессор гипервизора Hyper-V (root президент x) \\% общий счетчик времени выполнения** превышает 90% для некоторых, но не всех виртуальных процессоров и **процессор (x) \\% времени прерываний и процессора (x) \\% СЧЕТЧИКа времени DPC** приблизительно добавляется к значению для **корневого виртуального процессора (корневой вице-президент x) @no__t – 6% всего счетчика времени выполнения** , необходимо обеспечить включение VMQ на сетевых адаптерах.
