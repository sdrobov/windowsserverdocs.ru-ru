...
> [!NOTE]
> Основное внимание уделяется чтению базы данных, так как это, как правило, самый ресурсоемкий компонент, та же логика может применяться для записи в файл журнала путем замены LogicalDisk ( *\<журнала ntds\>* ) \авг дискового пространства (с *\<журнала NTDS\>* ) \ операций записи в секунду):
>  
...
>   - Если логический диск ( *\<ntds\>* ) \Авг для чтения с диска (сек.) находится в оптимальном диапазоне для серверного хранилища, LogicalDisk ( *\<NTDS\>* ) \ операции чтения/с можно использовать непосредственно для изменения размера хранилища.
>   - Если логический диск ( *\<NTDS\>* ) \Авг. чтение с диска (сек.) не находится в оптимальном диапазоне для серверного хранилища, требуется дополнительный ввод-вывод в соответствии со следующей формулой:
>     > (Логический диск ( *\<NTDS\>* ) \Авг время чтения с диска (сек)) &divide; (доступ к диску физического носителя) &times; ( *\<NTDS\>* ) \Авг время чтения с диска (с)
...
- Чтобы определить объем операций ввода-вывода, необходимый для хранилища, в котором превышена емкость базового хранилища:
  >*Необходимые операции ввода-вывода* : (логический диск *базы данных NTDS\>* ) \Авг время чтения с диска\<(сек.) &divide; *\<целевое среднее время чтения с диска (сек* .)\>) &times; LOGICALDISK ( *\<диск базы данных NTDS\>* ) \ "чтение/с"
...
> *New_MaxConcurrentApi_setting* &ge; (*semaphore_acquires* + *semaphore_time-аутов*) &times; *average_semaphore_hold_time* &divide; *time_collection_length*
...
|Категория|Счетчик производительности|Интервал и выборка|Target|Предупреждение|
|-|-|-|-|-|
|Процессор|Сведения о процессоре (_Total)\\% Processor Utility|60 мин|40 %|60 %|
...
|Хранилище|Логический диск ( *\<диск базы\>данных NTDS* ). \авг диска (сек)/чтение<p>Логический диск ( *\<диск базы\>данных NTDS* ). \авг диска (сек)/запись|60 мин|10 мс|15 мс|
...
1. Чтобы определить модификатор, используйте следующее уравнение:
   >((*Значение оценки "Целевая платформа для каждого ядра*") &times; (число*МГц на ядро базовой платформы*)) &divide; ((*значение оценки по базовому*показателю ядра) &times; (*МГц на ядро целевой платформы*))  

    Используя приведенный выше пример:
   >(35,83 &times; 2000) &divide; (33,75 &times; 2300) = 0,92
1. Умножьте предполагаемое количество процессоров на модификатор.  В приведенном выше примере, чтобы переключиться с процессора с частотой до 2650 процессоров 2630, умножьте вычисленный 11,25 процессоров &times; 0,92 = 10,35.
...
- **Шина PCI —** Это часто наподобный компонент. В этом примере это не будет узким местом. Однако при увеличении масштаба системы это может стать узким местом. Для справки 32-разрядная шина PCI, работающая на 33Mhz, может передавать данные 133 МБ/с. Ниже приведено уравнение.  
  ...
  64 66-битная шина PCI поддерживает теоретический максимум (64 бит &divide; 8 бит на байт &times; 66 МГц) = 528 МБ/с. Кроме того, любое другое устройство (например, сетевой адаптер, второй контроллер SCSI и т. д.) сокращает пропускную способность, доступную по мере предоставления общего доступа к пропускной способности, а устройства будут конкурировать за ограниченные ресурсы.
...
|Примечания|Анализ узкого места|Дисковое пространство|Bus|Адаптер|Шина PCI|
|-|-|-|-|-|-|
...
> *Чтение ввода-вывода* + 2 &times; *запись ввода-вывода* = *общий доступный дисковый ввод-вывод*  
...
> *Максимальное число операций ввода-вывода в секунду на* диск &times; 2 вращения &times; [( *% операций чтения* +  *% операций записи*) &divide; ( *% операций чтения* + 2 &times; *% операций записи*)] = *всего операций ввода-вывода в секунду*
...
> *Максимальное число операций ввода-вывода в секунду на* диск &times; 2 шпинделя &times; [( *% операций чтения* +  *% операций записи*) &divide; ( *% операций чтения* + 2 &times; *% операций записи*)] = *общий ввод/вывод*  

в наборе RAID 1, если кратность (*N*) наборов RAID 1 является полосковой, общая операция ввода-вывода, которую можно обработать, преобразуется в N &times; ввода-вывода на каждый набор RAID 1:  

> *N* &times; {*максимум операций ввода-вывода в секунду для каждого шпинделя* &times; 2 шпинделя &times; [( *% операций чтения* +  *% операций записи*) &divide; ( *% READS* + 2 &times; *% операций записи*)]} = *всего операций ввода-вывода*
...
> *Чтение ввода-вывода* + 4 &times; *запись ввода-* вывода = *всего операций ввода-вывода*  
...
> *Операций ввода-вывода на* диск &times; (*шпиндели* – 1) &times; [( *% операций чтения* +  *% операций записи*) &divide; ( *% операций чтения* + 4 &times; *% операций записи*)] = *всего операций ввода-вывода в секунду*
...
Наконец, адаптер HBA, установленный на сервере, также будет иметь ограничение на количество операций ввода-вывода, которое может быть обработано. Как правило, для обеспечения избыточности устанавливается второй HBA, но, как и при использовании коммутатора SAN, при вычислении максимального количества операций ввода-вывода, которые можно обработать, Общая пропускная способность *N* &ndash; 1 HBA — это максимальная масштабируемость системы.
...
- **Когда скорость будет запрогревить кэш —** Предполагая, что нагрузка клиента будет максимально увеличить пропускную способность в этом примере хранилища, кэш будет тепло с частотой 2400 операций ввода-вывода в секунду &times; 8 КБ на операцию ввода-вывода. Или примерно 20 МБ/с в секунду, Загрузка примерно 1 ГБ базы данных в ОЗУ каждые 53 секунд.
