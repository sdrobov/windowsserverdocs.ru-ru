...
Дополнительные сведения о горизонтальном масштабировании SMB см. в разделе [Масштабируемый файловый сервер (Общие сведения о данных приложений](https://technet.microsoft.com/library/hh831349.aspx) ) и в записи блога [для масштабирования, а не для масштабирования. это вопрос](http://blogs.technet.com/b/filecab/archive/2013/12/05/to-scale-out-or-not-to-scale-out-that-is-the-question.aspx).
...
-   **Связи счетчиков производительности "физический диск", "SMB" и "CSV FS"**
    ...
    Дополнительные сведения о том, как связаны счетчики физического диска, SMB и CSV (файловая система), см. в следующей записи блога: [Общий том кластера счетчики производительности](http://blogs.msdn.com/b/clustering/archive/2014/06/05/10531462.aspx).

## <a name="tuning-parameters-for-smb-file-servers"></a>Параметры настройки для файловых серверов SMB


Следующие параметры реестра REG @ no__t-0DWORD могут повлиять на производительность файловых серверов SMB.
...
- **Smb2CreditsMin** и **Smb2CreditsMax**
  ...
  > 
  > Вы можете отслеживать общие ресурсы SMB Client @ no__t-0Credit в секунду, чтобы узнать, есть ли какие-либо проблемы с кредитовыми данными.
...
- **аддитионалкритикалворкерсреадс**
    ...
    >[!TIP]
    > Значение может потребоваться увеличиться, если объем неизмененных данных диспетчера кэша (кэш счетчиков производительности @ no__t – 0Dirty страниц) увеличивается для использования большой части (свыше 25%). памяти или, если система выполняет множество синхронных операций ввода-вывода.
...
- **макссреадсперкуеуе**
  ...
  >[!TIP]
  > Указывает, что значение может потребоваться увеличиться, если рабочие очереди SMB2 увеличиваются очень большими (счетчик производительности "рабочие очереди сервера @ no__t-0Queue length @ no__t-1SMB2 неблокировка \*" постоянно превышает ~ 100).
...
Дополнительные сведения о счетчиках клиента SMB см. в разделе [Windows Server 2012 файловый сервер: Новые счетчики производительности для каждого общего ресурса SMB обеспечивают превосходную аналитику @ no__t-0.
