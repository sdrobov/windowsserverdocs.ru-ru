...
Функции по запросу — это функция, появившаяся в Windows 8 и Windows Server 2012, которая позволяет удалять файлы ролей и компонентов (иногда называемые *полезной*нагрузкой компонентов) из операционной системы для экономии места на диске, а также устанавливать роли и компоненты из Удаленные расположения или установочный носитель, а не локальные компьютеры. Можно удалять файлы компонентов с работающих физических или виртуальных компьютеров. Также можно добавлять или удалять файлы компонентов из файлов образа Windows (WIM) или автономных виртуальных жестких дисков, чтобы создать воспроизводимую копию конфигураций функции "Компоненты по требованию".

В конфигурации компонентов по требованию, когда файлы компонентов недоступны на компьютере, если для установки требуются эти файлы, Windows Server 2012 R2 или Windows Server 2012 можно направлять для получения файлов из параллельного хранилища компонентов (общей папки). , который содержит файлы компонентов и доступен для компьютера в сети, с Центр обновления Windows или с установочного носителя. По умолчанию, если файлы компонентов на целевом сервере недоступны, то для поиска отсутствующих файлов компонентов функция "Компоненты по требованию" выполняет следующие задачи в указанном ниже порядке:
...
## <a name="BKMK_store"></a>Создание файла компонентов или параллельного хранилища
...
2.  Проверьте, что хранилищу компонентов назначены правильные разрешения. Исходный путь или общая папка должны предоставлять разрешения на **Чтение** группе " **все** " (не рекомендуется по соображениям безопасности) или учетным записям компьютеров (*домен*\\*SERverNAME*$) серверов, на которые планируется установить функции с использованием этого хранилища возможностей; предоставление доступа к учетной записи пользователя недостаточно.
...
-   Параметр `remove` командлета `Uninstall-WindowsFeature` позволяет удалять файлы компонентов с сервера или с неподключенного виртуального жесткого диска (VHD), работающего под управлением Windows Server 2012 R2 или Windows Server 2012. Допустимые значения параметра `remove` — это имена ролей, служб ролей и компонентов.
...
## <a name="BKMK_remove"></a>Удаление файлов компонентов с помощью uninstall-WindowsFeature
...
2.  Введите следующую команду и нажмите клавишу **ВВОД**:
    ...
    **Например** Удаленный рабочий стол "Лицензирование" — Последняя остающаяся служба роли службы удаленных рабочих столов, которая установлена. Команда удаляет лицензирование удаленных рабочих столов, а затем удаляет файлы компонентов всей роли служб удаленных рабочих столов с указанного сервера *contoso_1*.

    ```
    Uninstall-WindowsFeature -Name rdS-Licensing -computerName contoso_1 -remove
    ```

    **Например** В следующем примере команда удаляет доменные службы Active Directory и групповая политика управления с автономного диска VHD. Сначала удаляются роль и компонент, затем файлы их компонентов окончательно удаляются с автономного виртуального жесткого диска, *Contoso.vhd*.
...
[включение и отключение компонентов Windows](https://technet.microsoft.com/library/hh824822.aspx)
[Общие сведения об обслуживании образов развертывания и управлении ими (DISM)](https://technet.microsoft.com/library/hh825236.aspx)
