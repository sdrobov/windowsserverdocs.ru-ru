...
2.  Выполните действия в отображающихся запросах и перезагрузите конечный компьютер. Если конечный компьютер находится в домене, отличном от домена компьютера-сборщика, может потребоваться определить их качестве доверенных узлов. Для этого выполните следующее действие.  
...
-   Имя целевого компьютера. В качестве имени компьютера можно использовать IP-адрес, имя узла или любое другое имя.  
...
3.  Скопируйте этот пример конфигурации в окне программы "Блокнот":  
    ...
    >   
    > В этом примере файла на двух целевых компьютерах определяется элемент \<Target >. Каждое определение указывает IP-адрес с \<> IPv4, но для определения целевого компьютера можно также использовать MAC-адрес (например, `<mac value=11:22:33:44:55:66/>` или `<mac value=11-22-33-44-55-66/>`) или GUID SMBIOS (например, `<guid value={269076F9-4B77-46E1-B03B-CA5003775B88}/>`). Также запишите ключ шифрования (тот же, который был указан или сформирован с помощью файлы Bcdedit на конечном компьютере) и имя компьютера.  
...
5.  Обновите компьютер сборщика, чтобы получить событие, отправленное компьютером Nano Server, добавив диапазон IPv4-адресов, конкретный адрес IPv4 или MAC для сервера Nano Server в файл Active. XML на компьютере сборщика (см. раздел Настройка компьютера сборщика этой статьи).  
...
Минимальный уровень ведения журнала можно задать в элементе сборщика \<> файла конфигурации. Например: < сборщик Конфигверсионмажор = 1 минлог\=подробных >.  
...
< сборщик Конфигверсионмажор = 1 минлог = журнал отладки\=К:\програмдата\микрософт\бутевентколлектор\логс\лог.ткст >  
...
5. На стороне конечного компьютера KDNET записывает некоторые диагностические сведения в реестр. Искать в:   
