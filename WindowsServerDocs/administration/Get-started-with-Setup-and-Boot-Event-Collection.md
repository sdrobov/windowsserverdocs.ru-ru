...
3.  На компьютере-сборщике выполните одну из следующих команд:  
    ...
    -   В командной строке Windows PowerShell: `Set-Item -Force WSMan:\localhost\Client\TrustedHosts "<target1>,<target2>,..."`, за которым следует `Set-Item -Force WSMan:\localhost\Client\AllowUnencrypted true` где \<Назначение1 > т. д. — это имена или IP-адресов на конечных компьютерах.  
  
    -   Или в командной строке: **winrm установить клиент winrm/config / @{TrustedHosts =»\<Назначение1 >,\<target2 >,...»; AllowUnencrypted = «true»}**  
...
4.  Проверьте удаленное подключение, перейдя на компьютер-сборщик и выполнив одну из следующих команд Windows PowerShell:  
    ...
    Если в том же домене, что на компьютере сборщика конечного компьютера, выполните команду `New-PSSession -Computer <target> | Remove-PSSession`  
...
1.  Запустите командную строку с повышенными привилегиями, а затем выполните следующие команды:  
    ...
    **bcdedit /eventsettings net key:a.b.c.d порт: 50000 hostip:1.2.3.4**  
...
**Теперь, когда сам транспорт событий включена, необходимо включить систему для фактической отправки событий трассировки событий Windows с помощью указанного транспорта.**  
...
2.  В открывшемся меню выберите **свойства**, а затем нажмите кнопку **сведения** вкладки. Разверните меню в **свойство** поле, прокрутите вниз до найти **сведения о расположении** (список, скорее всего, не является в алфавитном порядке) и затем щелкните его. Значение будет Строка вида **шины PCI X, Y устройства, функция Z**. Запишите X.Y.Z; Ниже приведены параметры шины, необходимое для следующей команды.  
  
3.  Выполните одну из следующих команд:  
  
    Из строки с повышенными привилегиями Windows PowerShell: `Enable-SbecBcd -ComputerName <target_name> -CollectorIP <ip> -CollectorPort <port> -Key <a.b.c.d> -BusParams <X.Y.Z>`  
...
-   Порт = \<номер порта, указанный для сборщика использовать >  
...
При включении службы создаются и хранятся в трех файлах конфигурации XML **c:\ProgramData\Microsoft\BootEventCollector\Config**:  
...
Один из решений, которое вам необходимо принять — это задать ограничение на размер файла. Наиболее подходящее ограничение на размер файла зависит от ожидаемого объема событий и доступного места на диске. Файлы меньшего размера являются более удобными с точки зрения очистки старых данных. Тем не менее каждый файл несет с собой дополнительную нагрузку, связанную с заголовком 64 КБ и чтение много файлов для получения объединенного журнала может быть неудобно. Абсолютное минимальное предельный размер составляет 256 КБ. Целесообразное ограничение на размер файла с точки зрения практичности должно составлять около 1 МБ, а 10 МБ, вероятно, будут хорошим стандартным значением. Более жесткое ограничение может быть целесообразным, если ожидается большое количество событий.  
...
3.  Скопируйте этот пример конфигурации в окне программы "Блокнот":  
    ...
    > [!NOTE]  
    > Корневым узлом является \<сборщика >. Его атрибуты задают версию синтаксиса файла конфигурации и имя файла журнала состояний.  
    >   
    > \<Общие > элемент группирует несколько целевых объектов, указание Общие элементы конфигурации для их, сильно, как группы пользователей можно указать разрешения, общие для нескольких пользователей.  
    >   
    > \<Collectorport > элемент определяет номер порта UDP, где сборщик данных будет прослушивать входящие данные. Это тот же порт, который был указан на этапе настройки конфигурации конечного компьютера для Bcdedit. Сборщик поддерживает только один порт и все остальные конечные компьютеры должны подключаться к тому же порту.  
    >   
    > \<Сервера пересылки > элемент указывает пересылкой событий трассировки событий Windows, полученных из конечных компьютеров. Существует только один тип сервера пересылки, который записывает события в ETL-файлы. Параметры определяют шаблон имени файла, предельный размер каждого файла в цикле и размер цикла для каждого компьютера. Параметр "toxml" определяет, что события трассировки событий Windows будут записываться в двоичном формате как и при их получении, то есть без преобразования в формат XML. В разделе "Преобразование событий XML" см. инструкции о том, в каких случаях следует преобразовывать события в формат XML. Шаблон имени файла содержит следующие подстановки: {computer} для имени компьютера и {#3} для индекса файла в цикле.  
    >   
    > В этом примере файла определяются два целевых компьютерах с помощью \<целевой > элемента. Каждое определение указывает IP-адрес с \<ipv4 >, но можно также использовать MAC-адрес (к примеру, < mac значение = «11:22:33:44:55:66»\/> или < mac value="11-22-33-44-55-66»\/>) или SMBIOS GUID (для Пример, < значение идентификатора guid = «{269076F9-4B77-46E1-B03B-CA5003775B88}»\/>) для определения конечного компьютера. Также запишите ключ шифрования (тот же, который был указан или сформирован с помощью файлы Bcdedit на конечном компьютере) и имя компьютера.  
  
4.  Введите сведения для каждого целевого компьютера, как отдельные \<целевой > элемент в файле конфигурации, а затем сохраните Newconfig.xml и закройте Блокнот.  
...
|Dism.exe|0x800f080c|Функция \<имя > неизвестно.||То же самое, что и выше|  
...
Минимальный журнал установить уровень в \<сборщика > в файле конфигурации. Например: < сборщика configVersionMajor = «1» minlog\=«verbose» >.  
...
На уровне отладки может быть полезно записывать журнал в файл вместо того, чтобы просматривать его в обычной системе ведения журналов. Чтобы сделать это, добавьте дополнительный элемент в \<сборщика > в файле конфигурации:  
...
2. Чтобы увидеть расширенной диагностики от сборщика, добавьте ее, \<сборщика > в файле конфигурации:  
   \<collector ... minlog="verbose">  
   Это позволит получать сообщения о каждом полученном пакете.  
3. Проверьте, происходит ли получение каких-либо пакетов в принципе. Кроме того, можно записать журнал в режиме подробного протоколирования непосредственно в файл, а не через трассировку событий Windows. Чтобы сделать это, добавьте ее, \<сборщика > в файле конфигурации:  
...
|Сборщик||Не создаются ETL-файлы.|`Get-SbecForwarding` Показывает, что целевой объект подключения без ошибок, но ETL-файлы не создаются.|Вероятно, конечный компьютер еще не отправил никакие данные; ETL-файлы создаются только при получении данных.|  
