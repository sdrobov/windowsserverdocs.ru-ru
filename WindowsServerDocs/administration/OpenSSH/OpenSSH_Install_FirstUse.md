...
Чтобы удалить OpenSSH через раздел "Параметры" в ОС Windows, откройте этот раздел и последовательно выберите "Приложения" > "Приложения и компоненты" > "Управление дополнительными возможностями". В списке установленных компонентов выберите компонент "Клиент OpenSSH" или "Сервер OpenSSH" и щелкните "Удалить".
