...
ms.locfileid: "71365690"
---
# <a name="remote-direct-memory-access-rdma-and-switch-embedded-teaming-set"></a>Удаленный прямой доступ к памяти \(RDMA @ no__t-1 и объединение внедренных команд \(SET @ no__t-3

>Область применения. Windows Server (Semi-Annual Channel), Windows Server 2016

В этом разделе приводятся сведения о настройке удаленного доступа к памяти \(RDMA @ no__t-1 с Hyper-V в Windows Server 2016, а также сведения о коммутаторе внедренных команд \(SET @ no__t-3.  

> [!NOTE]
> В дополнение к этому разделу доступны следующие сведения о включенном объединении для объединения коммутаторов. 
> - Загрузка из коллекции TechNet: [Windows Server 2016 сетевая карта и обзор коммутаторов для объединения внедренных](https://gallery.technet.microsoft.com/Windows-Server-2016-839cb607?redir=0)

## <a name="bkmk_rdma"></a>Настройка интерфейсов RDMA с помощью Hyper-V  
...
В следующих разделах приведены инструкции по использованию команд Windows PowerShell для включения моста центров обработки данных (DCB), создания виртуального коммутатора Hyper-V с виртуальным сетевым адаптером RDMA \(vNIC @ no__t-1 и создания виртуального коммутатора Hyper-V с SET и RDMA vNIC.

### <a name="enable-data-center-bridging-dcb"></a>Включение моста центра обработки данных \(DCB @ no__t-1

Перед использованием любого RDMA для роце \(\) версии RDMA для Ethernet необходимо включить DCB.  Хотя это и не требуется для протокола RDMA в Интернете \(iWARP @ no__t-1, тестирование определило, что все технологии RDMA на основе Ethernet работают лучше с DCB. По этой причине следует использовать DCB даже для развертываний RDMA iWARP.
...
###  <a name="bkmk_set-rdma"></a>Создание виртуального коммутатора Hyper-V с помощью SET и RDMA vNIC

Чтобы использовать RDMA модуля на виртуальных сетевых адаптерах узла Hyper-V \(vNICs @ no__t-1 на виртуальном коммутаторе Hyper-V, поддерживающем объединение RDMA, можно использовать приведенные ниже примеры команд Windows PowerShell.
...
## <a name="bkmk_over"></a>Настройка обзора

— Это альтернативное решение для объединения сетевых карт, которое можно использовать в средах, включающих Hyper-V и программно определенный сетевой \(SDN @ no__t-1 в Windows Server 2016. Установите некоторые функции объединения сетевых карт в виртуальный коммутатор Hyper-V.
...
## <a name="bkmk_avail"></a>ЗАДАТЬ доступность
...
## <a name="bkmk_nics"></a>Поддерживаемые сетевые адаптеры для SET

Вы можете использовать любой сетевой адаптер Ethernet, который прошел проверку оборудования и логотипа Windows \(WHQL @ no__t-1 в команде SET в Windows Server 2016. Для SET необходимо, чтобы все сетевые адаптеры, входящие в состав команды SET, совпадали \(i. e. тот же производитель, тот же самый и тот же, но и тот же самый встроенный микрокод и драйвер @ no__t-1. Параметр SET поддерживает от одного до восьми сетевых адаптеров в команде.
  
## <a name="bkmk_compat"></a>Настройка совместимости с сетевыми технологиями Windows Server

НАБОР совместим со следующими сетевыми технологиями в Windows Server 2016.

- Мост центра обработки данных \(DCB @ no__t-1
  
- Виртуализация сети Hyper-V-NV-GRE и Вкслан поддерживаются в Windows Server 2016.  
- Разгрузка контрольной суммы на стороне приема \(IPv4, IPv6, TCP @ no__t-1. они поддерживаются, если какой-либо из членов команды SET поддерживает их.

- Удаленный доступ к памяти \(RDMA @ no__t-1

- Виртуализация одиночного корневого ввода-вывода \(SR-IOV @ no__t-1

- Разгрузка контрольной суммы на стороне передачи \(IPv4, IPv6, TCP @ no__t-1 — поддерживаются, если все члены команды SET поддерживают их.

- Очереди виртуальных машин \(VMQ @ no__t-1

- Виртуальное масштабирование на стороне приема \(RSS @ no__t-1

НАБОР несовместим со следующими сетевыми технологиями в Windows Server 2016.

- Проверка подлинности 802.1 x. Протокол расширенной проверки подлинности 802.1 x \(EAP @ no__t-1 автоматически удаляется с помощью виртуального коммутатора Hyper @ no__t-2V в НАБОРе сценариев.
 
- Разгрузка задач IPsec \(IPsecTO @ no__t-1. Это устаревшая технология, которая не поддерживается большинством сетевых адаптеров и где она существует, по умолчанию отключена.

- Использование QoS @no__t -0pacer. exe @ no__t-1 в операционных системах узла или в собственной операционной системе. Эти сценарии качества обслуживания не являются сценариями Hyper @ no__t-0V, поэтому технологии не пересекаются. Кроме того, Служба QoS доступна, но не включена по умолчанию. необходимо намеренно включить QoS.

- Объединение на стороне приема \(RSC @ no__t-1. Функция RSC автоматически отключается с помощью виртуального коммутатора Hyper @ no__t-0V.

- Receive побочное масштабирование \(RSS @ no__t-1. Так как Hyper-V использует очереди для VMQ и ВММК, RSS всегда отключается при создании виртуального коммутатора.

- Разгрузка TCP Chimney. Эта технология отключена по умолчанию.

- QoS на виртуальной машине \(VM-QoS @ no__t-1. Служба QoS на виртуальной машине доступна, но отключена по умолчанию. Если вы настроили службу QoS для виртуальной машины в НАБОРе, параметры качества обслуживания приведут к непредсказуемым результатам.

## <a name="bkmk_modes"></a>Установка режимов и параметров
...
## <a name="bkmk_vmq"></a>НАБОРЫ и очереди виртуальных машин (ВМКС)
...
Большинство сетевых адаптеров имеют очереди, которые можно использовать для масштабирования на стороне приема \(RSS @ no__t-1 или VMQ, но не оба одновременно.
  
Некоторые параметры VMQ выглядят как параметры для очередей RSS, но на самом деле являются параметрами универсальных очередей, используемых как RSS, так и VMQ в зависимости от того, какая функция используется в настоящее время. Каждая сетевая карта имеет в своих дополнительных свойствах значения `*RssBaseProcNumber` и `*MaxRssProcessors`.

Ниже приведены несколько параметров VMQ, обеспечивающих лучшую производительность системы.

- В идеале каждый сетевой адаптер должен иметь значение `*RssBaseProcNumber`, равное значению, больше или равному двум (2). Это происходит потому, что первый физический процессор, Core 0 \(logical процессоров 0 и 1 @ no__t-1, обычно выполняет большую часть системной обработки, поэтому сетевая обработка должна быть направлена с этого физического процессора. 
...
## <a name="bkmk_live"></a>Установка и динамическая миграция

Динамическая миграция поддерживается в Windows Server 2016.

## <a name="bkmk_mac"></a>Использование MAC-адреса в передаваемых пакетах

При настройке команды SET с динамическим распределением нагрузки пакеты из одного источника \(such как одна виртуальная машина @ no__t-1 одновременно распределяются между несколькими членами группы. 
...
При обнаружении сбоя на основном сетевом адаптере программа установки программного обеспечения начинает использовать MAC-адрес виртуальной машины в члене команды, который выбирается в качестве временного привязаны участника команды \(i. e., который теперь будет отображаться в коммутаторе в качестве интерфейса виртуальной машины @ no_ _T-1.
...
## <a name="bkmk_manage"></a>Управление группой набора

Для управления командами SET рекомендуется использовать System Center Virtual Machine Manager \(VMM @ no__t-1, однако можно также использовать Windows PowerShell для управления НАБОРом. В следующих разделах приведены команды Windows PowerShell, которые можно использовать для управления НАБОРом.
...
SET позволяет создать сопоставление между виртуальным интерфейсом \(i. e., портом виртуального коммутатора Hyper-V @ no__t-1 и одним из физических сетевых адаптеров в группе. 

Например, если создать два узла vNIC для SMB @ no__t-0Direct, как описано в разделе [Создание виртуального коммутатора Hyper-V с Set и RDMA vNIC](#bkmk_set-rdma), можно убедиться, что эти две vNIC используют разные члены команды. 
