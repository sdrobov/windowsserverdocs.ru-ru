...
- Область просмотра ограничена только кластеризованными общими папками (без смешивания с некластеризованными общими папками): Когда пользователь просматривает общие папки, указывая путь к кластеризованному файловому серверу, отображение будет включать только общие папки, которые являются частью конкретной роли файлового сервера. Она будет исключать некластеризованные общие папки и использует часть отдельных ролей файлового сервера, которые находятся на узле кластера.

- Перечисление на основе доступа: Перечисление на основе доступа можно использовать для скрытия указанной папки из представления пользователей. Вместо того чтобы разрешить пользователям просматривать папку, но не обращаться к ней, можно запретить им просматривать папку. Перечисление на основе доступа для кластеризованной общей папки можно настроить так же, как и для некластеризованной общей папки.
...
- **Объема** Необходимо использовать общее хранилище, сертифицированное для Windows Server 2016 или Windows Server 2019.
...
- **СЕРВЕРЕ** Для разрешения имен серверы в кластере должны использовать систему доменных имен (DNS). Можно использовать протокол динамического обновления DNS.
...
### <a name="step-2-install-the-file-server-role-and-failover-cluster-feature"></a>Шаг 2. Установка роли файлового сервера и отказоустойчивого кластера
...
1. Откройте **Диспетчер сервера** и в раскрывающемся списке **Управление** выберите **Добавить роли и компоненты**.
   ...
   ![Добавить компонент](media/Cluster-File-Server/Cluster-FS-Add-Feature.png)
...
5. Для роли сервера в списке ролей откройте **Файловые службы**, выберите **файловый сервер**и **Далее**.
   ...
   ![Добавить роль](media/Cluster-File-Server/Cluster-FS-Add-FS-Role-1.png)
...
6. Для функций в списке компонентов выберите **отказоустойчивая кластеризация**.  Всплывающее диалоговое окно содержит список средств администрирования, которые также устанавливаются.  Не заключайте все выбранные **элементы, выберите Добавить компоненты** и **Далее**.
   ...
   ![Добавить компонент](media/Cluster-File-Server/Cluster-FS-Add-WSFC-1.png)

7. На странице Подтверждение нажмите кнопку установить.
...
8. Находясь на странице Сводка, щелкните Просмотреть отчет и прочтите результаты теста. Внесите необходимые изменения в конфигурацию и повторно запустите тесты. <br>Чтобы просмотреть результаты тестов после закрытия мастера, см. раздел *Системрут\клустер\репортс\валидатион Report Date и Time. HTML*.
...
2. Чтобы проверить компьютеры (например, имена компьютеров NODE1 и NODE2) для отказоустойчивой кластеризации, выполните команду:
    ...
    ```
4. Чтобы просмотреть результаты тестов после закрытия мастера, ознакомьтесь с указанным файлом (в Системрут\клустер\репортс @ no__t-0, внесите необходимые изменения в конфигурацию и повторно запустите тесты.
...
8. В окне **точка доступа клиента** введите имя файлового сервера, который будет использоваться.  Обратите внимание, что это не имя кластера.  Это для подключения к общей папке.  Например, если я хочу подключиться к \\SERVER, выводимое имя будет «SERVER».
