...
| Облачный следящий сервер     |  <ul><li>Файл следящего сервера, хранящийся в хранилище BLOB-объектов Azure</li><li> Рекомендуется, если все серверы в кластере имеют надежное подключение к Интернету.</li>      |  См. раздел [развертывание свидетеля облака](https://docs.microsoft.com/windows-server/failover-clustering/deploy-cloud-witness).       |
...
2. Выбрав кластер, в разделе **действия**выберите **Дополнительные действия**, а затем щелкните **Настройка параметров кворума кластера**. Появится мастер настройки кворума кластера. Нажмите кнопку **Далее**.
...
4. Нажмите кнопку **Далее**. Подтвердите выбранные параметры на странице подтверждения, а затем нажмите кнопку **Далее**.

После запуска мастера и появления страницы **Сводка** , если требуется просмотреть отчет о задачах, выполненных мастером, выберите **Просмотреть отчет**. Самый последний отчет останется в папке <em>systemroot</em> **\\Cluster\\Reports** с именем **куорумконфигуратион. MHT**.
...
В следующем примере конфигурация кворума в локальном кластере меняется на конфигурацию с большинством узлов и свидетелем. Ресурс общего файлового ресурса с именем *\\\\Contoso-FS\\FSW* настроен в качестве файлового ресурса-свидетеля.
...
* [Командлеты Windows PowerShell для отказоустойчивых кластеров](https://docs.microsoft.com/powershell/module/failoverclusters/?view=win10-ps)
