...
После запуска мастера и появления страницы **Сводка** , если требуется просмотреть отчет о задачах, выполненных мастером, выберите **Просмотреть отчет**. Самый последний отчет останется в папке <em>systemroot</em> **\\Cluster @ no__t-3Reports** с именем **куорумконфигуратион. MHT**.
...
В следующем примере конфигурация кворума в локальном кластере меняется на конфигурацию с большинством узлов и свидетелем. Ресурс общего файлового ресурса с именем *\\ @ no__t-2CONTOSO-FS @ no__t-3fsw* настраивается в качестве файлового ресурса-свидетеля.
