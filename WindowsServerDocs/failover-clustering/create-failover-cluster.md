...
description: Как создать отказоустойчивый кластер Windows Server 2012 R2, Windows Server 2012 и Windows Server 2016.
...
1. Чтобы на каждом узле отказоустойчивого кластера установить роль или компонент, необходимый для кластерной роли, используйте диспетчер сервера или Windows PowerShell. Например, чтобы создать кластерный файловый сервер, установите роль файлового сервера на всех узлах кластера.
    ...
    <thead>
    <tr class="header">
    <th>Кластерная роль</th>
    <th>Необходимая роль или компонент</th>
    </tr>
    </thead>
    <tbody>
    <tr class="odd">
    <td>Сервер пространства имен DFS</td>
    <td>Пространства имен DFS (часть роли файлового сервера)</td>
    </tr>
    <tr class="even">
    <td>DHCP-сервер</td>
    <td>Роль DHCP-сервера</td>
    </tr>
    <tr class="odd">
    ...
    <tr class="even">
    <td>Брокер реплики Hyper-V</td>
    <td>Роль Hyper-V</td>
    </tr>
    <tr class="odd">
    <td>Целевой сервер iSCSI</td>
    <td>Сервер цели iSCSI (часть роли файлового сервера)</td>
    </tr>
    <tr class="even">
    <td>iSNS-сервер</td>
    <td>Компоненты службы iSNS-сервера</td>
    </tr>
    <tr class="odd">
    <td>служба очередей сообщений</td>
    <td>Компонент службы очереди сообщений</td>
    </tr>
    <tr class="even">
    <td>Другой сервер</td>
    <td>Нет</td>
