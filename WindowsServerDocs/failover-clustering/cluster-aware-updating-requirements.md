...
> [!NOTE]  
> Может потребоваться независимое подтверждение того, что среда кластера готова к применению обновлений, если используется подключаемый модуль @ no__t-0in, отличный от **Microsoft. виндовсупдатеплугин**. Если вы используете не @ no__t-0Microsoft Plug @ no__t-1in, обратитесь к издателю за дополнительными сведениями. Дополнительные сведения о подключаемых модулях @ no__t-0ins см. в статье [как подключить @ no__t-2ins](cluster-aware-updating-plug-ins.md).   

## <a name="BKMK_REQ_CLUS"></a>Установка компонента отказоустойчивой кластеризации и средств отказоустойчивой кластеризации  
Для использования CAU необходимо установить компонент отказоустойчивости кластеров и средства отказоустойчивости кластеров. Средства отказоустойчивой кластеризации включают в себя средства CAU @no__t -0clusterawareupdating. dll @ no__t-1, командлеты отказоустойчивой кластеризации и другие компоненты, необходимые для операций CAU. Инструкции по установке компонента отказоустойчивости кластеров см. в записи блога [Установка компонента отказоустойчивости кластеров и средств отказоустойчивости кластеров](create-failover-cluster.md#install-the-failover-clustering-feature).  

Точные требования к установке средств отказоустойчивой кластеризации зависят от того, обновляются ли координаты CAU как кластерная роль в отказоустойчивом кластере \(by с помощью Self @ no__t-1updating Mode @ no__t-2 или с удаленного компьютера. В режиме Self @ no__t-0updating для CAU дополнительно требуется установка кластерной роли CAU в отказоустойчивом кластере с помощью средств CAU.    

В таблице ниже кратко описываются требования для установки компонента CAU в двух режимах обновления CAU.  

|Установленный компонент|Режим Self @ no__t-0updating|Remote @ no__t-0updating Mode|  
|-----------------------|-----------------------|-------------------------|  
|Средство отказоустойчивости кластеров|Требуется во всех узлах кластера|Требуется во всех узлах кластера|  
|Средства отказоустойчивости кластеров|Требуется во всех узлах кластера|— Требуется на удаленном компьютере @ no__t-0updating<br />— Требуется на всех узлах кластера для выполнения командлета [Save-каудебугтраце](https://docs.microsoft.com/powershell/module/clusterawareupdating/Save-CauDebugTrace?view=win10-ps)|  
...
-   Чтобы просмотреть или применить действия обновления с помощью пользовательского интерфейса CAU \(UI @ no__t-1 или командлетов обновления с поддержкой кластера, необходимо использовать учетную запись домена с правами локального администратора и разрешениями на всех узлах кластера. Если учетная запись не имеет достаточных привилегий на каждом узле, при выполнении этих действий появится запрос в окне кластерного обновления, чтобы предоставить необходимые учетные данные. Чтобы использовать командлеты [обновления с поддержкой кластера](https://docs.microsoft.com/powershell/module/clusterawareupdating/?view=win10-ps) , можно указать необходимые учетные данные в качестве параметра командлета.  

-   При использовании CAU в удаленном режиме @ no__t-0updating при входе с учетной записью, которая не имеет прав локального администратора и разрешений на узлах кластера, необходимо запускать средства CAU от имени администратора, используя учетную запись локального администратора в обновлении. Компьютер координатора или с помощью учетной записи, которая имеет право **Олицетворять клиента после проверки подлинности** пользователя. 

-   Чтобы запустить CAU анализатор соответствия рекомендациям, необходимо использовать учетную запись с правами администратора на узлах кластера и права локального администратора на компьютере, который используется для выполнения командлета [Test-каусетуп](https://docs.microsoft.com/powershell/module/clusterawareupdating/Test-CauSetup?view=win10-ps) или для анализа обновления кластера. готовность, используя окно обновления с поддержкой кластера. Подробнее см. в подразделе [Проверка готовности к обновлению кластера](#BKMK_BPA).  
...
-   Если в удаленном режиме @ no__t-0updating используется CAU, компьютер координатора обновлений должен иметь сетевое подключение к узлам отказоустойчивого кластера и находиться в том же домене Active Directory, что и отказоустойчивый кластер.  
...
-   Чтобы использовать скрипты PowerShell pre @ no__t-0update или POST @ no__t-1update во время прогона обновления CAU, убедитесь, что сценарии установлены на всех узлах кластера или доступны для всех узлов, например, в сетевой общей папке высокой доступности. Если сценарии сохранены в сетевой папке, настройте для нее разрешение на чтение для группы "Все".  
...
|Требование|Состояние по умолчанию|Режим Self @ no__t-0updating|Remote @ no__t-0updating Mode|  
|---------------|---|-----------------------|-------------------------|  
|[Включение автоматического перезапуска для правила брандмауэра](#BKMK_FW)|Отключено|Требуется во всех узлах кластера, если используется брандмауэр|Требуется во всех узлах кластера, если используется брандмауэр|  
|[Включить инструментарий управления Windows (WMI)](#BKMK_WMI)|Enabled|Требуется во всех узлах кластера|Требуется во всех узлах кластера|  
|[Включение Windows PowerShell 3,0 или 4,0 и удаленного взаимодействия Windows PowerShell](#BKMK_PS)|Enabled|Требуется во всех узлах кластера|Требуется во всех узлах кластера для выполнения следующих компонентов:<br /><br />— Командлет [Save-каудебугтраце](https://docs.microsoft.com/powershell/module/clusterawareupdating/Save-CauDebugTrace?view=win10-ps) .<br />— Скрипты PowerShell pre @ no__t-0update и POST @ no__t-1update во время прогона обновления<br />— Тестирование готовности к обновлению кластера с помощью окна "обновление с поддержкой кластера" или командлета [Test @ no__t-1CauSetup](https://docs.microsoft.com/powershell/module/clusterawareupdating/Test-CauSetup?view=win10-ps) Windows PowerShell.|  
|[Установка .NET Framework 4,6 или 4,5](#BKMK_NET)|Enabled|Требуется во всех узлах кластера|Требуется во всех узлах кластера для выполнения следующих компонентов:<br /><br />— Командлет [Save-каудебугтраце](https://docs.microsoft.com/powershell/module/clusterawareupdating/Save-CauDebugTrace?view=win10-ps) .<br />— Скрипты PowerShell pre @ no__t-0update и POST @ no__t-1update во время прогона обновления<br />— Тестирование готовности к обновлению кластера с помощью окна "обновление с поддержкой кластера" или командлета [Test @ no__t-1CauSetup](https://docs.microsoft.com/powershell/module/clusterawareupdating/Test-CauSetup?view=win10-ps) Windows PowerShell.|  

### <a name="BKMK_FW"></a>Включение автоматического перезапуска для правила брандмауэра  
Чтобы разрешить автоматическое перезагрузку после применения обновлений \(if установка обновления требует перезапуска @ no__t-1, если на узлах кластера используется брандмауэр Windows или брандмауэр, отличный от @ no__t-2Microsoft, на каждом узле должно быть включено правило брандмауэра, разрешающее следующий трафик:  
...
Если в узлах кластера используется брандмауэр Windows, сделать это можно путем включения группы правил брандмауэра Windows **Удаленное завершение работы** в каждом узле кластера. При использовании окна "кластерное обновление" для применения обновлений и настройки параметров Self @ no__t-0updating группа правил брандмауэра Windows для **удаленного завершения работы** автоматически включается на каждом узле кластера.  
...
Все узлы кластера должны быть настроены для удаленного управления с помощью инструментарий управления Windows (WMI) \(WMI @ no__t-1. Она включена по умолчанию.  
...
Для поддержки удаленного взаимодействия инструментария WMI, если на узлах кластера используется брандмауэр Windows, правило брандмауэра для входящего трафика для **служба удаленного управления Windows \(HTTP @ no__t-2In @ no__t-3** должно быть включено на каждом узле.  По умолчанию это правило включено.  

### <a name="BKMK_PS"></a>Включение удаленного взаимодействия Windows PowerShell и Windows PowerShell  
Чтобы включить режим "Self @ no__t-0updating" и некоторые функции CAU в удаленном режиме @ no__t-1updating, необходимо установить PowerShell и включить выполнение удаленных команд на всех узлах кластера. По умолчанию PowerShell устанавливается и включается для удаленного взаимодействия.  
...
-   Настройте параметр Domain @ no__t-0level групповая политика для служба удаленного управления Windows \(WinRM @ no__t-2.  
...
Чтобы включить режим @ no__t-0updating и некоторые функции CAU в удаленном режиме @ no__t-1updating, .NET Framework 4,6 или .NET Framework 4,5 (на Windows Server 2012 R2) должны быть установлены на всех узлах кластера. По умолчанию установлено значение NET Framework.  
...
Если вы приступите к использованию CAU для применения обновлений с помощью по умолчанию **Microsoft. виндовсупдатеплугин** Plug @ no__t-1in в кластере, то вы прекращаете использовать другие методы для установки обновлений программного обеспечения из корпорации Майкрософт на узлах кластера.  

> [!CAUTION]  
> Объединение CAU с методами, которые обновляют отдельные узлы автоматически \(on фиксированное расписание @ no__t-1 может привести к непредсказуемым результатам, включая прерывания в обслуживании и незапланированные простои.  
...
-   Не настраивайте систему обновления, например Windows Server Update Services \(WSUS @ no__t-1 для автоматического применения обновлений \(on с фиксированным расписанием @ no__t-3 на узлы кластера.  
...
-   Если внутренние серверы распространения программного обеспечения \(for, для хранения и развертывания обновлений используются серверы WSUS @ no__t-1. Убедитесь, что эти серверы правильно определяют утвержденные обновления для узлов кластера.  
...
При необходимости настройте параметры прокси-сервера WinHTTP на каждом узле, чтобы указать локальный прокси-сервер и настроить исключения локального адреса, \(that —, список обхода для локальных адресов @ no__t-1. Для этого можно выполнить следующую команду в каждом узле кластера из командной строки с повышенными привилегиями.  
...
-   Чтобы обеспечить целостность данных для подключений Server Message Block \(SMB @ no__t-1, которые используются для доступа к корневой папке исправления, следует настроить шифрование SMB в общей папке SMB, если это возможно. Подключаемый модуль **Microsoft.HotfixPlugin** требует настройки подписания SMB или шифрования SMB для обеспечения целостности данных, передаваемых через подключения SMB. 
...
Можно запустить модель CAU анализатор соответствия рекомендациям \(BPA @ no__t-1, чтобы проверить, удовлетворяет ли отказоустойчивый кластер и сетевая среда многим требованиям, чтобы обновления программного обеспечения были применены с помощью CAU. Многие тесты проверяют окружение на предмет готовности к применению обновлений Майкрософт с использованием подключаемого модуля @ no__t-0in, **Microsoft. виндовсупдатеплугин**по умолчанию.  

> [!NOTE]  
> Может потребоваться независимое подтверждение того, что среда кластера готова к применению обновлений программного обеспечения с помощью подключаемого модуля @ no__t-0in, отличного от **Microsoft. виндовсупдатеплугин**. Если используется не @ no__t-0Microsoft Plug @ no__t-1in, например поставщик оборудования, обратитесь к издателю за дополнительными сведениями.  
...
> -   После изменения источника обновления или изменения параметров обновления или конфигураций \(other, чем CAU @ no__t-1, которые могут повлиять на применение обновлений на узлах.  
...
|                    На узлах отказоустойчивого кластера должно быть включено удаленное управление через WMI                    |                                                Один или несколько узлов отказоустойчивого кластера не включены для удаленного управления с помощью инструментарий управления Windows (WMI) \(WMI @ no__t-1. Компонент CAU не может обновить узлы кластера, если для них не настроено удаленное управление.                                                 |                                                                                                  Убедитесь в том, что для узлов отказоустойчивого кластера включено удаленное управление через WMI. Подробнее см. в подразделе [Настройка узлов для удаленного управления](#BKMK_NODE_CONFIG) этого раздела.                                                                                                   |
|                      Удаленное взаимодействие PowerShell должно быть включено на каждом узле отказоустойчивого кластера.                       |                                                           PowerShell не установлен или не включен для удаленного взаимодействия на одном или нескольких узлах отказоустойчивого кластера. Невозможно настроить CAU для режима Self @ no__t-0updating или использовать некоторые функции в удаленном режиме @ no__t-1updating.                                                            |                                                                                             Убедитесь, что PowerShell установлен на всех узлах кластера и включен для удаленного взаимодействия.<br /><br />Подробнее см. в подразделе [Настройка узлов для удаленного управления](#BKMK_NODE_CONFIG) этого раздела.                                                                                             |
...
|       На каждом узле отказоустойчивого кластера должно быть включено правило брандмауэра, разрешающее удаленное выключение       |                 На одном или нескольких узлах отказоустойчивого кластера не включено правило брандмауэра, разрешающее удаленное выключение, либо включение этого правила запрещено параметром групповой политики. Прогоны обновления, требующие автоматической перезагрузки узлов, могут быть применены неверно.                  |                                                                    Если на узлах кластера используется брандмауэр Windows или брандмауэр, отличный от @ no__t-0Microsoft, настройте правило брандмауэра, разрешающее удаленное завершение работы.<br /><br />Подробнее см. в подразделе [Включение правила брандмауэра для обеспечения автоматического перезапуска](#BKMK_FW) этого раздела.                                                                    |
...
|        Для включения режима Self @ no__t-0updating необходимо установить в отказоустойчивом кластере кластерную роль CAU.        |                                                                                                   Кластерная роль CAU не установлена на этом отказоустойчивом кластере. Эта роль необходима для кластера Self @ no__t-0updating.                                                                                                   |      Чтобы использовать CAU в режиме Self @ no__t-0updating, добавьте кластерную роль CAU в отказоустойчивый кластер одним из следующих способов:<br /><br />— Выполните командлет PowerShell [Add-кауклустерроле](https://docs.microsoft.com/powershell/module/clusterawareupdating/Add-CauClusterRole) .<br />— Выберите действие **Настройка самостоятельного параметра @ no__t-1updating параметров** в окне Обновление с поддержкой кластера.      |
|         Для включения режима Self @ no__t-0updating в отказоустойчивом кластере должна быть включена кластерная роль CAU.         | Кластерная роль CAU отключена. Например, кластерная роль CAU не установлена или была отключена с помощью командлета PowerShell [Disable @ no__t-1CauClusterRole](https://docs.microsoft.com/powershell/module/clusterawareupdating/Disable-CauClusterRole) . Эта роль необходима для кластера Self @ no__t-0updating. | Чтобы использовать CAU в режиме Self @ no__t-0updating, включите кластерную роль CAU в этом отказоустойчивом кластере одним из следующих способов:<br /><br />— Запустите командлет PowerShell [Enable-кауклустерроле](https://docs.microsoft.com/powershell/module/clusterawareupdating/Enable-CauClusterRole) .<br />— Выберите действие **Настройка самостоятельного параметра @ no__t-1updating параметров** в окне Обновление с поддержкой кластера. |
|      Настроенный подключаемый модуль CAU @ no__t-0in для режима Self @ no__t-1updating должен быть зарегистрирован на всех узлах отказоустойчивого кластера.      |                                                              Кластерная роль CAU на одном или нескольких узлах этого отказоустойчивого кластера не может получить доступ к модулю CAU Plug @ no__t-0in, настроенному в параметрах Self @ no__t-1updating. Выполнение Self @ no__t-0updating может завершиться ошибкой.                                                              |           — Убедитесь, что настроенный подключаемый модуль CAU @ no__t-0in установлен на всех узлах кластера, следуя процедуре установки для продукта, который предоставляет подключаемый модуль CAU @ no__t-1in.<br />— Выполните командлет PowerShell [Register @ no__t-1CauPlugin](https://docs.microsoft.com/powershell/module/clusterawareupdating/Register-CauPlugin) , чтобы зарегистрировать подключаемый модуль @ no__t-2in на требуемых узлах кластера.           |
|                Все узлы отказоустойчивого кластера должны иметь одинаковый набор зарегистрированных подключаемых модулей CAU @ no__t-0ins                 |                                                                             Выполнение Self @ no__t-0updating может завершиться ошибкой, если параметр Plug @ no__t-1in, настроенный для использования в прогоне обновления, изменен на другой, недоступный на всех узлах кластера.                                                                              |           — Убедитесь, что настроенный подключаемый модуль CAU @ no__t-0in установлен на всех узлах кластера, следуя процедуре установки для продукта, который предоставляет подключаемый модуль CAU @ no__t-1in.<br />— Выполните командлет PowerShell [Register @ no__t-1CauPlugin](https://docs.microsoft.com/powershell/module/clusterawareupdating/Register-CauPlugin) , чтобы зарегистрировать подключаемый модуль @ no__t-2in на требуемых узлах кластера.           |
|                               Должны быть настроены допустимые параметры прогона обновления                                |                                                                          Расписание Self @ no__t-0updating и параметры выполнения обновления, настроенные для этого отказоустойчивого кластера, неполные или недопустимы. Выполнение Self @ no__t-0updating может завершиться ошибкой.                                                                           |                                                            Настройте допустимое расписание Self @ no__t-0updating и набор параметров прогона обновления. Например, с помощью командлета PowerShell [Set @ no__t-1CauClusterRole](https://docs.microsoft.com/powershell/module/clusterawareupdating/Set-CauClusterRole) можно настроить кластерную роль Cau.                                                            |
|                  По крайней мере два узла отказоустойчивого кластера должны быть владельцами кластерной роли CAU                  |                                                                                        Прогон обновления, запущенный в режиме Self @ no__t-0updating, завершится ошибкой, так как у кластерной роли CAU нет возможных узлов-владельцев для перемещения.                                                                                         |                                                                                                                С помощью средств отказоустойчивости кластеров настройте все узлы кластера в качестве потенциальных владельцев кластерной роли CAU. Это конфигурация по умолчанию.                                                                                                                |
|                  Все узлы отказоустойчивого кластера должны иметь доступ к сценариям Windows PowerShell                  |                                                                        Не все возможные узлы-владельцы в кластерной роли CAU имеют доступ к настроенным сценариям Windows PowerShell pre @ no__t-0update и POST @ no__t-1update. Выполнение Self @ no__t-0updating завершится ошибкой.                                                                        |                                                                                                                    Убедитесь, что все возможные узлы-владельцы в кластерной роли CAU имеют разрешения на доступ к настроенным сценариям PowerShell pre @ no__t-0update и POST @ no__t-1update.                                                                                                                     |
|                   На всех узлах отказоустойчивого кластера должны использоваться идентичные сценарии Windows PowerShell                   |                                                     Не все возможные узлы-владельцы в кластерной роли CAU используют одну и ту же копию указанных сценариев Windows PowerShell pre @ no__t-0update и POST @ no__t-1update. Выполнение Self @ no__t-0updating может завершиться ошибкой или отобразить непредвиденное поведение.                                                     |                                                                                                                                   Убедитесь, что все возможные узлы-владельцы кластерной роли CAU используют те же сценарии PowerShell pre @ no__t-0update и POST @ no__t-1update.                                                                                                                                   |
