...
Назначение: быстрое предоставление рабочей станции с привилегированным доступом, обеспечивающей защиту ролей администрирования локальных доменов и лесов.
      ...
      3. Настроив файл *proxy.pac*, обновите объект групповой политики "Конфигурация рабочей станции с привилегированным доступом — пользователь".
         1. Перейдите в раздел "Конфигурация пользователя\Параметры\Параметры Windows\Реестр". Щелкните "Реестр" правой кнопкой мыши, выберите **Создать** > **Элемент реестра** и настройте следующие параметры:
            ...
            6. Данные значения. Введите полный URL-адрес файла *proxy.pac* с частью http:// и именем файла. Например, http://proxy.fabrikam.com/proxy.pac.  URL-адрес также может быть однокомпонентным, например http://proxy/proxy.pac.
               ...
               > [!NOTE]
               > PAC-файл также может размещаться в общей папке и иметь синтаксис типа file://server.fabrikan.com/share/proxy.pac, но для этого требуется разрешить использование протокола file://. Дополнительные сведения о настройке требуемого значения регистра см. в разделе "ПРИМЕЧАНИЕ. Устаревшие скрипты прокси-сервера с использованием протокола file://" записи блога [Understanding Web Proxy Configuration](http://blogs.msdn.com/b/ieinternals/archive/2013/10/11/web-proxy-configuration-and-ie11-changes.aspx) (Основные сведения о конфигурации веб-прокси).
...
[Enabling Strict KDC Validation in Windows Kerberos](https://www.microsoft.com/en-us/download/details.aspx?id=6382) (Включение обязательной проверки KDC в Windows Kerberos)
