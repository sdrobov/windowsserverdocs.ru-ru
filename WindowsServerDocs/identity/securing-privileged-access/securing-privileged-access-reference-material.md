...
description: Элементы управления операционной безопасности для Windows Server Active Directory — домены
...
- Если ПО скачано из Интернета, проверьте его, скачав и сравнив две отдельные независимые копии:
   ...
   - Сравните скачанные файлы, используя программу, такую как certutil:  `certutil -hashfile <filename>`
...
Все исключения в отношении обязательных элементов (отмечены красным восьмиугольником или оранжевым треугольником в этом документе) следует рассматривать как временные, и они должны быть советом по Изменениям. Рекомендации:
...
Все исключения в отношении рекомендуемых элементов (отмечены желтым кругом в этом документе) следует рассматривать как временные и советом по Изменениям. Рекомендации:
...
Администраторам нельзя просматривать интернет-страницы, если вошли в систему, используя учетную запись администратора, или на рабочую станцию администрирования. К исключениям относятся веб-браузера для администрирования облачной службы.
...
- Такие учетные записи следует использовать только для:
   ...
   - выполнения редких задач, таких как:
      ...
      - Задачи уровня леса, требующих прав администратора предприятия

      > [!NOTE]
      > Чтобы ограничить использование этих привилегий делегируется управления топологией, включая управление сайтов и подсетей Active Directory.
...
- **Привилегии и усиление безопасности домена**. Административный лес должен быть настроен для предоставления минимальных прав доступа в соответствии с требованиями для администрирования AD.
   ...
   - Административный лес должен соответствовать конфигурации базовых показателей соответствия требованиям безопасности Майкрософт (SCB) для домена, включая надежные конфигурации для протоколов проверки подлинности.
...
- **Усиление безопасности серверов и контроллеров домена**. Для всех контроллеров домена и серверов в административном лесу:
   ...
   - Базовые конфигурации безопасности следует использовать в качестве начальных.
      ...
      > [!NOTE]
      > Клиенты могут использовать набор средств соответствия требованиям безопасности Майкрософт (SCT) для настройки базовых конфигураций на узлах администрирования.
...
- Владельцы-создатели групповых политик
- Криптографические операторы
- Пользователи DCOM
- Другие группы полномочные - пользовательские группы, которые могут быть созданы в организации для управления операциями с каталогами, которые также могут иметь действующие права доступа уровня 0.
...
|Использование оснасток MMC для удаленного компьютера|Network|-|Пример. Диспетчер устройств управления, средство просмотра событий, для компьютера, службы|
...
- **#** — Числовой идентификатор типа входа, которая передается в событиях аудита в журнале событий безопасности.
