...
description: Операционные средства управления безопасностью для доменов Active Directory Windows Server
...
- Если ПО скачано из Интернета, проверьте его, скачав и сравнив две отдельные независимые копии:
   ...
   - Сравните Скачанные файлы с помощью такой служебной программы, как certutil: `certutil -hashfile <filename>`
...
Дополнительные сведения об усилении защиты от основных рисков, связанных с AD, см. [на этой странице](http://aka.ms/hardenAD).
...
- Все рабочие станции, которыми необходимо управлять, используют Windows 7 или более поздней версии, и для них включен режим RestrictedAdmin RDP.
   ...
   > [!NOTE]
   > Дополнительные сведения о том, как включить режим RestrictedAdmin RDP, см. [на этой странице](http://aka.ms/RDPRA).
...
- На серверах и рабочих станциях развернуто решение [LAPS](http://aka.ms/laps) для управления паролями учетных записей локальных администраторов.
...
Все исключения для обязательных элементов (отмеченных красным восьмиугольник или оранжевым треугольником в этом документе) считаются временными и должны быть утверждены CAB-файлом. Рекомендации:
...
Все исключения для рекомендуемых элементов (отмеченные желтым кругом в этом документе) считаются временными и должны быть утверждены CAB-файлом. Рекомендации:
...
>
> Дополнительные сведения о типах входа, распространенных средствах управления и раскрытии учетных данных см. в подразделе [Средства администрирования и типы входа](http://aka.ms/admintoolsecurity).
...
- **Обслуживание удаленного сервера**. При удаленном входе на сервер администраторы уровня 0 должны следовать таким рекомендациям:
  - **Основной способ входа (с помощью средств)** . Применяйте инструменты удаленного доступа, которые используют сетевой вход (тип 3). Дополнительные сведения см. в разделе [Средства администрирования и типы входа](http://aka.ms/admintoolsecurity).
...
- **Удаленное предоставление поддержки пользователям.** . Обслуживающий персонал уровня 2 работает физически удаленно от пользователя.
   - **Основной способ**. Предполагает использование удаленного помощника, Skype для бизнеса или аналогичной программы демонстрации пользовательского экрана. Дополнительные сведения см. в разделе [Что такое удаленный помощник Windows?](https://windows.microsoft.com/en-us/windows/what-is-windows-remote-assistance).
...
Администраторам нельзя просматривать интернет-страницы, если вошли в систему, используя учетную запись администратора, или на рабочую станцию администрирования. Единственными разрешенными исключениями являются использование веб-браузера для администрирования облачной службы.
...
- Такие учетные записи следует использовать только для:
  ...
  - выполнения редких задач, таких как:
    ...
    - Задачи уровня леса, требующие административных привилегий предприятия

      > [!NOTE]
      > Для управления топологией, включая Управление сайтом и подсетями Active Directory, делегируется ограничение использования этих привилегий.
...
- **Привилегии и усиление безопасности домена**. Административный лес должен быть настроен для предоставления минимальных прав доступа в соответствии с требованиями для администрирования AD.
   ...
   - Административный лес должен соответствовать конфигурациям Microsoft Security Baseline (СКБ) для домена, включая надежные конфигурации для протоколов проверки подлинности.
...
- **Усиление безопасности серверов и контроллеров домена**. Для всех контроллеров домена и серверов в административном лесу:
   - Проверьте все носители на соответствие инструкциям руководства [Чистый источник установочного носителя](http://aka.ms/cleansource).
   ...
   - Базовые конфигурации безопасности следует использовать в качестве начальных.
      ...
      > [!NOTE]
      > Клиенты могут использовать набор средств обеспечения безопасности Майкрософт (SCT) для настройки базовых показателей на узлах администрирования.
   ...
   - Анализируйте возможные направления атак, чтобы предотвратить появление в Windows новых направлений атак во время установки нового программного обеспечения.
      ...
      > [!NOTE]
      > Использование таких средств, как анализатор [Attack Surface Analyzer](https://www.microsoft.com/en-us/download/details.aspx?id=24487), поможет оценить параметры конфигурации на узле и определить направления атак, риск которых представляют программное обеспечение или изменения конфигурации.
...
- Пользователи DCOM
- Другие делегированные группы — настраиваемые группы, которые могут быть созданы вашей организацией для управления операциями каталога, которые также могут иметь эффективный доступ к уровню 0.
...
|Метод подключения|Тип входа в систему|Повторное использование учетных данных на целевом компьютере|Комментарии|
|-----------|-------|--------------------|------|
...
|Использование оснасток MMC для удаленного компьютера|Network|-|Пример. Управление компьютером, Просмотр событий, Device Manager, службы|
|WinRM PowerShell|Network|-|Пример. Enter-PSSession Server|
|PowerShell WinRM с CredSSP|NetworkClearText|v|New-PSSession server<br />-Authentication Credssp<br />-Credential cred|
|PsExec без раскрытых учетных данных|Network|-|Пример. PsExec \\ \ Server cmd|
...
|Удаленный реестр|Network|-||
|Шлюз удаленных рабочих столов|Network|-|Проверка подлинности на шлюзе удаленных рабочих столов.|
|Запланированная задача|Batch|v|Пароль также сохраняется в качестве секрета LSA на диске.|
|Запуск средств в качестве службы|Обслуживание|v|Пароль также сохраняется в качестве секрета LSA на диске.|
|Сканеры уязвимостей|Network|-|По умолчанию большинство сканеров используют сетевой вход, хотя некоторые поставщики могут реализовать вход другого типа, тем самым увеличивая риск кражи учетных данных.|

Для проверки подлинности в сети используйте справочные данные из следующей таблицы:

|Метод подключения|Тип входа в систему|Повторное использование учетных данных на целевом компьютере|Комментарии|
|-----------|-------|--------------------|------|
|Обычная проверка подлинности IIS|NetworkClearText<br />(IIS 6.0 и более поздних версий)<br /><br />Interactive (Интерактивные)<br />(IIS 6.0 и предыдущих версий)|v||
|Встроенная проверка подлинности Windows IIS|Network|-|Поставщики NTLM и Kerberos|
...
|Network|3|Пароль,<br />хэш NT,<br />билет Kerberos|Нет (но если включено делегирование, используются билеты Kerberos)|NET USE,<br />RPC-вызовы,<br />удаленный реестр,<br />встроенная проверка подлинности Windows IIS;<br />проверка подлинности Windows SQL|
|Batch|4|Пароль (обычно хранится как секрет LSA)|Да|Запланированные задачи|
|Обслуживание|5|Пароль (обычно хранится как секрет LSA)|Да|Службы Windows|
...
- **#** — это числовой идентификатор для типа входа, который регистрируется в событиях аудита в журнале событий безопасности.
