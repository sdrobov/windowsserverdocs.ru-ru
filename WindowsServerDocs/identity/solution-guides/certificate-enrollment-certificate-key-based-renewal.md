...
В этой статье содержатся пошаговые инструкции по реализации веб-служба регистрации сертификатов (или политики регистрации сертификатов (CEP) или службы регистрации сертификатов (CES)) на настраиваемом порте, отличном от 443, для выполнения продления на основе ключа сертификата. преимущества функции автоматического продления CEP и CES.
...
В разделе **веб-сайт по умолчанию**выберите **KeyBasedRenewal_ADPolicyProvider_CEP_Certificate**, а затем откройте **Параметры приложения**. Запишите **идентификатор** и **URI**.
...
> [!Note]
> Если экземпляр установлен на новом сервере, а идентификатор отличается друг от друга и в случае, если идентификатор отличается, убедитесь, что он совпадает с ИДЕНТИФИКАТОРом, используемым в экземпляре CEPCES01. Значение можно скопировать и вставить напрямую.
...
> [!Note]
> В этой команде <cepcessvc> является учетной записью службы, а < CA1. contoso. com > является центром сертификации.
...
##### <a name="step-4-edit-the-ca-enrollment-services-object"></a>Шаг 4. изменение объекта служб регистрации ЦС

1. На контроллере домена откройте ADSIEdit. msc.

2. Подключитесь к разделу конфигурации и перейдите к объекту службы регистрации ЦС:
   
   CN = ЕНТКА, CN = службы регистрации, CN = открытые ключи Services, CN = Services, CN = Configuration, DC = contoso, DC = com

3. Измените объект и измените атрибут Мспки-регистрации-Servers, используя пользовательский порт с URI сервера CEP и CES, которые были найдены в параметрах приложения. Пример
...
Чтобы обеспечить работоспособность автоматического продления, убедитесь, что обновление вручную выполняется с помощью того же ключа. Не следует выдавать запрос на ввод имени пользователя и пароля.  Кроме того, при регистрации вам будет предложено выбрать сертификат. Это нормально.

Откройте личное хранилище сертификатов компьютера и добавьте представление "архивные сертификаты". Для этого используйте один из следующих методов.
...
Если срок действия сертификата истечет, то следует перейти к значению времени, кратному 8.

Например, пример сертификата был выпущен в 4:00 в 18-й день месяца, истекает в 4:00 в 20 раз и имеет срок действия в 2 дня и параметр продления на 8 часов. Подсистема автоматической регистрации активируется при перезапуске примерно в 8-часовом интервале.

Таким образом, если вы перейдете время на 8:10 P.M. на 19. выполнение команды **certutil-Pulse** (для активации подсистемы AE) регистрирует сертификат автоматически.
