...
|Этап|Описание  
|---------|---------------  
|[План: Определение необходимости в политике и конфигурации, необходимые для развертывания](Deploy-a-Central-Access-Policy--Demonstration-Steps-.md#BKMK_1.2)|Определение необходимости в политике и конфигурации для развертывания. 
...
|[Поддержка: Изменение и сверка политики](Deploy-a-Central-Access-Policy--Demonstration-Steps-.md#BKMK_1.5)|Изменение и сверка политики. 
  
## <a name="BKMK_1.1"></a>Настройка тестовой среды  
Прежде чем приступать, вам необходимо настроить лабораторию для тестирования этого сценария. Действия по настройке лаборатории рассматриваются подробно в [приложении б: Настройка тестовой среды](Appendix-B--Setting-Up-the-Test-Environment.md).  
  
## <a name="BKMK_1.2"></a>План: Определение необходимости в политике и конфигурации для развертывания.  
...
|1.3|Выражение политики доступа в конструкциях Windows Server 2012|Цель:<br /><br />-Resource.Department содержит процент<br /><br />Правила доступа:<br /><br />— Разрешите чтения User.Country=Resource.Country и User.department = Resource.Department<br />-Разрешить полный доступ User.MemberOf(FinanceAdmin)<br /><br />Исключение:<br /><br />Allow read memberOf(FinanceException)|  
...
|2.2|Создание свойства ресурсов|Создайте и включите следующие свойства ресурсов:<br /><br />-Отдела<br />-Страны|  
...
В следующей процедуре вы создадите два типа утверждений: "Страна" и "Отдел".  
  
#### <a name="to-create-claim-types"></a>Создание типов утверждений  
  
1.  Откройте сервер DC1 в диспетчере Hyper-V и войдите с учетной записью contoso\administrator, используя пароль **pass@word1**.  
...
Цель: Resource.Department Contains Finance  
...
5.  В разделе **Разрешения** выберите параметр **Использовать следующие разрешения как текущие**, нажмите кнопку **Изменить** и в диалоговом окне **Дополнительные параметры безопасности для разрешений** нажмите **Добавить**.  
    ...
    > [!NOTE]  
    > Параметр **Использовать следующие разрешения как текущие** позволяет создать политику для сверки. Дополнительные сведения см. в подразделе "Обслуживание: изменение и сверка политики" в этом разделе.  
...
7.  В диалоговом окне **Запись для разрешений** щелкните **Добавить условие** и введите следующие условия:   
    [**Пользователя**] [**страны**] [**любой из**] [**ресурсов**] [**страны**]   
     Щелкните **Добавить условие**.   
     [**И**]   
    Нажмите кнопку [**пользователя**] [**отдел**] [**любой из**] [**ресурсов**] [**отдел**]. Задайте для параметра **Разрешения** значение **Чтение**.  
...
Чтобы контроллеры домена предоставляли утверждения или данные авторизации устройств контроллеры домена должны быть настроен для поддержки динамического контроля доступа.  
...
3.  Вам также необходимо обновить глобальные свойства ресурсов из Active Directory. Откройте командное окно Windows PowerShell с повышенными привилегиями и введите: `Update-FSRMClassificationpropertyDefinition`. Нажмите клавишу ВВОД и закройте Windows PowerShell.  
    ...
    >   
    > 1.  Войдите на файловый сервер FILE1 как contoso\administrator, используя пароль **pass@word1**.  
...
3.  Чтобы просмотреть разрешения для пользователя, нажмите кнопку **выберите пользователя**, введите имя пользователя и нажмите кнопку **просмотреть действующие** для см. в разделе разрешения. Пример:  
...
8.  Щелкните ссылку **Добавить условие** еще раз и добавьте следующее условие:  
    [И]   
     [**Пользователя**] [**группы**] [**член любого**] [**значение**]\(**FinanceException**)  
...
1.  Подключитесь к файловому серверу FILE1 в диспетчере Hyper-V и войдите как contoso\administrator, используя пароль **pass@word1**.  
  
2.  В окне командной строке введите **gpupdate /force**. Эта команда применяет изменения групповой политики на сервере.  
  
3.  В диспетчере Hyper-V подключитесь к серверу CLIENT1. Завершите сеанс текущего пользователя. Перезапустите виртуальную машину CLIENT1. Затем войдите в компьютер с помощью contoso\EValle pass@word1.  
  
4.  Дважды щелкните ярлык на рабочем столе \\\FILE1\Finance документов. У пользователя EValle по-прежнему должен быть доступ к файлам. Переключитесь на сервер FILE1.  
  
5.  Откройте **средство просмотра событий** с помощью ярлыка на рабочем столе. Разверните узел **Журналы Windows** и выберите **Безопасность**. Откройте записи с **Event ID 4818**под **Сверка с централизованной политикой доступа** категории задач. Можно увидеть, что пользователю EValle был предоставлен доступ, однако согласно промежуточной политике в доступе было отказано.  
