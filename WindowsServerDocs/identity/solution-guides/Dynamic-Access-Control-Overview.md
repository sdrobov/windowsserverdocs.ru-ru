...
Централизованные политики доступа — это политики авторизации, включающие условные выражения. Например предположим, что у организации есть бизнес-требование, чтобы ограничить доступ к личными сведениями (PII) в файлах только владельцу файла и членами отдела кадров (HR), которым разрешено просматривать Персональные данные. Эта политика организации применяется к файлам персональных данных, на каком бы из файловых серверов организации они ни располагались. Для реализации этой политики организация должна иметь следующие возможности:  
...
Утверждение — это уникальная деталь информации о пользователе, устройстве или ресурсе, опубликованная контроллером домена. Название пользователя, отдел классификации файла или состояние работоспособности компьютера, например, утверждение. Объект может включать более одного утверждения, и любое сочетание утверждений можно использовать для авторизации доступа к ресурсам. В поддерживаемых версиях Windows доступны следующие типы утверждений:  
...
Следующий параметр групповой политики позволяет развертывать объекты централизованной политики доступа на файловых серверах в вашей организации: **Компьютер Windows Конфигурация компьютера\Политики\Параметры Windows\Параметры безопасности\Файловая система\централизованная доступа политика**.  
...
Если домены пользователя и файлового сервера находятся в разных лесах, все контроллеры домена в корне леса файлового сервера должны быть установлены Windows Server 2012 или высокого функционального уровня.  
...
Если утверждения преобразуются, когда они покидают лес, все контроллеры домена в корне леса пользователя должны быть установлены Windows Server 2012 или высокого функционального уровня.  
...
Сведения о реализации решений на основе этой технологии см. в разделе [динамический контроль доступа: Обзор сценария](Dynamic-Access-Control--Scenario-Overview.md).  
