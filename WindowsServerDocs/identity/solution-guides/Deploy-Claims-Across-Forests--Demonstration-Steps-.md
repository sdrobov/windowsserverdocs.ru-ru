...
Конфигурация теста включает в себя настройку двух лесов: Корпорация Adatum Corporation и Contoso, Ltd и имеет двустороннее доверие между Contoso и adatum. "adatum.com" — это доверенный лес, а "contoso.com" — доверяющий лес.  
...
Для лаборатории необходимо настроить следующие значения. Эти процедуры подробно описаны в @no__t 0Appendix B. Настройка тестовой среды](Appendix-B--Setting-Up-the-Test-Environment.md)  
...
|Утверждения пользователей в adatum и contoso|Идентификатор: ad://ext/Company:ContosoAdatum,<br /><br />Исходный атрибут: Company<br /><br />Предлагаемые значения: Contoso, adatum **Важно:** Необходимо задать идентификатор для типа утверждения "компания" в Contoso и adatum так, чтобы преобразование утверждений работало одинаково.|  
...
1. Войдите на клиентский компьютер КЛИЕНТ1 с помощью пароля <strong>pass@word1</strong>.  

2. Перейдите в папку \\ \ FILE1. contoso. Ком\еарнингс.  
...
| Разрешите всем заявкам, поступающим от adatum, за исключением "компания" и "Отдел", чтобы пройти до Contoso adatum | Код <br />-New-Адклаимтрансформатионполици \`<br />-Description: "политика преобразования утверждений для разрешения всех утверждений, кроме компании и отдела" \`<br /> -Name: "Алловаллклаимсексцепткомпанянддепартментполици" \`<br />-Алловаллексцепт: компания, Отдел \`<br />-Server:"Контосо. com" \`<br />Set-Адклаимтрансформлинк \`<br /> -Identity:"адатум. com" \`<br />-Policy: "Алловаллклаимсексцепткомпанянддепартментполици" \`<br /> -Трустроле: доверие \`<br />-Server:"Контосо. com" \` |
