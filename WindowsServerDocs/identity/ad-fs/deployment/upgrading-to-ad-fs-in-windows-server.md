...
> [!NOTE] 
> В AD FS 2019 существует известная ошибка PRT, если выполняется Windows Hello для бизнеса с гибридным доверием сертификатов. Эта ошибка может возникать в журналах событий администратора ADFS: получен недопустимый запрос OAuth. (Получен недопустимый запрос OAuth. Клиенту с именем NAME запрещено обращаться к ресурсу с областью ugs.) Чтобы исправить эту ошибку, сделайте следующее: 
...
> 8. На клиенте выполните перезагрузку клиента. Пользователю должно быть предложено подготовить WHFB.
