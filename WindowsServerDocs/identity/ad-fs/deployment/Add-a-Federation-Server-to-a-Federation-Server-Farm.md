...
Присоединение компьютера к ферме осуществляется с помощью мастера настройки сервера AD FS Федерации. При использовании этого мастера для приподключения компьютера к существующей ферме на компьютере настраивается копия базы данных конфигурации AD FS Read @ no__t-0only, и она должна получить обновления с основного сервера федерации.  
...
1.  Запустить мастер настройки сервера федерации AD FS можно двумя способами. Чтобы запустить мастер, выполните одно из следующих действий.  
    ...
    -   В любое время после завершения работы мастера установки откройте проводник Windows, перейдите в папку **C: \\Windows @ no__t-2ADFS** и дважды @ no__t-3click **фсконфигвизард. exe**.  
