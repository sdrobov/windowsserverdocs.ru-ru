---
title: Создание собственного клиентского приложения с помощью общедоступных клиентов OAuth с AD FS 2016
description: Пошаговое руководство, в котором содержатся инструкции, создания собственного клиентского приложения с помощью общедоступных клиентов OAuth и AD FS 2016
author: billmath
ms.author: billmath
ms.reviewer: anandy
manager: mtillman
...
ms.locfileid: "59846485"
---
# <a name="build-a-native-client-application-using-oauth-public-clients-with-ad-fs-2016"></a>Создание собственного клиентского приложения с помощью общедоступных клиентов OAuth с AD FS 2016

## <a name="overview"></a>Обзор

В этой статье показано, как создавать исходное приложение, которое взаимодействует с веб-API, защищаемые с помощью AD FS 2016.
...
2. Маркер доступа используется как токен носителя для проверки подлинности пользователя при вызове конечной точки /todolist TodoListService веб-API.
 Мы с помощью примера приложения для Azure AD здесь и затем изменить его для AD FS 2016.
...
* Добавьте ключ **ida: центр** со значением, изображающие AD FS службы, пример, https://fs.contoso.com/adfs/
* Изменить **ida: ClientId** ключа со значением из **собственное приложение** страницы во время создания группы приложений в AD FS. Для ex, 3f07368b-6efd-4f50-a330-d93853f4c855
* Изменить **todo:TodoListBaseAddress** на базовый адрес веб-API, например https://localhost:44321/
* Установите для параметра **ida: RedirectUri** к значению, поместить на странице приложения в машинном коде во время добавления группы приложений в AD FS.
* Для более удобного чтения можно удалить / комментарий ключ для **ida: клиента** и **ida: AADInstance**.

**MainWindow.xaml.cs**

* Удалите строку для aadInstance
...
* В функции **MainWindow**, изменить при создании экземпляра authContext как
...
* Удалить ключ **ida: клиента** как нам не нужен
