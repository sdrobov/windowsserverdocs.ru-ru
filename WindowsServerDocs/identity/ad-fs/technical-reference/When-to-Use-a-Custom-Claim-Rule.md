...
-   Отправка утверждений на основе значений, извлеченных из язык SQL \(хранилище атрибутов SQL\).  
  
-   Отправка утверждений на основе значений, извлеченных из протокола доступа к облегченному каталогу \(хранилище атрибутов LDAP\) с помощью настраиваемого фильтра LDAP.  
...
Правило утверждения представляет экземпляр бизнес-логики, который принимает входящее утверждение, применяет к нему условие \(если x, затем y\) и выдает исходящее утверждение на основе параметров условия.  
...
Это правило создается путем создания синтаксиса, необходимого для выполнения операции, с помощью языка правил заявок, а затем вставляется результат в текстовое поле, предоставленное в разделе Отправка утверждений с помощью настраиваемого шаблона правила, в свойствах отношения доверия поставщика утверждений или отношения доверия с проверяющей стороной в\-е оснастки управления AD FS.  
...
Чтобы лучше понять, как работает язык правил заявок, просмотрите синтаксис языка правил утверждений для других правил, которые уже существуют в\-привязки в, щелкнув вкладку **язык правил** в свойствах этого правила. Используя сведения данного раздела и информацию о синтаксисе на этой вкладке, можно глубже понять процесс создания собственных настраиваемых правил.  
...
Следующее правило выдает личный личный идентификатор \(PPID\) утверждения на основе атрибутов **виндовсаккаунтнаме** и **OriginalIssuer** пользователей в хранилище атрибутов LDAP:  
