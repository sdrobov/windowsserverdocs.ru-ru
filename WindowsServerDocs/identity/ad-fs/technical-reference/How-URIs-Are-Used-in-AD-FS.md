...
-   URL-адреса служба федерации, включая WS\-Federation, SAML, WS\-Trust, метаданные федерации, службы WS\-MetadataExchange, конфиденциальность и URL-адреса Организации.  
  
-   URL-адреса отношений доверия с проверяющей стороной, включая URL-адреса метаданных федерации\-WS, SAML и Federation  
  
-   URL-адреса отношений доверия поставщиков утверждений, включая URL-адреса метаданных федерации\-WS, SAML и Federation  
...
Синтаксис пути универсального кода ресурса (URI) организован иерархически и разделяются либо всеми символами «\/», либо всеми символами «:».  Поэтому путь можно разделить на разделы пути на основе символа разделителя.  При совпадении префиксов каждый раздел должен быть полным совпадением в соответствии с правилами сопоставления \(эти правила управляют регистром совпадений\). Дополнительные сведения о правилах сопоставления см. в документе RFC, упомянутом выше.  
...
-   Конечные разделители \(символы косой черты и двоеточия\) разделов пути или центров должны игнорироваться.  
  
-   Регистр частей схем и данных о пользователе и узле идентификаторов URI1 и URI2 должен строго совпадать.  
  
-   Каждый раздел пути URI1 должен быть точным совпадением \(на основе чувствительности регистра\) к соответствующему разделу пути URI2  
...
|http:\/\/contoso.com\/HR|http:\/\/contoso.com\/HR\/Web\/? m\=t|TRUE|По тем же причинам строка запроса не изменяет ничего|  
