...
> [!NOTE]  
> AD FS фермах, использующих внутреннюю базу данных Windows \(WID\) обеспечивают базовую избыточность данных с\/доступом на запись на основном узле сервера федерации и чтение\-доступ только на вторичных узлах.  Его можно использовать в географически локальной или географически распределенной топологии.  
>   
...
Группы доступности AlwaysOn появились в SQL Server 2012 и предоставляют новый способ создания экземпляра SQL Server высокой доступности.  Группы доступности AlwaysOn объединяют элементы кластеризации и зеркального отображения базы данных для обеспечения избыточности и отработки отказа на уровне экземпляра SQL и на уровне базы данных.  В отличие от предыдущих вариантов обеспечения высокой доступности, группам доступности AlwaysOn не требуется общая \(хранилища или сеть хранения\) на уровне базы данных.  
  
Группа доступности состоит из первичной реплики \(набор операций чтения\-записи баз данных-получателей\) и от одной до четырех реплик доступности \(наборов соответствующих баз данных-получателей\).  Группа доступности поддерживает одну копию\-записи \(первичной реплике\), а также одну до четырех операций чтения\-только реплики доступности.  Каждая реплика доступности должна находиться на другом узле одной отказоустойчивой кластеризации Windows Server \(кластере WSFC\).  Дополнительные сведения о группах доступности AlwaysOn см. в [обзоре группы доступности AlwaysOn \(SQL Server\)](https://technet.microsoft.com/library/ff877884.aspx).  
  
С точки зрения узлов фермы AD FS SQL Server группа доступности AlwaysOn заменяет один экземпляр SQL Server в качестве базы данных артефакта \/.  Прослушиватель группы доступности — это то, что клиент \(службе маркеров безопасности AD FS\) использует для подключения к SQL.  
...
> [!NOTE]  
> Для групп доступности AlwaysOn требуется, чтобы экземпляры SQL Server находились на отказоустойчивой кластеризации Windows Server \(узлах\) WSFC.  
...
-   Возможность чтения и записи на всех узлах \(не только основной\)  
  
-   Меньшие объемы данных, реплицируемых асинхронно во избежание задержки системы  
  
На следующей схеме показаны географически избыточные AD FS фермы SQL Server с репликацией слиянием \(1 издатель, два подписчика\):  
  
![ферма серверов, использующая SQL](media/ADFSSQLGeoRedundancy3.png)  
  
**Основные рекомендации по развертыванию для использования AD FS с SQL Serverной репликацией слиянием \(номера заметок на схеме выше\)**  
...
-   Если в группе доступности AlwaysOn, содержащей базу данных, которая является подписчиком репликации, произошел сбой, то подписка на репликацию завершится ошибкой. Для восстановления репликации администратор репликации должен вручную перенастроить подписчик.  См. SQL Server описание конкретной проблемы на [подписчиках репликации и группы доступности AlwaysOn \(SQL Server\)](https://technet.microsoft.com/library/hh882436.aspx) и общие инструкции по поддержке групп доступности AlwaysOn с параметрами репликации [репликацией, отслеживание изменений, отслеживание измененных данных и группы доступности AlwaysOn \(](https://technet.microsoft.com/library/hh403414.aspx)SQL Server\).  
