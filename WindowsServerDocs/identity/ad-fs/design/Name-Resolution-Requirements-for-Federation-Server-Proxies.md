...
Чтобы клиенты могли перенаправляться на сервер федерации учетных записей при попытке разрешить fs.fabrikam.com, в сети периметра настроен один узел \(\) записью ресурса для \(fs.fabrikam.com\) и IP-адрес сервера федерации учетной записи в корпоративную сеть. Это позволяет прокси-серверу федерации учетной записи разрешить имя узла fs.fabrikam.com на сервер федерации учетных записей, а не в себя, как это происходит при попытке найти fs.fabrikam.com с помощью DNS-службы, чтобы прокси-сервер федерации мог обмениваться данными с сервером федерации.  
