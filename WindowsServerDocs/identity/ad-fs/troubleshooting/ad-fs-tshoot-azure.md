...
1. Убедитесь, что ваш личный домен проверен, щелкнув домен рядом с параметром Федерация в портал Azure.
   ![](media/ad-fs-tshoot-azure/azure3.png)

2. Наконец, необходимо проверить [DNS](ad-fs-tshoot-dns.md) и убедиться, что AD FS серверы или серверы WAP разрешаются из Интернета.  Убедитесь, что это разрешение и вы можете переходить к нему.
3. Кроме того, для получения этих сведений можно также использовать PowerShell командлет `Get-AzureADDomain`.
...
В разделе **/адфс/ЛС/веб.конфиг**убедитесь, что указана запись для типа проверки подлинности.
...
- токен для Azure AD должен содержать следующие обязательные утверждения:
    - WSFED 
        - УЧАСТНИКА Значение этого утверждения должно соответствовать имени участника-пользователя (UPN) пользователей в Azure AD.
...
Дополнительные сведения см. [в статье Использование поставщика удостоверений SAML 2,0 для реализации единого входа](https://technet.microsoft.com/library/dn641269.aspx).
...
Чтобы устранить эту проблему, воспользуйтесь инструкциями в статье [Обновление сертификатов федерации для Office 365 и Azure Active Directory](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-o365-certs).
