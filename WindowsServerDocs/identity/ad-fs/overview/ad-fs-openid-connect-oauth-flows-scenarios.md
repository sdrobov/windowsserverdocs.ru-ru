...
|Параметр|Описание| 
|-----|-----|
|access_token|Включается, если response_type включает @ no__t-0.|
|token_type|Включается, если response_type включает @ no__t-0. Всегда будет носителя.| 
|expires_in| Включается, если response_type включает @ no__t-0. Указывает количество секунд, в течение которых маркер является допустимым, для кэширования.| 
|scope| Указывает области, для которых ACCESS_TOKEN будет допустимым.|  
|id_token|Включается, если response_type включает @ no__t-0. Подписанный JSON Web Token (JWT). Приложение может декодировать сегменты этого маркера, чтобы запросить сведения о пользователе, выполнившего вход. Приложение может кэшировать значения и отображать их, но не зависит от ограничений авторизации или безопасности.| 
...
|login_hint|необязательно|Можно использовать для предварительной заполнения поля имя пользователя или адрес электронной почты на странице входа пользователя, если вы уже знакомы с их именем пользователя заранее. Часто приложения будут использовать этот параметр во время повторной проверки подлинности, уже извлекли имя пользователя из предыдущего входа, `upn`используя утверждение `id_token`из.|
|domain_hint|необязательно|Если этот параметр включен, процесс обнаружения на основе домена, проходящего пользователем на странице входа, будет пропущен, что приведет к немного более быстрому взаимодействиям с пользователем.|
|code_challenge_method|необязательно|Метод, используемый для кодирования code_verifier для параметра code_challenge. Может принимать одно из следующих значений: </br>-Plain </br>- S256 </br>Если исключение исключено, предполагается, что code_challenge является открытым текстом, если включен @ no__t-0 @ no__t-1is. AD FS поддерживает как простые, так и S256. Дополнительные сведения см. в [документе RFC PKCE](https://tools.ietf.org/html/rfc7636).|
...
|code_verifier|необязательно|Тот же `code_verifier`, который использовался для получения код авторизации. Требуется, если PKCE использовался в запросе на предоставление кода авторизации. Дополнительные сведения см. в [документе RFC PKCE](https://tools.ietf.org/html/rfc7636).</br>Примечание. применяется к AD FS 2019 и более поздних версий.| 
