...
### <a name="scenario-1-block-all-external-access-to-office-365"></a>Сценарий 1. Блокировать весь внешний доступ к Office 365
...
### <a name="scenario-2-block-all-external-access-to-office-365-except-exchange-activesync"></a>Сценарий 2. Блокировать весь внешний доступ к Office 365, кроме Exchange ActiveSync
...
### <a name="scenario-3-block-all-external-access-to-office-365-except-browser-based-applications"></a>Сценарий 3. Блокировать весь внешний доступ к Office 365, за исключением браузерных приложений
...
### <a name="scenario-4-block-all-external-access-to-office-365-for-designated-active-directory-groups"></a>Сценарий 4. Блокировать весь внешний доступ к Office 365 для назначенных групп Active Directory

В следующем примере предоставляется доступ с внутренних клиентов на основе IP-адреса. Он блокирует доступ клиентов, находящихся за пределами корпоративной сети, у которых есть внешний IP-адрес клиента, за исключением тех, которые входят в указанную группу Active Directory. набор правил основан на правиле авторизации выдачи по умолчанию, озаглавленном "разрешение доступа к Все пользователи. Выполните следующие действия, чтобы добавить правило авторизации выдачи в отношение доверия с проверяющей стороной платформы удостоверений Microsoft Office 365 с помощью мастера правил утверждений:
...
|                                                         Используется для установления запроса от клиента с IP-адресом в определенном допустимом диапазоне.                                                         | НЕ существует ([type = = "<https://schemas.microsoft.com/2012/01/requestcontext/claims/x-ms-forwarded-client-ip>", значение = ~ "предоставленное клиентом регулярное выражение для общедоступного IP-адреса"]) |
...
Это дает возможность ввести любое количество адресов. Если необходимо разрешить диапазон адресов, например 192.168.1.1 – 192.168.1.25, сопоставление должно выполняться по символу: \b192 @ no__t-0168 @ no__t-11 @ no__t-2 ([1-9] | 1 [0-9] | 2 [0-5]) \b
...
При совпадении с блоком 192 можно написать аналогичное выражение для 10 блока: \b10 @ no__t-00 @ no__t-10 @ no__t-2 ([1-9] | 1 [0-4]) \b

И поместив их вместе, следующее выражение должно соответствовать всем адресам для "192.168.1.1 ~ 25" и "10.0.0.1 ~ 14": \b192 @ no__t-0168 @ no__t-11 @ no__t-2 ([1-9] | 1 [0-9] | 2 [0-5]) \b | \b10 @ no__t-30 @ no__t-40 @ no__t-5 ([1-9] | 1 [0-4]) \b
