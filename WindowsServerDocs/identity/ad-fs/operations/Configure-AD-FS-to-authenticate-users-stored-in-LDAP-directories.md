...
3. Наконец, необходимо зарегистрировать хранилище LDAP с AD FS в качестве отношения доверия с локальным поставщиком утверждений с помощью командлета **Add-адфслокалклаимспровидертруст** :
   ...
   В приведенном выше примере вы создаете отношение доверия с локальным поставщиком утверждений под названием "поставщики". Вы указываете сведения о соединении для AD FS подключиться к каталогу LDAP, который это отношение доверия с локальным поставщиком утверждений представляет, назначив параметру `$vendorDirectory` значение `-LdapServerConnection`. Обратите внимание, что на первом шаге вы назначили `$vendorDirectory` строку подключения для использования при подключении к конкретному каталогу LDAP. Наконец, вы указываете, что атрибуты LDAP `$GivenName`, `$Surname` и `$CommonName` (которые сопоставлены с утверждениями AD FS) используются для управления условным доступом, включая политики многофакторной идентификации и правила авторизации выдачи, а также для выдачи с помощью утверждений в маркерах безопасности, выданных AD FS. Чтобы использовать активные протоколы, такие как WS-Trust с AD FS, необходимо указать параметр Организатионалаккаунтсуффикс, который позволяет AD FS распознавать отношения доверия между локальными поставщиками утверждений при обслуживании активного запроса авторизации.
