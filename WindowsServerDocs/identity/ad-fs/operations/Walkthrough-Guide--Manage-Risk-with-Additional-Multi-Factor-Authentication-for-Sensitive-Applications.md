...
title: Пошаговое руководство — управление рисками с помощью дополнительной многофакторной идентификации для уязвимых приложений
description: ''
author: billmath
ms.author: billmath
manager: femila
...
-   Домен Active Directory с помощью тестового пользователя и учетные записи групп, работающих под управлением Windows Server 2012 R2 или домен Active Directory под управлением Windows Server 2008, Windows Server 2008 R2 или Windows Server 2012 со схемой, обновленной до Windows Server 2012 R2

-   На сервере федерации под управлением Windows Server 2012 R2
...
Инструкции о том, как настроить эту среду, см. в разделе [Настройка лабораторной среды для AD FS в Windows Server 2012 R2](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md).

## <a name="BKMK_2"></a>Шаг 2. Проверка стандартного механизма проверки подлинности через службы AD FS
В этом шаге вы проверите стандартный механизм управления доступом через службы AD FS (**проверка подлинности с помощью форм** для экстрасети и **проверка подлинности Windows** для интрасети), который перенаправляет пользователя на страницу входа AD FS, где ему нужно ввести действительные учетные данные, после чего пользователю будет предоставлен доступ к приложению. Можно использовать **Robert Hatley** учетной записи AD и **claimapp** пример приложения, настроенного в [Настройка лабораторной среды для AD FS в Windows Server 2012 R2](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md).
...
2.  Введите учетные данные из **Robert Hatley** в созданную учетную запись AD [Настройка лабораторной среды для AD FS в Windows Server 2012 R2](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md).
...
Настройка многофакторной проверки Подлинности в AD FS в Windows Server 2012 R2 двух этапов:
...
-   Выберите [аутентификации на основе сертификата](../../ad-fs/operations/Walkthrough-Guide--Manage-Risk-with-Additional-Multi-Factor-Authentication-for-Sensitive-Applications.md#BKMK_7) методов, доступных в AD FS в Windows Server 2012 R2 по умолчанию
...
#### <a name="BKMK_7"></a>Проверка подлинности сертификатов
...
1.  [Создание поставщика многофакторной проверки подлинности с помощью Windows Azure Portal](../../ad-fs/operations/Walkthrough-Guide--Manage-Risk-with-Additional-Multi-Factor-Authentication-for-Sensitive-Applications.md#BKMK_a)

2.  [Загрузить Windows Server многофакторная Идентификация Azure](../../ad-fs/operations/Walkthrough-Guide--Manage-Risk-with-Additional-Multi-Factor-Authentication-for-Sensitive-Applications.md#BKMK_b)

3.  [Установите сервер Azure многофакторной проверки подлинности Windows на сервере федерации](../../ad-fs/operations/Walkthrough-Guide--Manage-Risk-with-Additional-Multi-Factor-Authentication-for-Sensitive-Applications.md#BKMK_c)
...
##### <a name="BKMK_a"></a>Создание поставщика многофакторной проверки подлинности с помощью Windows Azure Portal
...
6.  Заполните следующие поля и выберите команду **Создать**.
    ...
    2.  **Модель использования** -модель использования поставщика многофакторной проверки подлинности.
        ...
        -   **На проверку подлинности** — модель покупки с оплатой на проверку подлинности. Обычно используется в сценариях применения Windows Azure Multi-Factor Authentication в потребительском приложении.

        -   **На включенного пользователя** — модель покупки с оплатой на включенного пользователя.  Обычно используется в сценариях приложений для сотрудников, например Office 365.
    ...
    3.  **Каталог** -клиента Windows Azure Active Directory, с которой связан поставщик многофакторной проверки подлинности. Этот параметр является необязательным, так как не требуется, чтобы поставщик был связан с Windows Azure Active Directory при обеспечении безопасности локальных приложений.
...
##### <a name="BKMK_b"></a>Загрузить Windows Server многофакторная Идентификация Azure
...
##### <a name="BKMK_c"></a>Установите сервер Azure многофакторной проверки подлинности Windows на сервере федерации
...
6.  В списке **Пользователи** выберите учетную запись **Robert Hatley** и нажмите кнопку **Проверка**. В окне **Тестовый пользователь** введите учетные данные для учетной записи **Robert Hatley** . Когда зазвонит мобильный телефон, нажмите клавишу «#», чтобы завершить проверку учетной записи.
...
Чтобы активировать многофакторную проверку подлинности, вы должны настроить ее политику на сервере федерации. В этом пошаговом руководстве, согласно политике многофакторной проверки Подлинности **Robert Hatley** необходима учетная запись, чтобы пройти Многофакторную проверку подлинности, так как он входит в **процент** группы, которую вы настроили в [Настройка лабораторной среды для служб AD FS Windows Server 2012 R2](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md).
...
##### <a name="to-configure-the-mfa-policy-based-on-users-group-membership-data-for-claimapp--via-the-ad-fs-management-console"></a>Настройка политики многофакторной проверки Подлинности на основе данных членства в группах пользователя для «claimapp» с помощью консоли управления AD FS
...
3.  В окне **Изменение отношения доверия с проверяющей стороной для claimapp** нажмите кнопку **Добавить** рядом со списком **Пользователи/Группы** . Введите в **процент** для имени группы AD, которую вы создали в [Настройка лабораторной среды для AD FS в Windows Server 2012 R2](../../ad-fs/deployment/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md)и нажмите кнопку **проверить имена**, а когда — имя устранены, нажмите кнопку **ОК**.

4.  Нажмите кнопку **ОК** в окне **Изменение отношения доверия с проверяющей стороной для claimapp** .

##### <a name="to-configure-the-mfa-policy-based-on-users-group-membership-data-for-claimapp--via-windows-powershell"></a>Настройка политики многофакторной проверки Подлинности на основе данных членства в группах пользователя для «claimapp» с помощью Windows PowerShell
...
2.  В том же окне команд Windows PowerShell выполните следующую команду:
    ...
    > [!NOTE]
    > Обязательно замените < group_SID > значением идентификатора безопасности для вашей группы AD **процент**.
...
2.  Введите учетные данные для учетной записи Active Directory **Robert Hatley** .
    ...
    Если вы настроили аутентификацию сертификата как дополнительного метода проверки подлинности, то текст сообщения по умолчанию будет **выберите сертификат, который вы хотите использовать для проверки подлинности. Если отменить операцию, закройте браузер и повторите попытку.**
