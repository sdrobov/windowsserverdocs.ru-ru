...
###  <a name="scenario1"></a>Сценарий 1. Блокировать весь внешний доступ к Office 365  
...
7.  Чтобы сохранить новые правила, в диалоговом окне **изменение правил утверждений** нажмите кнопку **ОК**. Полученный список должен выглядеть следующим образом.  
     ...
     ![Правила аутентификации выдачи](media/Access-Control-Policies-W2K12/clientaccess1.png "ADFS_Client_Access_1")  

###  <a name="scenario2"></a>Сценарий 2. Блокировать весь внешний доступ к Office 365, кроме Exchange ActiveSync  
...
###  <a name="scenario3"></a>Сценарий 3. Блокировать весь внешний доступ к Office 365, за исключением браузерных приложений  
...
###  <a name="scenario4"></a>Сценарий 4. Блокировать весь внешний доступ к Office 365, кроме назначенных групп Active Directory  
 В следующем примере предоставляется доступ с внутренних клиентов на основе IP-адреса. Он блокирует доступ клиентов, находящихся за пределами корпоративной сети, у которых есть внешний IP-адрес клиента, за исключением тех, которые входят в указанную группу Active Directory. выполните следующие действия, чтобы добавить правильные правила авторизации выдачи в  **Microsoft Office** отношение доверия с проверяющей стороной платформы удостоверений 365 с помощью мастера правил утверждений:  
...
##  <a name="buildingip"></a>Создание выражения диапазона IP-адресов  
...
- 10.0.0.1 – 10.0.0.14  
  ...
  Таким образом, мы можем сопоставить два разных IP-адреса с выражением или следующим образом: \b # #\\#. # #\\#. # #\\#. # # #&#124;\b \b # #\\#. #\\# #. #\\# #. # # # \b  

  Таким образом, пример для сопоставления только двух адресов (например, 192.168.1.1 или 10.0.0.1) будет выглядеть следующим образом\\: \b192\\.\\168 1 $&#124;\b10\\0\\0\\. 1 \ b  

  Это дает возможность ввести любое количество адресов. Если необходимо разрешить диапазон адресов, например 192.168.1.1 – 192.168.1.25, сопоставление должно выполняться по символу: \b192 @ no__t-0.168 @ no__t-1.1 @ no__t-2. ([1-9]&#124;1 [0-9]&#124;2 [0-5]) \b  

  Имейте в виду следующее:  

- IP-адрес рассматривается как строка, а не число.  

- Правило разбивается, как показано ниже:\\\b192.\\168\\-0,1.  
...
- При совпадении с блоком 192 можно написать аналогичное выражение для 10 блока: \b10 @ no__t-0.0 @ no__t-1.0 @ no__t-2. ([1-9]&#124;1 [0-4]) \b  

- И поместив их вместе, следующее выражение должно соответствовать всем адресам для "192.168.1.1 ~ 25" и "10.0.0.1 ~ 14": \b192 @ no__t-0.168 @ no__t-1.1 @ no__t-2. ([1-9]&#124;1 [0-9]&#124;2 [0-5]) \b&#124;\b10 @ no__t-6.0 @ no__t-7.0 @ no__t-8. ([1-9]&#124;1 [0-4]) \b  
