...
>[!NOTE] 
>Если используется встроенная база данных Windows (WID), это средство необходимо запустить на сервере-источнике AD FS.  С помощью командлета `Get-AdfsSyncProperties` PowerShell можно определить, является ли сервер сервером-источником.
