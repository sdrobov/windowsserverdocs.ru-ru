...
>[!NOTE]
>Функциональные возможности настройки заголовков ответа HTTP-безопасности (за исключением заголовков CORS) с помощью командлетов: `Get-AdfsResponseHeaders` и `Set-AdfsResponseHeaders` были отAD FSы до 2016. Вы можете добавить функциональные возможности в AD FS 2016, установив [KB4493473](https://support.microsoft.com/help/4493473/windows-10-update-kb4493473) и [KB4507459](https://support.microsoft.com/help/4507459/windows-10-update-kb4507459). 
...
Заголовки ответов включаются в исходящий HTTP-ответ, который отправляется AD FS в браузер. Заголовки могут быть перечислены с помощью командлета `Get-AdfsResponseHeaders`, как показано ниже.  

![Ответ заголовка](media/customize-http-security-headers-ad-fs/header1.png)

Атрибут `ResponseHeaders` на приведенном выше снимке экрана определяет заголовки безопасности, которые будут включаться AD FS в каждом HTTP-ответе. Заголовки ответа будут отправляться, только если `ResponseHeadersEnabled` имеет значение `True` (значение по умолчанию). Можно задать значение `False`, чтобы предотвратить AD FS включая любые заголовки безопасности в HTTP-ответе. Однако это не рекомендуется.  Для этого используйте следующую команду:
...
- **max-age =&lt;&gt;времени окончания** срока действия (в секундах) указывает, как долго сайт должен быть доступен только по протоколу HTTPS. По умолчанию и рекомендуемое значение — 31536000 секунд (1 год).  
...
По умолчанию заголовок включен и `max-age` установлен в 1 год; Однако администраторы могут изменить `max-age` (не рекомендуется использовать значение max-age) или включить HSTS для поддоменов с помощью командлета `Set-AdfsResponseHeaders`.  
...
По умолчанию заголовок включается в атрибут `ResponseHeaders`; Однако администраторы могут удалить заголовок с помощью командлета `Set-AdfsResponseHeaders`.  
...
Заголовок ответа безопасности HTTP используется для взаимодействия с браузером, может ли он отображать страницу в &lt;кадре&gt;/&lt;IFRAME&gt;. Для заголовка можно задать одно из следующих значений: 
...
- **allow-from <specified origin>** — страница будет отображаться в рамке только в том случае, если источник (например, https://www.). com) соответствует конкретному источнику в заголовке. 

#### <a name="x-frame-options-customization"></a>Настройка параметров кадра X  
По умолчанию заголовку будет присвоено значение Deny; Однако администраторы могут изменить это значение с помощью командлета `Set-AdfsResponseHeaders`.  
```PowerShell
...
По умолчанию для заголовка задается значение 1; Mode = Block; Однако администраторы могут изменить это значение с помощью командлета `Set-AdfsResponseHeaders`.  
