...
- **Екстранетлоккаутрекуирепдк &lt;Boolean&gt;** — при включении для блокировки экстрасети требуется основной контроллер домена (PDC). Если этот параметр отключен, блокировка экстрасети будет переключаться на другой контроллер домена в случае недоступности PDC.
...
- Чтобы избежать блокировки вредоносных учетных записей, убедитесь, что **ExtranetLockoutThreshold** < **порог блокировки учетной записи** и **Екстранетобсерватионвиндов** > **сбросить счетчик блокировки учетной записи** .
