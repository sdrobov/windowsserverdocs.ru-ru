...
2)  Убедитесь, что все брандмауэры перед AD FS или прокси веб-приложения (WAP) настроены на разрешение комбинации `hostname:port` для фермы AD FS. Чтобы выполнить этот шаг, вам потребуется обратиться к инженеру сети. 
...
Вы можете заметить, что некоторые устройства работают правильно, но другие устройства — нет. В этом случае, как правило, это результат неправильной подготовки сертификата пользователя на клиентском устройстве. Для этого сделайте следующее. 
...
Это редкие случаи.
1)  Если списки CRL являются очень длинными, при попытке загрузки может потребоваться время ожидания. В этом случае необходимо обновить "MaxFieldLength" и "Максрекуестбите" в соответствии с https://support.microsoft.com/help/820129/http-sys-registry-settings-for-windows
...
|           https://schemas.microsoft.com/2012/12/certificatecontext/extension/san           | Другое имя: имя участника =user@contoso.com, имя RFC822 =user@contoso.com |
