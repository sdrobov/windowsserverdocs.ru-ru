...
## <a name="BKMK_StagePS"></a>Этап RODC Windows PowerShell  
...
## <a name="BKMK_AttachPS"></a>Присоединение RODC Windows PowerShell  
...
Дополнительные сведения о центре администрирования Active Directory, см. в разделе [расширенные AD доменных служб Active Directory управления с помощью Active Directory центра администрирования &#40;Уровень200&#41; ](../../../ad-ds/get-started/adac/Advanced-AD-DS-Management-Using-Active-Directory-Administrative-Center--Level-200-.md) и просмотрите [Active Directory Центр администрирования: Приступая к работе](https://technet.microsoft.com/library/dd560651(WS.10).aspx).  
...
**Предварительное создание учетной записи контроллера домена только для чтения** ссылку в области задач Центр администрирования Active Directory эквивалентный командлет ADDSDeployment Windows PowerShell:  
...
> [!NOTE]  
> По умолчанию **- NoGlobalCatalog** значение $false. Это означает, что контроллер домена будет являться сервером глобального каталога в случае, если аргумент не задан.  
...
Также можно установить контроллер домена с помощью архивированных носителей, использовав параметр установки с носителя (IFM). При установке флажка **Установка с носителя** появляется возможность выбора носителя. Чтобы убедиться в том, что указанный путь является действительным путем к носителю, необходимо нажать кнопку **Проверить** . Носители, используемые параметром IFM, создаются с помощью системы архивации данных Windows Server или Ntdsutil.exe только из другого существующего компьютера с Windows Server 2012. Windows Server 2008 R2 или операционные системы более ранних версий не подходят для создания носителей для контроллера домена с Windows Server 2012.  В приложениях приведена дополнительная информация об изменениях в IFM. Если носители защищены SYSKEY, диспетчер сервера запрашивает пароль образа во время проверки.  
