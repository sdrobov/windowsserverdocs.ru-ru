...
|     62     |                                         В файле ответов отсутствует секция [DCInstall]                                          |                                                                                             *Only показан с помощью команды dcpromo/unattend, которая является устаревшей. См. более старую документацию.*                                                                                              |
...
|---------|------------------------------------------------------------------|  
|Симптомы|Возвращаемая ошибка:<br /><br />Код: не удалось проверить путь к носителю. Исключение при вызове "Жетдатабасеинфо" с аргументами "2". Недопустимая папка.|  
...
|---------|--------------------------------------------------------------------------------------------|  
|Симптомы|Диспетчер сервера возвращает следующую ошибку:<br /><br />Код-исключение при вызове "Днсоптион" с аргументами "6"<br /><br />Модуль Windows PowerShell ADDSDeployment возвращает следующую ошибку:<br /><br />Ошибка проверки кода разрешений пользователя. Необходимо указать имя домена, к которому принадлежит эта учетная запись пользователя.|  
...
|---------|---------------------------------------------------------------------------------------------------|  
|Симптомы|Если использовать средство Dism.exe для удаления роли доменных служб Active Directory перед безопасным понижением роли контроллера домена, сервер перестает загружаться нормально и выводится следующее сообщение об ошибке:<br /><br />Состояние кода: 0x000000000<br />контактные Произошла непредвиденная ошибка.|  
...
|---------|--------------------------------------------------------------------|  
|Симптомы|При повышении роли с помощью Windows PowerShell ADDSDeployment возвращается следующая ошибка:<br /><br />Code-Test. Верифидкпромокоре. DCPromo. General. 74<br /><br />Сбой проверки предварительных требований для повышения роли контроллера домена. Указан недопустимый режим работы домена|  
|Решение и примечания|Не указывайте режим работы леса Win2012, не указав *также* режим работы домена Win2012. Вот пример, который будет работать без ошибок:<br /><br />Code--леса Win2012-домаинмоде Win2012]|  
...
|Решение и примечания|Это ограничение диспетчера сервера. Чтобы получать информацию, используйте командлет Windows PowerShell ADDSDeployment:<br /><br />Код — удаление — аддсдомаинконтроллер|  
...
|Симптомы|При использовании модуля Windows PowerShell ADDSDeployment для повышения роли нового контроллера домена только для чтения с помощью промежуточной учетной записи компьютера появляется следующее сообщение об ошибке:<br /><br />Не удается разрешить набор параметров кода с использованием указанных именованных параметров.    <br />InvalidArgument: параметербиндинжексцептион<br />    + Фулликуалифиедеррорид: Амбигуауспараметерсет, Microsoft. DirectoryServices. Deployment. PowerShell. Commands. install|  
|Решение и примечания|Не указывайте параметры, которые уже определены в предварительно созданной учетной записи контроллера домена только для чтения. К ним можно отнести следующие.<br /><br />Код — реадонлиреплика<br />-инсталлднс<br />-донотконфигуреглобалкаталог<br />-sitename<br />-инсталлднс|  
...
|Симптомы|Понижение роли контроллера домена завершается без проблем, но в журнале dcpromo log содержится следующее сообщение об ошибке:<br /><br />Код-[Ошибка] не удалось установить безопасность для файлов сервера с 2|  
...
|Симптомы|Если вы используете аргумент **-whatif** при настройке контроллера домена с явным или неявным аргументом **-installdns:$true**, то в итоговых выходных данных будет указано:<br /><br />Код — DNS-сервер: Не|  
...
|Симптомы|Если после повышения роли нового контроллера домена вы выходите, а затем пытаетесь войти в интерактивном режиме, происходит следующая ошибка:<br /><br />Код-недостаточно места для обработки этой команды|  
...
|Решение и примечания|Имеется несколько сайтов доменных служб Active Directory и по крайней мере у одного нет подсетей. Создаваемый контроллер домена относится к одной из этих подсетей. Необходимо вручную выбрать подсеть в раскрывающемся меню "Имя сайта". Следует также проверить все сайты Active Directory с помощью средства DSSITE.MSC или воспользоваться следующей командой Windows PowerShell, чтобы найти все сайты без подсетей:<br /><br />Код-Get-адрепликатионсите-Filter \* — подсети свойств &#124; , где-Object {! $ _. подсети-eq "\*" &#124; } Format — имя таблицы|  

|||  
|-|-|  
|Проблемы|Повышение или понижение роли завершается сбоем с сообщением "Невозможно запустить службу"|  
|Симптомы|При попытке повысить роль, понизить роль или клонировать контроллер домена происходит следующая ошибка:<br /><br />Код — служба не может быть запущена, так как она отключена или не имеет связанных с ней устройств (0x80070422).<br /><br />Ошибка может быть интерактивной, происходить как событие или регистрироваться в журнале, например dcpromoui.log или dcpromo.log.|  
...
|-|-|  
|Симптомы|При повышении роли контроллера домена с помощью команды dcpromo /unattend с использованием следующего файла ответов:<br /><br />Приведен<br /><br />[ДЦинсталл]<br />Невдомаин = лес<br /><br />Репликаорневдомаин = домен<br /><br />Невдомаинднснаме = Corp. contoso. com<br /><br />Сафемодеадминпассворд = Safepassword@6<br /><br />Домаиннетбиоснаме = Corp<br /><br />Днсоннетворк = да<br /><br />Аутоконфигднс = да<br /><br />Ребутонсукцесс = Ноанднопромптеисер<br /><br />Ребутонкомплетион = нет<br /><br />*Домаинлевел = 0*<br /><br />*Форестлевел = 0*<br /><br />происходит сбой, а в журнале dcpromoui.log регистрируются следующие ошибки:<br /><br />Code-дкпромауи EA 4.5 B8 0089 13:31:50.783 ввод Каргументсспек:: Валидатеаргумент Домаинлевел<br /><br />дкпромауи EA 4.5 B8 008A 13:31: значение 50.783 для Домаинлевел равно 0<br /><br />дкпромауи EA 4.5 B8 008B 13:31: код выхода 50.783 — 77<br /><br />дкпромауи EA 4.5 B8 008C 13:31:50.783 указанный аргумент является недопустимым.<br /><br />дкпромауи EA 4.5 B8 008D 13:31:50.783 Закрытие журнала<br /><br />дкпромауи EA 4.5 B8 0032 13:31: код выхода 50.830 — 77<br /><br />Уровень 0 соответствует Windows 2000 и не поддерживается в Windows Server 2012.|  
...
|Проблемы|Продвижение "зависает" при создании объекта параметров NTDS, никогда не завершается|  
|-|-|  
