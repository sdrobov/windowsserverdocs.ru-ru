...
-   Настройка объектов групповой политики для ограничения учетных записей администраторов на контроллерах домена  
    -   В каждом домене леса следует изменить объект групповой политики контроллеров домена по умолчанию или политику, связанную с подразделением контроллеров домена, чтобы добавить учетную запись администратора каждого домена в следующие права пользователя в **параметрах компьютера \ политики \ Назначение прав Policies\User безопасности \ локальные права**:   
        -   Отказ в доступе к компьютеру из сети  
...
2.  В дереве консоли разверните узел <Forest> \ домены @ no__t-1 @ no__t-2, а затем **Групповая политика объекты** (где <Forest> — имя леса, а <Domain> — имя домена, в котором нужно создать групповая политика).  
...
7.  Настройте права пользователя, чтобы запретить учетной записи администратора доступ к серверам и рабочим станциям по сети, выполнив следующие действия.  
    ...
    3.  Введите **Администратор**, щелкните **Проверить имена**и нажмите кнопку **ОК**. Убедитесь, что учетная запись отображается в формате <DomainName> \ имя пользователя, как показано на следующем снимке экрана.  
