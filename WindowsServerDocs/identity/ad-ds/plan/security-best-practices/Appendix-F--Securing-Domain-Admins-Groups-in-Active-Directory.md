...
2.  В дереве консоли разверните узел \<лес\>\\домены\\\<домен\>, а затем **Групповая политика объекты** (где \<лес\> — это имя леса, \<домен\> — имя домена, в котором нужно задать групповая политика).  
...
4.  В диалоговом окне **Создание объекта групповой политики** введите \<имя объекта групповой политики\>и нажмите кнопку **ок** (где \<имя объекта групповой политики\> — имя этого объекта групповой политики).  

    ![группы администраторов защищенного домена](media/Appendix-F--Securing-Domain-Admins-Groups-in-Active-Directory/SAD_64.gif)  

5.  В области сведений щелкните правой кнопкой мыши \<имя объекта групповой политики\>и выберите команду **изменить**.  
