...
> [!NOTE]  
> Microsoft Windows по умолчанию и рекомендации базовых показателей были взяты из [средства Microsoft Security соответствию Manager](https://technet.microsoft.com/library/cc677002.aspx).  
...
|НЕТ|**Не** включайте в общих сценариях|  
|НАЛИЧИИ|Включить, если это необходимо, в конкретном сценарии или на компьютере установлены роль или компонент, для которых требуется аудит.|  
|DC|Включить на контроллерах домена|  
|Свобод|Нет рекомендаций|  

**Рекомендации по параметрам аудита Windows 10, Windows 8 и Windows 7**  

**Политика аудита**  

|Категория или подкатегория политики аудита|Windows по умолчанию<br /><br />Успешное завершение|Рекомендация по базовому плану<br /><br />Успешное завершение|Более надежная рекомендация<br /><br />Успешное завершение|  
|----------------------------------------|------------------------------------------|--------------------------------------------------|--------------------------------------------------|  
...
Полный список событий, которые следует включать при мониторинге подписывания нарушений, см. в разделе [Appendix L: События для отслеживания @ no__t-0.  
...
См. [Appendix L: События для мониторинга @ no__t-0, чтобы получить список рекомендуемых событий для отслеживания, их оценки критических результатов и сводку сообщений о событиях.  
...
-   Изменения свойств и членства в следующих группах AD DS: Администраторы предприятия (EA), Администраторы домена (DA), администратор (BA) и Администраторы схемы (SA)  
...
-   [Аудит доступа к глобальным объектам — это волшебная](http://blogs.technet.com/b/askds/archive/2011/03/10/global-object-access-auditing-is-magic.aspx) информация о настройке и использовании расширенной конфигурации политики аудита, добавленной в Windows 7 и windows Server 2008 R2.  

-   [Введение в аудит изменений в windows 2008](http://blogs.technet.com/b/askds/archive/2007/10/19/introducing-auditing-changes-in-windows-2008.aspx) — введение в аудит изменений, внесенных в Windows 2008.  

-   Полезные [Советы по аудиту в Vista и 2008](http://blogs.technet.com/b/askds/archive/2007/11/16/cool-auditing-tricks-in-vista-and-2008.aspx) — объясняются интересные новые функции аудита в Windows Vista и windows Server 2008, которые можно использовать для устранения неполадок или для просмотра данных в вашей среде.  

-   [Один и тот же магазин для аудита в Windows server 2008 и Windows Vista](http://blogs.technet.com/b/askds/archive/2008/03/27/one-stop-shop-for-auditing-in-windows-server-2008-and-windows-vista.aspx) — содержит компиляцию функций аудита и информации, содержащейся в windows Server 2008 и Windows Vista.  
...
**Высоком** Идентификаторы событий с высокой степенью важности должны всегда и немедленно получать оповещения и исследоваться.  

**Высокое** Идентификатор события со средним уровнем важности может указывать на вредоносные действия, но он должен сопровождаться некоторыми другими аномальными ошибками (например, необычное число, происходящее в определенный период времени, непредвиденные случаи или случаи возникновения на компьютере, на котором обычно не ожидается запись события в журнал.) Событие среднего уровня важности также можно собирать в виде метрики и сравнивать с течением времени.  

**Низшую** И события с низким уровнем важности не должны выискать внимание или вызывать предупреждения, если они не связаны с событиями среднего или высокого уровня важности.  
...
См. [Appendix L: События для мониторинга @ no__t-0 для просмотра списка рекомендуемых событий, которые необходимо отслеживать, их оценки важности и сводки сообщений о событиях.  
