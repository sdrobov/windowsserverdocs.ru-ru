...
|Machine|Конфигурация 1|Configuration 2|
|-------|---------------|---------------|
...
2. Откройте редактор реестра. Чтобы открыть редактор реестра, нажмите кнопку **Пуск**, выберите команду **выполнить**, введите **regedit**и нажмите кнопку ОК. Если появится диалоговое окно **контроля учетных записей** , подтвердите, что отображаемое в нем действие именно то, которое требуется, и нажмите кнопку **Да**. В редакторе реестра разверните следующий путь: **HKEY\_локальный\_компьютер\\SYSTEM\\CurrentControlSet\\Services\\NTDS\\параметры**. Найдите значение с именем **DSA Previous Restore**. Если это так, запишите значение параметра. Если значение отсутствует, параметр равен значению по умолчанию, что равно нулю. Не добавляйте значение, если оно не отображается.
...
## <a name="read-only-domain-controllers"></a>Контроллеры доменов только для чтения
