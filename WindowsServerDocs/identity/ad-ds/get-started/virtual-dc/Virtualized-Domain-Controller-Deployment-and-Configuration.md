...
-   [Виртуализированные домена при клонировании контроллера](../../../ad-ds/get-started/virtual-dc/Virtualized-Domain-Controller-Deployment-and-Configuration.md#BKMK_VDCCloning)  
  
    Этот раздел содержит подробное описание всего процесса клонирования виртуализированного контроллера домена.  
  
-   [Восстановление виртуализированного контроллера домена Safe](../../../ad-ds/get-started/virtual-dc/Virtualized-Domain-Controller-Deployment-and-Configuration.md#BKMK_VDCSafeRestore)  
...
|||  
|-|-|  
|**Программный продукт для виртуализации**|**Поддерживает виртуализированные контроллеры домена и VMGID**|  
|**Сервер Microsoft Windows Server 2012 с компонентом Hyper-V**|Да|  
|**Microsoft Windows Server 2012 Hyper-V Server**|Да|  
|**Функции Microsoft Windows 8 с помощью клиента Hyper-V**|Да|  
|**Windows Server 2008 R2 и Windows Server 2008**|Нет|  
|**Решения для виртуализации сторонних разработчиков**|Обратитесь к поставщику|  
...
Дополнительные сведения о номера последовательного обновления и устаревших объектах см. в разделе [операциями устранения неполадок Active Directory, завершающимися с ошибкой 8606: «Задано недостаточно атрибутов задано для создания объекта»](https://support.microsoft.com/kb/2028495).  
  
## <a name="BKMK_VDCCloning"></a>Виртуализированные домена при клонировании контроллера  
...
-   Шаг 2. Убедитесь, что роли эмулятора PDC размещается на контроллере домена под управлением Windows Server 2012, на, он подключен к сети и доступен с клонированного контроллера домена во время операции клонирования.  
...
Если в качестве низкоуровневой оболочки используется Microsoft Hyper-V, убедитесь, что он работает под управлением Windows Server 2012. Это можно проверить с помощью диспетчера устройств.  
...
> [!NOTE]  
> Windows Server 2012 Hyper-V поддерживает новые возможности импорта и экспорта, выходящие за рамки данного материала. Дополнительные сведения о статьях на сайте TechNet.  
  
#### <a name="exporting-merged-disks-using-hyper-v"></a>Экспорт объединенных дисков с помощью Hyper-V  
Последний вариант — использовать параметры слияния и преобразования разделов внутри Hyper-V. Это позволяет создать копию имеющейся структуры диска — даже при включении в нее файлов моментальных снимков AVHD/AVHDX — в виде одного нового диска. Как и сценарий ручного копирования дисков, обычно это используется для простых виртуальных машин, использующих только один диск, например C:\\. Единственное его преимущество по сравнению с ручным копированием заключается в том, что вам не нужно предварительно удалять моментальные снимки. Данная операция неизбежно будет выполняться дольше простого удаления снимков и копирования дисков.  
...
Чтобы создать клонированный контроллер домена с именем CloneDC1 в автономном режиме, в узел с названием «REDMOND» со статическим адресом IPv4, введите:  
...
>   
> **Get-VM - VMName**   
>  ***тест vm* | Get-VMNetworkAdapter | штат Флорида \***  
...
>   
> [Принудительное выполнения заслуживающей и не заслуживающей доверия синхронизации для папки SYSVOL с репликацией DFSR (как «D4/D2» для FRS)](https://support.microsoft.com/kb/2218556)  
