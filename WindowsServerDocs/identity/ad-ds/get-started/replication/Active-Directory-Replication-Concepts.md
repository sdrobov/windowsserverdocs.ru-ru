...
Для Windows Server 2008 RODC нормальная работа KCC обеспечивает некоторые средства для перераспределения, что устраняет необходимость использовать дополнительное средство, например Адлб. exe. Новые функции включены по умолчанию. Его можно отключить, добавив следующий набор разделов реестра на RODC:  
