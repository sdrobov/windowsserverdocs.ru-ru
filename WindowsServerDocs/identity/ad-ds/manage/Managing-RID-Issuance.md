...
-   [Устранение неполадок с выдачей RID](../../ad-ds/manage/Managing-RID-Issuance.md#BKMK_Tshoot)  
...
В Windows Server 2012 решаются проблемы, связанные с выделением идентификаторов RID, которые стали актуальны только тогда, когда среды Active Directory достигли определенного возраста. К ним относятся лучше ведение журнала событий, более подходящие предельные значения и возможность — при возникновении нештатной ситуации - дважды общий размер глобального пространства RID для домена.  
...
Контроллеры домена Windows Server 2003 и Windows Server 2008 не могут выдавать идентификаторы RID, если в глобальном пуле RID разблокирован 31-й<sup></sup> разряд. Контроллеры домена Windows Server 2008 R2 *можно* использовать 31<sup>st</sup> разрядные идентификаторы RID, *, но только если* исправление [KB 2642658](https://support.microsoft.com/kb/2642658) установлен. Неподдерживаемые контроллеры домена без установленного исправления рассматривают разблокированный глобальный пул RID как исчерпанный.  
...
6.  В **изменить атрибут записи**, тип:  
...
В предыдущих версиях операционной системы Windows Server при отсутствии атрибута rIDSetReferences происходила утечка пула RID. Чтобы устранить эту проблему в контроллерах домена под управлением Windows Server 2008 R2, установите исправление из [статье базы Знаний 2618669](https://support.microsoft.com/kb/2618669).  
...
## <a name="BKMK_Tshoot"></a>Устранение неполадок с выдачей RID  
...
3.  Упоминаются ли в полученном сообщении об ошибке относительные идентификаторы, но при этом оно является недостаточно конкретным? Пример: "Не удается создать объект, так как службе каталогов не удалось выделить относительный идентификатор".  
    ...
    1.  Проверьте журнал системных событий на контроллере домена «традиционные» (пред-Windows Server 2012) события RID, описанные в [запрос пула RID](https://technet.microsoft.com/library/ee406152(WS.10).aspx) (16642, 16643, 16644, 16645, 16656).  
...
|Сообщение|Размер пула для идентификаторов учетных записей (относительных идентификаторов (RID)), настроенный администратором, превышает поддерживаемое максимальное значение. Если контроллер домена является хозяином RID, будет использоваться максимальное значение %1.<br /><br />Дополнительные сведения см. в разделе [Ограничение на размер блока RID](../../ad-ds/manage/../../ad-ds/manage/../../ad-ds/manage/../../ad-ds/manage/Managing-RID-Issuance.md#BKMK_RIDBlockMaxSize).|  
