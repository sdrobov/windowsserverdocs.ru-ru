...
|       **Сообщение**        | На локальном компьютере *<COMPUTERNAME>* обнаружен файл конфигурации клонирования виртуального контроллера домена.<p>Файл конфигурации клонирования виртуального контроллера домена находится в: %1<p>Существование этого файла указывает на то, что локальный виртуальный контроллер домена является клоном другого виртуального контроллера домена. Начнет *<COMPUTERNAME>* клонирование самого себя. |
...
|        **Источник**        |                                                                                                          Microsoft-Windows-ActiveDirectory_DomainService                                                                                                          |
|       **Уровень серьезности**       |                                                                                                                           Informational                                                                                                                           |
|       **Сообщение**        | Внутреннее событие: *<COMPUTERNAME>* завершен запрос на клонирование агента системы удаленного каталога.<p>Имя исходного контроллера домена:%3<p>Имя клонируемого контроллера домена в запросе:%4<p>Сайт клонируемого контроллера домена в запросе:%5<p>Дополнительные данные<p>Значение ошибки:%1 %2 |
| **Примечания и решение** |                                                                                                     Это событие успешного выполнения операции, которое является неполадкой только в случае непредвиденного возникновения.                                                                                                      |
...
|        **Источник**        |                                                                                                                                                                                                     Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                     |
|       **Уровень серьезности**       |                                                                                                                                                                                                                      Informational                                                                                                                                                                                                                      |
|       **Сообщение**        | *<COMPUTERNAME>* Задайте следующее значение реестра, чтобы включить обновления DNS.<p>Раздел реестра:%1<p>Значение реестра: %2<p>Данные значения реестра: %3<p>В ходе клонирования локальный компьютер в течение короткого времени может иметь то же имя, что и клонируемый компьютер. Регистрация А- и АААА-записей DNS на это время отключается, чтобы клиенты не могли направлять запросы на локальный компьютер, участвующий в клонировании. |
| **Примечания и решение** |                                                                                                                                                                                                Это событие успешного выполнения операции, которое является неполадкой только в случае непредвиденного возникновения.                                                                                                                                                                                                 |
...
|        **Источник**        |                                                                                                                                                                                                                                                                                      Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                                                                                                                                                                      |
|       **Уровень серьезности**       |                                                                                                                                                                                                                                                                                                       Informational                                                                                                                                                                                                                                                                                                       |
|       **Сообщение**        | *<COMPUTERNAME>* Задайте следующие значения реестра, чтобы инициализировать реплику SYSVOL во время неполномочного восстановления:<p>Раздел реестра:%1<p>Значение реестра: %2<p>Данные значения реестра: %3<p>Active Directory обнаружил, что виртуальная машина, на которой размещен контроллер домена, возвращена в предыдущее состояние. *<COMPUTERNAME>* необходимо инициализировать неполномочное восстановление в локальной реплике SYSVOL. Для этого необходимо остановить службу FRS или DFSR, используемую для репликации папки SYSVOL, а затем запустить ее с надлежащими разделами и параметрами реестра, чтобы инициализировать восстановление. |
| **Примечания и решение** |                                                                                                                                                                                                                                                    Ожидается при восстановлении снимка. Все данные SYSVOL на этом контроллере домена заменяются копией с партнерского контроллера домена.                                                                                                                                                                                                                                                    |
...
|        **Источник**        |                                                                             Microsoft-Windows-ActiveDirectory_DomainService                                                                             |
|       **Уровень серьезности**       |                                                                                              Informational                                                                                              |
|       **Сообщение**        | Active Directory обнаружил, что виртуальная машина, на которой размещен контроллер домена, возвращена в предыдущее состояние. *<COMPUTERNAME>* завершена репликация для текущего контроллера домена. |
...
|        **Источник**        |                                                                                                                                         Microsoft-Windows-ActiveDirectory_DomainService                                                                                                                                         |
|       **Уровень серьезности**       |                                                                                                                                                          Informational                                                                                                                                                          |
|       **Сообщение**        |                                                                                                            *<COMPUTERNAME>* Удаленные базы данных DFSR инициализируют реплику SYSVOL во время неполномочного восстановления.                                                                                                             |
