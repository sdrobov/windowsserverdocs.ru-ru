...
@no__t 0MaxResultSetSize: 262 144 байт @ no__t-0  
...
Если события 2899 с пулом 250 МБ или больше сохраняются, скорее всего, существует много клиентов с большим количеством возвращаемых объектов с частым выполнением запросов. Данные, которые можно собрать с помощью [группы сборщиков данных Active Directory](http://blogs.technet.com/b/askds/archive/2010/06/08/son-of-spa-ad-data-collector-sets-in-win2008-and-beyond.aspx) , могут помочь найти повторяющиеся страничные запросы, в которых серверы LDAP заняты. Все эти запросы отображаются с рядом "возвращенных записей", которые соответствуют размеру используемой страницы.  
