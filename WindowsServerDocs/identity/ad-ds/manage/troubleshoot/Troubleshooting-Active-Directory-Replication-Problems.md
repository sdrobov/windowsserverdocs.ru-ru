...
### <a name="hardware-failures-or-upgradestitle"></a>Сбои оборудования или обновления @ no__t-0
...
- Сетевое подключение: Возможно, сетевое подключение недоступно или параметры сети настроены неправильно.
- Разрешение имен: Ошибки конфигурации DNS являются распространенной причиной сбоев репликации.
- Проверка подлинности и авторизация: Проблемы проверки подлинности и авторизации приводят к ошибкам "отказано в доступе", когда контроллер домена пытается подключиться к своему участнику репликации.
- База данных каталогов (хранилище): Возможно, база данных каталогов не сможет достаточно быстро обрабатывать транзакции, чтобы поддерживать время ожидания репликации.
...
- Топология репликации: Контроллеры домена должны иметь межсайтовые ссылки в AD DS, которые сопоставляются с подключениями к реальной глобальной сети (WAN) или виртуальными частными сетями (VPN). При создании объектов в AD DS для топологии репликации, которая не поддерживается реальной топологией сайта вашей сети, репликация, для которой требуется неправильно настроенная топология, завершается сбоем.
...
## <a name="using-repadmin-to-retrieve-replication-statustitle"></a>Использование средства Repadmin для получения сведений о состоянии репликации @ no__t-0
...
Дополнительные сведения, включая статьи о поддержке, относящиеся к кодам ошибок, см. в статье о поддержке. [Устранение распространенных ошибок репликации Active Directory](https://support.microsoft.com/help/3108513)
