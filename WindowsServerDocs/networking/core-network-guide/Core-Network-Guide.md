...
|Расположение папки базы данных доменных служб Active Directory|Е:\конфигуратион @ no__t-0<br /><br />Можно также принять расположение по умолчанию.|
|Расположение папки файлов журнала доменных служб Active Directory|Е:\конфигуратион @ no__t-0<br /><br />Можно также принять расположение по умолчанию.|
|Расположение папки SYSVOL доменных служб Active Directory|Е:\конфигуратион @ no__t-0<br /><br />Можно также принять расположение по умолчанию.|
|Пароль администратора режима восстановления каталога|**J @ no__t-1p2leO4 $ F**|
...
Например, если диапазон IP-адресов для подсети — 192.168.0.1 – 192.168.0.254 и у вас есть десять устройств, для которых требуется настроить статический IP-адрес, можно создать диапазон исключения для 192.168.0. область *x* , включающая десять или более IP-адресов: от 192.168.0.1 до 192.168.0.15.
...
2.  В области сведений щелкните правой кнопкой мыши папку, в которую следует добавить учетную запись пользователя.
    ...
    **Которому?**
    ...
    -   Active Directory*папки* "пользователи и компьютеры/*узел домена*/"
...
2.  В области сведений дважды щелкните папку, содержащую группу, в которую требуется добавить член.
    ...
    -   **Active Directory пользователи и компьютеры**/*узел домена*/ *, содержащий группу*
...
> [!NOTE]
> По умолчанию сервер политики сети прослушивает RADIUS-трафик на портах 1812, 1813, 1645 и 1646 для всех установленных сетевых адаптеров. Если при установке сервера политики сети включен брандмауэр Windows в повышенной безопасности, то исключения брандмауэра для этих портов автоматически создаются в процессе установки для протокола IP версии 6 \(IPv6 @ no__t-1 и трафика IPv4. Если серверы сетевого доступа настроены на отправку трафика RADIUS через порты, отличные от этих параметров по умолчанию, удалите исключения, созданные в брандмауэре Windows в процессе установки сервера политики сети, и создайте исключения для портов, используемых для Трафик RADIUS.
...
-   [Общие сведения о службах домен Active Directory](https://technet.microsoft.com/library/hh831484.aspx) в https://technet.microsoft.com/library/hh831484.aspx.

-   [Обзор системы доменных имен (DNS)](https://technet.microsoft.com/library/hh831667.aspx) в https://technet.microsoft.com/library/hh831667.aspx.
...
-   [Общие сведения о службах сетевой политики и доступа](https://technet.microsoft.com/library/hh831683.aspx) в https://technet.microsoft.com/library/hh831683.aspx.
