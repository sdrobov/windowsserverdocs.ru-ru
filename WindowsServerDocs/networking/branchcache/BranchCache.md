---
title: BranchCache
description: Этот раздел содержит общие сведения о BranchCache в Windows Server 2016
manager: brianlic
ms.custom: na
...
- Клиентские компьютеры в филиалах под управлением Windows 10, Windows 8.1, Windows 8 или Windows 7.
...
-   [Сведения о содержимом версии](#bkmk_version)  
  
-   [Как BranchCache обрабатывает обновление содержимого в файлах](#bkmk_handles)  
...
При развертывании BranchCache исходное содержимое хранится на BranchCache с поддержкой серверов содержимого в главном офисе или в облачном центре обработки данных. BranchCache поддерживает следующие типы серверов содержимого.

> [!NOTE]
> С BranchCache ускоряет загрузку только исходного содержимого — то есть содержимого, что клиентские компьютеры изначально получают с поддержкой BranchCache сервера содержимого. Содержимое, которое клиентские компьютеры получают напрямую из других источников, например с веб-серверов в Интернете или из Центра обновления Windows, не кэшируется клиентским компьютером или сервером размещенного кэша и недоступно другим компьютерам в филиале. Если вы хотите ускорить содержимое обновления Windows, тем не менее, можно установить сервер приложений служб Windows Server Update Services (WSUS) в главном офисе или облачного центра обработки данных и настроить его в качестве сервера содержимого BranchCache.

### <a name="web-servers"></a>Веб-серверы

Поддерживаемые веб-службы включают компьютеров, работающих под управлением Windows Server 2016, Windows Server 2012 R2, Windows Server 2012 или Windows Server 2008 R2 с установленной ролью сервера веб-сервер (IIS), использующим протокол передачи гипертекста (HTTP) или HTTP Secure) ПРОТОКОЛ HTTPS).

Кроме того, на веб-сервере должен быть установлен компонент BranchCache.

### <a name="file-servers"></a>Файловые серверы

Поддерживаемые файловые серверы включают компьютеры под управлением Windows Server 2016, Windows Server 2012 R2, Windows Server 2012 или Windows Server 2008 R2, имеющие роль сервера файловых служб и BranchCache для сетевых файлов службы роли установлены. 
...
Поддерживаемые серверы приложений включают компьютеры под управлением Windows Server 2016, Windows Server 2012 R2, Windows Server 2012 или Windows Server 2008 R2 с фоновой интеллектуальной службы передачи (BITS) установлен и включен. 

Кроме того, на сервере приложений должен быть установлен компонент BranchCache. В качестве примеров из серверов приложений можно развернуть серверы Microsoft Windows Server Update Services (WSUS) и точка распространения филиала Microsoft System Center Configuration Manager в качестве серверов содержимого BranchCache.

## <a name="BKMK_3a"></a>BranchCache и облако

Облако обладает огромным потенциалом с точки зрения сокращения операционных расходов и достижения новых уровней масштабирования, но удаление процессов от работающих с ними людей может увеличить сетевые расходы и снизить эффективность. Пользователям нужна высокая производительность и неважно, где размещаются их приложения и данные. 
...
> [!NOTE]
> В следующей таблице сокращение «ОС» означает, что операционная система.
...
При наличии серверов содержимого и серверы размещенного кэша, работающих под управлением Windows Server 2016, Windows Server 2012 R2 и Windows Server 2012, они используют версию сведений о содержимом, соответствующий операционной системы клиента BranchCache, запрашивает сведения. 

Когда компьютеры под управлением Windows Server 2012 и Windows 8 или более поздних операционных системах запрашивают содержимое, серверы содержимого и размещенного кэша используют сведения о содержимом версии 2; Когда компьютеры под управлением Windows Server 2008 R2 и Windows 7 запрашивают содержимое, серверы содержимого и размещенного кэша использовать сведения о содержимом версии 1.

>[!IMPORTANT]
>При развертывании BranchCache в режиме распределенного кэша клиенты, использующие разные версии сведений о содержимом, не могут совместно использовать содержимое. Например клиентский компьютер под управлением Windows 7 и клиентский компьютер под управлением Windows 10, которые устанавливаются в одном филиале не использовать содержимое совместно друг с другом.
  
## <a name="bkmk_handles"></a>Как BranchCache обрабатывает обновление содержимого в файлах

Когда пользователям в филиале, изменения или обновления содержимого документов, их изменения записываются непосредственно к серверу содержимого в главном офисе без участия в BranchCache. Это происходит независимо от того, загрузил ли пользователь документ с сервера содержимого или извлек его из размещенного или распределенного кэша филиала.
...
Чтобы установить компонент BranchCache или BranchCache для сетевых файлов службы роли, роли сервера файловых служб можно использовать диспетчера серверов в Windows Server 2016. Воспользуйтесь следующей таблицей, чтобы определить, следует ли устанавливать службу роли или компонент.

|Функция|Расположение компьютера|Элемент BranchCache, который следует установить|
|-----------------|---------------------|------------------------------------|
|Сервер содержимого \(сервера приложений на основе BITS\)|Главный офис или облачный центр обработки данных|Компонент BranchCache|
|Сервер содержимого \(веб-сервера\)|Главный офис или облачный центр обработки данных|Компонент BranchCache|
|Сервер содержимого \(файлового сервера с помощью протокола SMB\)|Главный офис или облачный центр обработки данных|Служба роли "Служба BranchCache для сетевых файлов" для роли сервера файловых служб|
|Сервер размещенного кэша|Филиал|Компонент BranchCache с включенным режимом сервера размещенного кэша|
|Клиентский компьютер с BranchCache|Филиал|Установка не нужна; просто включите BranchCache и режим BranchCache \(распределенного или размещенного\) на стороне клиента|
...
- На странице мастера **Выбор ролей сервера**, если вы устанавливаете сервер файл с поддержкой BranchCache, разверните **файловых служб и служб хранилища** и **служб файлов и iSCSI**, и затем выберите **BranchCache для сетевых файлов**.  Чтобы сэкономить место на диске, можно также выбрать **дедупликации данных** роли службы, а затем продолжайте мастера для установки и завершения. Если вы не хотите установить сервер файл с поддержкой BranchCache, не устанавливайте роль файловых служб и служб хранилища с помощью BranchCache для сетевых файлов службы роли.

- На странице мастера **Выбор компонентов**, если вы устанавливаете сервер содержимого, который не является сервером файла или при установке сервера размещенного кэша, выберите **BranchCache**, а затем следуйте указаниям мастера, для установки и завершения. Если вы не хотите устанавливать сервер содержимого, который не является файловым сервером, или сервер размещенного кэша, не устанавливайте компонент BranchCache.
...
Следующие операционные системы предоставляют BranchCache с поддержкой фоновой интеллектуальной службы передачи (BITS), Hyper протокола передачи текста (HTTP) и Server Message Block (SMB).
...
-   Windows 7 Профессиональная, BITS поддержка только

> [!NOTE]
> BranchCache не доступен по умолчанию в операционных системах Windows Server 2008 или Windows Vista. В этих операционных системах тем не менее, если вы загрузите и установите обновление Windows Management Framework, функциональные возможности BranchCache доступна фоновой интеллектуальной службы передачи (BITS) только для протокола. Дополнительные сведения и скачать Windows Management Framework, см. в разделе [Windows Management Framework (Windows PowerShell 2.0, WinRM 2.0 и BITS 4.0)](https://go.microsoft.com/fwlink/?LinkId=188677) в https://go.microsoft.com/fwlink/?LinkId=188677.
...
Кроме того в операционных системах семейства Windows Server 2008 R2 можно использовать в качестве серверов содержимого BranchCache со следующими исключениями:
...
Операционных систем семейства Windows Server 2016, Windows Server 2012 R2 и Windows Server 2012 можно использовать как BranchCache серверов размещенного кэша.

Кроме того можно использовать следующие операционные системы Windows Server 2008 R2 как BranchCache серверов размещенного кэша:
...
- Windows Server 2008 R2 Enterprise Server Core Installation с Hyper-V
...
- Windows Server 2008 R2 Datacenter Server Core Installation с Hyper-V
...
## <a name="bkmk_flow"></a>Поток содержимого и процессов
...
## <a name="BKMK_8"></a>Процессы BranchCache: Запрос содержимого

На первом этапе находящийся в филиале клиентский компьютер запрашивает содержимое, например веб-страницу, с сервера содержимого, который находится в удаленном расположении, например в главном офисе. Сервер содержимого проверяет, есть ли у клиентского компьютера права на получение запрошенного содержимого. Если клиентский компьютер имеет права и клиент и сервер содержимого BranchCache\-включен, сервер содержимого генерирует сведения о содержимом.
...
При получении действительного сообщения с запросом от клиента, узел содержимого проверяет, существует ли блок, указанный в запросе в кэш содержимого хранилища содержимого.
...
Прежде чем BranchCache пошлет сведения о содержимом или само содержимое, данные шифруются. BranchCache шифрует блок в ответном сообщении. В Windows 7 по умолчанию алгоритм шифрования, который использует BranchCache — AES-128, ключ шифрования Ke и размер ключа составляет 128 бит, в соответствии с требованиями алгоритма шифрования. 
...
- Для размещенного кэша, необходимы серверы, работающих под управлением Windows Server 2008 R2, сервер размещенного кэша, сертификат и связанный закрытый ключ, и центр сертификации (ЦС), выдавший этот сертификат должен быть доверенным на клиентских компьютерах в филиале. Это позволяет клиенту и серверу успешно участвовать в серверной проверке подлинности HTTPS.
...
В среде размещенного кэша, где сервер размещенного кэша под управлением Windows Server 2008 R2 использование технологии шифрования, например BitLocker или EFS является рекомендуется в том случае, если один из клиентов в филиале доступа к конфиденциальным данным через канал WAN. Это также необходимо, чтобы предотвратить физический доступ к размещенному кэшу, так как шифрование диска работает только в том случае, если компьютер был выключен в тот момент, когда злоумышленник получил к нему физический доступ.  Если компьютер был включен или находился в спящем режиме, шифрование диска не обеспечит защиту.

> [!NOTE]
> Серверы размещенного кэша, работающих под управлением Windows Server 2016, Windows Server 2012 R2 или Windows Server 2012 шифруют все данные в кэше по умолчанию, поэтому использование дополнительных технологий шифрования не является обязательным.
