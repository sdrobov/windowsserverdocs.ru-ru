...
Кроме того, на сервере приложений должен быть установлен компонент BranchCache. В качестве примеров серверов приложений можно развернуть Microsoft Windows Server Update Services (WSUS) и серверы точки распространения филиалов Microsoft System Center Configuration Manager как серверы содержимого BranchCache.
...
|-----------------|---------------------|------------------------------------|
|Сервер содержимого @no__t — сервер приложений на основе 0BITS — no__t-1|Главный офис или облачный центр обработки данных|Компонент BranchCache|
|Content Server \(Web Server @ no__t-1|Главный офис или облачный центр обработки данных|Компонент BranchCache|
|Сервер содержимого \(file Server по протоколу SMB @ no__t-1|Главный офис или облачный центр обработки данных|Служба роли "Служба BranchCache для сетевых файлов" для роли сервера файловых служб|
|Сервер размещенного кэша|Филиал|Компонент BranchCache с включенным режимом сервера размещенного кэша|
|Клиентский компьютер с BranchCache|Филиал|Установка не требуется; просто включите BranchCache и режим BranchCache \(distributed или Hosted @ no__t-1 на клиенте.|
