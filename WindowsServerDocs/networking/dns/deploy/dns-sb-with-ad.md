...
С помощью этого раздела вы можете использовать возможности управления трафиком политик DNS для раздельных развертываний @ no__t-0brain с Active Directory интегрированными зонами DNS в Windows Server 2016.

В Windows Server 2016 поддержка политик DNS расширена для Active Directory интегрированных зон DNS. Интеграция Active Directory предоставляет DNS-серверу несколько возможностей @ no__t-0master высокого уровня доступности. 

Ранее этот сценарий требовал, чтобы администраторы DNS поддерживали два разных DNS-сервера, каждый из которых предоставляет службы каждому набору пользователей, внутренним и внешним. Если только несколько записей в зоне были разделены: @ no__t-0brained или оба экземпляра зоны (внутренние и внешние) были делегированы в один и тот же родительский домен, это стало загадкаом управления.

> [!NOTE]
> - Развертывания DNS делятся на "@ no__t-0brain" при наличии двух версий одной зоны, одной версии для внутренних пользователей в интрасети организации и одной версии для внешних пользователей, которые обычно являются пользователями в Интернете.
> - В статье [Использование политики DNS для раздельного развертывания DNS](split-brain-DNS-deployment.md) . объясняется, как можно использовать политики DNS и области зоны для развертывания разделенной @ no__t-1BRAIN системы DNS на одном DNS-сервере Windows Server 2016.



##  <a name="example-split-brain-dns-in-active-directory"></a>Пример разбиения @ no__t-0Brain DNS in Active Directory
...
Если DNS-сервер для contoso.com Active Directory интегрирован и прослушивает два сетевых интерфейса, администратор Contoso DNS может выполнить действия, описанные в этом разделе, чтобы получить раздельное развертывание @ no__t-0brain.
...
## <a name="how-dns-policy-for-split-brain-dns-in-active-directory-works"></a>Как работает политика DNS для Split @ no__t-0Brain DNS в Active Directory
...
Поддержка динамических DNS-\(DDNS @ no__t-1 обновлений и очистки поддерживается только в области зоны по умолчанию. Так как внутренние клиенты обслуживаются по области зоны по умолчанию, администраторы Contoso DNS могут продолжать использовать существующие механизмы (динамические DNS или статические) для обновления записей в contoso.com. Для областей зоны, отличных от @ no__t-0default, \(such, так как внешняя область в этом примере @ no__t-2, поддержка DDNS или очистки недоступна.
...
## <a name="how-to-configure-dns-policy-for-split-brain-dns-in-active-directory"></a>Настройка политики DNS для Split @ no__t-0Brain DNS в Active Directory
...
Следующим шагом является добавление записей, представляющих узел веб-сервера, в две области зоны — внешние и по умолчанию — внутренние клиенты \(for @ no__t-1. 

В области внутренней зоны по умолчанию запись www.career.contoso.com добавляется с IP-адресом 10.0.0.39, который является частным IP-адресом. и в области внешней зоны одна и та же запись @no__t -0www. карьеры. contoso. com @ no__t-1 добавляется с общедоступным IP-адресом 65.55.39.10. 

Записи \(both в области внутренней зоны по умолчанию и области внешней зоны @ no__t-1 будут автоматически реплицироваться по домену с соответствующими областями зоны.
...
> [!NOTE]
> В этом примере используется параметр Server Interface \(the-ServerInterface в команде example ниже @ no__t-1 в качестве критерия для различения внутренних и внешних клиентов. Другим способом различения внешних и внутренних клиентов является использование подсетей клиента в качестве критерия. Если вы можете определить подсети, к которым принадлежат внутренние клиенты, можно настроить политику DNS, чтобы отличать их от подсети клиента. Сведения о настройке управления трафиком с помощью критериев подсети клиента см. в статье [Использование политики DNS для управления трафиком на основе географического расположения с основными серверами](primary-geo-location.md).
