...
ms.locfileid: "71356018"
---
# <a name="use-dns-policy-for-split-brain-dns-deployment"></a>Использовать политику DNS для раздельного развертывания DNS @ no__t-0Brain

>Относится к: Windows Server 2016

С помощью этого раздела вы узнаете, как настроить политику DNS в Windows Server @ no__t-0 2016 для раздельных развертываний DNS, где есть две версии одной зоны — один для внутренних пользователей в интрасети организации, а другой для внешних пользователей, которые Обычно это пользователи в Интернете.

>[!NOTE]
>Сведения о том, как использовать политику DNS для разбиения @ no__t-0brain для развертывания DNS с Active Directory интегрированными зоны DNS, см. [в статье Использование политики DNS для разделения DNS-имен](dns-sb-with-ad.md)с помощью Active Directory.
...
## <a name="bkmk_sbexample"></a>Пример развертывания DNS с разделением-мозгом
...
## <a name="bkmk_sbhow"></a>Как работает развертывание с разделением DNS
...
## <a name="bkmk_sbconfigure"></a>Настройка развернутого сервера с разделением DNS
...
## <a name="bkmk_recursion"></a>Пример управления выборочной рекурсией DNS
...
### <a name="bkmk_recursionhow"></a>Как работает управление селективной рекурсией DNS

Если получен запрос, для которого DNS-сервер Contoso является неполномочным, например для www.microsoft.com, запрос разрешения имени оценивается по политикам на DNS-сервере. 

Поскольку эти запросы не попадают ни в одну зону, политики уровня зоны \(as, определенные в примере Split-мозг @ no__t-1, не оцениваются. 
...
Затем DNS-сервер выполняет рекурсию, чтобы получить ответ на www.microsoft.com из Интернета, и кэширует ответ локально. 
...
### <a name="bkmk_recursionconfigure"></a>Настройка управления выборочной рекурсией DNS
...
Устаревший параметр рекурсии и список серверов пересылки называются областью рекурсии по умолчанию. Невозможно добавить или удалить область рекурсии по умолчанию, которая определяется по имени dot \( ". \).
