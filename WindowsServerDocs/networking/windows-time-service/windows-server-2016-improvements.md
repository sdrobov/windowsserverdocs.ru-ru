...
|Задержка кругового пути NTP| Последняя задержка кругового пути, которая возникла в NTP-клиенте при получении ответа от сервера в микросекундах. Это время, затраченное на NTP-клиент между передачей запроса на |NTP-сервер и получением допустимого ответа из сервера. Этот счетчик помогает определять задержки, испытываемые NTP-клиентом. Увеличение или изменение кругового пути может привести к появлению помех в вычислениях времени NTP, что, в свою очередь, может повлиять на точность синхронизации времени через NTP.|
