...
* [конфигурации](#hklmsystemcurrentcontrolsetservicesw32timeconfig)
* [NTP-клиент](#hklmsystemcurrentcontrolsetservicesw32timetimeprovidersntpclient)
...
В следующей таблице перечислены глобальные параметры групповой политики, связанные со службой времени Windows и предварительно заданное значение, связанное с каждым параметром. Дополнительные сведения о каждом параметре см. соответствующие записи реестра в [записи реестра для службы времени Windows](#windows-time-service-registry-entries) ранее в этой теме. Содержатся следующие параметры в вызове одного объекта GPO **глобальные параметры конфигурации**.  
...
В следующей таблице перечислены доступные параметры для **Настройка клиента Windows NTP** объекта групповой Политики и заранее настроенные значения, которые связаны со службой времени Windows. Дополнительные сведения о каждом параметре см. соответствующие записи реестра в [записи реестра для службы времени Windows](#windows-time-service-registry-entries) ранее в этой теме.  
