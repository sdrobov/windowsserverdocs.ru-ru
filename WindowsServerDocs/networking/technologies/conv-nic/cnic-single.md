...
В примере конфигурации в этом разделе описывается два узла Hyper-v, **узел а**и узел Hyper-v **B**. На обоих узлах установлен один физический сетевой адаптер (ПНИК), а сетевые карты подключены к верхней части стойки \(ToR\) физического коммутатора. Кроме того, узлы расположены в той же подсети, что и 192.168.1. x/24.
...
Убедитесь, что физический сетевой адаптер может подключаться к целевому узлу. В этом тесте демонстрируется подключение с использованием уровня 3 \(L3\)-или уровня IP, а также \(уровня 2\)L2.
...
5. Зарезервируйте процент пропускной способности для\)RDMA Direct \(.
...
1. Проверьте конфигурацию RDMA.
   ...
   |------|------------------------------------------|---------|
   |  M1  | Ethernet-адаптер Mellanox ConnectX-3 Pro |  Истина   |
...
2. Просмотрите сведения о RDMA сетевого адаптера. 
   ...
   |         Имя          |        интерфацедескриптион         | Активировать |
   |-----------------------|-------------------------------------|---------|
   | Адаптера vethernet \(ВМСТЕСТ\) | #2 адаптера виртуальной сети Ethernet Hyper-V |  Ложь  |
...
4. Включите RDMA на узле vNIC.
   ...
   |         Имя          |        интерфацедескриптион         | Активировать |
   |-----------------------|-------------------------------------|---------|
   | Адаптера vethernet \(ВМСТЕСТ\) | #2 адаптера виртуальной сети Ethernet Hyper-V |  Истина   |
