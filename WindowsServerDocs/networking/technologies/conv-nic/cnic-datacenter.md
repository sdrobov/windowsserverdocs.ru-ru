...
В этом разделе приведены инструкции по развертыванию Объединенных сетевых адаптеров в групповой конфигурации сетевого адаптера с помощью команды коммутатора Embedded \(SET\). 
...
Убедитесь, что физический сетевой адаптер может подключаться к целевому узлу.  В этом тесте демонстрируется подключение с использованием уровня 3 \(L3\) или уровня IP, а также слоя 2 \(L2\) виртуальных локальных сетей \(VLAN\).
...
Для этого шага сетевые карты находятся в режиме **доступа** . Однако при создании виртуального коммутатора Hyper-V \(vSwitch\) далее в этом разделе Свойства VLAN применяются на уровне порта vSwitch. 

Так как коммутатор может содержать несколько виртуальных ЛС, он должен находиться в верхней части стойки \(ToR\) физическом коммутаторе, чтобы порт, к которому подключен узел, был настроен в режиме магистрали.
...
>[!TIP]
>В соответствии с Институтом инженеров по электрической и электронике \(стандартов IEEE\), качество обслуживания \(качества обслуживания\) в физическом сетевом адаптере зависит от заголовка 802.1 p, который внедрен в заголовок 802.1 Q \(VLAN\) при настройке идентификатора виртуальной ЛС.
...
## <a name="step-4-configure-quality-of-service-qos"></a>Шаг 4. Настройка качества обслуживания \(качества обслуживания\)

>[!NOTE]
>Необходимо выполнить все этапы настройки DCB и QoS на всех узлах, предназначенных для взаимодействия друг с другом.

1. Установите мост центра обработки данных \(DCB\) на каждом узле Hyper-V.

   - **Необязательно** для сетевых конфигураций, использующих iWarp.
   - **Требуется** для сетевых конфигураций, использующих роце \(любой версии\) для служб RDMA.
   ...
   |---------|----------------|-----------|------------------------|
   |  Истина   |       Нет       |  Выполнено  | {Мост центра обработки данных} |
...
4. Включите **Управление потоком приоритета** для трафика SMB, который не требуется для iWarp.
   ...
   | Priority | Активировать | Параметр Policy | ifIndex | ифалиас |
   |----------|---------|-----------|---------|---------|
   |    0     |  Ложь  |  Глобальная   | &nbsp;  | &nbsp;  |
   |    1     |  Ложь  |  Глобальная   | &nbsp;  | &nbsp;  |
   |    2     |  Ложь  |  Глобальная   | &nbsp;  | &nbsp;  |
...
7. Резервирование половины пропускной способности для \(RDMA SMB Direct\)
...
Необходимо убедиться, что структура настроена правильно до создания vSwitch и перехода на RDMA \(mode 2\).
...
1. Проверьте конфигурацию RDMA.
   ...
   |    Имя    |        интерфацедескриптион        | Активировать |
   |------------|------------------------------------|---------|
   | TEST-40G-1 | Адаптер Mellanox ConnectX-4 VPI #2 |  Истина   |
   | TEST-40G-2 |  Адаптер Mellanox ConnectX-4 VPI   |  Истина   |
...
3. Скачайте [служебную программу DiskSpd. exe](https://aka.ms/diskspd) и извлеките ее в C:\TEST\.

4. Скачайте [скрипт PowerShell Test-RDMA](https://github.com/Microsoft/SDN/blob/master/Diagnostics/Test-Rdma.ps1) в тестовую папку на локальном диске, например C:\TEST\.
...
4. Просмотрите дополнительные свойства узла vNIC. 
   ...
   |---------|----------------|---------|--------------|------------|--------|-------------|
   | вмстест |      Истина      | вмстест | E41D2D074071 |    '    | &nbsp; |             |
...
4. Переименуйте сетевой адаптер управления.
   ...
   |         Имя         | исманажементос | VMName |      SwitchName      |  MacAddress  | Состояние | IPAddresses |
   |----------------------|----------------|--------|----------------------|--------------|--------|-------------|
   | CORP — внешний коммутатор |      Истина      | &nbsp; | CORP — внешний коммутатор | 001B785768AA |  '  |   &nbsp;    |
   |         СКЛАД          |      Истина      | &nbsp; |       вмстест        | E41D2D074071 |  '  |   &nbsp;    |
...
3. Просмотр свойств сетевого адаптера управления.
   ...
   | CORP — внешний коммутатор |      Истина      | CORP — внешний коммутатор | 001B785768AA |    '    | &nbsp; |             |
   |         Склад          |      Истина      |       вмстест        | E41D2D074071 |    '    | &nbsp; |             |
   |         SMB1         |      Истина      |       вмстест        | 00155D30AA00 |    '    | &nbsp; |             |
   |         SMB2         |      Истина      |       вмстест        | 00155D30AA01 |    '    | &nbsp; |             |
...
11. Проверьте подключение из удаленной системы, так как оба узла vNIC находятся в одной подсети и имеют один и тот же идентификатор виртуальной ЛС \(102\).
...
Необходимо проверить функциональность RDMA с удаленной системы на локальную систему, которая имеет vSwitch, для обоих членов команды Team SET vSwitch.<p>Так как оба узла vNIC \(SMB1 и SMB2\) назначаются виртуальной ЛС 102, можно выбрать адаптер VLAN 102 в удаленной системе. <p>В этом примере сетевая карта Test-40G-2 поддерживает RDMA для SMB1 (192.168.2.111) и SMB2 (192.168.2.222).
