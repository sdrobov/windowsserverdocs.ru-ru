...
1. Просмотр свойств первого сетевого адаптера.
   ...
   |    name    |           интерфацедескриптион           | ifIndex | Status |    MacAddress     | LinkSpeed |
   |------------|------------------------------------------|---------|--------|-------------------|-----------|
...
1. Установите мосты центра обработки \( данных \) на каждом узле Hyper-V.
   ...
   |---------|----------------|-----------|------------------------|
   |  Верно   |       Нет       |  Успешно  | {Мост центра обработки данных} |
...
4. Включите **Управление потоком приоритета** для трафика SMB, который не требуется для iWarp.
   ...
   | Priority | Активировано | Параметр Policy | IfIndex | ифалиас |
   |----------|---------|-----------|---------|---------|
...
7. Резервирование половины полосы пропускания в SMB Direct \( RDMA\)
   ...
   | name | Алгоритм | Пропускная способность (%) | Priority | Параметр Policy | IfIndex | ифалиас |
   |------|-----------|--------------|----------|-----------|---------|---------|
...
1. Проверьте конфигурацию RDMA.
   ...
   |    name    |        интерфацедескриптион        | Активировано |
   |------------|------------------------------------|---------|
   | TEST-40G-1 | Адаптер Mellanox ConnectX-4 VPI #2 |  Верно   |
   | TEST-40G-2 |  Адаптер Mellanox ConnectX-4 VPI   |  Верно   |
...
4. Просмотрите дополнительные свойства узла vNIC.
   ...
   |  name   | исманажементос | VMName  |  SwitchName  | MacAddress | Status | IPAddresses |
   |---------|----------------|---------|--------------|------------|--------|-------------|
   | вмстест |      Верно      | вмстест | E41D2D074071 |    '    | &nbsp; |             |
...
4. Переименуйте сетевой адаптер управления.
   ...
   |         name         | исманажементос | VMName |      SwitchName      |  MacAddress  | Status | IPAddresses |
   |----------------------|----------------|--------|----------------------|--------------|--------|-------------|
   | CORP — внешний коммутатор |      Верно      | &nbsp; | CORP — внешний коммутатор | 001B785768AA |  '  |   &nbsp;    |
   |         СКЛАД          |      Верно      | &nbsp; |       вмстест        | E41D2D074071 |  '  |   &nbsp;    |
...
3. Просмотр свойств сетевого адаптера управления.
   ...
   | CORP — внешний коммутатор |      Верно      | CORP — внешний коммутатор | 001B785768AA |    '    | &nbsp; |             |
   |         Склад          |      Верно      |       вмстест        | E41D2D074071 |    '    | &nbsp; |             |
   |         SMB1         |      Верно      |       вмстест        | 00155D30AA00 |    '    | &nbsp; |             |
   |         SMB2         |      Верно      |       вмстест        | 00155D30AA01 |    '    | &nbsp; |             |
