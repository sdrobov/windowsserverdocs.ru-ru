...
Сервер политики сети \(NPS @ no__t-1 использует политики сети и свойства удаленного доступа учетных записей пользователей, чтобы определить, имеет ли запрос на подключение разрешение на подключение к сети.
...
Если требуется, чтобы разрешения на доступ определялись свойствами удаленного доступа учетной записи пользователя в Active Directory @ no__t-0 Domain Services \(AD DS @ no__t-2, можно установить флажок **доступ определяется по свойствам входящих подключений пользователя** .
...
Эту процедуру можно использовать для создания политик запросов на подключение и сетевых политик, необходимых для развертывания серверов удаленного доступа или виртуальной частной сети \(VPN @ no__t-1 как протокол RADIUS \(RADIUS @ no__t-3. клиентов на сервер политики сети RADIUS.

>[!NOTE]
>Клиентские компьютеры, такие как переносные компьютеры и другие компьютеры с клиентскими операционными системами, не являются клиентами RADIUS. RADIUS-клиенты — это серверы сетевого доступа, такие как точки беспроводного доступа, коммутаторы 802.1 X проверки подлинности, виртуальная частная сеть \(VPN @ no__t-1 серверы и серверы удаленного доступа — так как эти устройства используют протокол RADIUS для взаимодействия с RADIUS. такие серверы, как НПСС.
...
6. В поле **Добавить стандартный атрибут RADIUS**в поле атрибуты прокрутите вниз до и добавьте следующие атрибуты:
    ...
    - **Туннель — средний тип**. Выберите значение, соответствующее ранее выбранным параметрам политики. Например, если настраивается политика беспроводной сети, выберите **Value: 802 (включает все Общепринятый формат носителей 802 и Ethernet)** .
...
При развертывании NPS с сетевыми политиками, использующими протокол расширенной проверки подлинности \(EAP @ no__t-1 с безопасностью уровня транспорта \(TLS @ no__t-3 или EAP-TLS, как метод проверки подлинности, максимальный единица передачи по умолчанию \(MTU @ no_ _T-5, который NPS использует для полезных данных EAP, — 1500 байт. 
