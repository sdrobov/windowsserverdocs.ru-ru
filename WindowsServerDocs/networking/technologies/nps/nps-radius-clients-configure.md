...
При добавлении нового сервера сетевого доступа \(VPN Server, беспроводной точки доступа, коммутатора проверки подлинности или удаленного доступа к серверу @ no__t-1 в сети необходимо добавить сервер в качестве RADIUS-клиента в NPS, а затем настроить клиент RADIUS для взаимодействия с сервером политики сети.

>[!IMPORTANT]
>Клиентские компьютеры и устройства, такие как переносные компьютеры, планшеты, телефоны и другие компьютеры с клиентскими операционными системами, не являются клиентами RADIUS. RADIUS-клиенты — это серверы сетевого доступа, такие как точки беспроводного доступа, коммутаторы с поддержкой 802.1 X, серверы виртуальной частной сети (VPN) и серверы удаленного доступа, так как они используют протокол RADIUS для взаимодействия с серверами RADIUS, например политики сети. Server \(NPS @ no__t-1.
...
Для выполнения процедур, описанных в этом разделе, необходимо иметь по крайней мере один сервер сетевого доступа \(VPN Server, беспроводная точка доступа, коммутатор проверки подлинности или сервер удаленного доступа @ no__t-1 или прокси-сервер NPS физически установлены в сети.
...
5. Если вы настраиваете точку доступа к беспроводной сети в **SSID**, укажите идентификатор набора служб \(SSID @ no__t-2, который представляет собой буквовую строку, которая служит в качестве сетевого имени. Это имя транслируется точками доступа для беспроводных клиентов и видимо для пользователей в точках беспроводной связи \(Wi-Fi @ no__t-1.
...
7. В **новом RADIUS-клиенте**в **дополнительных параметрах**при использовании любых методов проверки подлинности, отличных от EAP и PEAP. Если NAS поддерживает использование атрибута проверки подлинности сообщений, выберите пункт **сообщения запроса доступа должно содержать сообщение. Атрибут средства проверки подлинности**.
...
4. В поле **адрес \(IP или DNS @ no__t-2**введите диапазон IP-адресов для RADIUS-клиентов, используя нотацию междоменного маршрутизации \(CIDR @ no__t-4. Например, если диапазон IP-адресов для серверов NAS — 10.10.0.0, введите **10.10.0.0/16**.
5. В поле **поставщик** **нового RADIUS-клиента**укажите имя производителя NAS. Если вы не знаете имя изготовителя NAS, выберите **Стандарт RADIUS**.
6. В **новом RADIUS-клиенте**в поле **общий секрет**выполните одно из следующих действий.
    - Убедитесь, что выбран параметр **вручную** , а затем в поле **общий секрет**введите надежный пароль, который также вводится на NAS. Повторно введите общий секрет в поле **подтверждение общего секрета**.
    - Выберите **создать**, а затем — **создать** , чтобы автоматически создать общий секрет. Сохраните созданный общий секрет для конфигурации на NAS, чтобы он мог взаимодействовать с сервером политики сети.
7. В **новом RADIUS-клиенте**в **дополнительных параметрах**при использовании любых методов проверки подлинности, отличных от EAP и PEAP, а также если все серверы NAS поддерживают использование атрибута проверки подлинности сообщений, выберите пункт **сообщения запроса доступа должно содержать Атрибут проверки подлинности сообщения**.
