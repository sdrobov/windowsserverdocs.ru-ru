...
При развертывании сервера политики сети (NPS) в качестве протокол RADIUS \(прокси-\) RADIUS, NPS получает запросы на подключение от клиентов RADIUS, таких как серверы сетевого доступа или другие прокси-серверы RADIUS, а затем перенаправляет эти запросы к серверам, на которых выполняются службы NPS или другие серверы RADIUS. Эти рекомендации по планированию можно использовать для упрощения развертывания RADIUS.
...
Клиенты RADIUS — это серверы сетевого доступа, такие как точки беспроводного доступа, виртуальная частная сеть \(VPN-серверы\), коммутаторы с поддержкой 802.1 X и серверы удаленного доступа. Прокси-серверы RADIUS, которые перенаправляют сообщения запроса на подключение на серверы RADIUS, также являются RADIUS-клиентами. Сервер политики сети поддерживает все серверы доступа к сети и прокси-серверы RADIUS, соответствующие протоколу RADIUS, как описано в документе RFC 2865 "Служба удаленного входящего подключения к службе пользователей \(RADIUS-\)" и RFC 2866, "RADIUS Accounting".
