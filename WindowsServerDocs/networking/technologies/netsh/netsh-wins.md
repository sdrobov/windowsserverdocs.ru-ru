...
---
# <a name="network-shell-netsh-example-batch-file"></a>Сетевой интерпретатор \(пример пакетного файла Netsh\)
...
Вы можете использовать команды Netsh для службы Windows Internet Name Service \(WINS\) в пакетных файлах и других скриптах для автоматизации задач. В следующем примере пакетного файла показано, как использовать команды Netsh для WINS для выполнения различных связанных задач.

В этом примере пакетного файла WINS\-A — это WINS-сервер с IP адресом 192.168.125.30, а WINS\-B — WINS-сервер с IP адресом 192.168.0.189.

В примере пакетного файла выполняются следующие задачи.

- Добавляет запись динамического имени с IP-адресом 192.168.0.205, мою запись\_\[04h\], в WINS\-
- Устанавливает WINS\-B в качестве партнера по извещающей или опрашивающей репликации WINS\-A
- Подключается к WINS\-B, а затем настраивает WINS\-A в качестве партнера по извещающей или опрашивающей репликации WINS\-B
- Инициирует принудительную репликацию из WINS\-A в WINS\-б
- Подключается к WINS\-B, чтобы убедиться, что новая запись, моя запись\_, успешно реплицирована
...
- **сервер**. Сдвигает текущую командную строку WINS\-контекста на сервер, указанный по имени или IP-адресу.
