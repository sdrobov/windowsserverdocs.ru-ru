...
|             **сслктлсторенаме**              |                                                Указывает имя хранилища сертификатов в разделе LOCAL_MACHINE, где хранится Сслктлидентифиер.                                                | Необязательный |
...
|  **слушивают**  | Задает одно из следующих значений: Да. разрешает пользователю регистрировать URL-адреса. Это значение используется по умолчанию. Нет: запретить пользователю регистрировать URL-адреса. | Необязательный |
| **получить** |  Задает одно из следующих значений: Да: разрешить пользователю делегировать URL-адреса нет: запретить пользователю делегирование URL-адресов. Это значение используется по умолчанию.  | Необязательный |
...
- Добавление URL-адреса urlacl =https://+:80/MyUri пользователь = домен\\пользователь
- Add urlacl URL =<https://www.contoso.com:80/MyUri> пользователь = домен\\пользователь прослушивает = да
- Add urlacl URL =<https://www.contoso.com:80/MyUri> пользователь = домен\\пользовательский делегат = нет
