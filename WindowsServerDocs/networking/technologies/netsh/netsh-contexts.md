...
Netsh взаимодействует с другими компонентами операционной системы с помощью динамической библиотеки @ no__t-0link \(DLL @ no__t-2. 
...
Netsh — это служебная программа для работы со сценариями командной строки, которая позволяет локально или удаленно отображать или изменять конфигурацию сети текущего компьютера. При использовании без параметров **netsh** открывает командную строку Netsh. exe \( —, **netsh @ no__t-3**\).

#### <a name="syntax"></a>Синтаксис

**netsh**\[ **-a**&nbsp;*алиасфиле*\] \[ **-c**&nbsp;*Context* 0 1 **-r**3*ремотекомпутер*5 6 **-u** 8  *Имя_домена @ no__t-20* 1 *имя_пользователя* 3 4 **-p**6*Password*8 @ no__t-29 @ No__t-30 1 {*нетшкомманд*3 **-f**5*ScriptFile*} 7
...
Необязательно. Указывает домен, в котором находится учетная запись пользователя. По умолчанию используется локальный домен, если *имя_домена @ no__t-1* не указан.
