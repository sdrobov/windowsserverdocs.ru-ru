...
ИТ-отдел решает настроить политику качества обслуживания для определения приоритета конкретных приложений, используя значения DSCP \(\) для кодовой точки службы дифференцирования для классификации сетевого трафика и настройки маршрутизаторов для предоставления предварительных ссылок Обработка трафика с более высоким приоритетом. 
...
## <a name="scenario-2-prioritize-network-traffic-for-an-http-server-application"></a>Сценарий 2. Определение приоритетов сетевого трафика для серверного приложения HTTP
...
[4. URL-путь](#bkmk_QoS_UrlPath)

Сведения приведены ниже.

####  <a name="bkmk_QoS_UrlScheme"></a> 1. Схема URL-адресов

 `https://`имеет более высокий приоритет, `https://`чем.
...
####  <a name="bkmk_QoS_UrlPath"></a> 4. URL-путь
