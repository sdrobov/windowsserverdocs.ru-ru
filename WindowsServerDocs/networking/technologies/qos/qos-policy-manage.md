...
[Wi-Fi Alliance](https://go.microsoft.com/fwlink/?LinkId=160769) установила сертификацию для беспроводного \(мультимедиа\) WMM, определяющего четыре \(категории\) доступа WMM_AC для определения приоритетов сетевого трафика, передаваемого в\-сетиБеспроводная сеть Fi. Категории доступа включают в \(себя самый высокий и низкий приоритет\): голоса, видео, лучшие усилия и фоновый режим; соответственно сокращаются как во, VI, BK. Спецификация WMM определяет, какие значения DSCP соответствуют каждой из четырех категорий доступа:
...
Например, policy_A указывает только имя приложения (App. exe), а policy_B Указывает конечный IP-адрес 192.168.1.0/24. Когда эти политики QoS конфликтуют \(с приложением App. exe, он отправляет трафик на IP-адрес в диапазоне 192.168.4.0\)/24, policy_A применяется.
