...
В Windows Server 2016 vRSS включен по умолчанию, однако необходимо подготовить среду, чтобы обеспечить правильную работу vRSS на виртуальной машине \(VM @ no__t-1 или на виртуальном адаптере узла \(vNIC @ no__t-3. В Windows Server 2012 R2 vRSS был отключен по умолчанию.

При планировании и подготовке использования vRSS убедитесь, что:

- Физический сетевой адаптер совместим с очередь виртуальной машины \(VMQ @ no__t-1 и имеет скорость связи 10 Гбит/с или более.
- VMQ включается на физическом сетевом адаптере и на порте виртуального коммутатора Hyper @ no__t-0V.
- Отсутствует один корневой элемент входных данных @ no__t-0Output Virtualization \(SR @ no__t-2IOV @ no__t-3, настроенный для виртуальной машины.
- Объединение сетевых карт настроено правильно.
- Виртуальная машина имеет несколько логических процессоров \(LPs @ no__t-1.
...
3. **Отсутствие SR @ no__t-1IOV**. Убедитесь, что один корневой элемент входных данных @ no__t-0Output Virtualization \(SR @ no__t-2IOV @ no__t-3 — это не подключенный к сетевому интерфейсу виртуальной машины драйвер \(VF @ no__t-5. Это можно проверить с помощью команды **Get-нетадаптерсриов** . Если загружен драйвер VF, RSS использует параметры масштабирования из этого драйвера, а не настройки vRSS. Если драйвер VF не поддерживает RSS, то функция vRSS отключена.
