...
Это руководство содержит инструкции по развертыванию протокола конфигурации динамических узлов протокола IP версии 4 \(DHCP @ no__t-1 на сервере, который автоматически назначает IP-адреса и параметры DHCP DHCP-клиентам IPv4, подключение к одной или нескольким подсетям в сети.
...
Например, рекомендуется вручную настроить IP-адрес сервера DHCP, а также IP-адреса DNS-серверов и контроллеров домена, на которых выполняется домен Active Directory Services \(AD DS @ no__t-1.
...
## <a name="bkmk_lab"></a>Использование этого руководства в тестовой лаборатории
...
Требования к лаборатории различаются в зависимости от того, используете ли вы физические серверы или виртуальные машины \(VMs @ no__t-1, а также используется ли домен Active Directory или Развертывание автономного DHCP-сервера.
...
Для развертывания домена или автономного развертывания необходим один сервер, настроенный как узел Hyper @ no__t-0V.
...
1. Один **внутренний** виртуальный коммутатор. Не создавайте **внешний** виртуальный коммутатор, так как если узел Hyper @ No__t-1V находится в подсети, ВКЛЮЧАЮЩЕЙ DHCP-сервер, тестовые виртуальные машины получат IP-адрес от DHCP-сервера. Кроме того, тестовый DHCP-сервер может назначить IP-адреса другим компьютерам в подсети, где установлен узел Hyper @ no__t-0V.
...
Роль DHCP-сервера можно установить на физическом компьютере или на виртуальной машине \(VM @ no__t-1, установленной на узле Hyper @ no__t-2V. Если вы устанавливаете DHCP на виртуальной машине и хотите, чтобы DHCP-сервер предоставил назначения IP-адресов компьютерам в физической сети, к которой подключен узел Hyper-V, необходимо подключить адаптер виртуальной сети виртуальной машины к виртуальному коммутатору Hyper-V, **внешнему < /C0 >.**
...
### <a name="join-the-computer-to-the-domain-optional"></a>Присоедините компьютер к домену \(Optional @ no__t-1
...
Чтобы создать группы безопасности, необходимо запустить команду Network Shell \(netsh @ no__t-1 в Windows PowerShell, а затем перезапустить службу DHCP, чтобы новые группы стали активными.
...
### <a name="authorize-the-dhcp-server-in-active-directory-optional"></a>Авторизация DHCP-сервера в Active Directory \(Optional @ no__t-1
...
### <a name="notify-server-manager-that-post-install-dhcp-configuration-is-complete-optional"></a>Уведомлять диспетчер сервера о том, что конфигурация DHCP POST @ no__t-0install завершена \(Optional @ no__t-2

После завершения задач POST @ no__t-0installation, таких как создание групп безопасности и авторизация DHCP-сервера в Active Directory, диспетчер сервера может по-прежнему отображать предупреждение в пользовательском интерфейсе, сообщающем, что действия POST @ no__t-1installation должны быть завершено с помощью мастера настройки DHCP после установки.

Вы можете предотвратить появление этого сообщения @ no__t-0unnecessary и неточных сообщений в диспетчер сервера, настроив следующий раздел реестра с помощью этой команды Windows PowerShell.
...
### <a name="set-server-level-dns-dynamic-update-configuration-settings-optional"></a>Задайте параметры конфигурации динамического обновления DNS на уровне сервера \(Optional @ no__t-1
...
### <a name="configure-the-corpnet2-scope-optional"></a>Настройка области Corpnet2 \(Optional @ no__t-1

При наличии второй подсети, подключенной к первой подсети с маршрутизатором, на котором включена пересылка DHCP, можно использовать следующие команды, чтобы добавить вторую область с именем Corpnet2 для этого примера. В этом примере также настраивается диапазон исключения и IP-адрес для шлюза по умолчанию \(the для IP-адреса маршрутизатора @ no__t-1 в подсети Corpnet2.
...
Чтобы убедиться, что сервер DHCP обеспечивает динамическое выделение IP-адресов клиентам DHCP, можно подключить другой компьютер к обслуживаемой подсети. После подключения кабеля Ethernet к сетевому адаптеру и включения питания компьютера будет запрошен IP-адрес с DHCP-сервера. Вы можете проверить успешность настройки с помощью команды **ipconfig/all** , просмотреть результаты или выполнить проверки подключения, например, попытаться получить доступ к веб-ресурсам через обозреватель или файловые ресурсы с помощью проводника Windows или другого. приложений.
...
4. Убедитесь, что ваши области активированы, открыв консоль DHCP \(Server Manager, **средства**, **DHCP**\), развернув дерево сервера для просмотра областей, а затем щелкните правой кнопкой @ no__t-4clicking каждую область. Если итоговое меню включает выделение **активировать**, нажмите кнопку **активировать**. @no__t — 0If область уже активирована, выбор меню считывает значение **деактивировать**. \)
