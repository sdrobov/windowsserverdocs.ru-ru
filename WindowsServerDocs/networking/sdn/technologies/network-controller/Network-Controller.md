...
Сетевой контроллер — это высокодоступная и масштабируемая роль сервера, которая предоставляет один интерфейс программирования приложений \(API @ no__t-1, позволяющий сетевому контроллеру взаимодействовать с сетью, и второй API, позволяющий взаимодействовать с Сетевой контроллер.
...
API сетевого контроллера обмена позволяет настраивать, отслеживать, устранять неполадки и развертывать новые устройства в сети с помощью Windows PowerShell, переносить состояние представления \(REST @ no__t-1 API или приложение управления с графическим Пользовательский интерфейс, например System Center Virtual Machine Manager.

>[!NOTE]
>Северный API-интерфейс сетевого контроллера реализован как интерфейс REST.

Вы можете управлять сетью центра обработки данных с помощью сетевого контроллера, используя приложения управления, такие как System Center Virtual Machine Manager \(SCVMM @ no__t-1, и System Center Operations Manager \(SCOM @ no__t-3, так как сетевой контроллер позволяет настраивать, отслеживать, программировать и устранять неполадки сетевой инфраструктуры, которая находится под ее контролем.
...
- Служба удаленного доступа \(RAS @ no__t-1 многоклиентские шлюзы, виртуальные шлюзы и пулы шлюзов
...
При развертывании сетевого контроллера в тестовой среде можно запустить роль сервера сетевого контроллера на виртуальной машине Hyper-V \(VM @ no__t-1, установленной на узле Hyper-V.
