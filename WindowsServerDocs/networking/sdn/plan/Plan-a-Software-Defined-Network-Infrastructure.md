...
Пиринг BGP обычно настраивается в управляемом коммутаторе или маршрутизаторе в составе сетевой инфраструктуры. Узел BGP также можно настроить на сервере Windows Server с ролью сервера удаленного доступа (RAS), установленной в режиме только маршрутизации. На этом одноранговом узле маршрутизатора BGP в сетевой инфраструктуре необходимо настроить собственный ASN и разрешить пиринг из ASN, назначенного компонентам SDN, \(шлюзов SLB/МУЛЬТИПЛЕКСОРа и шлюза RAS\). Необходимо получить от физического маршрутизатора следующие сведения или сетевой администратор, управляющий этим маршрутизатором:
...
Чтобы учитывать расходы на трафик виртуальной сети клиента, вызванные заголовком инкапсуляции ВКСЛАН или NVGRE, значение MTU сети уровня 2 (коммутаторы и узлы) должно быть больше или равно 1674 байт \(включая заголовки Ethernet уровня 2\). 

Сетевые карты, поддерживающие новое ключевое слово New *енкаповерхеад* Adapter, устанавливают значение MTU автоматически через агент узла сетевого контроллера. Сетевые карты, не поддерживающие новое ключевое слово *енкаповерхеад* , должны вручную задать размер MTU на каждом физическом узле с помощью *жумбопаккет* \(или эквивалентного\) ключевого слова. 
...
- \(необходимые параметры MTU свитчпорт\)  
- MTU имеет значение > = 1674 байт \(включая заголовок L2-Ethernet\)  
- Требуется \(протоколов L3\)  
- ECMP  
- \(по протоколу BGP IETF RFC 4271\)на основе \-ECMP
...
- Качество обслуживания \(коэффициента мощности требуется только при использовании Роце\)
...
- При использовании сетевого контроллера для мониторинга физического коммутатора \(требуется протокол SNMP MIB\)  
...
> [!TIP]  
> Это удобно, если вы используете одно и то же имя для всех виртуальных коммутаторов, но оно не является обязательным. Если планируется развертывание с помощью скриптов, см. комментарий, связанный с переменной `vSwitchName` в файле config. PSD1.  
...
--------|-------------------------|-------------------------  
|Физический узел Hyper-v|4-ядерный ЦП с частотой 2,66 ГГц<br /><br />32 ГБ ОЗУ<br /><br />300 ГБ дискового пространства<br /><br />физический сетевой адаптер объемом 1 ГБ/с (или более мощный)|ОС: Windows Server 2016<br /><br />Роль Hyper-V установлена|  
